...
function bindCastboxRssPlayerInteractions(slot, options = {}) {
  ...
  function applyRssSearchFilter() {
    ...
    const rows = Array.from(root.querySelectorAll('.castbox-rss-item[data-castbox-ep-src]'));
    rows.forEach((btn) => {
      const titleText = String(btn.getAttribute('data-castbox-ep-title') || btn.querySelector('.castbox-rss-item-title')?.textContent || '').trim().toLowerCase();
      const match = !query || titleText.includes(query);
...
const playlistController = {
  ...
  _buildCardHtml(item, direction) {
    ...
    let extraCardClass = '';
    if (!isVideo && item.embedPlatform === 'apple' && !isEpisodeChartTypeValue(item.embedKind)) {
    ...
    const titleMarkup = `${(isVideo || platformHasDirectVideoBadge(item.embedPlatform)) ? buildVideoTitleBadgeMarkup() : ''}${this._escapeHtml(item.title)}`;
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', () => {
        ...
        if (platform === 'rss' || platform === 'castbox' || platform === 'pocketcasts') {
          ...
          renderCastboxRssPlayerInSlot(rssSlot, platform, item.embedUrl, item.title, true)
            .catch(() => {})
            .finally(() => {
