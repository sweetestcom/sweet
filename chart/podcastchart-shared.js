...
const WORKER_API_BASE = (() => {
  try {
    ...
    if (host === 'localhost' || host === '127.0.0.1') return 'https://swiest.com/api/podcast';
  } catch {}
  return '/api/podcast';
})();
...
const inflightJsonFetches = new Map();
const TEXT_RESULT_CACHE_VERSION = 'v1';
const TRANSLATION_RESULT_CACHE_PREFIX = 'textcache::translation::';
const TRANSLITERATION_RESULT_CACHE_PREFIX = 'textcache::transliteration::';
const TRANSLATION_RESULT_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const TRANSLITERATION_RESULT_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const TRANSLATION_RESULT_CACHE_MAX_ENTRIES = 2400;
const TRANSLITERATION_RESULT_CACHE_MAX_ENTRIES = 2400;
...
function getAppleEmbedThemeParam(theme = currentThemeMode) {
  return normalizeThemeMode(theme) === THEME_MODE_LIGHT ? 'light' : 'dark';
}

function appendAppleEmbedThemeParam(url, theme = currentThemeMode) {
  const parsed = safeUrlParse(url);
  if (!parsed) return String(url || '').trim();
  const host = String(parsed.hostname || '').trim().toLowerCase();
  if (!/embed\.podcasts\.apple\.com$/.test(host)) return parsed.toString();
  parsed.searchParams.set('theme', getAppleEmbedThemeParam(theme));
  return parsed.toString();
}

function refreshAppleEmbedThemeForNode(root = document) {
  const scope = root && typeof root.querySelectorAll === 'function' ? root : document;
  const playButtons = Array.from(scope.querySelectorAll('.play-link[data-embed-platform="apple"][data-embed-url]'));
  playButtons.forEach((btn) => {
    const currentUrl = String(btn.getAttribute('data-embed-url') || '').trim();
    if (!currentUrl) return;
    const themedUrl = appendAppleEmbedThemeParam(currentUrl, currentThemeMode);
    if (themedUrl && themedUrl !== currentUrl) btn.setAttribute('data-embed-url', themedUrl);
  });
  const favoriteButtons = Array.from(scope.querySelectorAll('.favorite-link[data-favorite-embed-platform="apple"][data-favorite-embed-url]'));
  favoriteButtons.forEach((btn) => {
    const currentUrl = String(btn.getAttribute('data-favorite-embed-url') || '').trim();
    if (!currentUrl) return;
    const themedUrl = appendAppleEmbedThemeParam(currentUrl, currentThemeMode);
    if (themedUrl && themedUrl !== currentUrl) btn.setAttribute('data-favorite-embed-url', themedUrl);
  });
  const frames = Array.from(scope.querySelectorAll('iframe[src*="embed.podcasts.apple.com"]'));
  frames.forEach((iframe) => {
    const currentSrc = String(iframe.getAttribute('src') || '').trim();
    if (!currentSrc) return;
    const themedSrc = appendAppleEmbedThemeParam(currentSrc, currentThemeMode);
    if (themedSrc && themedSrc !== currentSrc) iframe.setAttribute('src', themedSrc);
  });
}
...
function applyThemeMode(theme, options = {}) {
  ...
  const previousTheme = currentThemeMode;
  ...
  syncThemeToggleUi();
  if (options.refreshAppleEmbeds === false) return;
  if (!options.persist && previousTheme === nextTheme) return;
  refreshAppleEmbedThemeForNode(document);
}
...
const IDB_META_KEY_RSS_FEED_CACHE_PREFIX = 'rss_feed_cache_v1::';
const RSS_DEFAULT_PLAYER_MODE_STORAGE_KEY = 'rss_default_player_mode_v1';
const IDB_META_KEY_PREFETCH_STATE = 'prefetch_state_v1';
const IDB_META_KEY_BILIBILI_DETAIL_PREFIX = 'bilibili_detail_v1::';
const BILIBILI_DETAIL_CACHE_TTL_MS = 30 * 24 * 60 * 60 * 1000;
...
let myPodcastsOpmlExportEl = null;
...
let historyAllOriginsPrefetchTimer = 0;
let historyAllOriginsPrefetchPromise = null;
const NOTE_AUTOSAVE_DELAY_MS = 520;
const MY_PODCASTS_TAB_VALUES = ['podcasts', 'episodes', 'notes'];
const MY_PODCASTS_IMPORT_MODE_VALUES = ['rss', 'youtube', 'apple', 'spotify', 'bilibili', 'opml'];
...
const PLATFORM_ACCENT_COLORS = {
  ...
  snipd: ['#33D39E', '#20C08E', '#1B2B52'],
  netflix: ['#E00010', '#B00010', '#200000'],
  disneyplus: ['#003040', '#005060', '#00A0B0'],
  soundon: ['#2DAAE1', '#3CB8ED', '#178FC7'],
  kkbox: ['#39CBEE', '#57D7F4', '#1FBFE6'],
  podplay: ['#FF4B8C', '#FF7B3D', '#FFC24A'],
  xiaoyuzhou: ['#20B0E0', '#48B8D8', '#88C0C8'],
  ximalaya: ['#F86442', '#FF7B45', '#E84B2F'],
  bilibili: ['#F76A9D', '#F95A93', '#FF3F86'],
  bbcsounds: ['#F86000', '#D04010', '#A02800'],
  globalplayer: ['#1F72FE', '#238CFE', '#30A5FE'],
...
const SPOTIFY_CONFIRMED_COUNTRIES = new Set([
  'us', 'gb', 'ca', 'au', 'de', 'fr', 'it', 'es', 'nl', 'se', 'no', 'dk', 'fi',
  'at', 'ie', 'pl', 'jp', 'in', 'id', 'ph', 'br', 'mx', 'ar', 'cl', 'co', 'nz'
]);
...
const PLATFORM_APP_STORE_IDS = {
  ...
  snipd: { id: '1557206126', country: 'us' },
  netflix: { id: '363590051', country: 'us' },
  disneyplus: { id: '1446075923', country: 'us' },
  soundon: { id: '1476272964', country: 'tw' },
  kkbox: { id: '300915900', country: 'tw' },
  podplay: { id: '1458715963', country: 'se' },
  xiaoyuzhou: { id: '1488894313', country: 'cn' },
  ximalaya: { id: '876336838', country: 'cn' },
  bilibili: { id: '736536022', country: 'cn' },
  bbcsounds: { id: '1380676511', country: 'gb' },
  globalplayer: { id: '1142951331', country: 'gb' },
...
const PLATFORM_ICON_FALLBACK = {
  ...
  netflix: 'logo.webp',
  disneyplus: 'logo.webp',
  xiaoyuzhou: 'logo.webp',
  ximalaya: 'logo.webp',
  bilibili: 'logo.webp',
  bbcsounds: 'logo.webp',
  globalplayer: 'logo.webp',
  siriusxm: 'logo.webp',
  pandora: 'logo.webp',
...
const MICROSOFT_TRANSLATE_API_V3 = 'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0';
const MICROSOFT_TRANSLATE_API_PREVIEW = 'https://api.cognitive.microsofttranslator.com/translate?api-version=2025-10-01-preview';
const MICROSOFT_DETECT_API_V3 = 'https://api.cognitive.microsofttranslator.com/detect?api-version=3.0';
const MICROSOFT_TRANSLITERATE_API_PREVIEW = 'https://api.cognitive.microsofttranslator.com/transliterate?api-version=2025-10-01-preview&toScript=Latn';
const MICROSOFT_TRANSLITERATION_LANGUAGE_HINTS = {
  'ar': { language: 'ar', fromScript: 'Arab' },
  'as': { language: 'as', fromScript: 'Beng' },
  'be': { language: 'be', fromScript: 'Cyrl' },
  'bg': { language: 'bg', fromScript: 'Cyrl' },
  'bn': { language: 'bn', fromScript: 'Beng' },
  'brx': { language: 'brx', fromScript: 'Deva' },
  'el': { language: 'el', fromScript: 'Grek' },
  'fa': { language: 'fa', fromScript: 'Arab' },
  'gom': { language: 'gom', fromScript: 'Deva' },
  'gu': { language: 'gu', fromScript: 'Gujr' },
  'he': { language: 'he', fromScript: 'Hebr' },
  'hi': { language: 'hi', fromScript: 'Deva' },
  'ja': { language: 'ja', fromScript: 'Jpan' },
  'kk': { language: 'kk', fromScript: 'Cyrl' },
  'kn': { language: 'kn', fromScript: 'Knda' },
  'ko': { language: 'ko', fromScript: 'Kore' },
  'ks': { language: 'ks', fromScript: 'Arab' },
  'ky': { language: 'ky', fromScript: 'Cyrl' },
  'mai': { language: 'mai', fromScript: 'Deva' },
  'mk': { language: 'mk', fromScript: 'Cyrl' },
  'ml': { language: 'ml', fromScript: 'Mlym' },
  'mn-cyrl': { language: 'mn-Cyrl', fromScript: 'Cyrl' },
  'mni': { language: 'mni', fromScript: 'Mtei' },
  'mr': { language: 'mr', fromScript: 'Deva' },
  'ne': { language: 'ne', fromScript: 'Deva' },
  'or': { language: 'or', fromScript: 'Orya' },
  'pa': { language: 'pa', fromScript: 'Guru' },
  'pa-in': { language: 'pa', fromScript: 'Guru' },
  'pa-pk': { language: 'pa-Arab', fromScript: 'Arab' },
  'ru': { language: 'ru', fromScript: 'Cyrl' },
  'sa': { language: 'sa', fromScript: 'Deva' },
  'sd': { language: 'sd', fromScript: 'Arab' },
  'si': { language: 'si', fromScript: 'Sinh' },
  'sr-cyrl': { language: 'sr-Cyrl', fromScript: 'Cyrl' },
  'ta': { language: 'ta', fromScript: 'Taml' },
  'te': { language: 'te', fromScript: 'Telu' },
  'tg': { language: 'tg', fromScript: 'Cyrl' },
  'th': { language: 'th', fromScript: 'Thai' },
  'tt': { language: 'tt', fromScript: 'Cyrl' },
  'uk': { language: 'uk', fromScript: 'Cyrl' },
  'ur': { language: 'ur', fromScript: 'Arab' },
  'zh-hans': { language: 'zh-Hans', fromScript: 'Hans' },
  'zh-hant': { language: 'zh-Hant', fromScript: 'Hant' },
  'zh-cn': { language: 'zh-Hans', fromScript: 'Hans' },
  'zh-sg': { language: 'zh-Hans', fromScript: 'Hans' },
  'zh-tw': { language: 'zh-Hant', fromScript: 'Hant' },
  'zh-hk': { language: 'zh-Hant', fromScript: 'Hant' },
  'yue': { language: 'zh-Hant', fromScript: 'Hant' }
};
...
const NETFLIX_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' },
  { value: 'netflix-pop-culture', key: 'netflixPopCulture', defaultLabel: 'Pop Culture' },
  { value: 'netflix-knowledge', key: 'netflixCuriousMind', defaultLabel: 'Curious Mind' },
  { value: 'netflix-sports', key: 'typeSports', defaultLabel: 'Sports' }
];
const DISNEYPLUS_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' }
];
const SOUNDON_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' }
];
const KKBOX_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' }
];
const PODPLAY_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' }
];
...
function getXiaoyuzhouSpecialTypeLabels(pageLang = pageToLanguage(getCurrentFileName())) {
  ...
  return {
    skyrocket: 'Skyrocket Episodes',
    newStar: 'New Star Episodes'
  };
}

function getXiaoyuzhouTypeDefinitions() {
  const labels = getXiaoyuzhouSpecialTypeLabels();
  return [
    { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' },
    { value: 'top-episodes', key: 'topEpisodes', defaultLabel: 'Top Episodes' },
    { value: 'xiaoyuzhou-skyrocket-episodes', key: '', defaultLabel: labels.skyrocket },
    { value: 'xiaoyuzhou-new-star-episodes', key: '', defaultLabel: labels.newStar }
  ];
}
...
function getRuntimePageLanguageOverride() {
  try {
    const override = String(window.RUNTIME_PAGE_LANG || document.documentElement.getAttribute('data-runtime-lang') || '').trim().toLowerCase();
    return override || '';
  } catch {
    return '';
  }
...
function pageToLanguage(fileName) {
  ...
  if (normalizedFile === currentFile) {
    const runtimeOverride = getRuntimePageLanguageOverride();
    if (runtimeOverride) return runtimeOverride;
  }
  const match = normalizedFile.match(/^podcastchart(?:-([^.]+))?\.html$/);
...
function showToast(message) {
  if (!toastEl) return;
  const spotifyToastSuppressedUntil = typeof globalThis !== 'undefined'
    ? Number(globalThis.__spotifyRssSwitchSuppressToastUntil || 0)
    : 0;
  if (
    spotifyToastSuppressedUntil > Date.now()
    && String(message || '').trim() === String(t('loadFailedRefresh') || '').trim()
  ) {
    return;
  }
  if (toastTimer) {
...
function getAppleBaseTypeDefinitions() {
  return Array.isArray(APPLE_TYPE_DEFINITIONS) ? APPLE_TYPE_DEFINITIONS : [];
}

function parseAppleCategoryTypeValue(value) {
  const raw = String(value || '').trim().toLowerCase();
  const match = raw.match(/^apple-(\d{4})(?:-(podcasts|episodes))?$/);
  if (!match) return null;
  return {
    id: match[1],
    kind: match[2] || 'podcasts'
  };
}

function buildAppleCategoryTypeValue(categoryId, kind = 'podcasts') {
  const safeId = String(categoryId || '').trim();
  const safeKind = String(kind || '').trim().toLowerCase() === 'episodes' ? 'episodes' : 'podcasts';
  return `apple-${safeId}-${safeKind}`;
}

function getAppleExpandedTypeDefinitions() {
  const defs = getAppleBaseTypeDefinitions();
  const out = [];
  defs.forEach((def) => {
    if (!def) return;
    const parsed = parseAppleCategoryTypeValue(def.value);
    if (!parsed || String(def.value || '').trim().toLowerCase().endsWith('-episodes')) {
      out.push(def);
      return;
    }
    const baseLabel = buildTypeOptionLabel(def).replace(/^\s+/, '');
    out.push({
      ...def,
      value: buildAppleCategoryTypeValue(parsed.id, 'podcasts'),
      key: '',
      defaultLabel: `${baseLabel} - ${String(t('topPodcasts') || 'Top Podcasts').trim()}`
    });
    out.push({
      ...def,
      value: buildAppleCategoryTypeValue(parsed.id, 'episodes'),
      defaultLabel: `${baseLabel} - ${String(t('topEpisodes') || 'Top Episodes').trim()}`,
      key: '',
      indent: Number(def.indent) || 0
    });
  });
  return out;
}
...
function getTypeDefinitionsForPlatform(platform) {
  ...
  if (p === 'apple') return getAppleExpandedTypeDefinitions();
  if (p === 'spotify') return SPOTIFY_TYPE_DEFINITIONS;
  if (p === 'xiaoyuzhou') return getXiaoyuzhouTypeDefinitions();
  ...
  if (p === 'globalplayer') return TOP_PODCASTS_ONLY_TYPE_DEFINITIONS;
  ...
  if (p === 'netflix') return NETFLIX_TYPE_DEFINITIONS;
  if (p === 'disneyplus') return DISNEYPLUS_TYPE_DEFINITIONS;
  if (p === 'soundon') return SOUNDON_TYPE_DEFINITIONS;
  if (p === 'kkbox') return KKBOX_TYPE_DEFINITIONS;
  if (p === 'podplay') return PODPLAY_TYPE_DEFINITIONS;
...
function normalizeTypeForPlatform(platform, type) {
  ...
  if (safePlatform === 'apple') {
    const parsedAppleCategory = parseAppleCategoryTypeValue(type);
    if (parsedAppleCategory) {
      return buildAppleCategoryTypeValue(parsedAppleCategory.id, parsedAppleCategory.kind);
    }
  }
...
function buildTypeOptionLabel(def) {
  ...
  const platformDefault = String(def && def.defaultLabel || '').trim();
  const sharedLocalized = !localized && !platformDefault ? findLocalizedSharedTypeLabel(def) : '';
...
function findLocalizedSharedTypeLabel(def) {
  const key = String(def && def.key || '').trim();
  const value = String(def && def.value || '').trim().toLowerCase();
  const pools = [
    getAppleBaseTypeDefinitions(),
    Array.isArray(SPOTIFY_TYPE_DEFINITIONS) ? SPOTIFY_TYPE_DEFINITIONS : [],
    Array.isArray(PODCASTAPP_TYPE_DEFINITIONS) ? PODCASTAPP_TYPE_DEFINITIONS : [],
    Array.isArray(PODCASTADDICT_TYPE_DEFINITIONS) ? PODCASTADDICT_TYPE_DEFINITIONS : []
  ];
  for (const defs of pools) {
    for (const candidate of defs) {
      ...
      if (key && candidateKey === key) {
        const label = String(candidate.defaultLabel || '').trim();
        if (label) return label;
      }
      if (value && candidateValue === value) {
        const label = String(candidate.defaultLabel || '').trim();
        if (label) return label;
      }
    }
...
function isAppleTypeValue(value) {
  return /^apple-\d{4}(?:-(?:podcasts|episodes))?$/.test(String(value || '').trim().toLowerCase());
}

function isEpisodeChartTypeValue(type) {
  const raw = String(type || '').trim().toLowerCase();
  return raw === 'top-episodes'
    || raw === 'xiaoyuzhou-skyrocket-episodes'
    || raw === 'xiaoyuzhou-new-star-episodes'
    || /^apple-\d{4}-episodes$/.test(raw);
}
...
function getAppleMainPrefetchTypes() {
  return getAppleBaseTypeDefinitions()
    .map((def) => {
      const parsed = parseAppleCategoryTypeValue(def && def.value);
      if (!parsed || parsed.kind !== 'podcasts' || isAppleDerivedType(def && def.value) || Number(def && def.indent)) return '';
      return buildAppleCategoryTypeValue(parsed.id, 'podcasts');
    })
    .filter(Boolean);
}
...
function isWorkerAttachEligibleTask(platform, type) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const safeType = normalizeTypeForPlatform(safePlatform, type);
  if (safePlatform === 'apple') {
    return safeType === 'top-podcasts'
      || safeType === 'top-episodes'
      || /^apple-\d{4}-(podcasts|episodes)$/.test(safeType)
      || /^genre-\d{4}-(podcasts|episodes)$/.test(safeType);
  }
  if (safePlatform === 'spotify') return true;
  if (safePlatform === 'youtube') return safeType === 'top-podcasts' || safeType === 'top-episodes';
  return false;
}

function isAllOriginsHistoryEligibleTask(platform, type) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const safeType = normalizeTypeForPlatform(safePlatform, type);
  if (safePlatform === 'spotify') return true;
  if (safePlatform === 'apple') {
    return safeType === 'top-podcasts'
      || /^apple-\d{4}-podcasts$/.test(safeType)
      || /^genre-\d{4}-podcasts$/.test(safeType);
  }
  return false;
}

function buildDefaultWorkerPrefetchTasks(country) {
  const safeCountry = normalizeCountry(country);
  return dedupePrefetchTasks([
    buildPrefetchTask('apple', safeCountry, 'top-episodes', 'default-seed'),
    buildPrefetchTask('spotify', safeCountry, 'top-podcasts', 'default-seed')
  ]);
}

function buildHotWorkerFallbackTasks(country) {
  const safeCountry = normalizeCountry(country);
  return dedupePrefetchTasks([
    buildPrefetchTask('apple', safeCountry, 'top-podcasts', 'hot-fallback'),
    buildPrefetchTask('apple', safeCountry, 'top-episodes', 'hot-fallback'),
    buildPrefetchTask('spotify', safeCountry, 'top-podcasts', 'hot-fallback')
  ]);
}

async function selectWorkerPrefetchTasks(platform, country, type) {
  const currentTask = buildPrefetchTask(platform, country, type, 'current');
  if (!currentTask) return [];
  if (!isWorkerAttachEligibleTask(currentTask.platform, currentTask.type)) return [];

  const state = await loadPrefetchState();
  const picked = [];
  const seen = new Set([currentTask.key]);

  const addIfEligible = async (task) => {
    if (!task || seen.has(task.key)) return;
    if (!isWorkerAttachEligibleTask(task.platform, task.type)) return;
    if (await hasFreshResponseCacheForCombo(task.platform, task.country, task.type)) return;
    seen.add(task.key);
    picked.push(task);
  };

  if (currentTask.platform === 'apple' && currentTask.type === 'top-podcasts') {
    for (const task of buildDefaultWorkerPrefetchTasks(currentTask.country)) {
      await addIfEligible(task);
      if (picked.length >= 2) return picked.slice(0, 2);
    }
  }


  for (const [, entry] of recentEntriesFromBucket(state.seenCombos, PREFETCH_MAX_EXACT_COMBOS)) {
    const task = buildPrefetchTask(entry.platform, entry.country, entry.type, 'history-inline');
    await addIfEligible(task);
    if (picked.length >= 2) return picked.slice(0, 2);
  }

  for (const task of buildHotWorkerFallbackTasks(currentTask.country)) {
    await addIfEligible(task);
    if (picked.length >= 2) return picked.slice(0, 2);
  }
...
function serializeWorkerPrefetchTasks(tasks) {
  return (Array.isArray(tasks) ? tasks : [])
    .filter(Boolean)
    .map((task) => `${task.platform},${task.country},${task.type}`)
    .join('|');
}

async function appendWorkerPrefetchToCandidates(candidates, platform, type, country, options = {}) {
  const list = Array.isArray(candidates) ? candidates.filter(Boolean) : [];
  if (!list.length) return list;
  if (options.renderedCache) return list;
  const tasks = await selectWorkerPrefetchTasks(platform, country, type);
  if (!tasks.length) return list;
  const encoded = serializeWorkerPrefetchTasks(tasks);
  if (!encoded) return list;
  return list.map((url) => `${url}${url.includes('?') ? '&' : '?'}prefetch=${encodeURIComponent(encoded)}`);
}

function isWorkerPrefetchEnvelope(payload) {
  return !!(payload && typeof payload === 'object' && payload.__wrapped && payload.data && Array.isArray(payload.prefetched));
}

async function persistInlinePrefetchedPayloads(entries) {
  const list = Array.isArray(entries) ? entries : [];
  for (const entry of list) {
    const platform = String(entry && entry.platform || '').trim().toLowerCase();
    const country = normalizeCountry(entry && entry.country);
    const type = String(entry && entry.type || '').trim().toLowerCase();
    const data = entry && entry.data;
    if (!platform || !country || !type || !data) continue;
    try {
      const items = collectNormalizedItemsForData(data, platform, type);
      await persistPlatformPayloadAliases(platform, type, country, data, items, { fetchedAt: Date.now(), payload: data });
    } catch (error) {
      console.warn(`Inline prefetched cache persist failed for ${platform}/${country}/${type}:`, error);
    }
  }
}

async function buildHistoryAllOriginsPrefetchTasks(limit = 2) {
  const state = await loadPrefetchState();
  ...
  const currentKey = (!isLanguageScopeMode(currentScope) && currentPlatform !== 'all')
    ? prefetchComboStateKey(currentPlatform, currentCountry, currentType)
    : '';
  const out = [];

  for (const [, entry] of recentEntriesFromBucket(state.seenCombos, PREFETCH_MAX_EXACT_COMBOS)) {
    const task = buildPrefetchTask(entry.platform, entry.country, entry.type, 'history-allorigins');
    if (!task || task.key === currentKey) continue;
    if (!isAllOriginsHistoryEligibleTask(task.platform, task.type)) continue;
    if (isPrefetchFailureCoolingDown(state, task)) continue;
    if (await hasFreshResponseCacheForCombo(task.platform, task.country, task.type)) continue;
    out.push(task);
...
async function runHistoryAllOriginsPrefetchOnce() {
  const state = await loadPrefetchState();
  const tasks = await buildHistoryAllOriginsPrefetchTasks(2);
  for (const task of tasks) {
    try {
      const result = await fetchSinglePlatformNormalizedDataViaAllOrigins(task.platform, task.type, task.country, null);
      await persistPlatformPayloadAliases(task.platform, task.type, task.country, result.data, result.items, {
        fetchedAt: Date.now(),
        payload: result.data
      });
      markPrefetchTaskSuccess(state, task);
      await savePrefetchState(state);
      await delayPrefetch(1200, null);
    } catch (error) {
      markPrefetchTaskFailure(state, task);
      await savePrefetchState(state);
      console.warn(`History AllOrigins prefetch failed for ${task.key}:`, error);
    }
  }
}

function scheduleHistoryAllOriginsPrefetch() {
  if (isFavoritesScopeMode()) return;
  const currentPlatform = String((platformSelect && platformSelect.value) || 'apple').trim().toLowerCase();
  if (currentPlatform === 'xiaoyuzhou') return;
  if (historyAllOriginsPrefetchPromise) return;
  if (historyAllOriginsPrefetchTimer) clearTimeout(historyAllOriginsPrefetchTimer);
  historyAllOriginsPrefetchTimer = window.setTimeout(() => {
    historyAllOriginsPrefetchTimer = 0;
    if (historyAllOriginsPrefetchPromise) return;
    historyAllOriginsPrefetchPromise = runHistoryAllOriginsPrefetchOnce()
      .catch((error) => {
        console.warn('History AllOrigins prefetch run failed:', error);
      })
      .finally(() => {
        historyAllOriginsPrefetchPromise = null;
      });
  }, 1200);
}
...
function inferContentKindFromLink(platform, link, fallbackKind = '') {
  ...
  if (safePlatform === 'bilibili') {
    if (/\/lists\/\d+/i.test(rawLink)) return 'top-podcasts';
    return 'top-episodes';
  }
  return 'top-podcasts';
}
...
function favoriteTypeFromRecord(record) {
  ...
  if (isEpisodeChartTypeValue(kind)) return 'episodes';
  return 'podcasts';
}
...
async function downloadRemoteFile(url, filename = 'download.bin') {
  ...
  try {
    ...
    console.warn('Remote download failed:', error);
    return false;
  }
}
...
function getXiaoyuzhouPlatformDateKey(ts = Date.now()) {
  return getDateKeyInTimeZone(ts, 'Asia/Shanghai');
}
...
function getCurrentSnapshotDateKeyForPlatform(platform, ts = Date.now()) {
  ...
  if (safePlatform === 'xiaoyuzhou') return getXiaoyuzhouPlatformDateKey(ts);
...
function extractXiaoyuzhouSourceDateKey(payload) {
  if (!payload || typeof payload !== 'object') return '';
  const directCandidates = [
    payload.latestDateKey,
  ...
  if (payload.results && typeof payload.results === 'object') {
    const topEpisodesPayload = payload.results['top-episodes'];
    const nested = extractXiaoyuzhouSourceDateKey(topEpisodesPayload);
    if (nested) return nested;
  }
  const list = Array.isArray(payload.podcasts) ? payload.podcasts : [];
  for (const row of list) {
    const raw = String(row && (row.issueDate || row.latestDateKey || row.sourceDateKey) || '').trim();
    if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) return raw;
  }
  return '';
}
...
function getResponseCacheDateKeyForPlatform(platform, payload, updatedAt = Date.now()) {
  ...
  if (safePlatform === 'xiaoyuzhou') {
    return extractXiaoyuzhouSourceDateKey(payload) || getXiaoyuzhouPlatformDateKey(updatedAt);
  }
...
function ensureMyPodcastsEntryButton() {
  ...
  if (!btn) {
    ...
    btn.type = 'button';
    decorateCommandButton(btn, 'my-podcasts', buildMyPodcastsScopeLabel());
...
function supportsWorkerPlatformBatch(platforms, type, country) {
  const safeType = isEpisodeChartTypeValue(type) ? 'top-episodes' : 'top-podcasts';
  const safeCountry = normalizeCountry(country);
  ...
  return list.every((platform) => (
    supported.has(platform)
    && isPlatformAvailableForCountry(platform, safeCountry)
    && (safeType === 'top-podcasts' || platformSupportsEpisodes(platform))
  ));
}

const ALL_PLATFORM_SINGLE_ONLY_PLATFORMS = new Set([
  'bilibili'
]);

const ALL_PLATFORM_BATCH_STAGE_ONE_PLATFORMS = new Set([
  'apple',
  'spotify',
  'youtube',
  'castbox',
...
const ALL_PLATFORM_BATCH_STAGE_TWO_PLATFORMS = new Set([
  'xiaoyuzhou',
  'amazonmusic',
  'podcastapp',
  'rtlplus',
...
function classifyAllPlatformFetchTargets(platforms, type, country) {
  const safeType = isEpisodeChartTypeValue(type) ? 'top-episodes' : 'top-podcasts';
  const safeCountry = normalizeCountry(country);
  const list = (Array.isArray(platforms) ? platforms : [])
    .map((platform) => String(platform || '').trim().toLowerCase())
    .filter(Boolean)
    .filter((platform, index, arr) => arr.indexOf(platform) === index)
    .filter((platform) => isPlatformAvailableForCountry(platform, safeCountry))
    .filter((platform) => safeType === 'top-podcasts' || platformSupportsEpisodes(platform));
  ...
  list.forEach((platform) => {
    if (ALL_PLATFORM_SINGLE_ONLY_PLATFORMS.has(platform)) {
      single.push(platform);
      return;
    }
    if (ALL_PLATFORM_BATCH_STAGE_ONE_PLATFORMS.has(platform)) {
      stageOne.push(platform);
      return;
    }
    if (ALL_PLATFORM_BATCH_STAGE_TWO_PLATFORMS.has(platform)) {
      stageTwo.push(platform);
      return;
    }
    single.push(platform);
  });

  return { stageOne, stageTwo, single };
}

async function fetchWorkerPlatformBatchRaw(platforms, type, country) {
  const safeType = isEpisodeChartTypeValue(type) ? 'top-episodes' : 'top-podcasts';
  const safeCountry = normalizeCountry(country);
  const list = (Array.isArray(platforms) ? platforms : [])
    .map((platform) => String(platform || '').trim().toLowerCase())
  ...
  if (!supportsWorkerPlatformBatch(list, safeType, safeCountry)) {
    return { results: [], failures: list.slice() };
  }
  ...
  rows.forEach((row) => {
    ...
    seen.add(platform);
    if (row && row.ok && Object.prototype.hasOwnProperty.call(row, 'data')) {
      results.push({ platform, data: row.data });
      return;
    }
  ...
  list.forEach((platform) => {
    if (!seen.has(platform) && !failures.includes(platform)) failures.push(platform);
  });
...
function getPlatformRankCap(platform = (platformSelect ? platformSelect.value : ''), type = (typeSelect ? typeSelect.value : 'top-podcasts')) {
  ...
  if (p === 'apple') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'xiaoyuzhou') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'bilibili') return RANK_SCORE_EXTENDED_MAX;
  ...
  if (p === 'snipd' && rawType === 'top-podcasts') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'netflix') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'disneyplus' && rawType === 'top-podcasts') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'soundon' && rawType === 'top-podcasts') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'kkbox' && rawType === 'top-podcasts') return RANK_SCORE_EXTENDED_MAX;
  if (p === 'podplay' && rawType === 'top-podcasts') return RANK_SCORE_EXTENDED_MAX;
...
function shouldUncapSinglePlatformDisplay(platform = (platformSelect ? platformSelect.value : ''), type = (typeSelect ? typeSelect.value : 'top-podcasts')) {
  ...
  if (p === 'apple') return true;
  if (p === 'bilibili') return true;
  ...
  if (p === 'snipd' && rawType === 'top-podcasts') return true;
  if (p === 'netflix') return true;
  if (p === 'disneyplus' && rawType === 'top-podcasts') return true;
  if (p === 'soundon' && rawType === 'top-podcasts') return true;
  if (p === 'kkbox' && rawType === 'top-podcasts') return true;
  if (p === 'podplay' && rawType === 'top-podcasts') return true;
  return false;
}
...
function shouldUncapAggregateDisplay(platform = (platformSelect ? platformSelect.value : ''), scope = currentScopeValue()) {
  const p = String(platform || '').trim().toLowerCase();
  if (document.body && document.body.dataset && document.body.dataset.chartPageMode === 'topic') return true;
...
function getRenderDataCap() {
  if (document.body && document.body.dataset && document.body.dataset.chartPageMode === 'topic') return Number.POSITIVE_INFINITY;
...
function normalizeSnapshotItems(items, options = {}) {
  ...
  return source.map((item, index) => ({
    ...
    podcastTitle: String(item.podcastTitle || '').trim(),
    publishedText: String(item.publishedText || '').trim(),
    durationText: String(item.durationText || '').trim(),
    meta: String(item.meta || '').trim(),
    sourceType: String(item.sourceType || '').trim().toLowerCase(),
    originCountry: String(item.originCountry || '').trim().toLowerCase(),
    originCountryLabel: String(item.originCountryLabel || '').trim(),
    detectedLanguage: String(item.detectedLanguage || '').trim().toLowerCase(),
    detectedLanguageLabel: String(item.detectedLanguageLabel || '').trim(),
    appearanceText: String(item.appearanceText || '').trim(),
...
function resolveSnapshotWriteMeta(platform, payload, fetchedAt, chartFingerprint, latestExisting) {
  ...
  if (safePlatform === 'xiaoyuzhou') {
    const sourceDateKey = extractXiaoyuzhouSourceDateKey(payload);
    return {
      snapshotDate: sourceDateKey || getXiaoyuzhouPlatformDateKey(fetchedAt),
      storageDateBasis: sourceDateKey ? 'xiaoyuzhou-source-date' : 'xiaoyuzhou-shanghai-observed',
      sourceUpdatedAt: sourceDateKey || ''
    };
  }

  const snapshotDate = getStorageDateKey(fetchedAt);
  const latestSnapshotDate = String((latestExisting && latestExisting.snapshotDate) || '').trim();
  ...
  if (latestExisting && latestSnapshotDate === snapshotDate && latestFingerprint && latestFingerprint === chartFingerprint) {
    return {
      snapshotDate,
  ...
  dedupedRows.forEach((snapshot) => {
    ...
    list.forEach((item, index) => {
      ...
      const prev = map.get(key) || {
        ...
        originCountryLabel: item.originCountryLabel || '',
        detectedLanguage: item.detectedLanguage || '',
        detectedLanguageLabel: item.detectedLanguageLabel || '',
        appearanceText: item.appearanceText || '',
      ...
      if (dayKey >= prev.latestDate) {
        ...
        prev.podcastTitle = item.podcastTitle || prev.podcastTitle;
        prev.publishedText = item.publishedText || prev.publishedText;
        prev.durationText = item.durationText || prev.durationText;
        prev.meta = item.meta || prev.meta;
        prev.sourceType = item.sourceType || prev.sourceType;
        prev.originCountry = item.originCountry || prev.originCountry;
        prev.originCountryLabel = item.originCountryLabel || prev.originCountryLabel;
        prev.detectedLanguage = item.detectedLanguage || prev.detectedLanguage;
        prev.detectedLanguageLabel = item.detectedLanguageLabel || prev.detectedLanguageLabel;
        prev.appearanceText = item.appearanceText || prev.appearanceText;
        prev.appleId = item.appleId || prev.appleId;
...
function shouldApplyDataBackfill(platform, type) {
  ...
  const allowCustomPodcastType = safePlatform === 'rtlplus' || safePlatform === 'ximalaya';
  if (!allowCustomPodcastType && safeType !== 'top-podcasts' && !isEpisodeChartTypeValue(safeType)) {
    return false;
  }
  ...
  const backfillPlatforms = [
    ...
    'bbcsounds', 'globalplayer', 'deezer', 'abclisten', 'nrk', 'yleareena',
    'sverigesradio', 'podplay', 'radionet', 'podcastaddict', 'radiko', 'orfsound', 'raiplaysound', 'srfaudio', 'drlyd', 'rtbfauvio', 'rneaudio', 'npoluister', 'snipd', 'netflix', 'disneyplus', 'soundon', 'kkbox',
    'ivoox', 'podimo', 'rtlplus', 'xiaoyuzhou'
  ];

  return backfillPlatforms.includes(safePlatform);
}

function getDataBackfillSourcePlatforms(targetPlatform) {
  return ['apple', 'spotify'];
}

async function buildDataBackfillMapFromSnapshots(country, type = 'top-podcasts', targetPlatform = '') {
  const safeType = normalizeTopType(type);
  if (safeType !== 'top-podcasts' && safeType !== 'top-episodes') return new Map();

  const now = Date.now();
  const cacheKey = `${country}_${safeType}_${String(targetPlatform || '').trim().toLowerCase()}`;
  ...
  const backfillSources = getDataBackfillSourcePlatforms(targetPlatform);

  const rowsHaveDescriptions = (rows) => {
    const list = Array.isArray(rows) ? rows : [];
    return list.some((row) => Array.isArray(row && row.items) && row.items.some((item) => String(item && item.description || '').trim()));
  };

  const sourceNeedsDescriptionRefresh = (platform) => {
    const safePlatform = String(platform || '').trim().toLowerCase();
    return safePlatform === 'apple';
  };
  ...
  for (const sourcePlatform of backfillSources) {
    ...
    } else if (sourceNeedsDescriptionRefresh(sourcePlatform) && !rowsHaveDescriptions(rows)) {
      try {
        console.log(`Refreshing backfill snapshot descriptions for ${sourcePlatform} (${country})...`);
        await fetchCountryPlatformItemsWithCache(sourcePlatform, safeType, country, { forceRefresh: true });
        rows = await idbGetAllByIndex(IDB_SNAPSHOT_STORE, 'comboKey', comboKey);
      } catch (error) {
        console.warn(`Failed to refresh backfill descriptions for ${sourcePlatform}:`, error);
      }
    }
    ...
    for (const row of sortedRows) {
      ...
      for (const item of list) {
        ...
        const nextDescription = String(item && item.description ? item.description : '').trim();
        ...
        if (!entry.url && hasHttpUrl(nextUrl)) {
          ...
          if (hasHttpUrl(nextEmbedUrl) && nextEmbedPlatform) {
            ...
            entry.embedKind = isEpisodeChartTypeValue(nextEmbedKind) ? 'top-episodes' : 'top-podcasts';
...
async function enrichItemsFromLocalCacheWithBackfill(items, platform, country, type) {
  ...
  const backfillMap = await buildDataBackfillMapFromSnapshots(country, type, platform);
  ...
  const enriched = list.map((item) => {
    ...
    if (!String(next.description || '').trim() && String(backfill.description || '').trim()) {
      next.description = String(backfill.description || '').trim();
      changed = true;
    }
...
function getEpisodePlaySvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><g class="play-icon"><path fill="currentColor" d="M6 14.723V5.277a1.25 1.25 0 0 1 1.921-1.054l7.422 4.722a1.25 1.25 0 0 1 0 2.11L7.92 15.778A1.25 1.25 0 0 1 6 14.723"/></g><g class="close-icon"><path d="M6.7 6.7 13.3 13.3M13.3 6.7l-6.6 6.6" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"/></g></svg>';
}
...
function getRssPlayerPlayPauseSvg() {
  return '<svg class="castbox-rss-play-icon" viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path data-glyph="play" fill="currentColor" d="M6 14.723V5.277a1.25 1.25 0 0 1 1.921-1.054l7.422 4.722a1.25 1.25 0 0 1 0 2.11L7.92 15.778A1.25 1.25 0 0 1 6 14.723"/><g data-glyph="pause" fill="currentColor"><rect x="5.85" y="4.65" width="3.1" height="10.7" rx="1.15"/><rect x="11.05" y="4.65" width="3.1" height="10.7" rx="1.15"/></g></svg>';
}

function getRssDownloadSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="currentColor" d="M10 3.75a.75.75 0 0 1 .75.75v6.19l1.97-1.97a.75.75 0 1 1 1.06 1.06l-3.25 3.25a.75.75 0 0 1-1.06 0L6.22 9.78a.75.75 0 1 1 1.06-1.06l1.97 1.97V4.5A.75.75 0 0 1 10 3.75M4.5 13.25a.75.75 0 0 1 .75.75v.5c0 .414.336.75.75.75h8a.75.75 0 0 0 .75-.75V14a.75.75 0 0 1 1.5 0v.5A2.25 2.25 0 0 1 15 16.75H5A2.25 2.25 0 0 1 2.75 14.5V14a.75.75 0 0 1 .75-.75"/></svg>';
}

function getRssFeedSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="currentColor" d="M4.5 13.75a1.75 1.75 0 1 0 0 3.5 1.75 1.75 0 0 0 0-3.5m-1-5.2a1 1 0 0 0 0 2c3.59 0 6.5 2.91 6.5 6.5a1 1 0 1 0 2 0c0-4.694-3.806-8.5-8.5-8.5m0-4.8a1 1 0 0 0 0 2c6.241 0 11.3 5.059 11.3 11.3a1 1 0 1 0 2 0c0-7.346-5.954-13.3-13.3-13.3"/></svg>';
}

function getRssQueueSvg() {
  return '<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path fill="currentColor" d="M2.634 5.537a.906.906 0 1 0 0-1.813.906.906 0 1 0 0 1.813zm3.192-.325h9.865a.576.576 0 0 0 .585-.578.578.578 0 0 0-.585-.585H5.826a.574.574 0 0 0-.585.585c0 .325.253.578.585.578zM2.634 9.906c.506 0 .91-.404.91-.91a.906.906 0 0 0-.91-.91.906.906 0 0 0-.91.91c0 .506.405.91.91.91zm3.192-.325h9.865a.582.582 0 1 0 0-1.162H5.826a.572.572 0 0 0-.585.577c0 .325.253.585.585.585zm-3.192 4.694a.91.91 0 1 0-.001-1.82.91.91 0 0 0 0 1.82zm3.192-.332h9.865a.576.576 0 0 0 .585-.577.578.578 0 0 0-.585-.585H5.826a.574.574 0 0 0-.585.585c0 .324.253.577.585.577z"/></svg>';
}

function getRssQueueAddSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="currentColor" d="M10 4.25a.75.75 0 0 1 .75.75v4.25H15a.75.75 0 0 1 0 1.5h-4.25V15a.75.75 0 0 1-1.5 0v-4.25H5a.75.75 0 0 1 0-1.5h4.25V5a.75.75 0 0 1 .75-.75"/><path fill="currentColor" opacity=".34" d="M3.5 4.75a.75.75 0 0 1 .75-.75h2.2a.75.75 0 0 1 0 1.5h-2.2a.75.75 0 0 1-.75-.75m0 5a.75.75 0 0 1 .75-.75h1.2a.75.75 0 0 1 0 1.5h-1.2a.75.75 0 0 1-.75-.75m0 5a.75.75 0 0 1 .75-.75h2.2a.75.75 0 0 1 0 1.5h-2.2a.75.75 0 0 1-.75-.75"/></svg>';
}

function getRssQueueAddedSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M5.2 10.4 8.3 13.5 14.9 6.9"/></svg>';
}

function getQueueRemoveSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" d="M6.2 6.2 13.8 13.8M13.8 6.2l-7.6 7.6"/></svg>';
}

function getAppleModeSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M16.64 12.27c.02 2.23 1.95 2.98 1.97 2.99-.02.05-.31 1.07-1.03 2.12-.62.9-1.27 1.8-2.28 1.82-.98.02-1.3-.58-2.43-.58-1.13 0-1.49.56-2.4.6-.98.04-1.72-.98-2.35-1.88-1.27-1.84-2.24-5.2-.94-7.46.64-1.12 1.78-1.83 3.02-1.85.94-.02 1.83.63 2.43.63.6 0 1.72-.78 2.89-.67.49.02 1.87.2 2.75 1.5-.07.04-1.64.95-1.63 2.78M14.8 5.54c.52-.63.88-1.5.78-2.37-.75.03-1.66.5-2.19 1.12-.48.56-.91 1.44-.8 2.28.83.06 1.69-.42 2.21-1.03"/></svg>';
}

function getRssSearchSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M10.5 2a8.5 8.5 0 1 0 5.262 15.176l4.53 4.531a1 1 0 0 0 1.415-1.414l-4.531-4.531A8.5 8.5 0 0 0 10.5 2m-6.5 8.5a6.5 6.5 0 1 1 13 0 6.5 6.5 0 0 1-13 0"/></svg>';
}

function getRssSeekBackSvg() {
  return '<svg viewBox="84.21 19.76 99.61 109.08" aria-hidden="true" focusable="false"><path fill="currentColor" d="M84.205 79.035c0 27.246 22.608 49.804 49.805 49.804 27.246 0 49.805-22.558 49.805-49.804 0-24.024-17.53-44.385-40.381-48.877v-6.934c0-3.467-2.393-4.395-5.03-2.49l-15.576 10.888c-2.246 1.563-2.295 3.907 0 5.518l15.528 10.938c2.685 1.953 5.078 1.025 5.078-2.49v-6.934c18.457 4.199 32.031 20.605 32.031 40.38 0 23.047-18.408 41.504-41.455 41.504-23.047 0-41.553-18.457-41.504-41.503.049-13.868 6.787-26.124 17.188-33.545 2.002-1.514 2.636-3.809 1.416-5.86-1.221-2.002-3.907-2.539-6.055-.879-12.549 9.131-20.85 23.877-20.85 40.284Zm35.547 20.02c1.904 0 3.125-1.32 3.125-3.37v-30.81c0-2.49-1.27-3.81-3.467-3.81-1.367 0-2.343.44-4.052 1.612l-6.739 4.541c-1.123.83-1.66 1.71-1.66 2.832 0 1.611 1.27 2.979 2.881 2.979.928 0 1.416-.196 2.344-.928l4.59-3.37v26.954c0 2.002 1.172 3.37 2.978 3.37Zm24.805.585c8.105 0 13.476-5.225 13.476-13.086 0-7.178-4.834-12.305-11.816-12.305-2.88 0-6.006 1.319-7.47 3.516l.83-10.547h14.648c1.416 0 2.637-1.172 2.637-2.832s-1.221-2.734-2.637-2.734H138.6c-2.54 0-3.955 1.465-4.2 4.052l-1.025 14.063c-.244 2.441 1.026 3.613 3.174 3.613 1.611 0 2.295-.293 3.71-1.367 1.856-1.709 3.37-2.393 5.274-2.393 3.809 0 6.348 2.784 6.348 7.032 0 4.297-2.978 7.422-7.031 7.422-2.881 0-5.371-1.66-6.64-4.2-.733-1.27-1.612-1.953-2.833-1.953-1.611 0-2.734 1.172-2.734 2.832 0 .684.146 1.319.39 1.953 1.319 3.516 5.616 6.934 11.524 6.934Z"/></svg>';
}

function getRssSeekForwardSvg() {
  return '<svg viewBox="84.21 19.78 99.61 109.06" aria-hidden="true" focusable="false"><path fill="currentColor" d="M84.205 79.035c0 27.246 22.608 49.804 49.805 49.804 27.246 0 49.805-22.558 49.805-49.804 0-16.407-8.301-31.153-20.85-40.284-2.148-1.66-4.834-1.123-6.055.88-1.22 2.05-.586 4.345 1.416 5.859 10.4 7.421 17.139 19.677 17.188 33.545.049 23.046-18.457 41.503-41.504 41.503-23.047 0-41.455-18.457-41.455-41.503 0-19.776 13.574-36.182 32.031-40.381v6.982c0 3.467 2.393 4.395 5.078 2.49L145.24 37.19c2.198-1.514 2.247-3.858 0-5.469l-15.527-10.937c-2.734-1.954-5.127-1.026-5.127 2.49v6.885c-22.851 4.492-40.38 24.853-40.38 48.877ZM118.58 99.59c7.666 0 13.135-4.638 13.135-11.23 0-4.541-3.027-8.301-7.324-9.082 3.467-.977 6.103-4.59 6.103-8.692 0-5.859-4.785-9.96-11.62-9.96-5.909 0-10.84 3.32-11.866 8.056a6.314 6.314 0 0 0-.195 1.611c0 1.758 1.123 2.979 2.783 2.979 1.416 0 2.295-.733 2.832-2.295 1.025-3.076 3.076-4.688 6.299-4.688 3.418 0 5.761 2.051 5.761 5.03 0 3.125-2.343 5.42-5.81 5.42h-2.1c-1.611 0-2.685 1.123-2.685 2.832 0 1.611 1.123 2.832 2.685 2.832h2.344c4.102 0 6.69 2.295 6.69 5.713 0 3.32-2.93 5.859-6.983 5.859-3.418 0-5.86-1.709-6.933-4.736-.586-1.612-1.465-2.295-2.784-2.295-1.66 0-2.832 1.27-2.832 2.978 0 .537.05 1.074.196 1.612 1.074 4.59 6.347 8.056 12.304 8.056Zm31.494 0c8.106 0 13.428-7.666 13.428-19.385 0-11.816-5.322-19.58-13.428-19.58-8.154 0-13.427 7.715-13.427 19.58 0 11.72 5.322 19.385 13.427 19.385Zm0-5.908c-4.296 0-7.08-5.127-7.08-13.477 0-8.496 2.735-13.671 7.08-13.671 4.346 0 7.032 5.126 7.032 13.671 0 8.35-2.735 13.477-7.032 13.477Z"/></svg>';
}

function getRssVolumeSvg() {
  return '<svg viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path class="volume-body" fill="currentColor" d="M25.477 51.06c1.4129 0 2.431-1.0389 2.431-2.431V15.488c0-1.3921-1.0181-2.5349-2.4726-2.5349-1.0181 0-1.7038.43634-2.805 1.4752l-9.2046 8.6644c-.14545.12464-.31166.18698-.51945.18698H6.6948c-2.9297 0-4.5088 1.5999-4.5088 4.7374v8.0411c0 3.1167 1.5791 4.7166 4.5088 4.7166h6.2126c.20779 0 .374.06234.51945.18698l9.2046 8.7475c.99732.93501 1.8285 1.3506 2.8466 1.3506z"/><path class="volume-wave volume-wave-1" fill="currentColor" d="M36.864 41.108c.70647.49868 1.7246.35323 2.3271-.47787 1.6205-2.1817 2.5971-5.3815 2.5971-8.6436 0-3.2621-.9766-6.4411-2.5971-8.6436-.60255-.83111-1.5999-.97655-2.3271-.49868-.89345.62336-1.0181 1.683-.35319 2.5765 1.2051 1.6207 1.9323 4.0932 1.9323 6.5658 0 2.4726-.76881 4.9451-1.9531 6.5866-.62332.89345-.51945 1.9116.374 2.5349z"/><path class="volume-wave volume-wave-2" fill="currentColor" d="M45.154 46.718c.81021.54023 1.8077.33245 2.3894-.49867 2.7426-3.8231 4.3426-8.9137 4.3426-14.233 0-5.3399-1.5583-10.451-4.3426-14.254-.60255-.81034-1.5791-1.0181-2.3894-.47787-.78979.54021-.91447 1.5583-.29106 2.4518 2.2647 3.3245 3.6779 7.6878 3.6779 12.28s-1.3923 8.9969-3.6779 12.28c-.60255.89345-.49872 1.9116.29106 2.4518z"/><path class="volume-wave volume-wave-3" fill="currentColor" d="M53.527 52.39c.76894.51945 1.7872.31166 2.3898-.54021 3.8438-5.423 6.0255-12.446 6.0255-19.864s-2.2443-14.42-6.0255-19.864c-.60255-.87268-1.6209-1.0805-2.3898-.54021-.78936.56098-.91404 1.5791-.31149 2.4518 3.3451 4.9244 5.423 11.241 5.423 17.952 0 6.7113-1.9945 13.132-5.423 17.952-.60255.87268-.47787 1.8908.31149 2.4518z"/></svg>';
}

function getVideoTitleBadgeSvg() {
  return '<svg viewBox="0 0 18 18" aria-hidden="true" focusable="false"><path fill="currentColor" d="M2.228 13.717h13.544c1.24 0 1.983-.748 1.983-1.982V3.818c0-1.235-.744-1.983-1.983-1.983H2.228c-1.235 0-1.983.748-1.983 1.983v7.917c0 1.234.748 1.982 1.983 1.982m.12-1.465c-.404 0-.637-.223-.637-.633V3.936c0-.413.233-.633.638-.633H15.65c.404 0 .638.22.638.633v7.683c0 .41-.234.633-.638.633zm3.11 3.913h7.084c.37 0 .67-.307.67-.669s-.3-.669-.67-.669H5.458a.675.675 0 0 0-.67.67c0 .361.3.668.67.668"/></svg>';
}

function buildVideoTitleBadgeMarkup() {
  return `<span class="video-title-badge" aria-label="Video podcast" title="Video podcast">${getVideoTitleBadgeSvg()}</span>`;
}

function platformHasDirectVideoBadge(platform) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  return safePlatform === 'youtube'
    || safePlatform === 'bilibili'
    || safePlatform === 'netflix'
    || safePlatform === 'disneyplus';
}

function setTitleVideoBadgeState(titleEl, shouldShow) {
  if (!titleEl) return;
  let badge = titleEl.querySelector('.video-title-badge');
  if (shouldShow) {
    if (!badge) {
      titleEl.insertAdjacentHTML('afterbegin', buildVideoTitleBadgeMarkup());
    }
    return;
  }
...
function syncRowVideoBadgeState(row, shouldShow) {
  if (!row) return;
  const titleEl = row.querySelector('.title-copy-target');
  if (!titleEl) return;
  setTitleVideoBadgeState(titleEl, !!shouldShow);
}
...
function getNoteSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="currentColor" d="M12.03 4.113a3.612 3.612 0 0 1 5.108 5.108l-6.292 6.29c-.324.324-.56.561-.791.752l-.235.176q-.308.21-.65.36l-.229.093c-.181.066-.369.115-.586.16l-.764.134-2.394.4c-.142.024-.294.05-.423.06-.098.007-.232.01-.378-.026l-.149-.05a1.08 1.08 0 0 1-.521-.474l-.046-.093a1.1 1.1 0 0 1-.075-.527c.01-.129.035-.28.06-.422l.398-2.394c.1-.602.162-.987.295-1.35l.093-.23q.15-.342.36-.65l.176-.235c.19-.232.428-.467.751-.79zM6.68 11.345c-.35.35-.534.535-.66.688l-.11.147a2.7 2.7 0 0 0-.24.433l-.062.154c-.08.22-.124.462-.232 1.112l-.398 2.394-.001.001h.003l2.393-.399.717-.126a3 3 0 0 0 .394-.105l.154-.063q.228-.1.433-.24l.147-.11c.153-.126.339-.31.688-.66l4.988-4.988-3.227-3.226zm9.517-6.291a2.28 2.28 0 0 0-3.225 0l-.364.362 3.226 3.227.363-.364c.89-.89.89-2.334 0-3.225M4.583 1.783a.3.3 0 0 1 .294.241c.117.585.347 1.092.707 1.48.357.385.859.668 1.549.783a.3.3 0 0 1 0 .592c-.69.115-1.192.398-1.549.783-.315.34-.53.77-.657 1.265l-.05.215a.3.3 0 0 1-.588 0c-.117-.585-.347-1.092-.707-1.48-.357-.384-.859-.668-1.549-.783a.3.3 0 0 1 0-.592c.69-.115 1.192-.398 1.549-.783.36-.388.59-.895.707-1.48l.015-.05a.3.3 0 0 1 .279-.19"/></svg>';
}

function getSearchCatalogSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" fill-rule="evenodd" clip-rule="evenodd"><path fill="currentColor" d="M10.5 2a8.5 8.5 0 1 0 5.262 15.176l4.53 4.531a1 1 0 0 0 1.415-1.414l-4.531-4.531A8.5 8.5 0 0 0 10.5 2M4 10.5a6.5 6.5 0 1 1 13 0 6.5 6.5 0 0 1-13 0"/></svg>';
}

function getTransportPrevSvg() {
  return '<svg viewBox="0 0 32 28" aria-hidden="true" focusable="false"><g transform="translate(32 0) scale(-1 1)"><path fill="currentColor" fill-rule="nonzero" d="M18.14 20.68c.365 0 .672-.107 1.038-.323l8.508-4.997c.623-.365.938-.814.938-1.37 0-.564-.307-.988-.938-1.361l-8.508-4.997c-.366-.216-.68-.324-1.046-.324-.73 0-1.337.556-1.337 1.569v4.773c-.108-.399-.406-.73-.904-1.021L7.382 7.632c-.357-.216-.672-.324-1.037-.324-.73 0-1.345.556-1.345 1.569v10.235c0 1.013.614 1.569 1.345 1.569.365 0 .68-.108 1.037-.324l8.509-4.997c.49-.29.796-.631.904-1.038v4.79c0 1.013.615 1.569 1.345 1.569z"/></g></svg>';
}

function getTransportNextSvg() {
  return '<svg viewBox="0 0 32 28" aria-hidden="true" focusable="false"><path fill="currentColor" fill-rule="nonzero" d="M18.14 20.68c.365 0 .672-.107 1.038-.323l8.508-4.997c.623-.365.938-.814.938-1.37 0-.564-.307-.988-.938-1.361l-8.508-4.997c-.366-.216-.68-.324-1.046-.324-.73 0-1.337.556-1.337 1.569v4.773c-.108-.399-.406-.73-.904-1.021L7.382 7.632c-.357-.216-.672-.324-1.037-.324-.73 0-1.345.556-1.345 1.569v10.235c0 1.013.614 1.569 1.345 1.569.365 0 .68-.108 1.037-.324l8.509-4.997c.49-.29.796-.631.904-1.038v4.79c0 1.013.615 1.569 1.345 1.569z"/></svg>';
}

function getShareExportSvg() {
  return '<svg viewBox="0 0 16 16" aria-hidden="true" focusable="false"><path fill="currentColor" d="M8.007 10.42c-.338 0-.617-.293-.617-.643V2.752l.046-1.041-.41.517-.922 1.04a.549.549 0 0 1-.418.189.542.542 0 0 1-.55-.559c0-.174.06-.3.172-.419L7.536.223C7.695.056 7.848 0 8.006 0c.166 0 .312.056.471.223l2.228 2.256a.575.575 0 0 1 .179.42.547.547 0 0 1-.557.558.559.559 0 0 1-.418-.189l-.921-1.04-.411-.51.053 1.034v7.025c0 .35-.279.643-.623.643zM4.18 16C2.75 16 2 15.218 2 13.723V6.858c0-1.501.75-2.277 2.181-2.277h1.81v1.39H4.267c-.61 0-.948.335-.948 1.006v6.62c0 .671.338 1.006.948 1.006h7.466c.603 0 .948-.335.948-1.005V6.977c0-.67-.345-1.006-.948-1.006h-1.704v-1.39h1.79c1.438 0 2.181.783 2.181 2.277v6.865C14 15.218 13.257 16 11.819 16H4.18z"/></svg>';
}

function getSurpriseSparkSvg() {
  return '<svg viewBox="0 0 56 56" aria-hidden="true" focusable="false"><path fill="currentColor" d="M 26.6875 12.6602 C 26.9687 12.6602 27.1094 12.4961 27.1797 12.2383 C 27.9062 8.3242 27.8594 8.2305 31.9375 7.4570 C 32.2187 7.4102 32.3828 7.2461 32.3828 6.9648 C 32.3828 6.6836 32.2187 6.5195 31.9375 6.4726 C 27.8828 5.6524 28.0000 5.5586 27.1797 1.6914 C 27.1094 1.4336 26.9687 1.2695 26.6875 1.2695 C 26.4062 1.2695 26.2656 1.4336 26.1953 1.6914 C 25.3750 5.5586 25.5156 5.6524 21.4375 6.4726 C 21.1797 6.5195 20.9922 6.6836 20.9922 6.9648 C 20.9922 7.2461 21.1797 7.4102 21.4375 7.4570 C 25.5156 8.2774 25.4687 8.3242 26.1953 12.2383 C 26.2656 12.4961 26.4062 12.6602 26.6875 12.6602 Z M 15.3438 28.7852 C 15.7891 28.7852 16.0938 28.5039 16.1406 28.0821 C 16.9844 21.8242 17.1953 21.8242 23.6641 20.5821 C 24.0860 20.5117 24.3906 20.2305 24.3906 19.7852 C 24.3906 19.3633 24.0860 19.0586 23.6641 18.9883 C 17.1953 18.0977 16.9609 17.8867 16.1406 11.5117 C 16.0938 11.0899 15.7891 10.7852 15.3438 10.7852 C 14.9219 10.7852 14.6172 11.0899 14.5703 11.5352 C 13.7969 17.8164 13.4687 17.7930 7.0469 18.9883 C 6.6250 19.0821 6.3203 19.3633 6.3203 19.7852 C 6.3203 20.2539 6.6250 20.5117 7.1406 20.5821 C 13.5156 21.6133 13.7969 21.7774 14.5703 28.0352 C 14.6172 28.5039 14.9219 28.7852 15.3438 28.7852 Z M 31.2344 54.7305 C 31.8438 54.7305 32.2891 54.2852 32.4062 53.6524 C 34.0703 40.8086 35.8750 38.8633 48.5781 37.4570 C 49.2344 37.3867 49.6797 36.8945 49.6797 36.2852 C 49.6797 35.6758 49.2344 35.2070 48.5781 35.1133 C 35.8750 33.7070 34.0703 31.7617 32.4062 18.9180 C 32.2891 18.2852 31.8438 17.8633 31.2344 17.8633 C 30.6250 17.8633 30.1797 18.2852 30.0860 18.9180 C 28.4219 31.7617 26.5938 33.7070 13.9140 35.1133 C 13.2344 35.2070 12.7891 35.6758 12.7891 36.2852 C 12.7891 36.8945 13.2344 37.3867 13.9140 37.4570 C 26.5703 39.1211 28.3281 40.8321 30.0860 53.6524 C 30.1797 54.2852 30.6250 54.7305 31.2344 54.7305 Z"/></svg>';
}

function getAudioModeSvg() {
  return '<svg viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path fill="currentColor" d="M4 13c-.6 0-1 .4-1 1v4c0 .6.4 1 1 1s1-.4 1-1v-4c0-.6-.4-1-1-1Zm4-2c-.6 0-1 .4-1 1v8c0 .6.4 1 1 1s1-.4 1-1v-8c0-.6-.4-1-1-1Zm4-5c-.6 0-1 .4-1 1v18c0 .6.4 1 1 1s1-.4 1-1V7c0-.6-.4-1-1-1Zm4 7c-.6 0-1 .4-1 1v4c0 .6.4 1 1 1s1-.4 1-1v-4c0-.6-.4-1-1-1Zm4-4c-.6 0-1 .4-1 1v12c0 .6.4 1 1 1s1-.4 1-1V10c0-.6-.4-1-1-1Zm4-3c-.6 0-1 .4-1 1v18c0 .6.4 1 1 1s1-.4 1-1V7c0-.6-.4-1-1-1Zm4 7c-.6 0-1 .4-1 1v4c0 .6.4 1 1 1s1-.4 1-1v-4c0-.6-.4-1-1-1Z"/></svg>';
}

function getBackToTopSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M11.2 6.06a1.1 1.1 0 0 1 1.6 0l5.02 5.28a1.1 1.1 0 0 1-1.6 1.5l-3.14-3.31v7.37a1.1 1.1 0 1 1-2.2 0V9.53l-3.14 3.3a1.1 1.1 0 1 1-1.6-1.49l5.02-5.28ZM5.4 3.1a1 1 0 0 1 1-1h11.2a1 1 0 1 1 0 2H6.4a1 1 0 0 1-1-1Z"/></svg>';
}

function getEyeClosedRevealSvg() {
  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M21 12c-1.75 3.58-5.12 6-9 6s-7.25-2.42-9-6M12 18v3M19.42 14.42l2.08 2.08M16.23 16.97 17.5 19.5M4.58 14.42 2.5 16.5M7.77 16.97 6.5 19.5" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg>';
}

function getGlobalPlayerCollapseSvg() {
  return '<svg viewBox="0 0 90.64 30.831" aria-hidden="true" focusable="false"><path fill="currentColor" d="m4.486 14.456 32.352 13.938c3.156 1.387 5.552 2.437 8.48 2.437 2.932 0 5.357-1.05 8.484-2.437l32.353-13.938c2.612-1.192 4.485-3.514 4.485-6.42C90.64 3.184 87.085 0 83 0c-2.279 0-5.172 1.325-7.569 2.42L42.845 16.358h4.95L15.21 2.42C12.812 1.325 9.948 0 7.636 0 3.55 0 0 3.184 0 8.036c0 2.906 1.873 5.228 4.486 6.42z"/></svg>';
}
...
function getAppleEpisodeEmbedUrl(url) {
  ...
  parsed.hash = '';
  parsed.searchParams.set('theme', getAppleEmbedThemeParam());
  return parsed.toString();
}
...
function getInlineEmbedFrameHeight(platform, kind) {
  const p = String(platform || '').trim().toLowerCase();
  const safeKind = isEpisodeChartTypeValue(kind) ? 'top-episodes' : 'top-podcasts';
...
function buildEpisodePlayerIframeMarkup(embedUrl, platform, title, kind, options = {}) {
  ...
  if (p === 'apple') {
    if (safeKind === 'top-episodes') {
      ...
      const initialHeight = appleEpisodeMode === 'video' ? 371 : 175;
      return `<div class="episode-player-apple-shell ${cssClass}" data-apple-episode-mode="${appleEpisodeMode}"><iframe class="episode-player-frame apple ${cssClass}" allow="autoplay *; encrypted-media *; fullscreen *; clipboard-write" frameborder="0" height="${initialHeight}" style="width:100%;overflow:hidden;border-radius:10px;height:${initialHeight}px" sandbox="allow-forms allow-popups allow-same-origin allow-scripts allow-storage-access-by-user-activation allow-top-navigation-by-user-activation" src="${safeUrl}" title="${safeTitle}"></iframe></div>`;
    }
...
const CASTBOX_RSS_CACHE_TTL_MS = 3 * 60 * 60 * 1000;
...
function buildRssProxyCandidates(rssUrl) {
  ...
  return [
    ...
    `https://proxy.killcors.com/?url=${encoded}`,
...
async function fetchAnyTextWithFallback(urls, options = {}) {
  const queue = [];
  const seen = new Set();
  const maxAttempts = Math.max(2, Number(options && options.maxAttempts) || 8);
  let attempts = 0;
  let lastError = null;
  ...
  while (queue.length && attempts < maxAttempts) {
    ...
    try {
      ...
      const text = unwrapProxyWrappedText(await response.text());
      if (!String(text || '').trim()) {
        throw new Error(`Empty response body for ${candidate}`);
      }
      return { text, finalUrl: String(response.url || candidate).trim() || candidate };
    } catch (error) {
      lastError = error;
    }
  }
...
function extractEpisodeTranscriptSources(node, baseUrl = '') {
  const sources = [];
  const seen = new Set();
  const push = (urlValue, typeValue = '', languageValue = '') => {
    const url = normalizeXmlLinkHref(urlValue, baseUrl);
    if (!url || seen.has(url)) return;
    seen.add(url);
    sources.push({
      url,
      type: String(typeValue || '').trim().toLowerCase(),
      language: String(languageValue || '').trim().toLowerCase()
    });
  };

  const directEls = collectXmlElementsByTagNames(node, ['podcast:transcript', 'transcript']);
  directEls.forEach((el) => {
    push(el.getAttribute('url') || el.getAttribute('href') || '', el.getAttribute('type') || '', el.getAttribute('language') || '');
  });

  try {
    const descendants = node && typeof node.getElementsByTagName === 'function'
      ? Array.from(node.getElementsByTagName('*'))
      : [];
    descendants.forEach((el) => {
      const local = String(el.localName || '').trim().toLowerCase();
      const tag = String(el.tagName || '').trim().toLowerCase();
      if (local !== 'transcript' && tag !== 'podcast:transcript' && tag !== 'transcript') return;
      push(el.getAttribute('url') || el.getAttribute('href') || '', el.getAttribute('type') || '', el.getAttribute('language') || '');
    });
  } catch {}
  ...
  if (!sources.length) {
    try {
      const rawXml = typeof XMLSerializer !== 'undefined' ? new XMLSerializer().serializeToString(node) : '';
      const pattern = /<(?:[\w-]+:)?transcript\b([^>]*)\/?>/gi;
      let match;
      while ((match = pattern.exec(rawXml))) {
        const attrs = String(match[1] || '');
        const urlMatch = attrs.match(/\burl\s*=\s*["']([^"']+)["']/i) || attrs.match(/\bhref\s*=\s*["']([^"']+)["']/i);
        const typeMatch = attrs.match(/\btype\s*=\s*["']([^"']+)["']/i);
        const languageMatch = attrs.match(/\blanguage\s*=\s*["']([^"']+)["']/i);
        push(urlMatch ? urlMatch[1] : '', typeMatch ? typeMatch[1] : '', languageMatch ? languageMatch[1] : '');
      }
    } catch {}
  }
...
function extractTranscriptSourcesFromRawItemXml(itemXml, baseUrl = '') {
  ...
  while ((match = pattern.exec(raw))) {
    ...
    const typeMatch = attrs.match(/\btype\s*=\s*["']([^"']+)["']/i);
    const languageMatch = attrs.match(/\blanguage\s*=\s*["']([^"']+)["']/i);
    const url = normalizeXmlLinkHref(urlMatch ? urlMatch[1] : '', baseUrl);
    if (!url || seen.has(url)) continue;
    seen.add(url);
    sources.push({
      url,
      type: String(typeMatch ? typeMatch[1] : '').trim().toLowerCase(),
      language: String(languageMatch ? languageMatch[1] : '').trim().toLowerCase()
    });
  }
  return sources;
}

function extractTranscriptSourcesByItemIndex(rawText, baseUrl = '') {
  const raw = String(rawText || '');
  if (!raw) return [];
  return [...raw.matchAll(/<item\b[\s\S]*?<\/item>/gi)].map((match) => extractTranscriptSourcesFromRawItemXml(match[0], baseUrl));
}

function findTranscriptSourcesForEpisodeInRawFeed(rawText, episode, baseUrl = '') {
  const raw = String(rawText || '');
  const row = episode && typeof episode === 'object' ? episode : null;
  if (!raw || !row) return [];

  const guid = decodeHtmlEntities(String(row.guid || '').trim());
  const audioUrl = decodeHtmlEntities(String(row.audioUrl || '').trim());
  const normalizedAudio = normalizeXmlLinkHref(audioUrl, baseUrl);
  const itemMatches = [...raw.matchAll(/<item\b[\s\S]*?<\/item>/gi)];

  for (const match of itemMatches) {
    const itemXml = String(match[0] || '');
    if (!itemXml) continue;
    ...
    if (guid) {
      const guidValueMatch = itemXml.match(/<guid\b[^>]*>([\s\S]*?)<\/guid>/i)
        || itemXml.match(/<id\b[^>]*>([\s\S]*?)<\/id>/i);
      const guidValue = guidValueMatch ? decodeHtmlEntities(String(guidValueMatch[1] || '').trim()) : '';
      guidMatch = !!guidValue && guidValue === guid;
    }

    if (normalizedAudio) {
      const enclosureMatch = itemXml.match(/<enclosure\b[^>]*\burl\s*=\s*["']([^"']+)["']/i);
      const mediaMatch = itemXml.match(/<(?:[\w-]+:)?content\b[^>]*\burl\s*=\s*["']([^"']+)["']/i);
      const linkMatch = itemXml.match(/<(?:audio|source|link)\b[^>]*(?:\bsrc|\bhref)\s*=\s*["']([^"']+)["']/i);
      const candidateUrls = [enclosureMatch, mediaMatch, linkMatch]
        .map((m) => normalizeXmlLinkHref(m && m[1] ? m[1] : '', baseUrl))
        .filter(Boolean);
      audioMatch = candidateUrls.includes(normalizedAudio);
    }

    if (!guidMatch && !audioMatch) continue;
    const sources = extractTranscriptSourcesFromRawItemXml(itemXml, baseUrl);
...
function pickPreferredTranscriptSource(list) {
  const sources = Array.isArray(list) ? list.filter(Boolean) : [];
  if (!sources.length) return null;
  const scored = sources.map((item) => {
    const type = String(item.type || '').trim().toLowerCase();
    let score = 10;
    if (type.includes('vtt')) score = 120;
    else if (type.includes('srt') || type.includes('subrip')) score = 110;
    else if (type.includes('text/plain')) score = 90;
    return { item, score };
  }).sort((a, b) => b.score - a.score);
  return scored[0] ? scored[0].item : null;
}

function sortTranscriptSourcesByPreference(list) {
  const sources = Array.isArray(list) ? list.filter(Boolean) : [];
...
function parseTranscriptTimestamp(value) {
  ...
  if (nums.some((num) => !Number.isFinite(num))) return NaN;
  if (parts.length === 2) {
...
function normalizeTranscriptCueText(text) {
...
function parseVttOrSrtTranscript(text) {
  ...
  blocks.forEach((block) => {
    ...
    const end = parseTranscriptTimestamp(match[2]);
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) return;
    const cueText = normalizeTranscriptCueText(lines.slice(timingIndex + 1).join(' '));
    if (!cueText) return;
...
function parseTimestampedPlainTranscript(text) {
  const lines = String(text || '').replace(/\r/g, '').split('\n').map((line) => line.trim()).filter(Boolean);
  const cues = [];
  lines.forEach((line) => {
    const match = line.match(/^([0-9]{1,2}:\d{2}(?::\d{2})?(?:[.,]\d{1,3})?)\s+(.*)$/);
    if (!match) return;
    const start = parseTranscriptTimestamp(match[1]);
    const cueText = normalizeTranscriptCueText(match[2]);
    if (!Number.isFinite(start) || !cueText) return;
    cues.push({ start, end: start + 6, text: cueText });
  });
...
function parseTranscriptTextToCues(text, type = '') {
  const safeType = String(type || '').trim().toLowerCase();
  if (safeType.includes('vtt') || safeType.includes('srt') || safeType.includes('subrip')) {
    return parseVttOrSrtTranscript(text);
  }
  const fromBlocks = parseVttOrSrtTranscript(text);
  if (fromBlocks.length) return fromBlocks;
  return parseTimestampedPlainTranscript(text);
}
...
function parseCastboxRssFeed(xmlText, rssUrl) {
  ...
  const rawTranscriptSourcesByIndex = atomMode ? [] : extractTranscriptSourcesByItemIndex(rawText, String(rssUrl || '').trim());
  ...
  const items = sourceNodes
    .map((itemNode, index) => {
      ...
      const transcriptSources = extractEpisodeTranscriptSources(itemNode, String(rssUrl || '').trim());
      ...
      return {
        ...
        duration,
        link,
        transcripts: transcriptSources.length ? transcriptSources : (rawTranscriptSourcesByIndex[index] || []),
        pubDateRaw,
        pubDate: Number.isFinite(pubDate) ? pubDate : 0
      };
    })
...
async function loadCastboxRssFeed(rssUrl, options = {}) {
  ...
  if (!forceRefresh) {
    try {
      const record = await idbGet(IDB_META_STORE, rssFeedCacheMetaKey(key));
      const cachedFeed = record && record.value && typeof record.value === 'object' ? record.value.feed : null;
      const updatedAt = Number(record && record.updatedAt) || 0;
      if (cachedFeed && updatedAt && (Date.now() - updatedAt) < CASTBOX_RSS_CACHE_TTL_MS) {
        castboxRssFeedCache.set(key, { updatedAt, feed: cachedFeed });
        return cachedFeed;
      }
    } catch (error) {
      console.warn('RSS feed cache read failed:', error);
    }
  }
  ...
  const task = (async () => {
    ...
    try {
      const payload = { key: rssFeedCacheMetaKey(key), value: { feed: normalizedFeed }, updatedAt: now };
      await idbPut(IDB_META_STORE, payload);
      if (finalFeedUrl && finalFeedUrl !== key) {
        await idbPut(IDB_META_STORE, { key: rssFeedCacheMetaKey(finalFeedUrl), value: { feed: normalizedFeed }, updatedAt: now });
      }
    } catch (error) {
      console.warn('RSS feed cache write failed:', error);
    }
...
function formatCastboxEpisodeMeta(ep) {
  ...
  if (ep && ep.duration) parts.push(formatLooseMediaDurationLabel(ep.duration));
  return parts.join(' · ');
}
...
function formatLooseMediaDurationLabel(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  if (/^\d+(?:\.\d+)?$/.test(raw)) {
    return formatCastboxAudioTime(Number(raw));
  }
  return raw;
}

function parseLooseMediaDurationSeconds(value) {
  const raw = String(value || '').trim();
  if (!raw) return 0;
...
function formatPublishedDisplayDate(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
...
function normalizeLooseMediaMetaText(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  if (!raw.includes('·')) return formatLooseMediaDurationLabel(raw);
  return raw
    .split('·')
    .map((part) => formatLooseMediaDurationLabel(part))
    .filter(Boolean)
    .join(' · ');
}

function buildCompactMediaMetaText(publishedText = '', durationText = '', fallbackText = '') {
  const parts = [];
  const published = String(publishedText || '').trim();
  const duration = formatLooseMediaDurationLabel(durationText);
  const fallback = normalizeLooseMediaMetaText(fallbackText);
...
function buildCastboxEpisodeRowMarkup(ep, index, activeSrc = '') {
  ...
  const rawAudioUrl = String(ep.audioUrl || '').trim();
  const audioUrl = escapeHtml(rawAudioUrl);
  const itemLink = escapeHtml(String(ep.link || ''));
  const guid = escapeHtml(String(ep.guid || ep.audioUrl || `${index}`));
  const isActive = activeSrc && rawAudioUrl && activeSrc === rawAudioUrl;
  const cls = isActive ? 'castbox-rss-item is-active' : 'castbox-rss-item';
  const addLabel = escapeHtml(tOr('playlistMode', 'Queue'));
  return `<li class="castbox-rss-item-row"><button class="${cls}" type="button" data-castbox-ep-index="${index}" data-castbox-ep-src="${audioUrl}" data-castbox-ep-guid="${guid}" data-castbox-ep-title="${title}" data-castbox-ep-meta="${meta}" ${itemLink ? `data-castbox-ep-link="${itemLink}"` : ''}><span class="castbox-rss-item-index">#${index + 1}</span><span class="castbox-rss-item-body"><span class="castbox-rss-item-title">${title}</span>${meta ? `<span class="castbox-rss-item-meta">${meta}</span>` : ''}</span></button><button class="castbox-rss-item-add" type="button" data-castbox-queue-add="${index}" aria-label="${addLabel}" title="${addLabel}">${getRssQueueAddSvg()}</button></li>`;
}

function buildGlobalRssQueueItemFromEpisodeButton(root, btn) {
  if (!root || !btn) return null;
  const feed = root.__castboxFeed && typeof root.__castboxFeed === 'object' ? root.__castboxFeed : {};
  const src = String(btn.getAttribute('data-castbox-ep-src') || '').trim();
  const guid = String(btn.getAttribute('data-castbox-ep-guid') || '').trim() || src;
  const title = String(btn.getAttribute('data-castbox-ep-title') || '').trim();
  if (!src || !title) return null;
  return normalizeGlobalRssQueueItem({
    id: `${normalizeFavoriteFeedUrl(root.__castboxRssUrl || feed.rssUrl || '') || 'feed'}::${guid}`,
    src,
    guid,
    title,
    meta: String(btn.getAttribute('data-castbox-ep-meta') || '').trim(),
    link: String(btn.getAttribute('data-castbox-ep-link') || '').trim(),
    feedUrl: String(root.__castboxRssUrl || feed.rssUrl || '').trim(),
    podcastTitle: String(feed.title || root.__castboxFallbackTitle || '').trim(),
    accentPlatform: String(root.__castboxPlatform || 'rss').trim().toLowerCase() || 'rss',
    image: String(feed.image || '').trim()
  });
}

function syncCastboxQueueAddButtons(root) {
  if (!root) return;
  const globalPlayer = globalRssPlayerController.ensureMounted();
  root.querySelectorAll('[data-castbox-queue-add]').forEach((btn) => {
    const itemBtn = btn.previousElementSibling;
    const item = itemBtn ? buildGlobalRssQueueItemFromEpisodeButton(root, itemBtn) : null;
    const queued = !!(item && globalPlayer.findItemIndex(item) >= 0);
    btn.classList.toggle('is-added', queued);
    btn.setAttribute('aria-pressed', queued ? 'true' : 'false');
    btn.setAttribute('title', queued ? 'Added to queue' : 'Add to queue');
    btn.setAttribute('aria-label', queued ? 'Added to queue' : 'Add to queue');
    btn.innerHTML = queued ? getRssQueueAddedSvg() : getRssQueueAddSvg();
  });
}
...
function buildCastboxEpisodesRowsMarkupByIndexes(episodes, indexes, activeSrc = '') {
  const list = Array.isArray(episodes) ? episodes : [];
  const picks = Array.isArray(indexes) ? indexes : [];
  const htmlParts = [];
  picks.forEach((index) => {
    const safeIndex = Number(index);
    if (!Number.isFinite(safeIndex) || safeIndex < 0 || safeIndex >= list.length) return;
    htmlParts.push(buildCastboxEpisodeRowMarkup(list[safeIndex], safeIndex, activeSrc));
  });
  return htmlParts.join('');
}
...
const rssTranscriptCache = new Map();
const rssTranscriptPending = new Map();
const RSS_SKIP_BACK_SECONDS = 15;
const RSS_SKIP_FORWARD_SECONDS = 30;

function formatRssPlaybackRateLabel(rate) {
  const safeRate = Number(rate) || 1;
  return Number.isInteger(safeRate) ? `${safeRate}x` : `${safeRate.toFixed(2).replace(/\.?0+$/, '')}x`;
}

function clampRssPlaybackRate(rate) {
  return Math.max(0.5, Math.min(4, Number(rate) || 1));
}

function clampRssVolumeLevel(volume) {
  return Math.max(0, Math.min(1, Number(volume) || 0));
}

function formatRssVolumeLabel(volume) {
  return `${Math.round(clampRssVolumeLevel(volume) * 100)}%`;
}
...
function getHorizontalGapPx(el) {
...
function measureChildrenInlineWidth(el) {
  if (!el) return 0;
  const children = Array.from(el.children || []).filter((node) => {
    if (!(node instanceof HTMLElement)) return false;
    return node.offsetParent !== null;
  });
  if (!children.length) return 0;
  const gap = getHorizontalGapPx(el);
  const widths = children.reduce((sum, node) => sum + node.getBoundingClientRect().width, 0);
  return widths + Math.max(0, children.length - 1) * gap;
}

function applyResponsiveActionTrim(actionsWrap, maxTrim = 4) {
  if (!actionsWrap) return;
  const availableWidth = actionsWrap.getBoundingClientRect().width || actionsWrap.clientWidth || 0;
  let trim = 0;
  actionsWrap.dataset.trim = '0';
  while (trim < maxTrim && measureChildrenInlineWidth(actionsWrap) > availableWidth + 1) {
    trim += 1;
    actionsWrap.dataset.trim = String(trim);
  }
}

function clearResponsiveTrimHidden(actionsWrap) {
  if (!actionsWrap) return;
  Array.from(actionsWrap.children || []).forEach((node) => {
    if (!(node instanceof HTMLElement)) return;
    if (node.dataset.trimHidden === 'true') {
      node.dataset.trimHidden = 'false';
      node.style.removeProperty('display');
    }
  });
}

function applyResponsiveActionTrimWithCandidates(actionsWrap, candidates = []) {
  if (!actionsWrap) return;
  clearResponsiveTrimHidden(actionsWrap);
  const availableWidth = actionsWrap.getBoundingClientRect().width || actionsWrap.clientWidth || 0;
  let hiddenCount = 0;
  for (const candidate of candidates) {
    if (!(candidate instanceof HTMLElement) || candidate.offsetParent === null) continue;
    if (measureChildrenInlineWidth(actionsWrap) <= availableWidth + 1) break;
    candidate.dataset.trimHidden = 'true';
    candidate.style.display = 'none';
    hiddenCount += 1;
  }
  actionsWrap.dataset.trim = String(hiddenCount);
}

const INLINE_FEED_RESOLVE_CACHE_KEY = 'podcast_inline_feed_resolve_v1';
const INLINE_FEED_RESOLVE_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000;
const INLINE_FEED_RESOLVE_CACHE_MAX = 800;
let inlineFeedResolvePersistentLoaded = false;
const inlineFeedResolvePersistentCache = new Map();

function loadInlineFeedResolvePersistentCache() {
  if (inlineFeedResolvePersistentLoaded) return;
  inlineFeedResolvePersistentLoaded = true;
  if (typeof localStorage === 'undefined') return;
  try {
    const raw = localStorage.getItem(INLINE_FEED_RESOLVE_CACHE_KEY);
    if (!raw) return;
    const parsed = JSON.parse(raw);
    ...
    parsed.forEach((entry) => {
      ...
      if (!key || !updatedAt || (now - updatedAt) > INLINE_FEED_RESOLVE_CACHE_TTL_MS) return;
      inlineFeedResolvePersistentCache.set(key, { feedUrl, updatedAt });
    });
  } catch {}
}

function persistInlineFeedResolvePersistentCache() {
  if (typeof localStorage === 'undefined') return;
  try {
    const now = Date.now();
    const entries = Array.from(inlineFeedResolvePersistentCache.entries())
      .map(([key, value]) => ({
        key,
        feedUrl: String(value && value.feedUrl || '').trim(),
        updatedAt: Number(value && value.updatedAt) || 0
      }))
      .filter((entry) => entry.key && entry.updatedAt && (now - entry.updatedAt) <= INLINE_FEED_RESOLVE_CACHE_TTL_MS)
      .sort((a, b) => b.updatedAt - a.updatedAt)
      .slice(0, INLINE_FEED_RESOLVE_CACHE_MAX);
    localStorage.setItem(INLINE_FEED_RESOLVE_CACHE_KEY, JSON.stringify(entries));
  } catch {}
}

function getInlineFeedResolveCachedValue(key) {
  const safeKey = String(key || '').trim();
  if (!safeKey) return null;
  loadInlineFeedResolvePersistentCache();
  const cached = inlineFeedResolvePersistentCache.get(safeKey);
  if (!cached) return null;
  if ((Date.now() - cached.updatedAt) > INLINE_FEED_RESOLVE_CACHE_TTL_MS) {
    inlineFeedResolvePersistentCache.delete(safeKey);
    persistInlineFeedResolvePersistentCache();
    return null;
  }
  return cached;
}

function setInlineFeedResolveCachedValue(key, feedUrl) {
  const safeKey = String(key || '').trim();
  if (!safeKey) return;
  loadInlineFeedResolvePersistentCache();
  const safeFeedUrl = String(feedUrl || '').trim();
  if (!safeFeedUrl) {
    inlineFeedResolvePersistentCache.delete(safeKey);
    persistInlineFeedResolvePersistentCache();
    return;
  }
  inlineFeedResolvePersistentCache.set(safeKey, {
    feedUrl: safeFeedUrl,
    updatedAt: Date.now()
  });
  persistInlineFeedResolvePersistentCache();
...
function clearInlineFeedResolveCachedValue(key) {
  const safeKey = String(key || '').trim();
  if (!safeKey) return;
  loadInlineFeedResolvePersistentCache();
  if (!inlineFeedResolvePersistentCache.has(safeKey)) return;
  inlineFeedResolvePersistentCache.delete(safeKey);
  persistInlineFeedResolvePersistentCache();
}

function buildCastboxSpeedControlMarkup(activeRate = 1) {
  const currentRate = clampRssPlaybackRate(activeRate);
  return `
    <div class="castbox-rss-speed-control" data-castbox-speed-control data-open="false">
      <button class="castbox-rss-stepper-btn" type="button" data-castbox-speed-decrease aria-label="Decrease playback speed" title="Decrease playback speed">
        ${escapeHtml('−')}
      </button>
      <button class="castbox-rss-speed" type="button" data-castbox-speed-trigger aria-label="Playback speed" title="Playback speed" aria-expanded="false">
        ${escapeHtml(formatRssPlaybackRateLabel(currentRate))}
      </button>
      <button class="castbox-rss-stepper-btn" type="button" data-castbox-speed-increase aria-label="Increase playback speed" title="Increase playback speed">
        ${escapeHtml('+')}
      </button>
      <input class="castbox-rss-speed-slider" data-castbox-speed-slider type="range" min="0.5" max="4" step="0.05" value="${currentRate}" aria-label="Playback speed">
    </div>
  `;
}

function buildCastboxSearchControlMarkup() {
  return `
    <div class="castbox-rss-search-control" data-castbox-search-control data-open="false">
      <button class="castbox-rss-search-btn" type="button" data-castbox-search-trigger aria-label="${escapeHtml(tOr('searchPlaceholder', 'Search episodes'))}" title="${escapeHtml(tOr('searchPlaceholder', 'Search episodes'))}" aria-expanded="false">
        ${getRssSearchSvg()}
      </button>
      <input class="castbox-rss-search-input" data-castbox-search-input type="search" inputmode="search" autocomplete="off" spellcheck="false" placeholder="${escapeHtml(tOr('searchPlaceholder', 'Search episodes'))}" aria-label="${escapeHtml(tOr('searchPlaceholder', 'Search episodes'))}">
    </div>
  `;
}

function buildAppleRssModeControlsMarkup(mode = 'apple', canUseRss = true) {
  const activeMode = String(mode || 'apple').trim().toLowerCase() === 'rss' ? 'rss' : 'apple';
  const rssDisabled = canUseRss ? '' : ' disabled aria-disabled="true"';
  return `
    <div class="episode-player-mode-switch" data-episode-mode-switch data-mode="${activeMode}">
      <button class="episode-player-mode-btn${activeMode === 'apple' ? ' is-active' : ''}" type="button" data-episode-mode="apple" aria-label="Apple Podcasts" title="Apple Podcasts" aria-pressed="${activeMode === 'apple' ? 'true' : 'false'}">
        ${getAppleModeSvg()}
      </button>
      <button class="episode-player-mode-btn${activeMode === 'rss' ? ' is-active' : ''}" type="button" data-episode-mode="rss" aria-label="RSS" title="RSS"${rssDisabled} aria-pressed="${activeMode === 'rss' ? 'true' : 'false'}">
        ${getRssFeedSvg()}
      </button>
    </div>
  `;
...
function buildPlatformRssModeControlsMarkup(platform = 'spotify', mode = 'platform', canUseRss = true) {
  const safePlatform = String(platform || 'spotify').trim().toLowerCase() || 'spotify';
  const activeMode = String(mode || 'platform').trim().toLowerCase() === 'rss' ? 'rss' : safePlatform;
  const rssDisabled = canUseRss ? '' : ' disabled aria-disabled="true"';
  return `
    <div class="episode-player-mode-switch" data-episode-mode-switch data-mode="${escapeHtml(activeMode)}">
      <button class="episode-player-mode-btn${activeMode === safePlatform ? ' is-active' : ''}" type="button" data-episode-mode="${escapeHtml(safePlatform)}" aria-label="${escapeHtml(getMyPodcastsImportModeLabel(safePlatform) || safePlatform)}" title="${escapeHtml(getMyPodcastsImportModeLabel(safePlatform) || safePlatform)}" aria-pressed="${activeMode === safePlatform ? 'true' : 'false'}">
        ${getMyPodcastsImportModeIconSvg(safePlatform)}
      </button>
      <button class="episode-player-mode-btn${activeMode === 'rss' ? ' is-active' : ''}" type="button" data-episode-mode="rss" aria-label="RSS" title="RSS"${rssDisabled} aria-pressed="${activeMode === 'rss' ? 'true' : 'false'}">
        ${getRssFeedSvg()}
      </button>
...
function buildPlatformQueueControlsMarkup(platform = 'youtube', queued = false) {
  const safePlatform = String(platform || 'youtube').trim().toLowerCase() || 'youtube';
  const addTitle = queued ? 'Added to queue' : 'Add to queue';
  return `
    <div class="episode-player-mode-switch" data-episode-mode-switch data-mode="${escapeHtml(safePlatform)}">
      <button class="episode-player-mode-btn is-active" type="button" data-episode-mode="${escapeHtml(safePlatform)}" aria-label="${escapeHtml(getMyPodcastsImportModeLabel(safePlatform) || safePlatform)}" title="${escapeHtml(getMyPodcastsImportModeLabel(safePlatform) || safePlatform)}" aria-pressed="true">
        ${getMyPodcastsImportModeIconSvg(safePlatform)}
      </button>
      <button class="episode-player-mode-btn${queued ? ' is-active is-queue-added' : ''}" type="button" data-episode-queue-add aria-label="${addTitle}" title="${addTitle}" aria-pressed="${queued ? 'true' : 'false'}">
        ${queued ? getRssQueueAddedSvg() : getRssQueueAddSvg()}
      </button>
    </div>
  `;
}

function buildSourceRssPlayerModeAreaMarkup(primaryPlatform = 'apple', mode = 'platform', canUseRss = true, descriptionMarkup = '', includeAd = true) {
  const safePrimaryPlatform = String(primaryPlatform || 'apple').trim().toLowerCase() || 'apple';
  const safeMode = String(mode || '').trim().toLowerCase();
  const resolvedMode = safeMode === 'rss' ? 'rss' : safePrimaryPlatform;
  const controls = safePrimaryPlatform === 'apple'
    ? buildAppleRssModeControlsMarkup(resolvedMode, canUseRss)
    : buildPlatformRssModeControlsMarkup(safePrimaryPlatform, resolvedMode, canUseRss);
  return `
    <div class="episode-player-mode-host" data-episode-mode-host></div>
    <div class="episode-player-mode-controls-wrap">
      ${controls}
    </div>
    ${descriptionMarkup}
    ${includeAd ? buildPlayerAdMarkup() : ''}
  `;
}

function getPlatformAccentPair(platform) {
  const topicOverride = getTopicAccentOverridePalette();
  if (Array.isArray(topicOverride) && topicOverride.length) {
    return [String(topicOverride[0] || '#ff435f'), String(topicOverride[1] || topicOverride[0] || '#ff5d67')];
  }
  const safePlatform = String(platform || '').trim().toLowerCase();
  return PLATFORM_ACCENT_COLORS[safePlatform] || PLATFORM_ACCENT_COLORS.apple || ['#6F8BFF', '#00C5B0'];
}

function applyPlayerAccentVars(node, platform) {
  if (!node || !node.style) return;
  const accentPair = getPlatformAccentPair(platform);
  const c1 = String(accentPair[0] || '#6F8BFF');
  const c2 = String(accentPair[1] || accentPair[0] || '#00C5B0');
  node.style.setProperty('--rss-accent', c1);
  node.style.setProperty('--rss-accent-soft', c2);
}

function getCurrentPlayerAccentPlatform(row, fallback = 'apple') {
  const selectedPlatform = String((platformSelect && platformSelect.value) || '').trim().toLowerCase();
  if (selectedPlatform && selectedPlatform !== 'all') return selectedPlatform;
  const favoriteOriginPlatform = String(row?.querySelector('.favorite-link')?.getAttribute('data-favorite-origin-platform') || '').trim().toLowerCase();
  if (favoriteOriginPlatform) return favoriteOriginPlatform;
  return String(fallback || 'apple').trim().toLowerCase() || 'apple';
}

const GLOBAL_RSS_PLAYER_STORAGE_KEY = 'global_rss_player_v1';
const GLOBAL_RSS_PLAYER_QUEUE_LIMIT = 240;
const GLOBAL_RSS_PLAYER_EVENT = 'global-rss-player-update';
const AUDIO_MODE_RETRY_DELAYS_MS = [1200, 3200];
const audioModeJobQueue = [];
let audioModeProcessing = false;
let audioModeActiveSignature = '';

function normalizeGlobalRssQueueItem(raw = {}) {
  const platform = String(raw.platform || raw.embedPlatform || raw.originPlatform || '').trim().toLowerCase();
  const isYouTube = platform === 'youtube';
  const isBilibili = platform === 'bilibili';
  const src = String(raw.src || raw.audioUrl || '').trim();
  const feedUrl = normalizeFavoriteFeedUrl(raw.feedUrl || '');
  const videoId = String(raw.videoId || '').trim();
  const playlistId = String(raw.playlistId || '').trim();
  const aid = normalizeBilibiliAid(raw.aid || '');
  const bvid = String(raw.bvid || '').trim();
  const cid = String(raw.cid || '').trim();
  const guid = String(raw.guid || '').trim() || src || videoId || playlistId || aid || bvid || cid;
  const title = String(raw.title || '').trim();
  const embedUrl = String(raw.embedUrl || '').trim() || (isBilibili ? buildBilibiliEmbedUrl(aid, bvid, cid) : '');
  if ((!src && !isYouTube && !isBilibili) || !title || (isYouTube && !videoId && !playlistId) || (isBilibili && !embedUrl)) return null;
  const accentPlatform = String(raw.accentPlatform || raw.embedPlatform || raw.originPlatform || platform || 'rss').trim().toLowerCase() || 'rss';
  const publishedText = String(raw.publishedText || '').trim();
  const durationText = formatLooseMediaDurationLabel(raw.durationText || raw.duration || '');
  const durationSeconds = Math.max(0, Number(raw.durationSeconds) || parseLooseMediaDurationSeconds(raw.durationText || raw.duration || ''));
  const fallbackPodcastTitle = isYouTube ? String(raw.meta || '').trim() : '';
  const podcastTitle = String(raw.podcastTitle || raw.feedTitle || fallbackPodcastTitle).trim();
  const meta = buildCompactMediaMetaText(publishedText, durationText, String(raw.meta || '').trim() !== podcastTitle ? raw.meta : '');
  return {
    id: String(raw.id || `${isYouTube ? 'youtube' : isBilibili ? 'bilibili' : (feedUrl || 'feed')}::${guid}`).trim(),
    src,
    guid,
    title,
    meta,
    ...
    feedUrl,
    platform: isYouTube ? 'youtube' : isBilibili ? 'bilibili' : 'audio',
    videoId,
    playlistId,
    embedUrl,
    aid,
...
function readGlobalRssPlayerState() {
  try {
    const raw = localStorage.getItem(GLOBAL_RSS_PLAYER_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
...
function writeGlobalRssPlayerState(state) {
  try {
    localStorage.setItem(GLOBAL_RSS_PLAYER_STORAGE_KEY, JSON.stringify(state || {}));
  } catch { }
}

function readDefaultRssPlayerModePreference() {
  try {
    const raw = localStorage.getItem(RSS_DEFAULT_PLAYER_MODE_STORAGE_KEY);
    return String(raw || '').trim().toLowerCase() === 'rss' ? 'rss' : '';
  } catch {
    return '';
  }
}

function getDefaultRssPlayerMode(platform = '') {
  const safePlatform = String(platform || '').trim().toLowerCase();
  if (safePlatform !== 'apple' && safePlatform !== 'spotify') return safePlatform;
  return readDefaultRssPlayerModePreference() === 'rss' ? 'rss' : safePlatform;
}

function setDefaultRssPlayerModePreference(mode = '') {
  const safeMode = String(mode || '').trim().toLowerCase() === 'rss' ? 'rss' : '';
  try {
    if (safeMode) localStorage.setItem(RSS_DEFAULT_PLAYER_MODE_STORAGE_KEY, safeMode);
    else localStorage.removeItem(RSS_DEFAULT_PLAYER_MODE_STORAGE_KEY);
  } catch {}
}

function rssFeedCacheMetaKey(feedUrl = '') {
  const safe = normalizeFavoriteFeedUrl(feedUrl);
  return safe ? `${IDB_META_KEY_RSS_FEED_CACHE_PREFIX}${safe}` : '';
}

function buildMediaSessionArtworkList(imageUrl = '') {
...
function buildGlobalRssPlayerMarkup() {
  return `
    <div class="global-rss-player" data-global-rss-player hidden>
      <audio class="global-rss-audio" preload="none"></audio>
      <div class="global-rss-youtube-host" data-global-rss-youtube-host aria-hidden="true"></div>
      <div class="global-rss-bilibili-host" data-global-rss-bilibili-host aria-hidden="true"></div>
      <div class="global-rss-player-inner">
        <div class="global-rss-summary" data-global-rss-summary-toggle role="button" tabindex="0" aria-label="Open player details">
          <div class="global-rss-cover-wrap global-rss-cover-wrap-summary">
            <img class="global-rss-cover" data-global-rss-summary-cover src="" alt="" hidden>
          </div>
          <div class="global-rss-summary-copy">
            <div class="global-rss-summary-kicker" data-global-rss-summary-kicker></div>
            <div class="global-rss-summary-title" data-global-rss-summary-title></div>
          </div>
          <div class="global-rss-summary-actions">
            <button class="global-rss-btn global-rss-btn-play" type="button" data-global-rss-summary-play aria-label="${escapeHtml(t('playEpisode'))}" title="${escapeHtml(t('playEpisode'))}">
              ${getRssPlayerPlayPauseSvg()}
            </button>
            <button class="global-rss-btn" type="button" data-global-rss-summary-next aria-label="Next" title="Next">
              ${getTransportNextSvg()}
            </button>
          </div>
        </div>
        <button class="global-rss-collapse" type="button" data-global-rss-collapse aria-label="Collapse player">
          ${getGlobalPlayerCollapseSvg()}
        </button>
        <div class="global-rss-main">
          <div class="global-rss-cover-wrap">
            <img class="global-rss-cover" data-global-rss-cover src="" alt="" hidden>
          </div>
          <div class="global-rss-info">
            <div class="global-rss-kicker" data-global-rss-kicker></div>
            <div class="global-rss-title" data-global-rss-title></div>
            <div class="global-rss-meta" data-global-rss-meta></div>
          </div>
          <div class="global-rss-controls">
            <button class="global-rss-btn" type="button" data-global-rss-prev aria-label="Previous" title="Previous">
              ${getTransportPrevSvg()}
            </button>
            <button class="global-rss-btn global-rss-seek-btn global-rss-seek-back" type="button" data-global-rss-seek-back aria-label="Back 15 seconds" title="Back 15 seconds">
              ${getRssSeekBackSvg()}
            </button>
            <button class="global-rss-btn global-rss-btn-play" type="button" data-global-rss-play aria-label="${escapeHtml(t('playEpisode'))}" title="${escapeHtml(t('playEpisode'))}">
              ${getRssPlayerPlayPauseSvg()}
            </button>
            <button class="global-rss-btn global-rss-seek-btn global-rss-seek-forward" type="button" data-global-rss-seek-forward aria-label="Forward 30 seconds" title="Forward 30 seconds">
              ${getRssSeekForwardSvg()}
            </button>
            <button class="global-rss-btn" type="button" data-global-rss-next aria-label="Next" title="Next">
              ${getTransportNextSvg()}
            </button>
          </div>
          <div class="global-rss-progress-wrap">
            <input class="global-rss-progress" data-global-rss-progress type="range" min="0" max="1000" step="1" value="0" aria-label="Playback progress">
            <div class="global-rss-time"><span data-global-rss-time-current>00:00</span><span>/</span><span data-global-rss-time-total>00:00</span></div>
          </div>
          <div class="global-rss-actions">
            <div class="global-rss-inline-control" data-global-rss-volume data-open="false">
              <button class="global-rss-stepper-btn" type="button" data-global-rss-volume-decrease aria-label="Decrease volume" title="Decrease volume">
                ${escapeHtml('−')}
              </button>
              <button class="global-rss-btn" type="button" data-global-rss-volume-btn aria-label="Volume" title="Volume" aria-expanded="false">
                ${getRssVolumeSvg()}
                <span class="global-rss-inline-value global-rss-inline-value-volume" data-global-rss-volume-value>100%</span>
              </button>
              <button class="global-rss-stepper-btn" type="button" data-global-rss-volume-increase aria-label="Increase volume" title="Increase volume">
                ${escapeHtml('+')}
              </button>
              <input class="global-rss-inline-slider" data-global-rss-volume-slider type="range" min="0" max="100" step="1" value="100" aria-label="Volume">
            </div>
            <div class="global-rss-inline-control" data-global-rss-speed data-open="false">
              <button class="global-rss-stepper-btn" type="button" data-global-rss-speed-decrease aria-label="Decrease playback speed" title="Decrease playback speed">
                ${escapeHtml('−')}
              </button>
              <button class="global-rss-btn global-rss-btn-label" type="button" data-global-rss-speed-btn aria-label="Playback speed" title="Playback speed" aria-expanded="false">1x</button>
              <button class="global-rss-stepper-btn" type="button" data-global-rss-speed-increase aria-label="Increase playback speed" title="Increase playback speed">
                ${escapeHtml('+')}
              </button>
              <input class="global-rss-inline-slider" data-global-rss-speed-slider type="range" min="0.5" max="4" step="0.05" value="1" aria-label="Playback speed">
            </div>
            <button class="global-rss-btn global-rss-seek-btn global-rss-seek-back global-rss-mobile-seek" type="button" data-global-rss-mobile-seek-back aria-label="Back 15 seconds" title="Back 15 seconds">
              ${getRssSeekBackSvg()}
            </button>
            <button class="global-rss-btn global-rss-seek-btn global-rss-seek-forward global-rss-mobile-seek" type="button" data-global-rss-mobile-seek-forward aria-label="Forward 30 seconds" title="Forward 30 seconds">
              ${getRssSeekForwardSvg()}
            </button>
            <button class="global-rss-btn global-rss-queue-btn" type="button" data-global-rss-queue-toggle aria-label="Queue" title="Queue" aria-expanded="false">
              ${getRssQueueSvg()}
              <span class="global-rss-queue-count" data-global-rss-queue-count>0</span>
            </button>
          </div>
        </div>
        <div class="global-rss-queue" data-global-rss-queue hidden>
          <div class="global-rss-queue-head">
            <strong>Up Next</strong>
            <span data-global-rss-queue-summary>0</span>
          </div>
          <ol class="global-rss-queue-list" data-global-rss-queue-list></ol>
        </div>
      </div>
    </div>
  `;
...
const globalRssPlayerController = {
  mounted: false,
  root: null,
  audio: null,
  youtubeHost: null,
  ...
  bilibiliElapsedSeconds: 0,
  bilibiliStartedAtMs: 0,
  bilibiliProgressTimer: 0,
  queue: [],
  currentIndex: -1,
  queueOpen: false,
  ...
  ensureMounted() {
    if (this.mounted && this.root && this.audio) return this;
    const host = document.createElement('div');
    host.innerHTML = buildGlobalRssPlayerMarkup();
    this.root = host.firstElementChild;
    document.body.appendChild(this.root);
    this.audio = this.root.querySelector('.global-rss-audio');
    if (this.audio) {
    }
    this.youtubeHost = this.root.querySelector('[data-global-rss-youtube-host]');
    this.bilibiliHost = this.root.querySelector('[data-global-rss-bilibili-host]');
    this.bind();
    ...
    try {
      if (!item) {
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
          title: String(item.title || '').trim() || document.title || 'Podcast',
          artist: String(item.podcastTitle || item.meta || '').trim() || document.title || 'Podcast',
          album: String(item.meta || '').trim() || String(item.accentPlatform || '').trim(),
          artwork: buildMediaSessionArtworkList(item.image)
        });
      }
    } catch { }
    try {
      navigator.mediaSession.playbackState = item
        ? (this.isMediaPaused() ? 'paused' : 'playing')
        : 'none';
    } catch { }
    try {
      const duration = this.getMediaDuration();
      const position = this.getMediaCurrentTime();
      const playbackRate = this.getMediaPlaybackRate();
      if (Number.isFinite(duration) && duration > 0 && Number.isFinite(position) && position >= 0 && Number.isFinite(playbackRate) && playbackRate > 0 && typeof navigator.mediaSession.setPositionState === 'function') {
  ...
  bindMediaSessionHandlers() {
    if (this.mediaSessionBound || typeof navigator === 'undefined' || !navigator.mediaSession || typeof navigator.mediaSession.setActionHandler !== 'function') return;
    const safeSet = (action, handler) => {
      try { navigator.mediaSession.setActionHandler(action, handler); } catch { }
    };
    safeSet('play', () => { this.playCurrent(); });
    safeSet('pause', () => {
      if (this.isYouTubeItem() && this.youtubePlayer && typeof this.youtubePlayer.pauseVideo === 'function') this.youtubePlayer.pauseVideo();
      else if (this.isBilibiliItem()) {
        this.stopBilibiliPlayback(false);
        this.persist(true);
        this.emit();
        this.updateUi();
      }
    ...
    safeSet('previoustrack', () => { this.playPrevious(); });
    safeSet('nexttrack', () => { this.playNext(); });
    safeSet('seekbackward', (details = {}) => {
      const offset = Math.max(1, Number(details.seekOffset) || 15);
      this.seekMediaTo(this.getMediaCurrentTime() - offset);
    });
    safeSet('seekforward', (details = {}) => {
      const offset = Math.max(1, Number(details.seekOffset) || 30);
      this.seekMediaTo(this.getMediaCurrentTime() + offset);
    });
    safeSet('seekto', (details = {}) => {
      if (!Number.isFinite(Number(details.seekTime))) return;
      this.seekMediaTo(Number(details.seekTime) || 0);
    });
    safeSet('stop', () => {
      if (this.isYouTubeItem() && this.youtubePlayer && typeof this.youtubePlayer.pauseVideo === 'function') this.youtubePlayer.pauseVideo();
      else if (this.isBilibiliItem()) {
  ...
  isYouTubeItem(item = null) {
    return String((item || this.getCurrentItem() || {}).platform || '').trim().toLowerCase() === 'youtube';
  },
  isBilibiliItem(item = null) {
    return String((item || this.getCurrentItem() || {}).platform || '').trim().toLowerCase() === 'bilibili';
  },
  getMediaCurrentTime() {
    if (this.isBilibiliItem()) {
      const duration = this.getMediaDuration();
      const runningSeconds = this.bilibiliPlaying && this.bilibiliStartedAtMs > 0
        ? Math.max(0, (Date.now() - this.bilibiliStartedAtMs) / 1000)
        : 0;
      const elapsed = Math.max(0, Number(this.bilibiliElapsedSeconds) || 0) + runningSeconds;
      return duration > 0 ? Math.min(duration, elapsed) : elapsed;
    }
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.getCurrentTime === 'function') {
      try { return Number(this.youtubePlayer.getCurrentTime()) || 0; } catch { return 0; }
    }
  ...
  getMediaDuration() {
    if (this.isBilibiliItem()) {
      const item = this.getCurrentItem();
      return Math.max(0, Number(item && item.durationSeconds) || 0);
    }
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.getDuration === 'function') {
      try { return Number(this.youtubePlayer.getDuration()) || 0; } catch { return 0; }
    }
    return Number(this.audio?.duration) || 0;
  },
  isMediaPaused() {
    if (this.isBilibiliItem()) return !this.bilibiliPlaying;
    if (this.isYouTubeItem()) return this.youtubeState !== 1;
    return !this.audio || this.audio.paused || this.audio.ended;
  },
  getMediaVolume() {
    if (this.isBilibiliItem()) return 1;
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.getVolume === 'function') {
      try { return Math.max(0, Math.min(1, (Number(this.youtubePlayer.getVolume()) || 0) / 100)); } catch { return 1; }
    }
    if (!this.audio) return 1;
    const rawVolume = Number(this.audio.volume);
    const safeVolume = Number.isFinite(rawVolume) ? rawVolume : 1;
    return this.audio.muted ? 0 : Math.max(0, Math.min(1, safeVolume));
  },
  getMediaPlaybackRate() {
    if (this.isBilibiliItem()) return 1;
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.getPlaybackRate === 'function') {
      try { return Number(this.youtubePlayer.getPlaybackRate()) || 1; } catch { return 1; }
    }
    return Number(this.audio?.playbackRate) || 1;
  },
  setMediaVolume(nextVolume) {
    ...
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.setVolume === 'function') {
      try { this.youtubePlayer.setVolume(Math.round(safeVolume * 100)); } catch { }
      this.persist(false);
      this.emit();
      this.updateUi();
      return;
    ...
    this.audio.muted = safeVolume <= 0.001;
    this.persist(false);
    this.emit();
    this.updateUi();
  },
  setMediaPlaybackRate(nextRate) {
    const safeRate = Math.max(0.5, Math.min(4, Number(nextRate) || 1));
    if (this.isBilibiliItem()) {
      this.persist(false);
      this.emit();
      this.updateUi();
    ...
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.setPlaybackRate === 'function') {
      try { this.youtubePlayer.setPlaybackRate(safeRate); } catch { }
      this.persist(false);
      this.emit();
      this.updateUi();
      return;
    }
    this.audio.playbackRate = Number(safeRate.toFixed(2));
    this.persist(false);
    this.emit();
    this.updateUi();
  },
  seekMediaTo(seconds) {
    ...
    if (this.isYouTubeItem() && this.youtubePlayer && this.youtubeReady && typeof this.youtubePlayer.seekTo === 'function') {
      try { this.youtubePlayer.seekTo(safeSeconds, true); } catch { }
      this.persist(true);
      this.emit();
      this.updateUi();
      return;
    }
    if (Number.isFinite(this.audio?.duration) && this.audio.duration > 0) this.audio.currentTime = safeSeconds;
  },
  stopYoutubePolling() {
    if (this.youtubePollTimer) {
      clearInterval(this.youtubePollTimer);
      this.youtubePollTimer = 0;
    }
  },
  startYoutubePolling() {
    this.stopYoutubePolling();
    this.youtubePollTimer = window.setInterval(() => {
      if (!this.isYouTubeItem()) return;
      this.persist(false);
      this.emit();
      this.updateUi();
    }, 1000);
  },
  stopBilibiliProgressTimer() {
    if (this.bilibiliProgressTimer) {
      clearInterval(this.bilibiliProgressTimer);
      this.bilibiliProgressTimer = 0;
    }
  },
  syncBilibiliElapsedState() {
    if (!this.bilibiliPlaying || !this.bilibiliStartedAtMs) return;
    const duration = this.getMediaDuration();
    const elapsed = Math.max(0, Number(this.bilibiliElapsedSeconds) || 0) + Math.max(0, (Date.now() - this.bilibiliStartedAtMs) / 1000);
    this.bilibiliElapsedSeconds = duration > 0 ? Math.min(duration, elapsed) : elapsed;
    this.bilibiliStartedAtMs = Date.now();
  },
  resetBilibiliProgressState(seconds = 0) {
    this.stopBilibiliProgressTimer();
    this.bilibiliElapsedSeconds = Math.max(0, Number(seconds) || 0);
    this.bilibiliStartedAtMs = 0;
  },
  startBilibiliProgressTimer() {
    this.stopBilibiliProgressTimer();
    if (!this.isBilibiliItem() || !this.bilibiliPlaying) return;
    if (!this.bilibiliStartedAtMs) this.bilibiliStartedAtMs = Date.now();
    this.bilibiliProgressTimer = window.setInterval(() => {
      if (!this.isBilibiliItem() || !this.bilibiliPlaying) {
        this.stopBilibiliProgressTimer();
        return;
      }
      this.persist(false);
  ...
  stopBilibiliPlayback(clearHost = false) {
    this.syncBilibiliElapsedState();
    this.stopBilibiliProgressTimer();
    this.bilibiliPlaying = false;
    this.bilibiliStartedAtMs = 0;
    if (this.bilibiliIframe) {
      try { this.bilibiliIframe.src = 'about:blank'; } catch {}
    }
    if (clearHost && this.bilibiliHost) {
      this.bilibiliHost.innerHTML = '';
      this.bilibiliHost.removeAttribute('data-current-key');
      this.bilibiliIframe = null;
      this.bilibiliElapsedSeconds = 0;
    }
  },
  async ensureBilibiliPlayerForItem(item, options = {}) {
    if (!this.bilibiliHost || !item) return false;
    const autoplay = options.autoplay !== false;
    const seekTo = Math.max(0, Number(options.seekTo) || 0);
    const baseEmbedUrl = String(item.embedUrl || '').trim() || buildBilibiliEmbedUrl(item.aid, item.bvid, item.cid);
    if (!baseEmbedUrl) return false;
    const nextKey = String(item.id || baseEmbedUrl).trim();
    const playbackUrl = buildBilibiliPlaybackEmbedUrl(baseEmbedUrl, { autoplay, seekTo });
    const currentKey = String(this.bilibiliHost.getAttribute('data-current-key') || '').trim();
    if (this.bilibiliIframe && currentKey === nextKey) {
      try { this.bilibiliIframe.src = playbackUrl; } catch {}
      this.bilibiliElapsedSeconds = Math.max(0, seekTo);
      this.bilibiliPlaying = autoplay;
      this.bilibiliStartedAtMs = autoplay ? Date.now() : 0;
      if (autoplay) this.startBilibiliProgressTimer();
      else this.stopBilibiliProgressTimer();
      this.updateUi();
      this.emit();
      return true;
    }
    this.bilibiliHost.innerHTML = '';
    this.bilibiliHost.setAttribute('data-current-key', nextKey);
    const iframe = document.createElement('iframe');
    iframe.className = 'global-rss-bilibili-frame';
    iframe.src = playbackUrl;
    iframe.setAttribute('title', String(item.title || 'BiliBili').trim() || 'BiliBili');
    iframe.setAttribute('frameborder', '0');
    iframe.setAttribute('scrolling', 'no');
    iframe.setAttribute('allowfullscreen', 'true');
    iframe.setAttribute('allow', 'autoplay; fullscreen');
    iframe.setAttribute('loading', 'eager');
    this.bilibiliHost.appendChild(iframe);
    this.bilibiliIframe = iframe;
    this.bilibiliElapsedSeconds = Math.max(0, seekTo);
    this.bilibiliPlaying = autoplay;
    this.bilibiliStartedAtMs = autoplay ? Date.now() : 0;
    if (autoplay) this.startBilibiliProgressTimer();
  ...
  async ensureYoutubePlayerForItem(item, options = {}) {
    if (!this.youtubeHost || !item) return false;
    const autoplay = options.autoplay !== false;
    const seekTo = Math.max(0, Number(options.seekTo) || 0);
    const initialVolume = Math.max(0, Math.min(1, Number(options.volume)));
    const initialRate = Math.max(0.5, Math.min(4, Number(options.playbackRate) || 1));
    const videoId = String(item.videoId || '').trim();
    const playlistId = String(item.playlistId || '').trim();
    await loadYouTubeIframeApi();
    if (!window.YT || !window.YT.Player) return false;
    const currentKey = String(this.youtubeHost.getAttribute('data-current-key') || '').trim();
    const nextKey = `${playlistId || ''}::${videoId || ''}`;
    if (this.youtubePlayer && currentKey === nextKey && this.youtubeReady) {
      try {
        if (seekTo > 0 && typeof this.youtubePlayer.seekTo === 'function') this.youtubePlayer.seekTo(seekTo, true);
        if (autoplay && typeof this.youtubePlayer.playVideo === 'function') this.youtubePlayer.playVideo();
      } catch { }
      this.startYoutubePolling();
      return true;
    }
    try {
      if (this.youtubePlayer && typeof this.youtubePlayer.destroy === 'function') this.youtubePlayer.destroy();
    } catch { }
    this.youtubePlayer = null;
    this.youtubeReady = false;
    this.youtubeState = -1;
    this.youtubeHost.innerHTML = '';
    this.youtubeHost.setAttribute('data-current-key', nextKey);
    const hostId = `global-rss-yt-${Math.random().toString(36).slice(2, 10)}`;
    this.youtubeHost.innerHTML = `<div id="${hostId}"></div>`;
    await new Promise((resolve) => {
      const playerOptions = {
        ...
        events: {
          onReady: () => {
            ...
            try {
              if (Number.isFinite(initialVolume) && typeof this.youtubePlayer.setVolume === 'function') this.youtubePlayer.setVolume(Math.round(initialVolume * 100));
              if (Number.isFinite(initialRate) && typeof this.youtubePlayer.setPlaybackRate === 'function') this.youtubePlayer.setPlaybackRate(initialRate);
              if (seekTo > 0 && typeof this.youtubePlayer.seekTo === 'function') this.youtubePlayer.seekTo(seekTo, true);
              if (autoplay && typeof this.youtubePlayer.playVideo === 'function') this.youtubePlayer.playVideo();
            } catch { }
            this.startYoutubePolling();
          ...
          onStateChange: (event) => {
            this.youtubeState = Number(event && event.data);
            if (this.youtubeState === 1) this.startYoutubePolling();
            else if (this.youtubeState === 0) {
              this.stopYoutubePolling();
      ...
      this.youtubePlayer = new window.YT.Player(hostId, playerOptions);
    });
    return true;
  },
  bind() {
    const playBtn = this.root.querySelector('[data-global-rss-play]');
    const prevBtn = this.root.querySelector('[data-global-rss-prev]');
    const nextBtn = this.root.querySelector('[data-global-rss-next]');
    const seekBackBtn = this.root.querySelector('[data-global-rss-seek-back]');
    const seekForwardBtn = this.root.querySelector('[data-global-rss-seek-forward]');
    const mobileSeekBackBtn = this.root.querySelector('[data-global-rss-mobile-seek-back]');
    const mobileSeekForwardBtn = this.root.querySelector('[data-global-rss-mobile-seek-forward]');
    const summaryToggle = this.root.querySelector('[data-global-rss-summary-toggle]');
    const summaryPlayBtn = this.root.querySelector('[data-global-rss-summary-play]');
    const summaryNextBtn = this.root.querySelector('[data-global-rss-summary-next]');
    const collapseBtn = this.root.querySelector('[data-global-rss-collapse]');
    const progress = this.root.querySelector('[data-global-rss-progress]');
    const volumeWrap = this.root.querySelector('[data-global-rss-volume]');
    const volumeBtn = this.root.querySelector('[data-global-rss-volume-btn]');
    const volumeValue = this.root.querySelector('[data-global-rss-volume-value]');
    const volumeSlider = this.root.querySelector('[data-global-rss-volume-slider]');
    const volumeDecreaseBtn = this.root.querySelector('[data-global-rss-volume-decrease]');
    const volumeIncreaseBtn = this.root.querySelector('[data-global-rss-volume-increase]');
    const speedWrap = this.root.querySelector('[data-global-rss-speed]');
    const speedBtn = this.root.querySelector('[data-global-rss-speed-btn]');
    const speedSlider = this.root.querySelector('[data-global-rss-speed-slider]');
    const speedDecreaseBtn = this.root.querySelector('[data-global-rss-speed-decrease]');
    const speedIncreaseBtn = this.root.querySelector('[data-global-rss-speed-increase]');
    const queueBtn = this.root.querySelector('[data-global-rss-queue-toggle]');
    const queueList = this.root.querySelector('[data-global-rss-queue-list]');
    const actionsWrap = this.root.querySelector('.global-rss-actions');

    const setControlOpen = (wrap, open, trigger) => {
      if (!wrap) return;
      wrap.dataset.open = open ? 'true' : 'false';
      if (trigger) trigger.setAttribute('aria-expanded', open ? 'true' : 'false');
      updateMobileActionTrim();
    };

    const updateMobileActionTrim = () => {
      if (!actionsWrap) return;
      const activeControl = volumeWrap?.dataset.open === 'true'
        ? 'volume'
        : speedWrap?.dataset.open === 'true'
          ? 'speed'
          : '';
      actionsWrap.dataset.activeControl = activeControl;
      clearResponsiveTrimHidden(actionsWrap);
      actionsWrap.dataset.trim = '0';
      if (!activeControl || !isCompactPlayerViewport()) return;
      requestAnimationFrame(() => {
        applyResponsiveActionTrimWithCandidates(actionsWrap, [
          queueBtn,
          mobileSeekForwardBtn,
          mobileSeekBackBtn
        ]);
      });
    };
    ...
    if (playBtn) {
      playBtn.addEventListener('click', () => {
        if (this.isMediaPaused()) this.playCurrent();
        else if (this.isYouTubeItem() && this.youtubePlayer && typeof this.youtubePlayer.pauseVideo === 'function') this.youtubePlayer.pauseVideo();
        else if (this.isBilibiliItem()) {
          this.stopBilibiliPlayback(false);
          this.persist(true);
          this.emit();
    ...
    if (summaryPlayBtn) {
      summaryPlayBtn.addEventListener('click', (event) => {
        event.stopPropagation();
        if (this.isMediaPaused()) this.playCurrent();
        else if (this.isYouTubeItem() && this.youtubePlayer && typeof this.youtubePlayer.pauseVideo === 'function') this.youtubePlayer.pauseVideo();
        else if (this.isBilibiliItem()) {
    ...
    if (seekBackBtn) seekBackBtn.addEventListener('click', () => this.seekMediaTo(this.getMediaCurrentTime() - RSS_SKIP_BACK_SECONDS));
    if (seekForwardBtn) seekForwardBtn.addEventListener('click', () => this.seekMediaTo(this.getMediaCurrentTime() + RSS_SKIP_FORWARD_SECONDS));
    if (mobileSeekBackBtn) mobileSeekBackBtn.addEventListener('click', () => this.seekMediaTo(this.getMediaCurrentTime() - RSS_SKIP_BACK_SECONDS));
    if (mobileSeekForwardBtn) mobileSeekForwardBtn.addEventListener('click', () => this.seekMediaTo(this.getMediaCurrentTime() + RSS_SKIP_FORWARD_SECONDS));
    if (summaryNextBtn) {
      summaryNextBtn.addEventListener('click', (event) => {
        event.stopPropagation();
        this.playNext();
      });
    }
    if (summaryToggle) {
      summaryToggle.addEventListener('click', () => {
        ...
        if (!this.mobileExpanded) this.queueOpen = false;
        this.updateUi();
      });
    ...
    if (progress) {
      progress.addEventListener('input', () => {
        ...
        const currentEl = this.root.querySelector('[data-global-rss-time-current]');
        if (currentEl) currentEl.textContent = formatCastboxAudioTime(duration * ratio);
      });
      progress.addEventListener('change', () => {
        const duration = this.getMediaDuration();
        if (duration > 0) this.seekMediaTo(Math.max(0, Math.min(duration, duration * ((Number(progress.value) || 0) / 1000))));
        this.persist(true);
        this.updateUi();
      });
    }
    if (volumeBtn && volumeSlider) {
      volumeBtn.addEventListener('click', () => {
        setControlOpen(speedWrap, false, speedBtn);
        const nextOpen = !(volumeWrap && volumeWrap.dataset.open === 'true');
        setControlOpen(volumeWrap, nextOpen, volumeBtn);
        if (nextOpen) volumeSlider.focus();
      });
      volumeSlider.addEventListener('input', () => {
        const nextVolume = clampRssVolumeLevel((Number(volumeSlider.value) || 0) / 100);
        this.setMediaVolume(nextVolume);
      });
    }
    if (volumeDecreaseBtn) {
      volumeDecreaseBtn.addEventListener('click', () => {
        setControlOpen(speedWrap, false, speedBtn);
        setControlOpen(volumeWrap, true, volumeBtn);
        this.setMediaVolume(clampRssVolumeLevel(this.getMediaVolume() - 0.05));
      });
    }
    if (volumeIncreaseBtn) {
    ...
    if (speedBtn && speedSlider) {
      speedBtn.addEventListener('click', () => {
        setControlOpen(volumeWrap, false, volumeBtn);
        const nextOpen = !(speedWrap && speedWrap.dataset.open === 'true');
        setControlOpen(speedWrap, nextOpen, speedBtn);
        if (nextOpen) speedSlider.focus();
      });
      speedSlider.addEventListener('input', () => {
        const nextRate = clampRssPlaybackRate(Number(speedSlider.value) || 1);
        this.setMediaPlaybackRate(nextRate);
      });
    }
    if (speedDecreaseBtn) {
      speedDecreaseBtn.addEventListener('click', () => {
        setControlOpen(volumeWrap, false, volumeBtn);
        setControlOpen(speedWrap, true, speedBtn);
        this.setMediaPlaybackRate(clampRssPlaybackRate(this.getMediaPlaybackRate() - 0.05));
      });
    }
    if (speedIncreaseBtn) {
    ...
    if (queueBtn) {
      queueBtn.addEventListener('click', () => {
        ...
        if (this.queueOpen) this.mobileExpanded = true;
        this.updateUi();
      });
    }
    if (typeof window !== 'undefined') {
      window.addEventListener('resize', updateMobileActionTrim);
    }
    if (queueList) {
      queueList.addEventListener('click', (event) => {
        const removeBtn = event.target && event.target.closest ? event.target.closest('[data-global-rss-remove-index]') : null;
        if (removeBtn) {
          event.preventDefault();
          const idx = parseInt(removeBtn.getAttribute('data-global-rss-remove-index') || '-1', 10);
          this.removeAt(idx);
          return;
        }
        const itemBtn = event.target && event.target.closest ? event.target.closest('[data-global-rss-index]') : null;
        if (!itemBtn) return;
        event.preventDefault();
        const idx = parseInt(itemBtn.getAttribute('data-global-rss-index') || '-1', 10);
        if (idx >= 0) this.playIndex(idx, { autoplay: true });
      });
      queueList.addEventListener('dragstart', (event) => {
        const item = event.target && event.target.closest ? event.target.closest('[data-global-rss-draggable-index]') : null;
        if (!item) return;
        this.dragIndex = parseInt(item.getAttribute('data-global-rss-draggable-index') || '-1', 10);
      ...
      queueList.addEventListener('dragend', (event) => {
        ...
        this.dragIndex = -1;
        queueList.querySelectorAll('.is-drop-target').forEach((node) => node.classList.remove('is-drop-target'));
      });
      ...
      queueList.addEventListener('drop', (event) => {
        ...
        if (!item) return;
        event.preventDefault();
        const targetIndex = parseInt(item.getAttribute('data-global-rss-draggable-index') || '-1', 10);
    ...
    this.audio.addEventListener('play', () => {
      if (this.isYouTubeItem()) return;
      setAudioPlaybackActive(this.audio, true);
      this.persist(false);
      this.emit();
      this.updateUi();
    });
    this.audio.addEventListener('pause', () => {
      if (this.isYouTubeItem()) return;
      setAudioPlaybackActive(this.audio, false);
      this.persist(true);
      this.emit();
      this.updateUi();
    });
    this.audio.addEventListener('ended', () => {
      if (this.isYouTubeItem()) return;
      setAudioPlaybackActive(this.audio, false);
      if (!this.advanceAfterCompletion()) {
        this.persist(true);
        this.emit();
        this.updateUi();
      }
    ...
    this.audio.addEventListener('loadedmetadata', () => {
      if (this.isYouTubeItem()) return;
      const pendingSeek = Number(this.audio.dataset.pendingSeek || 0);
      if (Number.isFinite(pendingSeek) && pendingSeek > 0) {
        try { this.audio.currentTime = pendingSeek; } catch { }
      }
      delete this.audio.dataset.pendingSeek;
      this.updateUi();
      this.emit();
    });
    ...
    document.addEventListener('click', (event) => {
      if (volumeWrap && !volumeWrap.contains(event.target)) setControlOpen(volumeWrap, false, volumeBtn);
      if (speedWrap && !speedWrap.contains(event.target)) setControlOpen(speedWrap, false, speedBtn);
      if (this.queueOpen && this.root && !this.root.contains(event.target)) {
        this.queueOpen = false;
        this.updateUi();
      }
    });
  ...
  getAccentPlatform() {
    const selectedPlatform = String((platformSelect && platformSelect.value) || '').trim().toLowerCase();
    if (selectedPlatform && selectedPlatform !== 'all') return selectedPlatform;
    return String(this.getCurrentItem()?.accentPlatform || 'rss').trim().toLowerCase() || 'rss';
  },
  syncAccent() {
    if (!this.root) return;
    applyPlayerAccentVars(this.root, this.getAccentPlatform());
  },
  emit() {
    this.syncAccent();
    this.syncMediaSession();
    document.dispatchEvent(new CustomEvent(GLOBAL_RSS_PLAYER_EVENT, {
      detail: {
        ...
        paused: this.isMediaPaused(),
        playbackRate: this.getMediaPlaybackRate(),
        volume: this.getMediaVolume(),
        muted: !this.isYouTubeItem() && !this.isBilibiliItem() && !!(this.audio && this.audio.muted)
      }
    }));
  },
  updateUi() {
    ...
    document.body.classList.toggle('has-global-rss-player', !!item);
    this.syncAccent();
    const titleEl = this.root.querySelector('[data-global-rss-title]');
    const kickerEl = this.root.querySelector('[data-global-rss-kicker]');
    const metaEl = this.root.querySelector('[data-global-rss-meta]');
    const coverEl = this.root.querySelector('[data-global-rss-cover]');
    const summaryKickerEl = this.root.querySelector('[data-global-rss-summary-kicker]');
    const summaryTitleEl = this.root.querySelector('[data-global-rss-summary-title]');
    const summaryCoverEl = this.root.querySelector('[data-global-rss-summary-cover]');
    const playBtn = this.root.querySelector('[data-global-rss-play]');
    const summaryPlayBtn = this.root.querySelector('[data-global-rss-summary-play]');
    const prevBtn = this.root.querySelector('[data-global-rss-prev]');
    const nextBtn = this.root.querySelector('[data-global-rss-next]');
    ...
    const progress = this.root.querySelector('[data-global-rss-progress]');
    const currentEl = this.root.querySelector('[data-global-rss-time-current]');
    const totalEl = this.root.querySelector('[data-global-rss-time-total]');
    const volumeWrap = this.root.querySelector('[data-global-rss-volume]');
    const volumeValue = this.root.querySelector('[data-global-rss-volume-value]');
    const volumeSlider = this.root.querySelector('[data-global-rss-volume-slider]');
    const speedBtn = this.root.querySelector('[data-global-rss-speed-btn]');
    ...
    const actionsWrap = this.root.querySelector('.global-rss-actions');
    const queueBtn = this.root.querySelector('[data-global-rss-queue-toggle]');
    const queueCount = this.root.querySelector('[data-global-rss-queue-count]');
    const queueWrap = this.root.querySelector('[data-global-rss-queue]');
    const queueSummary = this.root.querySelector('[data-global-rss-queue-summary]');
    const queueList = this.root.querySelector('[data-global-rss-queue-list]');

    if (titleEl) titleEl.textContent = item ? item.title : '';
    if (kickerEl) kickerEl.textContent = item ? String(item.podcastTitle || '').trim() : '';
    if (metaEl) metaEl.textContent = item ? String(item.meta || '').trim() : '';
    if (summaryKickerEl) summaryKickerEl.textContent = item ? String(item.podcastTitle || '').trim() : '';
    if (summaryTitleEl) summaryTitleEl.textContent = item ? item.title : '';
    this.root.dataset.platform = item ? String(item.platform || 'audio').trim().toLowerCase() : '';
    if (coverEl) {
      const src = String(item && item.image || '').trim();
      if (src) {
        coverEl.hidden = false;
        coverEl.src = src;
        coverEl.alt = item && item.podcastTitle ? item.podcastTitle : item && item.title ? item.title : '';
      } else {
        coverEl.hidden = true;
    ...
    if (summaryCoverEl) {
      const src = String(item && item.image || '').trim();
      if (src) {
        summaryCoverEl.hidden = false;
        summaryCoverEl.src = src;
        summaryCoverEl.alt = item && item.podcastTitle ? item.podcastTitle : item && item.title ? item.title : '';
      } else {
        summaryCoverEl.hidden = true;
        summaryCoverEl.removeAttribute('src');
        summaryCoverEl.alt = '';
      }
    }
    const isPlaying = !!item && !this.isMediaPaused();
    if (playBtn) playBtn.classList.toggle('is-playing', isPlaying);
    if (summaryPlayBtn) summaryPlayBtn.classList.toggle('is-playing', isPlaying);
    if (prevBtn) prevBtn.disabled = !(this.currentIndex > 0);
    if (nextBtn) nextBtn.disabled = !(this.currentIndex >= 0 && this.currentIndex < this.queue.length - 1);
    if (summaryToggle) summaryToggle.setAttribute('aria-expanded', this.mobileExpanded ? 'true' : 'false');
    if (collapseBtn) collapseBtn.hidden = !this.mobileExpanded;
    this.root.dataset.mobileExpanded = this.mobileExpanded ? 'true' : 'false';
    const mediaCurrent = this.getMediaCurrentTime();
    const mediaDuration = this.getMediaDuration();
    if (currentEl) currentEl.textContent = formatCastboxAudioTime(mediaCurrent);
    if (totalEl) totalEl.textContent = formatCastboxAudioTime(mediaDuration);
    if (progress) {
      progress.disabled = !!item && this.isBilibiliItem(item);
      progress.setAttribute('aria-disabled', progress.disabled ? 'true' : 'false');
      progress.value = mediaDuration > 0 ? String(Math.max(0, Math.min(1000, Math.round((mediaCurrent / mediaDuration) * 1000)))) : '0';
    }
    if (volumeWrap) {
      const safeVolume = clampRssVolumeLevel(this.getMediaVolume());
      const muted = !this.isYouTubeItem() && !this.isBilibiliItem() && (this.audio?.muted || safeVolume <= 0.001);
      volumeWrap.dataset.volumeState = muted ? 'muted' : (safeVolume < 0.34 ? 'low' : (safeVolume < 0.67 ? 'medium' : 'high'));
    }
    const volumePercent = Math.round(clampRssVolumeLevel(this.getMediaVolume()) * 100);
    if (volumeSlider) volumeSlider.value = String(volumePercent);
    if (volumeValue) volumeValue.textContent = `${volumePercent}%`;
    if (speedBtn) speedBtn.textContent = formatRssPlaybackRateLabel(this.getMediaPlaybackRate());
    if (speedSlider) speedSlider.value = String(clampRssPlaybackRate(this.getMediaPlaybackRate()));
    if (queueCount) queueCount.textContent = String(this.queue.length);
    if (queueBtn) queueBtn.setAttribute('aria-expanded', this.queueOpen ? 'true' : 'false');
    if (queueWrap) queueWrap.hidden = !this.queueOpen || !this.queue.length;
    if (queueSummary) queueSummary.textContent = String(this.queue.length);
    if (queueList) {
      queueList.innerHTML = this.queue.map((entry, index) => `
        <li class="global-rss-queue-item${index === this.currentIndex ? ' is-active' : ''}" draggable="true" data-global-rss-draggable-index="${index}">
          <button class="global-rss-queue-play" type="button" data-global-rss-index="${index}">
            <span class="global-rss-queue-title">${escapeHtml(entry.title)}</span>
            <span class="global-rss-queue-meta">${escapeHtml(entry.meta || entry.podcastTitle || '')}</span>
          </button>
          <button class="global-rss-queue-remove" type="button" data-global-rss-remove-index="${index}" aria-label="Remove from queue" title="Remove from queue">
            ${getQueueRemoveSvg()}
          </button>
        </li>
      `).join('');
    ...
    if (actionsWrap) {
      const activeControl = actionsWrap.dataset.activeControl || '';
      clearResponsiveTrimHidden(actionsWrap);
      actionsWrap.dataset.trim = '0';
      if (activeControl && isCompactPlayerViewport()) {
        requestAnimationFrame(() => {
          applyResponsiveActionTrimWithCandidates(actionsWrap, [
            queueBtn,
            mobileSeekForwardBtn,
  ...
  persist(force = false) {
    ...
    if (!force && Math.abs(current - this.lastSavedSecond) < 2) return;
    this.lastSavedSecond = current;
    const state = {
      queue: this.queue.slice(0, GLOBAL_RSS_PLAYER_QUEUE_LIMIT),
      currentIndex: this.currentIndex,
      currentTime: current,
      playbackRate: this.getMediaPlaybackRate(),
      volume: this.getMediaVolume(),
      muted: !this.isYouTubeItem() && !!this.audio.muted,
      isPlaying: !this.isMediaPaused(),
      updatedAt: Date.now()
    };
    writeGlobalRssPlayerState(state);
    const item = this.getCurrentItem();
    if (item && item.feedUrl && item.guid && !this.isYouTubeItem(item) && !this.isBilibiliItem(item)) {
      void setRssPlaybackProgress(item.feedUrl, item.guid, {
        currentTime: current,
        duration: this.getMediaDuration(),
        title: item.title,
        updatedAt: Date.now()
  ...
  async restore() {
    const state = readGlobalRssPlayerState();
    if (!state) {
      this.updateUi();
      return;
    }
    this.queue = (Array.isArray(state.queue) ? state.queue : [])
      .map((entry) => normalizeGlobalRssQueueItem(entry))
      .filter(Boolean)
      .slice(0, GLOBAL_RSS_PLAYER_QUEUE_LIMIT);
    this.currentIndex = Math.max(-1, Math.min(this.queue.length - 1, Number(state.currentIndex) || 0));
    this.audio.volume = Math.max(0, Math.min(1, Number(state.volume) || 1));
    this.audio.muted = !!state.muted;
    this.audio.playbackRate = Math.max(0.5, Math.min(4, Number(state.playbackRate) || 1));
    const currentItem = this.getCurrentItem();
    if (currentItem) {
      if (this.isYouTubeItem(currentItem)) {
        void this.ensureYoutubePlayerForItem(currentItem, {
          autoplay: !!state.isPlaying,
          seekTo: Math.max(0, Number(state.currentTime) || 0),
          volume: Number(state.volume),
          playbackRate: Number(state.playbackRate) || 1
        });
      } else if (this.isBilibiliItem(currentItem)) {
        void this.ensureBilibiliPlayerForItem(currentItem, {
          autoplay: !!state.isPlaying,
          seekTo: Math.max(0, Number(state.currentTime) || 0)
        });
      } else {
        this.audio.src = currentItem.src;
        this.audio.dataset.pendingSeek = String(Math.max(0, Number(state.currentTime) || 0));
        this.audio.load();
        if (state.isPlaying) {
          setTimeout(() => {
            try {
  ...
  findItemIndex(item) {
    const normalized = normalizeGlobalRssQueueItem(item);
    if (!normalized) return -1;
    return this.queue.findIndex((entry) => entry && (
      entry.id === normalized.id
      || (String(entry.platform || '').trim().toLowerCase() === 'audio'
        && String(normalized.platform || '').trim().toLowerCase() === 'audio'
        && entry.src === normalized.src
        && entry.feedUrl === normalized.feedUrl)
      || (this.isYouTubeItem(entry) && entry.videoId === normalized.videoId && entry.playlistId === normalized.playlistId)
      || (this.isBilibiliItem(entry) && entry.aid === normalized.aid && entry.bvid === normalized.bvid && entry.cid === normalized.cid)
    ));
  },
  enqueue(item, options = {}) {
    const normalized = normalizeGlobalRssQueueItem(item);
    if (!normalized) return -1;
    const existingIndex = this.findItemIndex(normalized);
    if (existingIndex >= 0) {
      if (options.playNow) this.playIndex(existingIndex, { autoplay: options.autoplay !== false });
      this.updateUi();
      this.emit();
      return existingIndex;
    }
    this.queue.push(normalized);
    if (this.queue.length > GLOBAL_RSS_PLAYER_QUEUE_LIMIT) this.queue = this.queue.slice(-GLOBAL_RSS_PLAYER_QUEUE_LIMIT);
    const nextIndex = this.queue.length - 1;
    if (this.currentIndex < 0) this.currentIndex = nextIndex;
    this.persist(true);
    this.updateUi();
    this.emit();
    if (options.playNow) this.playIndex(nextIndex, { autoplay: options.autoplay !== false });
    return nextIndex;
  },
  replaceQueue(items, options = {}) {
    const normalized = (Array.isArray(items) ? items : [])
      .map((item) => normalizeGlobalRssQueueItem(item))
      .filter(Boolean)
      .slice(0, GLOBAL_RSS_PLAYER_QUEUE_LIMIT);
    this.queue = normalized;
    this.currentIndex = normalized.length ? 0 : -1;
    this.persist(true);
    this.updateUi();
    this.emit();
    if (normalized.length && options.playNow !== false) {
      this.playIndex(Math.max(0, Number(options.startIndex) || 0), { autoplay: options.autoplay !== false });
    }
    return normalized.length;
  },
  removeAt(index) {
    const safeIndex = Number(index);
    if (!Number.isInteger(safeIndex) || safeIndex < 0 || safeIndex >= this.queue.length) return;
    const removingCurrent = safeIndex === this.currentIndex;
    this.queue.splice(safeIndex, 1);
    if (!this.queue.length) {
      this.currentIndex = -1;
      this.stopYoutubePolling();
      this.stopBilibiliPlayback(true);
      try { if (this.youtubePlayer && typeof this.youtubePlayer.stopVideo === 'function') this.youtubePlayer.stopVideo(); } catch { }
      this.audio.pause();
      this.audio.removeAttribute('src');
      this.audio.load();
    } else if (removingCurrent) {
      this.currentIndex = Math.max(0, Math.min(safeIndex, this.queue.length - 1));
      this.playIndex(this.currentIndex, { autoplay: !this.isMediaPaused() });
      return;
  ...
  moveItem(fromIndex, toIndex) {
    ...
    if (!Number.isInteger(from) || !Number.isInteger(to) || from < 0 || to < 0 || from >= this.queue.length || to >= this.queue.length || from === to) {
      return;
    }
  ...
  advanceAfterCompletion() {
    ...
    if (finishedIndex < 0 || finishedIndex >= this.queue.length) return false;
    this.queue.splice(finishedIndex, 1);
    if (!this.queue.length) {
      this.currentIndex = -1;
      this.queueOpen = false;
      this.stopYoutubePolling();
    ...
    this.currentIndex = Math.max(0, Math.min(finishedIndex, this.queue.length - 1));
    this.playIndex(this.currentIndex, { autoplay: true });
    return true;
  },
  async playIndex(index, options = {}) {
    const safeIndex = Number(index);
    const item = this.queue[safeIndex];
    ...
    if (this.isYouTubeItem(item)) {
      ...
      this.audio.removeAttribute('src');
      this.audio.load();
      await this.ensureYoutubePlayerForItem(item, {
        autoplay: shouldAutoplay,
        seekTo: Math.max(0, Number(options.seekTo) || 0)
      });
      this.persist(true);
      this.emit();
      this.updateUi();
    ...
    if (this.isBilibiliItem(item)) {
      this.stopYoutubePolling();
      try { if (this.youtubePlayer && typeof this.youtubePlayer.pauseVideo === 'function') this.youtubePlayer.pauseVideo(); } catch { }
      this.audio.pause();
      this.audio.removeAttribute('src');
      this.audio.load();
      this.resetBilibiliProgressState(Math.max(0, Number(options.seekTo) || 0));
      await this.ensureBilibiliPlayerForItem(item, {
        autoplay: shouldAutoplay,
        seekTo: Math.max(0, Number(options.seekTo) || 0)
      });
      this.persist(true);
    ...
    this.stopBilibiliPlayback(true);
    try { if (this.youtubePlayer && typeof this.youtubePlayer.pauseVideo === 'function') this.youtubePlayer.pauseVideo(); } catch { }
    const srcChanged = String(this.audio.getAttribute('src') || '').trim() !== item.src;
    if (srcChanged) {
      this.audio.src = item.src;
      const stored = await getRssPlaybackProgress(item.feedUrl, item.guid).catch(() => null);
      this.audio.dataset.pendingSeek = String(Math.max(0, Number(stored && stored.currentTime || 0)));
      this.audio.load();
    }
    if (shouldAutoplay) {
      try {
  ...
  playCurrent() {
    if (this.currentIndex < 0 && this.queue.length) {
      return this.playIndex(0, { autoplay: true });
    }
    const currentItem = this.getCurrentItem();
    if (this.isYouTubeItem(currentItem)) {
      if (!this.youtubePlayer || !this.youtubeReady || typeof this.youtubePlayer.playVideo !== 'function') {
        if (this.currentIndex >= 0) {
          return this.playIndex(this.currentIndex, { autoplay: true });
        }
        if (this.queue.length) {
          return this.playIndex(0, { autoplay: true });
        }
      ...
      try { this.youtubePlayer.playVideo(); } catch { }
      return true;
    }
    if (this.isBilibiliItem(currentItem)) {
      if (this.currentIndex >= 0) {
        return this.ensureBilibiliPlayerForItem(currentItem, {
          autoplay: true,
          seekTo: this.getMediaCurrentTime()
        });
      }
      if (this.queue.length) return this.playIndex(0, { autoplay: true });
      return false;
    }
    if (currentItem) {
      const currentSrc = String(this.audio?.getAttribute('src') || '').trim();
      const targetSrc = String(currentItem.src || '').trim();
  ...
  playPrevious() {
    ...
    this.playIndex(this.currentIndex - 1, { autoplay: true });
    return true;
  },
  playNext() {
    if (this.currentIndex < 0 || this.currentIndex >= this.queue.length - 1) return false;
    this.playIndex(this.currentIndex + 1, { autoplay: true });
    return true;
  }
};

function buildApplePlayerModeAreaMarkup(mode = 'apple', canUseRss = true, descriptionMarkup = '', includeAd = true) {
  return buildSourceRssPlayerModeAreaMarkup('apple', mode, canUseRss, descriptionMarkup, includeAd);
}

async function loadRssTranscriptCues(source) {
  const transcript = source && typeof source === 'object' ? source : null;
  const url = transcript ? String(transcript.url || '').trim() : '';
  const type = transcript ? String(transcript.type || '').trim().toLowerCase() : '';
  if (!url) return [];

  const cached = rssTranscriptCache.get(url);
  if (Array.isArray(cached)) return cached;
  if (rssTranscriptPending.has(url)) return rssTranscriptPending.get(url);

  const task = (async () => {
    try {
      const loaded = await fetchAnyTextWithFallback(buildRssProxyCandidates(url), { maxAttempts: 8 });
      const cues = parseTranscriptTextToCues(loaded.text, type);
      rssTranscriptCache.set(url, cues);
      return cues;
    } catch (error) {
      console.warn('RSS transcript load failed:', error);
      rssTranscriptCache.set(url, []);
      return [];
    } finally {
      rssTranscriptPending.delete(url);
    }
  })();

  rssTranscriptPending.set(url, task);
  return task;
}
  ...
  return `
    <div class="castbox-rss-player" data-castbox-rss-player data-castbox-rss-url="${safeRssUrl}" data-castbox-fallback-title="${safeFallbackTitle}" data-castbox-visible-limit="${initialLimit}">
      ...
      <div class="castbox-rss-controls">
        <div class="castbox-rss-transport">
          <button class="castbox-rss-seek castbox-rss-seek-back" type="button" data-castbox-seek-back aria-label="Back 15 seconds" title="Back 15 seconds">
            ${getRssSeekBackSvg()}
          </button>
          ...
          <button class="castbox-rss-seek castbox-rss-seek-forward" type="button" data-castbox-seek-forward aria-label="Forward 30 seconds" title="Forward 30 seconds">
            ${getRssSeekForwardSvg()}
          </button>
        </div>
        ...
        <div class="castbox-rss-actions">
          <div class="castbox-rss-volume" data-castbox-volume data-volume-state="high" data-open="false">
            <button class="castbox-rss-stepper-btn" type="button" data-castbox-volume-decrease aria-label="Decrease volume" title="Decrease volume">
              ${escapeHtml('−')}
            </button>
            <button class="castbox-rss-volume-btn" type="button" data-castbox-volume-btn aria-label="Volume" title="Volume" aria-expanded="false">
              ${getRssVolumeSvg()}
              <span class="castbox-rss-volume-value" data-castbox-volume-value>100%</span>
            </button>
            <button class="castbox-rss-stepper-btn" type="button" data-castbox-volume-increase aria-label="Increase volume" title="Increase volume">
              ${escapeHtml('+')}
            </button>
            <input class="castbox-rss-volume-slider" data-castbox-volume-slider type="range" min="0" max="100" step="1" value="100" aria-label="Volume">
          </div>
          ${buildCastboxSpeedControlMarkup(1)}
          ${buildCastboxSearchControlMarkup()}
          <a class="castbox-rss-download" data-castbox-download href="${audioSrc}" download rel="noopener" aria-label="${escapeHtml(noteText('noteDownload'))}" title="${escapeHtml(noteText('noteDownload'))}">
            ${getRssDownloadSvg()}
          </a>
          <a class="castbox-rss-feed-link" data-castbox-feed-link href="${safeRssUrl || '#'}" target="_blank" rel="noopener" aria-label="${escapeHtml(tOr('myPodcastsImportActionRss', 'RSS'))}" title="${escapeHtml(tOr('myPodcastsImportActionRss', 'RSS'))}">
            ${getRssFeedSvg()}
          </a>
        </div>
      </div>
      <div class="castbox-rss-transcript" data-castbox-transcript hidden style="display:none">
        <div class="castbox-rss-transcript-text" data-castbox-transcript-text></div>
      </div>
...
async function renderCastboxRssPlayerInSlot(slot, embedPlatform, rssUrl, fallbackTitle, forceRefresh = true, options = {}) {
  ...
  const includeAd = options && options.includeAd === false ? false : true;

  slot.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${platform}"><div class="castbox-rss-loading">${escapeHtml(t('loading'))}…</div></div>${includeAd ? buildPlayerAdMarkup() : ''}`;
  applyPlayerAccentVars(slot, platform);
  ...
  try {
    ...
    slot.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${platform}">${buildCastboxRssPlayerMarkup(feed, safeFallbackTitle, safeRssUrl)}</div>${includeAd ? buildPlayerAdMarkup() : ''}`;
    applyPlayerAccentVars(slot, platform);
    ...
    slot.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${platform}"><div class="castbox-rss-error-wrap"><div class="castbox-rss-error">${escapeHtml(t('loadFailedRefresh'))}</div><button class="castbox-rss-refresh" type="button" data-castbox-retry>${escapeHtml(t('refresh'))}</button></div></div>${includeAd ? buildPlayerAdMarkup() : ''}`;
    applyPlayerAccentVars(slot, platform);
    ...
    if (retryBtn) {
      retryBtn.addEventListener('click', () => {
        const retryKey = normalizeFavoriteFeedUrl(safeRssUrl) || safeRssUrl;
        castboxRssFeedCache.delete(retryKey);
...
function bindCastboxRssPlayerInteractions(slot, options = {}) {
  ...
  const seekBackBtn = root.querySelector('[data-castbox-seek-back]');
  const seekForwardBtn = root.querySelector('[data-castbox-seek-forward]');
  const speedControl = root.querySelector('[data-castbox-speed-control]');
  const speedTrigger = root.querySelector('[data-castbox-speed-trigger]');
  const speedSlider = root.querySelector('[data-castbox-speed-slider]');
  const speedDecreaseBtn = root.querySelector('[data-castbox-speed-decrease]');
  const speedIncreaseBtn = root.querySelector('[data-castbox-speed-increase]');
  const searchControl = root.querySelector('[data-castbox-search-control]');
  const searchTrigger = root.querySelector('[data-castbox-search-trigger]');
  const searchInput = root.querySelector('[data-castbox-search-input]');
  const feedLink = root.querySelector('[data-castbox-feed-link]');
  const downloadLink = root.querySelector('[data-castbox-download]');
  const volumeWrap = root.querySelector('[data-castbox-volume]');
  const volumeBtn = root.querySelector('[data-castbox-volume-btn]');
  const volumeValue = root.querySelector('[data-castbox-volume-value]');
  const volumeSlider = root.querySelector('[data-castbox-volume-slider]');
  const volumeDecreaseBtn = root.querySelector('[data-castbox-volume-decrease]');
  const volumeIncreaseBtn = root.querySelector('[data-castbox-volume-increase]');
  ...
  const transcriptBox = root.querySelector('[data-castbox-transcript]');
  const transcriptText = root.querySelector('[data-castbox-transcript-text]');
  const actionsWrap = root.querySelector('.castbox-rss-actions');
  if (!audio || !listEl) return;

  const rootKey = normalizeFavoriteFeedUrl(root.__castboxRssUrl) || root.__castboxFallbackTitle || String(Math.random());

  let lastSavedSecond = -1;
  let transcriptRequestToken = 0;
  root.__castboxTranscriptCues = [];
  root.__castboxActiveEpisode = null;
  root.__castboxLastVolume = Math.max(0.05, Math.min(1, Number(root.__castboxLastVolume) || 1));
  root.__castboxSearchQuery = String(root.__castboxSearchQuery || '').trim().toLowerCase();

  function updateDownloadLink() {
    if (!downloadLink) return;
    const src = getActiveEpisodeSource();
    const title = nowTitle ? String(nowTitle.textContent || '').trim() : '';
    downloadLink.setAttribute('href', src || '#');
    const suffix = src && /\.(m4a|mp3|aac|ogg|wav)(?:$|\?)/i.test(src)
      ? (src.match(/\.(m4a|mp3|aac|ogg|wav)(?:$|\?)/i)?.[1] || 'mp3')
      : 'mp3';
    const safeName = (title || 'podcast-audio')
      .replace(/[\\/:*?"<>|]+/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()
      .slice(0, 120) || 'podcast-audio';
    downloadLink.setAttribute('download', `${safeName}.${suffix}`);
    downloadLink.classList.toggle('is-disabled', !src);
    downloadLink.setAttribute('aria-disabled', src ? 'false' : 'true');
  }

  function syncVolumeUi() {
    const safeVolume = clampRssVolumeLevel(audio.volume);
    const muted = !!audio.muted || safeVolume <= 0.001;
    if (volumeSlider) volumeSlider.value = String(Math.round(safeVolume * 100));
    if (volumeValue) volumeValue.textContent = formatRssVolumeLabel(safeVolume);
    if (volumeWrap) {
      volumeWrap.dataset.volumeState = muted ? 'muted' : (safeVolume < 0.34 ? 'low' : (safeVolume < 0.67 ? 'medium' : 'high'));
    }
    if (volumeBtn) {
      const label = muted ? 'Volume (muted)' : 'Volume';
  ...
  function setEpisodeVolume(nextVolume) {
    const safeVolume = clampRssVolumeLevel(nextVolume);
    audio.volume = safeVolume;
    audio.muted = safeVolume <= 0.001;
    if (safeVolume > 0.001) root.__castboxLastVolume = safeVolume;
    syncVolumeUi();
  }
  ...
  function syncSpeedButtonState() {
    ...
    const label = formatRssPlaybackRateLabel(rate);
    speedTrigger.textContent = label;
    speedTrigger.setAttribute('aria-label', `Playback speed ${label}`);
    speedTrigger.setAttribute('title', `Playback speed ${label}`);
    if (speedSlider) speedSlider.value = String(Math.max(0.5, Math.min(4, rate)));
  }

  function setSpeedControlOpen(open) {
    if (!speedControl) return;
    const next = !!open;
    speedControl.dataset.open = next ? 'true' : 'false';
    if (speedTrigger) speedTrigger.setAttribute('aria-expanded', next ? 'true' : 'false');
    updateMobileActionTrim();
  }
  ...
  function setVolumeControlOpen(open) {
    ...
    const next = !!open;
    volumeWrap.dataset.open = next ? 'true' : 'false';
    if (volumeBtn) volumeBtn.setAttribute('aria-expanded', next ? 'true' : 'false');
    updateMobileActionTrim();
  }

  function setSearchControlOpen(open) {
    if (!searchControl) return;
    const next = !!open;
    searchControl.dataset.open = next ? 'true' : 'false';
    if (searchTrigger) searchTrigger.setAttribute('aria-expanded', next ? 'true' : 'false');
    updateMobileActionTrim();
  }

  function updateMobileActionTrim() {
    if (!actionsWrap) return;
    const activeControl = searchControl?.dataset.open === 'true'
      ? 'search'
      : volumeWrap?.dataset.open === 'true'
        ? 'volume'
      : speedControl?.dataset.open === 'true'
          ? 'speed'
          : '';
    actionsWrap.dataset.activeControl = activeControl;
    clearResponsiveTrimHidden(actionsWrap);
    ...
    requestAnimationFrame(() => {
      ...
      if (activeControl !== 'search') candidates.push(searchControl);
      applyResponsiveActionTrimWithCandidates(actionsWrap, candidates);
    });
  }

  function syncFeedLinkState() {
    if (!feedLink) return;
    const href = String(root.__castboxRssUrl || '').trim();
    feedLink.setAttribute('href', href || '#');
    feedLink.classList.toggle('is-disabled', !href);
    feedLink.setAttribute('aria-disabled', href ? 'false' : 'true');
  }

  function applyRssSearchFilter() {
    const query = String(root.__castboxSearchQuery || '').trim().toLowerCase();
    const feed = root.__castboxFeed;
    const episodes = feed && Array.isArray(feed.episodes) ? feed.episodes : [];
    const activeSrc = getActiveEpisodeSource();
    if (!query) {
      listEl.innerHTML = buildCastboxEpisodesRowsMarkup(episodes, 0, root.__castboxVisibleLimit, activeSrc);
      updateCastboxLoadMoreButton(root);
      syncCastboxQueueAddButtons(root);
      return;
    }
    const matchedIndexes = [];
    episodes.forEach((episode, index) => {
      const titleText = String(episode && episode.title || '').trim().toLowerCase();
      if (titleText.includes(query)) matchedIndexes.push(index);
    });
    listEl.innerHTML = buildCastboxEpisodesRowsMarkupByIndexes(episodes, matchedIndexes, activeSrc);
    const loadMoreBtn = root.querySelector('[data-castbox-load-more]');
    if (loadMoreBtn) loadMoreBtn.hidden = true;
    syncCastboxQueueAddButtons(root);
  }

  function syncTranscriptDisplay() {
    if (!transcriptBox || !transcriptText) return;
    const cues = Array.isArray(root.__castboxTranscriptCues) ? root.__castboxTranscriptCues : [];
    if (!cues.length) {
      transcriptBox.hidden = true;
      transcriptBox.style.display = 'none';
      transcriptBox.classList.remove('is-loading');
      transcriptText.textContent = '';
      return;
    }
    const current = Number(audio.currentTime) || 0;
    const cue = cues.find((entry) => current >= entry.start && current < entry.end)
      || cues.find((entry) => entry.start > current)
      || null;
    ...
    transcriptText.textContent = String(cue.text || '').trim();
  ...
  async function loadActiveTranscript() {
    ...
    const currentToken = transcriptRequestToken;
    const activeEpisode = root.__castboxActiveEpisode;
    let sources = sortTranscriptSourcesByPreference(activeEpisode && activeEpisode.transcripts);
    if (!sources.length && activeEpisode) {
      const rawFeedText = root.__castboxFeed && typeof root.__castboxFeed.rawText === 'string'
        ? root.__castboxFeed.rawText
        : '';
      const recovered = findTranscriptSourcesForEpisodeInRawFeed(rawFeedText, activeEpisode, String(root.__castboxRssUrl || '').trim());
      if (recovered.length) {
        activeEpisode.transcripts = recovered.slice();
        sources = sortTranscriptSourcesByPreference(recovered);
      }
    }
    if (!sources.length || !transcriptBox || !transcriptText) {
      root.__castboxTranscriptCues = [];
    ...
    transcriptBox.hidden = false;
    transcriptBox.style.display = 'flex';
    transcriptBox.classList.add('is-loading');
    transcriptText.textContent = '…';
    let cues = [];
    for (const source of sources) {
      cues = await loadRssTranscriptCues(source);
      if (Array.isArray(cues) && cues.length) break;
    }
    if (currentToken !== transcriptRequestToken) return;
    root.__castboxTranscriptCues = Array.isArray(cues) ? cues : [];
    syncTranscriptDisplay();
  }
  ...
  async function persistActiveProgress(force = false) {
    const currentItem = globalPlayer.getCurrentItem();
    if (!currentItem) return;
    const feedUrl = normalizeFavoriteFeedUrl(root.__castboxRssUrl);
    if (normalizeFavoriteFeedUrl(currentItem.feedUrl) !== feedUrl) return;
    const current = Number(audio.currentTime) || 0;
    if (!force && Math.abs(current - lastSavedSecond) < 2) return;
    lastSavedSecond = current;
    await setRssPlaybackProgress(currentItem.feedUrl, currentItem.guid, {
      currentTime: current,
  ...
  function syncPlayButtonState() {
    if (!playBtn) return;
    const currentItem = globalPlayer.getCurrentItem();
    const sameFeed = currentItem && normalizeFavoriteFeedUrl(currentItem.feedUrl) === normalizeFavoriteFeedUrl(root.__castboxRssUrl);
    const sameItem = sameFeed && String(currentItem.src || '').trim() === getActiveEpisodeSource();
    const isPlaying = !!(sameItem && !audio.paused && !audio.ended);
  ...
  function seekAudioBy(deltaSeconds) {
    ...
    try { audio.currentTime = nextValue; } catch {}
  ...
  function setActiveEpisodeButton(btn) {
    ...
    const episodeIndex = Math.max(0, parseInt(btn.getAttribute('data-castbox-ep-index') || '-1', 10));
    const title = String(btn.getAttribute('data-castbox-ep-title') || '').trim();
    ...
    const feedEpisodes = root.__castboxFeed && Array.isArray(root.__castboxFeed.episodes) ? root.__castboxFeed.episodes : [];
    root.__castboxActiveEpisode = (
      (episodeIndex >= 0 && feedEpisodes[episodeIndex])
      || feedEpisodes.find((episode) => (
        String(episode && episode.guid || '').trim() === guid
        || String(episode && episode.audioUrl || '').trim() === src
      ))
      || null
    );
  ...
  function syncRootFromGlobal() {
    if (!root.isConnected) {
      document.removeEventListener(GLOBAL_RSS_PLAYER_EVENT, syncRootFromGlobal);
      return;
    }
    const currentItem = globalPlayer.getCurrentItem();
    const sameFeed = currentItem && normalizeFavoriteFeedUrl(currentItem.feedUrl) === normalizeFavoriteFeedUrl(root.__castboxRssUrl);
    ...
    const activeBtn = Array.from(root.querySelectorAll('.castbox-rss-item[data-castbox-ep-src]')).find((node) => {
      return String(node.getAttribute('data-castbox-ep-guid') || '').trim() === String(currentItem.guid || '').trim()
        || String(node.getAttribute('data-castbox-ep-src') || '').trim() === String(currentItem.src || '').trim();
    }) || root.querySelector('.castbox-rss-item');
    if (activeBtn) setActiveEpisodeButton(activeBtn);
    syncAudioTimelineFromElement();
    syncPlayButtonState();
    syncSpeedButtonState();
    syncVolumeUi();
  ...
  function isCurrentRootFeedActive() {
    const currentItem = globalPlayer.getCurrentItem();
    return !!(currentItem && normalizeFavoriteFeedUrl(currentItem.feedUrl) === normalizeFavoriteFeedUrl(root.__castboxRssUrl));
  }

  document.addEventListener(GLOBAL_RSS_PLAYER_EVENT, syncRootFromGlobal);

  const initialActiveBtn = root.querySelector('.castbox-rss-item.is-active') || root.querySelector('.castbox-rss-item');
  if (initialActiveBtn) {
  ...
  audio.addEventListener('play', () => {
    if (!isCurrentRootFeedActive()) return;
    syncPlayButtonState();
  });
  ...
  audio.addEventListener('volumechange', () => {
    if (!isCurrentRootFeedActive()) return;
    const currentVolume = Math.max(0, Math.min(1, Number(audio.volume) || 0));
    if (!audio.muted && currentVolume > 0.001) root.__castboxLastVolume = currentVolume;
    syncVolumeUi();
  });
  ...
  if (playBtn) {
    playBtn.addEventListener('click', () => {
      ...
      const sameItem = currentItem
        && normalizeFavoriteFeedUrl(currentItem.feedUrl) === normalizeFavoriteFeedUrl(root.__castboxRssUrl)
        && String(currentItem.src || '').trim() === getActiveEpisodeSource();
      if (sameItem) {
        try {
      ...
      const nextBtn = root.querySelector('.castbox-rss-item.is-active') || root.querySelector('.castbox-rss-item');
      if (!nextBtn) return;
      const item = buildGlobalRssQueueItemFromEpisodeButton(root, nextBtn);
      if (!item) return;
      setActiveEpisodeButton(nextBtn);
      globalPlayer.enqueue(item, { playNow: true, autoplay: true });
    });
  }

  if (seekBackBtn) {
    seekBackBtn.addEventListener('click', () => {
      seekAudioBy(-RSS_SKIP_BACK_SECONDS);
    });
  }

  if (seekForwardBtn) {
    seekForwardBtn.addEventListener('click', () => {
      seekAudioBy(RSS_SKIP_FORWARD_SECONDS);
    });
  }
  ...
  if (volumeBtn) {
    volumeBtn.addEventListener('click', () => {
      ...
      setVolumeControlOpen(nextOpen);
      if (nextOpen && volumeSlider) {
        try {
  ...
  if (volumeDecreaseBtn) {
    volumeDecreaseBtn.addEventListener('click', () => {
      ...
      setEpisodeVolume(currentVolume - 0.05);
    });
  }
  ...
  if (speedSlider) {
    speedSlider.addEventListener('input', () => {
      const nextRate = clampRssPlaybackRate(Number(speedSlider.value) || 1);
      audio.playbackRate = Number(nextRate.toFixed(2));
      syncSpeedButtonState();
    });
  }

  if (speedTrigger && speedSlider) {
    speedTrigger.addEventListener('click', () => {
      const nextOpen = !(speedControl && speedControl.dataset.open === 'true');
      setSearchControlOpen(false);
      setVolumeControlOpen(false);
      setSpeedControlOpen(nextOpen);
      try {
  ...
  if (speedDecreaseBtn) {
    speedDecreaseBtn.addEventListener('click', () => {
      ...
      audio.playbackRate = Number(clampRssPlaybackRate(audio.playbackRate - 0.05).toFixed(2));
      syncSpeedButtonState();
    });
  }
  ...
  if (speedSlider) {
    speedSlider.addEventListener('blur', () => {
      setTimeout(() => {
        ...
        if (speedControl && active && speedControl.contains(active)) return;
        setSpeedControlOpen(false);
      }, 0);
    });
  }
  ...
  if (volumeSlider) {
    volumeSlider.addEventListener('blur', () => {
      setTimeout(() => {
        const active = document.activeElement;
        if (volumeWrap && active && volumeWrap.contains(active)) return;
        setVolumeControlOpen(false);
      }, 0);
    });
  }
  ...
  if (searchInput) {
    searchInput.addEventListener('input', () => {
      root.__castboxSearchQuery = String(searchInput.value || '').trim().toLowerCase();
      if (root.__castboxSearchQuery) {
        while (Number(root.__castboxVisibleLimit) < (root.__castboxFeed?.episodes?.length || 0)) {
          appendMoreEpisodes();
        }
      }
      applyRssSearchFilter();
  ...
  if (searchTrigger && searchInput) {
    searchTrigger.addEventListener('click', () => {
      const nextOpen = !(searchControl && searchControl.dataset.open === 'true');
      setVolumeControlOpen(false);
      setSpeedControlOpen(false);
      setSearchControlOpen(nextOpen);
      if (nextOpen) {
        try {
  ...
  if (searchInput) {
    searchInput.addEventListener('blur', () => {
      setTimeout(() => {
        const active = document.activeElement;
        if (searchControl && active && searchControl.contains(active)) return;
        setSearchControlOpen(false);
      }, 0);
    });
  }
  ...
  if (downloadLink) {
    downloadLink.addEventListener('click', async (event) => {
      ...
      updateDownloadLink();
      const filename = String(downloadLink.getAttribute('download') || 'podcast-audio.mp3').trim() || 'podcast-audio.mp3';
      downloadLink.classList.add('is-loading');
      downloadLink.setAttribute('aria-busy', 'true');
      const ok = await downloadRemoteFile(src, filename);
  ...
  root.addEventListener('click', (event) => {
    ...
    if (searchControl && !searchControl.contains(event.target)) {
      setSearchControlOpen(false);
    }
    ...
    const queueAddBtn = event.target && event.target.closest ? event.target.closest('[data-castbox-queue-add]') : null;
    if (queueAddBtn) {
      event.preventDefault();
      const itemBtn = queueAddBtn.previousElementSibling;
      const item = itemBtn ? buildGlobalRssQueueItemFromEpisodeButton(root, itemBtn) : null;
      if (!item) return;
      globalPlayer.enqueue(item, { playNow: false });
      syncCastboxQueueAddButtons(root);
    ...
    const item = buildGlobalRssQueueItemFromEpisodeButton(root, btn);
    if (!item) return;
...
function getAppleFeedResolutionMetaForRow(row, button = null) {
  const playBtn = button || (row ? row.querySelector('.play-link') : null);
  const favoriteBtn = row ? row.querySelector('.favorite-link') : null;
  const directFeedUrl = normalizeFavoriteFeedUrl(favoriteBtn?.getAttribute('data-favorite-feed') || '');
  const pageUrl = String(favoriteBtn?.getAttribute('data-favorite-url') || row?.querySelector('.open-link')?.getAttribute('href') || '').trim();
  const embedUrl = String(playBtn?.getAttribute('data-embed-url') || '').trim();
  const countryCode = normalizeCountry((countrySelect && countrySelect.value) || 'us');
  const episodeIds = extractAppleEpisodeIdsFromEmbedUrl(embedUrl);
  const showId = extractApplePodcastIdFromUrl(pageUrl)
    || extractApplePodcastIdFromUrl(embedUrl)
    || String(episodeIds && episodeIds.showId || '').trim();
  return {
...
async function resolveAppleFeedUrlForRow(row, button = null) {
  const meta = getAppleFeedResolutionMetaForRow(row, button);
  if (meta.feedUrl) return meta;
  if (!meta.showId) return { ...meta, feedUrl: '' };
  const cacheKey = `apple::${String(meta.showId || '').trim()}::${normalizeCountry(meta.countryCode || 'us')}`;
  const cached = getInlineFeedResolveCachedValue(cacheKey);
  if (cached) {
    const feedUrl = normalizeFavoriteFeedUrl(cached.feedUrl || '');
    if (feedUrl) {
      const favBtn = row ? row.querySelector('.favorite-link') : null;
      if (favBtn && !String(favBtn.getAttribute('data-favorite-feed') || '').trim()) {
        favBtn.setAttribute('data-favorite-feed', feedUrl);
      }
    }
    return { ...meta, feedUrl };
  }
  const resolved = await resolveApplePodcastImportMeta(`https://podcasts.apple.com/${meta.countryCode}/podcast/id${meta.showId}`, meta.countryCode).catch(() => null);
  const feedUrl = normalizeFavoriteFeedUrl(resolved && resolved.feedUrl);
  if (feedUrl) setInlineFeedResolveCachedValue(cacheKey, feedUrl);
  else clearInlineFeedResolveCachedValue(cacheKey);
  if (feedUrl) {
    const favBtn = row ? row.querySelector('.favorite-link') : null;
    if (favBtn && !String(favBtn.getAttribute('data-favorite-feed') || '').trim()) {
      favBtn.setAttribute('data-favorite-feed', feedUrl);
...
function getAppleFeedResolutionMetaForPlaylistItem(item) {
  const favAttrs = item && item.favAttrs && typeof item.favAttrs === 'object' ? item.favAttrs : {};
  const directFeedUrl = normalizeFavoriteFeedUrl(favAttrs['data-favorite-feed'] || item?.resolvedFeedUrl || '');
  const pageUrl = String(favAttrs['data-favorite-url'] || item?.openUrl || '').trim();
...
async function resolveAppleFeedUrlForPlaylistItem(item) {
  const meta = getAppleFeedResolutionMetaForPlaylistItem(item);
  if (meta.feedUrl) return meta;
  if (!meta.showId) return { ...meta, feedUrl: '' };
  const cacheKey = `apple::${String(meta.showId || '').trim()}::${normalizeCountry(meta.countryCode || 'us')}`;
  const cached = getInlineFeedResolveCachedValue(cacheKey);
  if (cached) {
    const feedUrl = normalizeFavoriteFeedUrl(cached.feedUrl || '');
    if (feedUrl) {
      item.favAttrs = item.favAttrs || {};
      item.favAttrs['data-favorite-feed'] = feedUrl;
      item.resolvedFeedUrl = feedUrl;
    }
    return { ...meta, feedUrl };
  }
  const resolved = await resolveApplePodcastImportMeta(`https://podcasts.apple.com/${meta.countryCode}/podcast/id${meta.showId}`, meta.countryCode).catch(() => null);
...
const spotifyFeedResolveCache = new Map();

function normalizePodcastMatchText(value) {
  return String(value || '')
    .toLowerCase()
    .normalize('NFKC')
    .replace(/&/g, ' and ')
    .replace(/[^\p{L}\p{N}\s]+/gu, ' ')
    .replace(/\b(the|podcast|official|show|with)\b/gu, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function scorePodcastSearchCandidate(candidate, title, publisher = '') {
  const targetTitle = normalizePodcastMatchText(title);
  const targetPublisher = normalizePodcastMatchText(publisher);
  const candidateTitle = normalizePodcastMatchText(candidate && (candidate.collectionName || candidate.trackName) || '');
  const candidatePublisher = normalizePodcastMatchText(candidate && candidate.artistName || '');
  let score = 0;
  if (!targetTitle || !candidateTitle) return -1;
  if (candidateTitle === targetTitle) score += 80;
  else if (candidateTitle.includes(targetTitle) || targetTitle.includes(candidateTitle)) score += 58;
  else {
    const targetTokens = new Set(targetTitle.split(' ').filter(Boolean));
    const candidateTokens = new Set(candidateTitle.split(' ').filter(Boolean));
    let overlap = 0;
    targetTokens.forEach((token) => { if (candidateTokens.has(token)) overlap += 1; });
    score += Math.round((overlap / Math.max(1, targetTokens.size)) * 42);
  }
  if (targetPublisher && candidatePublisher) {
    if (candidatePublisher === targetPublisher) score += 28;
    else if (candidatePublisher.includes(targetPublisher) || targetPublisher.includes(candidatePublisher)) score += 18;
    else {
      const targetTokens = new Set(targetPublisher.split(' ').filter(Boolean));
      const candidateTokens = new Set(candidatePublisher.split(' ').filter(Boolean));
      let overlap = 0;
      targetTokens.forEach((token) => { if (candidateTokens.has(token)) overlap += 1; });
      score += Math.round((overlap / Math.max(1, targetTokens.size)) * 14);
    }
  }
  return score;
}

function searchItunesPodcastsByJsonp(term, countryCode, timeoutMs = 9000) {
  return new Promise((resolve, reject) => {
    const query = String(term || '').trim();
    if (!query) {
      reject(new Error('Missing search term'));
      return;
    }
    const callbackName = '__podcastItunesSearchCb_' + Date.now() + '_' + Math.floor(Math.random() * 10000);
    const script = document.createElement('script');
    let done = false;
    const cleanup = () => {
      if (done) return;
    ...
    script.async = true;
    script.src =
      'https://itunes.apple.com/search?media=podcast&entity=podcast&limit=12&country=' +
      encodeURIComponent(normalizeCountry(countryCode || 'us')) +
      '&term=' +
...
function getSpotifyFeedResolutionMetaForRow(row, button = null) {
  const playBtn = button || (row ? row.querySelector('.play-link') : null);
  const favoriteBtn = row ? row.querySelector('.favorite-link') : null;
  const embedKind = String(playBtn?.getAttribute('data-embed-kind') || '').trim().toLowerCase();
  const rowTitle = String(row?.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row?.querySelector('.title')?.textContent || '').trim();
  const rowPublisher = String(row?.querySelector('.publisher')?.textContent || '').trim();
  const isEpisode = isEpisodeChartTypeValue(embedKind);
  const searchTitle = isEpisode ? rowPublisher : rowTitle;
  const searchPublisher = isEpisode ? '' : rowPublisher;
  return {
    feedUrl: normalizeFavoriteFeedUrl(favoriteBtn?.getAttribute('data-favorite-feed') || ''),
    title: rowTitle,
    publisher: rowPublisher,
    searchTitle: String(searchTitle || '').trim(),
    searchPublisher: String(searchPublisher || '').trim(),
    embedKind,
    embedUrl: String(playBtn?.getAttribute('data-embed-url') || '').trim(),
    countryCode: normalizeCountry((countrySelect && countrySelect.value) || 'us')
  };
}

async function resolveSpotifyFeedUrlForRow(row, button = null) {
  const meta = getSpotifyFeedResolutionMetaForRow(row, button);
  if (meta.feedUrl) return meta;
  const queryTitle = String(meta.searchTitle || meta.title || '').trim();
  const queryPublisher = String(meta.searchPublisher || meta.publisher || '').trim();
  const cacheKey = `${normalizePodcastMatchText(queryTitle)}::${normalizePodcastMatchText(queryPublisher)}`;
  const cached = spotifyFeedResolveCache.get(cacheKey);
  if (cached && (Date.now() - cached.updatedAt) < (24 * 60 * 60 * 1000)) {
    return { ...meta, feedUrl: cached.feedUrl || '' };
  }
  const persisted = getInlineFeedResolveCachedValue(`spotify::${cacheKey}`);
  if (persisted) {
    const feedUrl = normalizeFavoriteFeedUrl(persisted.feedUrl || '');
    spotifyFeedResolveCache.set(cacheKey, { feedUrl, updatedAt: Date.now() });
    if (feedUrl) {
      const favBtn = row ? row.querySelector('.favorite-link') : null;
      if (favBtn && !String(favBtn.getAttribute('data-favorite-feed') || '').trim()) {
        favBtn.setAttribute('data-favorite-feed', feedUrl);
  ...
  const terms = [
    `${queryTitle} ${queryPublisher}`.trim(),
    queryTitle
  ].filter(Boolean);
  let best = null;
  for (const term of terms) {
    let payload = null;
    try {
      payload = await searchItunesPodcastsByJsonp(term, meta.countryCode);
    } catch {
      payload = null;
    }
    const results = Array.isArray(payload && payload.results) ? payload.results : [];
    for (const candidate of results) {
      const score = scorePodcastSearchCandidate(candidate, queryTitle, queryPublisher);
  ...
  const feedUrl = best && best.score >= 62 ? normalizeFavoriteFeedUrl(best.candidate && best.candidate.feedUrl) : '';
  if (feedUrl) {
    spotifyFeedResolveCache.set(cacheKey, { feedUrl, updatedAt: Date.now() });
    setInlineFeedResolveCachedValue(`spotify::${cacheKey}`, feedUrl);
  } else {
    spotifyFeedResolveCache.delete(cacheKey);
    clearInlineFeedResolveCachedValue(`spotify::${cacheKey}`);
  }
  if (feedUrl) {
    const favBtn = row ? row.querySelector('.favorite-link') : null;
    if (favBtn && !String(favBtn.getAttribute('data-favorite-feed') || '').trim()) {
...
function getSpotifyFeedResolutionMetaForPlaylistItem(item) {
  const favAttrs = item && item.favAttrs && typeof item.favAttrs === 'object' ? item.favAttrs : {};
  const embedKind = String(item?.embedKind || '').trim().toLowerCase();
  const rowTitle = String(item?.title || '').trim();
  const rowPublisher = String(item?.publisher || '').trim();
  const isEpisode = isEpisodeChartTypeValue(embedKind);
  const searchTitle = isEpisode ? rowPublisher : rowTitle;
  const searchPublisher = isEpisode ? '' : rowPublisher;
  return {
    feedUrl: normalizeFavoriteFeedUrl(favAttrs['data-favorite-feed'] || item?.resolvedFeedUrl || ''),
    title: rowTitle,
    publisher: rowPublisher,
    searchTitle: String(searchTitle || '').trim(),
    searchPublisher: String(searchPublisher || '').trim(),
...
async function resolveSpotifyFeedUrlForPlaylistItem(item) {
  const meta = getSpotifyFeedResolutionMetaForPlaylistItem(item);
  if (meta.feedUrl) return meta;
  const queryTitle = String(meta.searchTitle || meta.title || '').trim();
  const queryPublisher = String(meta.searchPublisher || meta.publisher || '').trim();
  const cacheKey = `${normalizePodcastMatchText(queryTitle)}::${normalizePodcastMatchText(queryPublisher)}`;
...
function bindInlineRssResolveRetry(row, modeHost, platform, retryFn) {
  if (!row || !modeHost || typeof retryFn !== 'function') return;
  bindInlinePlayerModeControls(row);
  const retryBtn = modeHost.querySelector('[data-inline-rss-retry]');
  if (!retryBtn) return;
  retryBtn.addEventListener('click', (event) => {
    event.preventDefault();
    retryFn(retryBtn).catch(() => {});
  });
}

async function switchAppleRowPlayerMode(row, mode, trigger = null) {
  if (!row) return false;
  const slot = row.querySelector('.episode-player-wrap');
  const playBtn = row.querySelector('.play-link');
  if (!slot || !playBtn) return false;
  const requestedMode = String(mode || 'apple').trim().toLowerCase() === 'rss' ? 'rss' : 'apple';
  const currentMode = String(row.dataset.appleInlineMode || 'apple').trim().toLowerCase() || 'apple';
  const isVisibleOpen = row.classList.contains('player-open') && !slot.hasAttribute('hidden') && !!slot.querySelector('[data-episode-mode-host], .castbox-rss-player, iframe');
  if (currentMode === requestedMode && isVisibleOpen) return true;
  const embedUrl = String(playBtn.getAttribute('data-embed-url') || '').trim();
  const embedKind = String(playBtn.getAttribute('data-embed-kind') || 'top-podcasts').trim().toLowerCase();
  const titleText = String(row.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row.querySelector('.title')?.textContent || '').trim();
  const rowDescription = String(row.getAttribute('data-description') || '').trim();
  const descriptionMarkup = buildEpisodePlayerDescriptionMarkup(rowDescription);
  ...
  try {
    if (requestedMode === 'rss') {
      const accentPlatform = getCurrentPlayerAccentPlatform(row, 'apple');
      slot.innerHTML = buildApplePlayerModeAreaMarkup('rss', true, descriptionMarkup, true);
      applyPlayerAccentVars(slot, accentPlatform);
      slot.removeAttribute('hidden');
      row.classList.add('player-open');
      document.body.classList.add('player-active');
      playBtn.classList.add('is-active');
      playBtn.setAttribute('aria-pressed', 'true');
      playBtn.setAttribute('aria-label', getClosePlayerActionLabel());
      playBtn.setAttribute('title', getClosePlayerActionLabel());
      activeEpisodePlayerRow = row;
      const modeHost = slot.querySelector('[data-episode-mode-host]');
      if (!modeHost) return false;
      modeHost.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${accentPlatform}"><div class="castbox-rss-loading">${escapeHtml(t('loading'))}…</div></div>`;
      setIframePlaybackActive(row, false);
      const resolved = await resolveAppleFeedUrlForRow(row, playBtn);
      if (!resolved.feedUrl) {
        const cacheKey = `apple::${String(resolved.showId || '').trim()}::${normalizeCountry(resolved.countryCode || 'us')}`;
        clearInlineFeedResolveCachedValue(cacheKey);
        const favBtn = row ? row.querySelector('.favorite-link') : null;
        if (favBtn) favBtn.removeAttribute('data-favorite-feed');
        modeHost.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${accentPlatform}"><div class="castbox-rss-error-wrap"><div class="castbox-rss-error">${escapeHtml(t('loadFailedRefresh'))}</div><button class="castbox-rss-refresh" type="button" data-inline-rss-retry>${escapeHtml(t('refresh'))}</button></div></div>`;
        bindInlineRssResolveRetry(row, modeHost, 'apple', async (retryBtn) => {
          retryBtn.disabled = true;
          retryBtn.classList.add('is-loading');
          try {
            await switchAppleRowPlayerMode(row, 'rss', retryBtn);
          } finally {
      ...
      await renderCastboxRssPlayerInSlot(modeHost, accentPlatform, resolved.feedUrl, titleText, true, { includeAd: false });
      row.dataset.appleInlineMode = 'rss';
      setDefaultRssPlayerModePreference('rss');
      bindInlinePlayerModeControls(row);
      syncEpisodePlayerDescription(row);
      const adNode = slot.querySelector('.episode-player-ad .adsbygoogle');
      if (adNode) initializeAdsbygooglePush();
      return true;
    }

    setDefaultRssPlayerModePreference('apple');
    return await openEpisodePlayerRow(row, playBtn, { respectPreferredMode: false });
  } finally {
...
async function switchSpotifyRowPlayerMode(row, mode, trigger = null) {
  if (!row) return false;
  const slot = row.querySelector('.episode-player-wrap');
  const playBtn = row.querySelector('.play-link');
  if (!slot || !playBtn) return false;
  const requestedMode = String(mode || 'spotify').trim().toLowerCase() === 'rss' ? 'rss' : 'spotify';
  const currentMode = String(row.dataset.spotifyInlineMode || 'spotify').trim().toLowerCase() || 'spotify';
  const isVisibleOpen = row.classList.contains('player-open') && !slot.hasAttribute('hidden') && !!slot.querySelector('[data-episode-mode-host], .castbox-rss-player, iframe');
  if (requestedMode === currentMode && isVisibleOpen) return true;
  const titleText = String(row.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row.querySelector('.title')?.textContent || '').trim();
  const rowDescription = String(row.getAttribute('data-description') || '').trim();
  const descriptionMarkup = buildEpisodePlayerDescriptionMarkup(rowDescription);
  if (trigger) {
  ...
  try {
    if (requestedMode === 'rss') {
      if (typeof globalThis !== 'undefined') globalThis.__spotifyRssSwitchSuppressToastUntil = Date.now() + 3000;
      const accentPlatform = getCurrentPlayerAccentPlatform(row, 'spotify');
      slot.innerHTML = buildSourceRssPlayerModeAreaMarkup('spotify', 'rss', true, descriptionMarkup, true);
      applyPlayerAccentVars(slot, accentPlatform);
      slot.removeAttribute('hidden');
      row.classList.add('player-open');
      document.body.classList.add('player-active');
      ...
      modeHost.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${accentPlatform}"><div class="castbox-rss-loading">${escapeHtml(t('loading'))}…</div></div>`;
      setIframePlaybackActive(row, false);
      const resolved = await resolveSpotifyFeedUrlForRow(row, playBtn);
      if (!resolved.feedUrl) {
        if (typeof globalThis !== 'undefined') globalThis.__spotifyRssSwitchSuppressToastUntil = 0;
        const cacheKey = `${normalizePodcastMatchText(String(resolved.searchTitle || resolved.title || '').trim())}::${normalizePodcastMatchText(String(resolved.searchPublisher || resolved.publisher || '').trim())}`;
        spotifyFeedResolveCache.delete(cacheKey);
        clearInlineFeedResolveCachedValue(`spotify::${cacheKey}`);
        const favBtn = row ? row.querySelector('.favorite-link') : null;
        if (favBtn) favBtn.removeAttribute('data-favorite-feed');
        modeHost.innerHTML = `<div class="episode-player-shell rss-player-shell platform-${accentPlatform}"><div class="castbox-rss-error-wrap"><div class="castbox-rss-error">${escapeHtml(t('loadFailedRefresh'))}</div><button class="castbox-rss-refresh" type="button" data-inline-rss-retry>${escapeHtml(t('refresh'))}</button></div></div>`;
        bindInlineRssResolveRetry(row, modeHost, 'spotify', async (retryBtn) => {
          retryBtn.disabled = true;
          retryBtn.classList.add('is-loading');
          try {
            await switchSpotifyRowPlayerMode(row, 'rss', retryBtn);
          } finally {
            if (retryBtn.isConnected) {
              retryBtn.disabled = false;
              retryBtn.classList.remove('is-loading');
      ...
      await renderCastboxRssPlayerInSlot(modeHost, accentPlatform, resolved.feedUrl, titleText, true, { includeAd: false });
      row.dataset.spotifyInlineMode = 'rss';
      setDefaultRssPlayerModePreference('rss');
      bindInlinePlayerModeControls(row);
      syncEpisodePlayerDescription(row);
      const adNode = slot.querySelector('.episode-player-ad .adsbygoogle');
    ...
    setDefaultRssPlayerModePreference('spotify');
    return await openEpisodePlayerRow(row, playBtn, { respectPreferredMode: false });
  } finally {
    if (requestedMode !== 'rss' && typeof globalThis !== 'undefined') globalThis.__spotifyRssSwitchSuppressToastUntil = 0;
    if (trigger && trigger.isConnected) {
      trigger.disabled = false;
      trigger.classList.remove('is-loading');
    }
...
function buildGlobalYouTubeQueueItemFromRow(row, button = null) {
  const playBtn = button || (row ? row.querySelector('.play-link') : null);
  if (!row || !playBtn) return null;
  const embedUrl = String(playBtn.getAttribute('data-embed-url') || '').trim();
  const videoId = extractVideoIdFromEmbedUrl(embedUrl);
  const playlistId = extractPlaylistIdFromEmbedUrl(embedUrl);
  if (!videoId && !playlistId) return null;
  const title = String(row.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row.querySelector('.title')?.textContent || '').trim();
  if (!title) return null;
  const podcastTitle = String(row.getAttribute('data-item-podcast-title') || row.querySelector('.publisher')?.textContent || '').trim();
  const publishedText = String(row.getAttribute('data-item-published') || '').trim();
  const durationText = formatLooseMediaDurationLabel(row.getAttribute('data-item-duration') || '');
  const rowMeta = String(row.getAttribute('data-item-meta') || '').trim();
  return normalizeGlobalRssQueueItem({
    id: `youtube::${playlistId || videoId}`,
    platform: 'youtube',
    videoId,
    playlistId,
    title,
    meta: buildCompactMediaMetaText(publishedText, durationText, rowMeta && rowMeta !== podcastTitle ? rowMeta : ''),
    link: String(row.querySelector('.open-link')?.getAttribute('href') || '').trim(),
    image: String(row.querySelector('.cover')?.getAttribute('src') || '').trim() || buildYouTubeThumbnailFromUrl(`https://www.youtube.com/watch?v=${videoId}`),
    podcastTitle,
    publishedText,
    durationText,
    accentPlatform: getCurrentPlayerAccentPlatform(row, 'youtube'),
    embedUrl
  });
}

function buildGlobalBilibiliQueueItemFromRow(row, button = null) {
  const playBtn = button || (row ? row.querySelector('.play-link') : null);
  if (!row || !playBtn) return null;
  const embedUrl = String(playBtn.getAttribute('data-embed-url') || '').trim();
  const meta = parseBilibiliEmbedMetaFromUrl(embedUrl);
  if (!embedUrl || !meta) return null;
  const title = String(row.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row.querySelector('.title')?.textContent || '').trim();
  if (!title) return null;
  const podcastTitle = String(row.getAttribute('data-item-podcast-title') || row.querySelector('.publisher')?.textContent || '').trim();
  const publishedText = String(row.getAttribute('data-item-published') || '').trim();
  const durationText = formatLooseMediaDurationLabel(row.getAttribute('data-item-duration') || '');
  const durationSeconds = Math.max(0, Number(row.getAttribute('data-item-duration-seconds')) || parseLooseMediaDurationSeconds(durationText));
  const rowMeta = String(row.getAttribute('data-item-meta') || '').trim();
  const description = String(row.getAttribute('data-description') || '').trim();
  return normalizeGlobalRssQueueItem({
    id: `bilibili::${meta.aid || meta.bvid || meta.cid}`,
    platform: 'bilibili',
    aid: meta.aid,
    bvid: meta.bvid,
    cid: meta.cid,
    ...
    meta: buildCompactMediaMetaText(publishedText, durationText, rowMeta && rowMeta !== podcastTitle ? rowMeta : ''),
    link: String(row.querySelector('.open-link')?.getAttribute('href') || '').trim(),
    image: String(row.querySelector('.cover')?.getAttribute('src') || '').trim(),
    podcastTitle,
    description,
    publishedText,
    durationText,
    durationSeconds,
    accentPlatform: getCurrentPlayerAccentPlatform(row, 'bilibili'),
    embedUrl
  });
}

function buildGlobalVideoQueueItemFromRow(row, button = null) {
  const playBtn = button || (row ? row.querySelector('.play-link') : null);
  const platform = String(playBtn?.getAttribute('data-embed-platform') || '').trim().toLowerCase();
  if (platform === 'youtube') return buildGlobalYouTubeQueueItemFromRow(row, playBtn);
  if (platform === 'bilibili') return buildGlobalBilibiliQueueItemFromRow(row, playBtn);
  return null;
}

function collectAudioModeTargetRows() {
  const rows = Array.from(chartContainer.querySelectorAll('.chart-row:not(.chart-ad)'));
  const query = String(searchInput && searchInput.value || '').trim().toLowerCase();
  if (!query) return rows;
  return rows.filter((row) => String(row.dataset.search || '').toLowerCase().includes(query));
}

function buildAudioModeJobSignature(rows) {
  const sourceRows = Array.isArray(rows) ? rows : [];
  return sourceRows.map((row) => {
    const playBtn = row ? row.querySelector('.play-link') : null;
    const embedPlatform = String(playBtn?.getAttribute('data-embed-platform') || '').trim().toLowerCase();
    const embedUrl = String(playBtn?.getAttribute('data-embed-url') || '').trim();
    const title = String(row?.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row?.querySelector('.title')?.textContent || '').trim();
    const publisher = String(row?.getAttribute('data-item-podcast-title') || row?.querySelector('.publisher')?.textContent || '').trim();
    return `${embedPlatform}::${embedUrl}::${title}::${publisher}`;
  }).join('|');
}

function normalizeAudioModeEpisodeTitle(value) {
  return String(value || '')
    .trim()
    .toLowerCase()
    .replace(/[’'"]/g, '')
...
function scoreAudioModeEpisodeMatch(targetTitle, candidateTitle) {
  const target = normalizeAudioModeEpisodeTitle(targetTitle);
  const candidate = normalizeAudioModeEpisodeTitle(candidateTitle);
  if (!target || !candidate) return 0;
  if (target === candidate) return 120;
  if (candidate.includes(target)) return 94;
  if (target.includes(candidate)) return 88;
  const targetWords = target.split(' ').filter(Boolean);
  const candidateWords = new Set(candidate.split(' ').filter(Boolean));
  if (!targetWords.length || !candidateWords.size) return 0;
  let matches = 0;
...
async function resolveAudioModeFeedUrlForRow(row) {
  if (!row) return '';
  const favoriteBtn = row.querySelector('.favorite-link');
  const directFeed = normalizeFavoriteFeedUrl(favoriteBtn?.getAttribute('data-favorite-feed') || '');
  if (directFeed) return directFeed;
  const playBtn = row.querySelector('.play-link');
  if (!playBtn) return '';
  const embedPlatform = String(playBtn.getAttribute('data-embed-platform') || '').trim().toLowerCase();
  const embedUrl = String(playBtn.getAttribute('data-embed-url') || '').trim();
  if ((embedPlatform === 'rss' || embedPlatform === 'castbox' || embedPlatform === 'pocketcasts') && embedUrl) {
    return normalizeFavoriteFeedUrl(embedUrl);
  }
  if (embedPlatform === 'apple') {
    const resolved = await resolveAppleFeedUrlForRow(row, playBtn).catch(() => null);
    return normalizeFavoriteFeedUrl(resolved && resolved.feedUrl);
  }
  if (embedPlatform === 'spotify') {
    const resolved = await resolveSpotifyFeedUrlForRow(row, playBtn).catch(() => null);
    return normalizeFavoriteFeedUrl(resolved && resolved.feedUrl);
  }
  return normalizeFavoriteFeedUrl(favoriteBtn?.getAttribute('data-favorite-feed') || '');
}

function buildAudioModeQueueItemFromFeedEpisode(row, feed, episode, feedUrl) {
  if (!row || !episode || !feed) return null;
  const openLink = String(row.querySelector('.open-link')?.getAttribute('href') || episode.link || '').trim();
  const accentPlatform = getCurrentPlayerAccentPlatform(row, 'rss');
  return normalizeGlobalRssQueueItem({
    id: `${normalizeFavoriteFeedUrl(feedUrl) || 'feed'}::${String(episode.guid || episode.audioUrl || '').trim()}`,
    src: String(episode.audioUrl || '').trim(),
    guid: String(episode.guid || episode.audioUrl || '').trim(),
    title: String(episode.title || '').trim(),
    link: openLink,
    feedUrl: normalizeFavoriteFeedUrl(feedUrl),
    podcastTitle: String(feed.title || row.getAttribute('data-item-podcast-title') || row.querySelector('.publisher')?.textContent || '').trim(),
    publishedText: String(episode.pubDateRaw || '').trim(),
    duration: String(episode.duration || '').trim(),
    image: String(feed.image || row.querySelector('.cover')?.getAttribute('src') || '').trim(),
    accentPlatform,
    meta: buildCompactMediaMetaText(String(episode.pubDateRaw || '').trim(), formatLooseMediaDurationLabel(episode.duration || ''), '')
  });
}

function buildAudioModeDirectQueueItemForRow(row) {
  if (!row) return null;
  const playBtn = row.querySelector('.play-link');
  const embedPlatform = String(playBtn?.getAttribute('data-embed-platform') || '').trim().toLowerCase();
  if (embedPlatform === 'youtube' || embedPlatform === 'bilibili') {
    return buildGlobalVideoQueueItemFromRow(row, playBtn);
  }
  return null;
}

function pickAudioModeEpisodeFromFeed(row, feed) {
  const episodes = feed && Array.isArray(feed.episodes) ? feed.episodes : [];
  if (!episodes.length) return null;
  const playBtn = row ? row.querySelector('.play-link') : null;
  const embedKind = String(playBtn?.getAttribute('data-embed-kind') || '').trim().toLowerCase();
  if (!isEpisodeChartTypeValue(embedKind)) {
    return episodes[0];
  }
  const targetTitle = String(row.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row.querySelector('.title')?.textContent || '').trim();
  let best = null;
  episodes.forEach((episode, index) => {
    const score = scoreAudioModeEpisodeMatch(targetTitle, episode && episode.title);
    if (!best || score > best.score || (score === best.score && Number(episode && episode.pubDate) > Number(best.episode && best.episode.pubDate)) || (score === best.score && index < best.index)) {
      best = { episode, score, index };
    }
  });
  return best && best.score >= 52 ? best.episode : null;
}

function withPromiseTimeout(promise, timeoutMs = 15000, label = 'Request timed out') {
  const safeTimeout = Math.max(2000, Number(timeoutMs) || 15000);
  return Promise.race([
    Promise.resolve(promise),
    new Promise((_, reject) => {
      setTimeout(() => reject(new Error(String(label || 'Request timed out'))), safeTimeout);
    })
  ]);
}

function delayAudioMode(ms = 0) {
  return new Promise((resolve) => {
    setTimeout(resolve, Math.max(0, Number(ms) || 0));
  });
}

function appendAudioModeItemToPlayer(player, item, state) {
  if (!player || !item || !state) return false;
  if (player.findItemIndex(item) >= 0) {
    state.skippedCount = Number(state.skippedCount || 0) + 1;
    return false;
  }
  if (!state.started && !player.queue.length) {
    player.replaceQueue([item], { playNow: true, autoplay: true, startIndex: 0 });
  } else {
    player.enqueue(item, { playNow: false });
  }
  state.started = true;
...
async function processAudioModeJob(job) {
  const rows = Array.isArray(job && job.rows) ? job.rows : [];
  const trigger = job && job.trigger ? job.trigger : null;
  const signature = String(job && job.signature || '').trim();
  ...
  setStatus('loading');
  audioModeActiveSignature = signature;
  try {
    const player = globalRssPlayerController.ensureMounted();
    const state = { resolvedCount: 0, skippedCount: 0, started: false };
    let pendingRows = Array.isArray(rows) ? rows.slice() : [];
    for (let round = 0; round < (AUDIO_MODE_RETRY_DELAYS_MS.length + 1) && pendingRows.length; round += 1) {
      if (round > 0) {
        await delayAudioMode(AUDIO_MODE_RETRY_DELAYS_MS[round - 1] || 0);
      }
      const failedRows = [];
      for (let i = 0; i < pendingRows.length; i += 1) {
        const row = pendingRows[i];
        const directItem = buildAudioModeDirectQueueItemForRow(row);
        if (directItem) {
          appendAudioModeItemToPlayer(player, directItem, state);
          continue;
        }
        const feedUrl = await withPromiseTimeout(resolveAudioModeFeedUrlForRow(row), 12000, 'RSS link timeout').catch(() => '');
        if (!feedUrl) continue;
        const feed = await withPromiseTimeout(loadRssFeedWithRetry(feedUrl, 1), 16000, 'RSS load timeout').catch(() => null);
        if (!feed) {
          failedRows.push(row);
          continue;
        }
        const episode = pickAudioModeEpisodeFromFeed(row, feed);
        if (!episode) continue;
        const item = buildAudioModeQueueItemFromFeedEpisode(row, feed, episode, feedUrl);
        if (!item) continue;
        appendAudioModeItemToPlayer(player, item, state);
      }
      pendingRows = failedRows;
    }
    if (!state.resolvedCount) {
      showToast(t('loadFailedRefresh'));
      return false;
    }
    showToast(`${state.resolvedCount} ${state.resolvedCount === 1 ? 'item' : 'items'} queued`);
    return true;
  } finally {
    audioModeActiveSignature = '';
    setStatus('cache');
    if (trigger && trigger.isConnected) {
      trigger.disabled = false;
      trigger.classList.remove('is-loading');
...
async function drainAudioModeQueue() {
  if (audioModeProcessing) return;
  audioModeProcessing = true;
  try {
    while (audioModeJobQueue.length) {
      const job = audioModeJobQueue.shift();
      try {
        await processAudioModeJob(job);
      } catch (error) {
        console.warn('Audio mode job failed:', error);
      }
    }
  } finally {
    audioModeProcessing = false;
  }
}

async function enterAudioModeFromCurrentChart(trigger = null) {
  const rows = collectAudioModeTargetRows();
  if (!rows.length) {
    showToast(t('noData'));
    return false;
  }
  const signature = buildAudioModeJobSignature(rows);
  if ((audioModeProcessing && signature && signature === audioModeActiveSignature)
    || audioModeJobQueue.some((job) => String(job && job.signature || '').trim() === signature)) {
    showToast('Audio Mode is already processing');
    return true;
  }
  audioModeJobQueue.push({ rows, trigger, signature });
  if (audioModeProcessing) {
    showToast('Audio Mode queued');
    return true;
  }
  void drainAudioModeQueue();
  return true;
}

function syncInlineYoutubeQueueButton(row) {
  if (!row) return;
  const btn = row.querySelector('[data-episode-queue-add]');
  if (!btn) return;
  const globalPlayer = globalRssPlayerController.ensureMounted();
  const item = buildGlobalYouTubeQueueItemFromRow(row);
  const queued = !!(item && globalPlayer.findItemIndex(item) >= 0);
  btn.classList.toggle('is-active', queued);
  btn.classList.toggle('is-queue-added', queued);
  btn.setAttribute('aria-pressed', queued ? 'true' : 'false');
  btn.setAttribute('aria-label', queued ? 'Added to queue' : 'Add to queue');
  btn.setAttribute('title', queued ? 'Added to queue' : 'Add to queue');
  btn.innerHTML = queued ? getRssQueueAddedSvg() : getRssQueueAddSvg();
...
function syncInlineVideoQueueButton(row) {
  if (!row) return;
  const playBtn = row.querySelector('.play-link');
  const platform = String(playBtn?.getAttribute('data-embed-platform') || '').trim().toLowerCase();
  if (platform === 'youtube') {
    syncInlineYoutubeQueueButton(row);
    return;
  }
  const btn = row.querySelector('[data-episode-queue-add]');
  if (!btn) return;
  const globalPlayer = globalRssPlayerController.ensureMounted();
  const item = buildGlobalVideoQueueItemFromRow(row);
  const queued = !!(item && globalPlayer.findItemIndex(item) >= 0);
  btn.classList.toggle('is-active', queued);
  btn.classList.toggle('is-queue-added', queued);
  btn.setAttribute('aria-pressed', queued ? 'true' : 'false');
...
function bindInlinePlayerModeControls(row) {
  if (!row) return;
  const slot = row.querySelector('.episode-player-wrap');
  if (!slot || slot.dataset.modeControlsBound === '1') return;
  slot.dataset.modeControlsBound = '1';
  slot.addEventListener('click', async (event) => {
    const modeBtn = event.target && event.target.closest ? event.target.closest('[data-episode-mode]') : null;
    if (modeBtn) {
      event.preventDefault();
      const mode = String(modeBtn.getAttribute('data-episode-mode') || '').trim().toLowerCase();
      if (mode === 'apple' || mode === 'rss') {
        if (row.querySelector('.play-link')?.getAttribute('data-embed-platform') === 'apple') {
          await switchAppleRowPlayerMode(row, mode, modeBtn);
          bindInlinePlayerModeControls(row);
          return;
        }
      }
      if (mode === 'spotify' || mode === 'rss') {
        if (row.querySelector('.play-link')?.getAttribute('data-embed-platform') === 'spotify') {
          await switchSpotifyRowPlayerMode(row, mode, modeBtn);
          bindInlinePlayerModeControls(row);
          return;
        }
      }
    ...
    const queueBtn = event.target && event.target.closest ? event.target.closest('[data-episode-queue-add]') : null;
    if (queueBtn) {
      event.preventDefault();
      const item = buildGlobalVideoQueueItemFromRow(row);
      if (!item) return;
      globalRssPlayerController.ensureMounted().enqueue(item, { playNow: false });
      syncInlineVideoQueueButton(row);
    }
  });
  document.addEventListener(GLOBAL_RSS_PLAYER_EVENT, () => {
    try { syncInlineVideoQueueButton(row); } catch {}
  });
}
...
async function performShareAction(url) {
...
function buildCommandButtonIconSvg(kind) {
  const safeKind = String(kind || '').trim().toLowerCase();
  if (safeKind === 'surprise') return getSurpriseSparkSvg();
  if (safeKind === 'share') return getShareExportSvg();
  if (safeKind === 'refresh') return getEpisodeRefreshSvg();
  if (safeKind === 'my-podcasts') return getFavoriteHeartSvg();
  if (safeKind === 'playlist') return PLM_ICON_SVG;
  if (safeKind === 'audio-mode') return getAudioModeSvg();
  return '';
}

function decorateCommandButton(button, kind, label) {
  if (!button) return;
  const safeKind = String(kind || '').trim().toLowerCase();
  const safeLabel = String(label || '').trim();
  const svg = buildCommandButtonIconSvg(safeKind);
  if (!svg) return;
  button.dataset.commandKind = safeKind;
  button.setAttribute('aria-label', safeLabel);
  button.setAttribute('title', safeLabel);
  button.innerHTML = `<span class="command-btn-icon" aria-hidden="true">${svg}</span><span class="sr-only">${escapeHtml(safeLabel)}</span>`;
}

function decorateCommandActionButtons() {
  const surprise = document.getElementById('random-btn');
  if (surprise) decorateCommandButton(surprise, 'surprise', t('surprise'));
  const share = document.getElementById('share-btn');
  if (share) decorateCommandButton(share, 'share', t('share'));
  const refresh = document.getElementById('refresh-btn');
  if (refresh) decorateCommandButton(refresh, 'refresh', t('refresh'));
  const myBtn = document.getElementById('my-podcasts-btn');
  if (myBtn) decorateCommandButton(myBtn, 'my-podcasts', buildMyPodcastsScopeLabel());
  const playlistBtn = document.getElementById('playlist-mode-btn');
  if (playlistBtn) decorateCommandButton(playlistBtn, 'playlist', tOr('playlistMode', 'Playlist Mode'));
  const audioBtn = document.getElementById('audio-mode-btn');
  if (audioBtn) decorateCommandButton(audioBtn, 'audio-mode', tOr('audioMode', 'Audio Mode'));
}
...
function applyAppleEpisodeShellLayout(shell) {
  ...
  const iframe = shell.querySelector('.episode-player-frame.apple.is-episode');
  if (shell.classList.contains('is-video')) {
    const width = Math.max(0, Number(shell.clientWidth) || 0) || 660;
...
function buildSanitizedPlayerDescription(raw, options = {}) {
  const source = String(raw || '').trim();
  if (!source) return { text: '', html: '' };
  const allowHtml = Boolean(options.allowHtml);
  const parser = new DOMParser();
  const doc = parser.parseFromString(source, 'text/html');
  doc.querySelectorAll('script,style,iframe,object,embed,svg,video,audio,img,picture,source').forEach((node) => node.remove());
  ...
  textNodes.forEach((textNode) => {
    ...
    if (textNode.parentNode.nodeName.toLowerCase() === 'a') return;
    const value = String(textNode.nodeValue || '');
    if (!/https?:\/\//i.test(value)) return;
    const frag = doc.createDocumentFragment();
    const pattern = /(https?:\/\/[^\s<]+)/gi;
    let lastIndex = 0;
    value.replace(pattern, (match, url, offset) => {
      if (offset > lastIndex) frag.appendChild(doc.createTextNode(value.slice(lastIndex, offset)));
    ...
    if (lastIndex < value.length) frag.appendChild(doc.createTextNode(value.slice(lastIndex)));
    textNode.parentNode.replaceChild(frag, textNode);
  });
  const text = String(doc.body.textContent || '').replace(/\s+/g, ' ').trim();
  if (!allowHtml) return { text, html: '' };
  return {
    text,
    html: String(doc.body.innerHTML || '').trim()
  };
}

function buildEpisodePlayerDescriptionMarkup(rawDescription) {
  const prepared = buildSanitizedPlayerDescription(rawDescription, { allowHtml: true });
  if (!prepared.text) return '';
  return `
    <div class="episode-player-description">
      <div class="episode-player-description-preview">
        <div class="episode-player-description-source">${prepared.html || escapeHtml(prepared.text)}</div>
        <div class="episode-player-description-translation episode-player-description-translation-preview" hidden>
          <div class="episode-player-description-divider" aria-hidden="true"></div>
          <div class="episode-player-description-translation-body"></div>
        </div>
      </div>
      <button class="episode-player-description-toggle" type="button" aria-expanded="false" aria-label="${escapeHtml(tOr('open', 'Open'))}" hidden>...</button>
      <div class="episode-player-description-expanded" hidden>
        <div class="episode-player-description-source">${prepared.html || escapeHtml(prepared.text)}</div>
        <div class="episode-player-description-translation episode-player-description-translation-expanded" hidden>
          <div class="episode-player-description-divider" aria-hidden="true"></div>
          <div class="episode-player-description-translation-body"></div>
        </div>
      </div>
...
function resetEpisodePlayerDescriptionTranslation(wrap) {
  if (!wrap) return;
  wrap.dataset.translationState = '';
  wrap.dataset.translationEmpty = '';
  wrap.dataset.translationSource = '';
  wrap.querySelectorAll('.episode-player-description-translation-body').forEach((node) => {
    node.innerHTML = '';
  });
  wrap.querySelectorAll('.episode-player-description-translation').forEach((node) => {
    node.setAttribute('hidden', '');
  });
}

async function maybeTranslateEpisodePlayerDescription(wrap, sourceText) {
  if (!wrap) return;
  const translationWraps = Array.from(wrap.querySelectorAll('.episode-player-description-translation'));
  const translationBodies = Array.from(wrap.querySelectorAll('.episode-player-description-translation-body'));
  if (!translationWraps.length || !translationBodies.length) return;
  const text = String(sourceText || '').trim();
  const sourceKey = text.replace(/\s+/g, ' ').trim();

  if (wrap.dataset.translationSource !== sourceKey) {
    resetEpisodePlayerDescriptionTranslation(wrap);
    wrap.dataset.translationSource = sourceKey;
  }

  if (wrap.dataset.translationState === 'loading') return;
  if (wrap.dataset.translationState === 'done') {
    const hasRenderedTranslation = translationBodies.some((node) => String(node.textContent || '').trim());
    if (hasRenderedTranslation || wrap.dataset.translationEmpty === '1') return;
    wrap.dataset.translationState = '';
  }

  if (!text) {
    wrap.dataset.translationState = 'done';
    wrap.dataset.translationEmpty = '1';
    return;
  }

  wrap.dataset.translationState = 'loading';
  try {
    const result = await translateText(text);
    if (!result || !String(result.displayText || '').trim()) {
      wrap.dataset.translationState = 'done';
      wrap.dataset.translationEmpty = '1';
      return;
    }
    const translationItems = Array.isArray(result.results) && result.results.length
      ? result.results
      : [{ provider: 'translation', translatedText: String(result.displayText || '').trim() }];
    const translationMarkup = translationItems
      .map((item) => {
        const translatedText = String(item && item.translatedText || '').trim();
        if (!translatedText) return '';
        return `
          <div class="episode-player-description-translation-entry">
            <span class="translation-marker" aria-hidden="true">${getTranslationMarkerSvg()}</span>
            <div class="episode-player-description-translation-text">${escapeHtml(translatedText)}</div>
          </div>
        `;
      })
      .filter(Boolean)
      .join('');
    if (!translationMarkup) {
      wrap.dataset.translationState = 'done';
      wrap.dataset.translationEmpty = '1';
      return;
    }
    translationBodies.forEach((node) => { node.innerHTML = translationMarkup; });
    translationWraps.forEach((node) => node.removeAttribute('hidden'));
    wrap.dataset.translationState = 'done';
    wrap.dataset.translationEmpty = '0';
    if (wrap.closest('.chart-row')) syncEpisodePlayerDescription(wrap.closest('.chart-row'));
  } catch (error) {
    console.warn('Description translation failed:', error);
    wrap.dataset.translationState = '';
  }
}

function refreshEpisodePlayerDescriptionWrap(wrap) {
  if (!wrap) return;
  const btn = wrap.querySelector('.episode-player-description-toggle');
  const preview = wrap.querySelector('.episode-player-description-preview');
  const expanded = wrap.querySelector('.episode-player-description-expanded');
  if (!btn || !preview || !expanded) return;
  const wasExpanded = btn.getAttribute('aria-expanded') === 'true' || wrap.classList.contains('is-expanded');
  const needsToggle = preview.scrollHeight > preview.clientHeight + 4;
  ...
  const sourceNode = preview.querySelector('.episode-player-description-source')
    || expanded.querySelector('.episode-player-description-source');
  const plainText = String(sourceNode && sourceNode.textContent || '').trim();
  if (plainText) void maybeTranslateEpisodePlayerDescription(wrap, plainText);
  ...
  btn.addEventListener('click', () => {
    ...
    if (isExpanded) expanded.setAttribute('hidden', '');
    else expanded.removeAttribute('hidden');
    wrap.classList.toggle('is-expanded', !isExpanded);
  });
}

function syncEpisodePlayerDescription(row) {
  if (!row) return;
  const slot = row.querySelector('.episode-player-wrap');
  if (!slot) return;
  slot.querySelectorAll('.episode-player-description').forEach((wrap) => {
    refreshEpisodePlayerDescriptionWrap(wrap);
  });
}

function refreshVisibleEpisodeDescriptions() {
  document.querySelectorAll('.chart-row.player-open').forEach((row) => {
    try { syncEpisodePlayerDescription(row); } catch {}
  });
}

async function refreshInlineEpisodePlayerRow(row, triggerBtn = null) {
  if (!row || !row.classList.contains('player-open')) return false;
  const playBtn = row.querySelector('.play-link');
  const slot = row.querySelector('.episode-player-wrap');
  if (!playBtn || !slot) return false;

  const embedPlatform = String(playBtn.getAttribute('data-embed-platform') || '').trim().toLowerCase();
  const embedUrl = String(playBtn.getAttribute('data-embed-url') || '').trim();
  const titleText = String(row.querySelector('.title-copy-target')?.getAttribute('data-copy-title') || row.querySelector('.title')?.textContent || '').trim();
  if (!embedPlatform || !embedUrl) return false;
  ...
  try {
    if (embedPlatform === 'rss' || embedPlatform === 'castbox' || embedPlatform === 'pocketcasts') {
      await renderCastboxRssPlayerInSlot(slot, embedPlatform, embedUrl, titleText, true);
      const rowDescription = String(row.getAttribute('data-description') || '').trim();
      const descriptionMarkup = buildEpisodePlayerDescriptionMarkup(rowDescription);
      if (descriptionMarkup && !slot.querySelector('.episode-player-description')) {
        const adNode = slot.querySelector('.episode-player-ad');
        if (adNode) adNode.insertAdjacentHTML('beforebegin', descriptionMarkup);
        else slot.insertAdjacentHTML('beforeend', descriptionMarkup);
        syncEpisodePlayerDescription(row);
      }
    } else if (embedPlatform === 'apple' && String(row.dataset.appleInlineMode || '').trim().toLowerCase() === 'rss') {
      await switchAppleRowPlayerMode(row, 'rss', triggerBtn);
    } else if (embedPlatform === 'spotify' && String(row.dataset.spotifyInlineMode || '').trim().toLowerCase() === 'rss') {
      await switchSpotifyRowPlayerMode(row, 'rss', triggerBtn);
    } else {
...
function closeEpisodePlayerRow(row) {
  ...
  delete row.dataset.appleInlineMode;
  delete row.dataset.spotifyInlineMode;
  setIframePlaybackActive(row, false);
...
async function openEpisodePlayerRow(row, btn, options = {}) {
  ...
  const slot = row.querySelector('.episode-player-wrap');
  if (!slot) return false;
  const shouldRespectPreferredMode = options && options.respectPreferredMode !== false;
  const preferredMode = shouldRespectPreferredMode ? getDefaultRssPlayerMode(embedPlatform) : embedPlatform;
  if (embedPlatform === 'apple' && preferredMode === 'rss') {
    return await switchAppleRowPlayerMode(row, 'rss', btn);
  }
  if (embedPlatform === 'spotify' && preferredMode === 'rss') {
    return await switchSpotifyRowPlayerMode(row, 'rss', btn);
  }
  const rowDescription = String(row.getAttribute('data-description') || '').trim();
  const descriptionMarkup = buildEpisodePlayerDescriptionMarkup(rowDescription);
  ...
  if (embedPlatform === 'castbox' || embedPlatform === 'pocketcasts' || embedPlatform === 'rss') {
    setIframePlaybackActive(row, false);
    slot.removeAttribute('hidden');
    applyPlayerAccentVars(slot, embedPlatform);
    row.classList.add('player-open');
    document.body.classList.add('player-active');
    btn.classList.add('is-active');
  ...
  let iframeOptions = {};
  let appleResolvedMeta = null;
  if (embedPlatform === 'spotify' && isEpisodeChartTypeValue(embedKind)) {
  ...
  if (embedPlatform === 'apple' && isEpisodeChartTypeValue(embedKind)) {
    try {
      appleResolvedMeta = await resolveAppleEpisodeEmbedMeta(embedUrl);
    } catch {
      appleResolvedMeta = null;
    }
    iframeOptions = {
      ...iframeOptions,
      appleEpisodeMode: appleResolvedMeta && appleResolvedMeta.isVideo ? 'video' : 'audio'
    };
  }
  ...
  const detectedVideoBadge = platformHasDirectVideoBadge(embedPlatform)
    || (embedPlatform === 'spotify' && (iframeOptions.spotifyEpisodeMode === 'video' || iframeOptions.spotifyShowMode === 'video'))
    || (embedPlatform === 'apple' && !!(appleResolvedMeta && appleResolvedMeta.isVideo));
  syncRowVideoBadgeState(row, detectedVideoBadge);

  if (embedPlatform === 'apple') {
    const accentPlatform = getCurrentPlayerAccentPlatform(row, 'apple');
    slot.innerHTML = buildApplePlayerModeAreaMarkup('apple', true, descriptionMarkup, true);
    applyPlayerAccentVars(slot, accentPlatform);
    const modeHost = slot.querySelector('[data-episode-mode-host]');
    if (!modeHost) return false;
    modeHost.innerHTML = `<div class="episode-player-shell platform-${embedPlatform}">${iframeHtml}</div>`;
    row.dataset.appleInlineMode = 'apple';
    bindInlinePlayerModeControls(row);
  } else if (embedPlatform === 'spotify') {
    const accentPlatform = getCurrentPlayerAccentPlatform(row, 'spotify');
    slot.innerHTML = buildSourceRssPlayerModeAreaMarkup('spotify', 'spotify', true, descriptionMarkup, true);
    applyPlayerAccentVars(slot, accentPlatform);
    const modeHost = slot.querySelector('[data-episode-mode-host]');
    if (!modeHost) return false;
    modeHost.innerHTML = `<div class="episode-player-shell platform-${embedPlatform}">${iframeHtml}</div>`;
    row.dataset.spotifyInlineMode = 'spotify';
    bindInlinePlayerModeControls(row);
  } else if (embedPlatform === 'youtube') {
    const accentPlatform = getCurrentPlayerAccentPlatform(row, 'youtube');
    const queued = globalRssPlayerController.ensureMounted().findItemIndex(buildGlobalYouTubeQueueItemFromRow(row, btn)) >= 0;
    slot.innerHTML = `<div class="episode-player-shell platform-${embedPlatform}">${iframeHtml}</div><div class="episode-player-mode-controls-wrap">${buildPlatformQueueControlsMarkup('youtube', queued)}</div>${descriptionMarkup}${buildPlayerAdMarkup()}`;
    applyPlayerAccentVars(slot, accentPlatform);
    bindInlinePlayerModeControls(row);
    syncInlineYoutubeQueueButton(row);
  } else if (embedPlatform === 'bilibili') {
    const accentPlatform = getCurrentPlayerAccentPlatform(row, 'bilibili');
    const queued = globalRssPlayerController.ensureMounted().findItemIndex(buildGlobalBilibiliQueueItemFromRow(row, btn)) >= 0;
    slot.innerHTML = `<div class="episode-player-shell platform-${embedPlatform}">${iframeHtml}</div><div class="episode-player-mode-controls-wrap">${buildPlatformQueueControlsMarkup('bilibili', queued)}</div>${descriptionMarkup}${buildPlayerAdMarkup()}`;
    applyPlayerAccentVars(slot, accentPlatform);
    bindInlinePlayerModeControls(row);
    syncInlineVideoQueueButton(row);
  } else {
    slot.innerHTML = `<div class="episode-player-shell platform-${embedPlatform}">${iframeHtml}</div>${descriptionMarkup}${buildPlayerAdMarkup()}`;
  }
  slot.removeAttribute('hidden');
  syncEpisodePlayerDescription(row);
  const playerAd = slot.querySelector('.episode-player-ad .adsbygoogle');
  ...
  if (embedPlatform === 'apple' && isEpisodeChartTypeValue(embedKind)) {
    ...
    Promise.resolve(appleResolvedMeta || resolveAppleEpisodeEmbedMeta(embedUrl)).then((meta) => {
      if (!row.classList.contains('player-open')) return;
      ...
      const previousMode = String(liveShell.getAttribute('data-apple-episode-mode') || '').trim().toLowerCase();
      ...
      syncRowVideoBadgeState(row, isVideo);
      applyAppleEpisodeShellLayout(liveShell);
      if (previousMode !== (isVideo ? 'video' : 'audio')) {
        const iframe = liveShell.querySelector('.episode-player-frame.apple.is-episode');
        if (iframe) {
          const currentSrc = String(iframe.getAttribute('src') || '').trim();
  ...
  row.dataset.appleInlineMode = embedPlatform === 'apple' ? 'apple' : '';
  row.dataset.spotifyInlineMode = embedPlatform === 'spotify' ? 'spotify' : '';
  row.classList.add('player-open');
  document.body.classList.add('player-active');
  btn.classList.add('is-active');
  btn.setAttribute('aria-pressed', 'true');
...
function buildHistoryPanelMarkup(series, platform, titleText = '') {
  ...
  const docDir = String(document && document.documentElement && document.documentElement.getAttribute('dir') || '').trim().toLowerCase();
  const isRtlDoc = docDir === 'rtl';
  const startAxisX = isRtlDoc ? (width - pad.right) : pad.left;
  const startAxisAnchor = isRtlDoc ? 'end' : 'start';
  const endAxisX = isRtlDoc ? pad.left : (width - pad.right);
  const endAxisAnchor = isRtlDoc ? 'start' : 'end';
  ...
  return `
    <div class="history-panel" data-history-panel style="--history-color-1:${escapeHtml(c1)};--history-color-2:${escapeHtml(c2)};--history-path-len:${pathLen};">
      ...
      <svg class="history-curve" viewBox="0 0 ${width} ${height}" role="img" aria-label="${safeTitle}">
        ...
        ${pointLabels}
        ${pointDots}
        ${startDate ? `<text class="history-axis-label" x="${startAxisX}" y="${height - 10}" text-anchor="${startAxisAnchor}">${escapeHtml(startDate)}</text>` : ''}
        ${endDate ? `<text class="history-axis-label" x="${endAxisX}" y="${height - 10}" text-anchor="${endAxisAnchor}">${escapeHtml(endDate)}</text>` : ''}
      </svg>
    </div>
  `;
}
...
function setMyPodcastsBusyState(busy) {
  ...
  if (myPodcastsOpmlExportEl) myPodcastsOpmlExportEl.disabled = myPodcastsBusy;
...
function getMyPodcastsOpmlImportSvg() {
  return '<svg viewBox="0 0 512 512" aria-hidden="true" focusable="false"><g transform="translate(42.666667 85.333333)"><path fill="currentColor" d="M405.333333 0v362.666667H21.333333V85.333333H64V320h298.666667V42.666667h-85.333334V0h128zM128 0c57.3601 0 104.145453 45.275876 106.568117 102.039688l.09855 4.626979-.000667 76.485333 48.915055-48.903611 30.169889 30.169889L213.333333 264.836556 112.915055 164.418278l30.16989-30.169889L192 183.152V106.666667c0-34.083859-26.643626-61.944766-60.239514-63.891356L128 42.666667H0V0h128z"/></g></svg>';
}

function getMyPodcastsOpmlExportSvg() {
  return '<svg viewBox="0 0 512 512" aria-hidden="true" focusable="false"><g transform="translate(64 48.915055)"><path fill="currentColor" d="M170.666667 36.418278v42.666667h-128v277.333333h298.666666v-128H384v170.666667H0V36.418278h170.666667zM313.751611 0l100.418278 100.418278-100.418278 100.418278-30.169889-30.169889 48.903278-48.915667-55.151667.000611c-34.083859 0-61.944765 26.643626-63.891356 60.239514l-.108644 3.760486v106.666667h-42.666666V185.751611c0-57.3601 45.275876-104.145453 102.039688-106.568117l4.627-0.09855 55.150667-.000944-48.903278-48.915055L313.751611 0z"/></g></svg>';
}

function getMyPodcastsImportActionSvg(mode = myPodcastsImportMode) {
  const safeMode = normalizeMyPodcastsImportMode(mode);
  if (safeMode === 'opml') {
    return getMyPodcastsOpmlImportSvg();
  }
...
function getMyPodcastsImportModeLabel(mode = myPodcastsImportMode) {
  ...
  if (safeMode === 'bilibili') return String(t('platformBilibili') || '').trim() || 'BiliBili';
...
function getMyPodcastsOpmlExportLabel() {
  return 'Export OPML';
}
...
function updateMyPodcastsImportUi() {
  ...
  if (myPodcastsOpmlPickEl) {
    ...
    const pickLabel = getMyPodcastsLocalizedText('myPodcastsImportPickOpml') || 'Import OPML';
    myPodcastsOpmlPickEl.hidden = !show;
    myPodcastsOpmlPickEl.disabled = myPodcastsBusy;
    myPodcastsOpmlPickEl.textContent = pickLabel;
    myPodcastsOpmlPickEl.setAttribute('title', pickLabel);
    myPodcastsOpmlPickEl.setAttribute('aria-label', pickLabel);
  }

  if (myPodcastsOpmlExportEl) {
    const show = myPodcastsImportMode === 'opml';
    const label = getMyPodcastsOpmlExportLabel();
    myPodcastsOpmlExportEl.hidden = !show;
    myPodcastsOpmlExportEl.disabled = myPodcastsBusy || !favoritePodcastMap || !favoritePodcastMap.size;
    myPodcastsOpmlExportEl.innerHTML = getMyPodcastsOpmlExportSvg();
    myPodcastsOpmlExportEl.setAttribute('title', label);
    myPodcastsOpmlExportEl.setAttribute('aria-label', label);
  }

  if (myPodcastsAddBtnEl) {
    myPodcastsAddBtnEl.hidden = false;
    myPodcastsAddBtnEl.disabled = myPodcastsBusy;
...
function buildMyPodcastsOpmlFilename() {
  const date = new Date();
  const y = date.getFullYear();
  const m = String(date.getMonth() + 1).padStart(2, '0');
  const d = String(date.getDate()).padStart(2, '0');
  return `my-podcasts-${y}-${m}-${d}.opml`;
}

function isFavoriteRecordPodcastLevel(record) {
  const row = normalizeFavoriteRecord(record || {});
  if (!row.embedKind || isEpisodeChartTypeValue(row.embedKind)) return false;
  const platform = String(row.embedPlatform || row.originPlatform || '').trim().toLowerCase();
  return platform !== 'youtube' && platform !== 'bilibili';
}

function buildPlaylistLikeItemFromFavoriteRecord(record) {
  const row = normalizeFavoriteRecord(record || {});
  return {
    title: row.title,
    publisher: row.publisher,
    embedUrl: row.embedUrl,
    embedKind: row.embedKind,
    resolvedFeedUrl: row.feedUrl,
    openUrl: row.url,
    favAttrs: {
      'data-favorite-feed': row.feedUrl || '',
      'data-favorite-url': row.url || ''
    }
  };
}

async function resolveFavoriteRecordFeedUrlForExport(entryKey, record) {
  const row = normalizeFavoriteRecord(record || {});
  const directFeedUrl = normalizeFavoriteFeedUrl(inferFavoriteFeedUrlFromRecord(row));
  if (directFeedUrl) return directFeedUrl;
  if (!isFavoriteRecordPodcastLevel(row)) return '';
  const platform = String(row.embedPlatform || row.originPlatform || '').trim().toLowerCase();
  if (platform !== 'apple' && platform !== 'spotify') return '';
  const item = buildPlaylistLikeItemFromFavoriteRecord(row);
  const resolved = platform === 'apple'
    ? await resolveAppleFeedUrlForPlaylistItem(item).catch(() => null)
    : await resolveSpotifyFeedUrlForPlaylistItem(item).catch(() => null);
  const feedUrl = normalizeFavoriteFeedUrl(resolved && resolved.feedUrl);
  if (!feedUrl) return '';
  const previous = favoritePodcastMap.get(entryKey);
  if (previous) {
    favoritePodcastMap.set(entryKey, mergeFavoriteRecords(previous, {
      ...row,
      favoriteKey: entryKey,
      feedUrl,
      updatedAt: Date.now()
    }));
  }
...
function buildOpmlOutlineXml(entry) {
  const title = String(entry && entry.title || '').trim() || 'Podcast';
  const attrs = [
    `text="${escapeXmlAttr(title)}"`,
    `title="${escapeXmlAttr(title)}"`,
    `type="${escapeXmlAttr(entry && entry.type || 'link')}"`
  ];
  if (entry && entry.type === 'rss') {
    attrs.push(`xmlUrl="${escapeXmlAttr(entry.xmlUrl || '')}"`);
    if (entry.htmlUrl) attrs.push(`htmlUrl="${escapeXmlAttr(entry.htmlUrl)}"`);
  } else if (entry && entry.url) {
    attrs.push(`url="${escapeXmlAttr(entry.url)}"`);
  }
  return `    <outline ${attrs.join(' ')} />`;
}

async function buildMyPodcastsOpmlExportXml() {
  const entries = [];
  let cacheUpdated = false;
  const records = Array.from(favoritePodcastMap.entries());
  for (const [entryKey, rawRecord] of records) {
    const row = normalizeFavoriteRecord({ ...(rawRecord || {}), favoriteKey: entryKey });
    const platform = String(row.embedPlatform || row.originPlatform || '').trim().toLowerCase();
    const title = String(row.title || row.publisher || '').trim() || 'Podcast';
    const htmlUrl = normalizeFavoriteUrlKey(row.url || '');
    const linkUrl = htmlUrl || normalizeFavoriteUrlKey(row.embedUrl || '') || normalizeFavoriteFeedUrl(row.feedUrl || '');
    let feedUrl = '';
    if (isFavoriteRecordPodcastLevel(row)) {
      const before = normalizeFavoriteFeedUrl(inferFavoriteFeedUrlFromRecord(row));
      feedUrl = await resolveFavoriteRecordFeedUrlForExport(entryKey, row);
      if (feedUrl && feedUrl !== before) cacheUpdated = true;
    }
    if (feedUrl) {
      entries.push({
        type: 'rss',
  ...
  const lines = [
    ...
    '<opml version="2.0">',
    '  <head>',
    `    <title>${escapeXmlAttr('My Podcasts Export')}</title>`,
    `    <dateCreated>${escapeXmlAttr(new Date().toUTCString())}</dateCreated>`,
    '  </head>',
    '  <body>',
    ...entries.map(buildOpmlOutlineXml),
    '  </body>',
    '</opml>'
  ];
  return {
...
async function handleMyPodcastsExportOpml() {
  if (myPodcastsBusy) return;
  if (!favoritePodcastMap || !favoritePodcastMap.size) {
    showToast(t('noData'));
    return;
  }
  ...
  try {
    const result = await buildMyPodcastsOpmlExportXml();
    if (!result || !result.xml || !result.count) {
    ...
    link.href = objectUrl;
    link.download = buildMyPodcastsOpmlFilename();
    document.body.appendChild(link);
    link.click();
    link.remove();
    setTimeout(() => URL.revokeObjectURL(objectUrl), 1000);
    setMyPodcastsProgress(`↑${result.count}`);
    showToast({
      icon: '↗',
      message: `${getMyPodcastsOpmlExportLabel()} (${result.count})`,
      duration: 2200
    });
  } catch (error) {
    console.warn('OPML export failed:', error);
    setMyPodcastsProgress('!');
    showToast(t('loadFailedRefresh'));
  } finally {
...
async function resolvePlatformImportMeta(candidate) {
  ...
  const embedKind = isEpisodeChartTypeValue(candidate.embedKind) ? 'top-episodes' : 'top-podcasts';

  if (platform === 'spotify') {
    const meta = isEpisodeChartTypeValue(embedKind)
...
function ensureMyPodcastsControls() {
    ...
    node.innerHTML = `
      <div class="my-podcasts-library-head">
        ...
        <div class="my-podcasts-import-row">
          ...
          <button id="my-opml-export" class="my-podcasts-action-btn" type="button" hidden aria-label="${escapeHtml(getMyPodcastsOpmlExportLabel())}" title="${escapeHtml(getMyPodcastsOpmlExportLabel())}">${getMyPodcastsOpmlExportSvg()}</button>
  ...
  myPodcastsOpmlExportEl = document.getElementById('my-opml-export');
  ...
  if (myPodcastsAddBtnEl && !myPodcastsAddBtnEl.dataset.boundClick) {
    myPodcastsAddBtnEl.addEventListener('click', () => {
      if (myPodcastsImportMode === 'opml') {
        if (myPodcastsBusy || !myPodcastsOpmlFileEl) return;
  ...
  if (myPodcastsOpmlExportEl && !myPodcastsOpmlExportEl.dataset.boundClick) {
    myPodcastsOpmlExportEl.addEventListener('click', () => {
      handleMyPodcastsExportOpml().catch((error) => {
        console.warn('OPML export handler failed:', error);
      });
    });
    myPodcastsOpmlExportEl.dataset.boundClick = '1';
  }
...
function setLoading(show, textKey = 'loadingDots') {
  ...
  if (show) {
    const topicOverride = getTopicAccentOverridePalette();
    if (topicOverride && topicOverride.length) {
      setChartLoadVisual(true, { platform: 'all', colors: topicOverride });
      return;
    }
...
html[data-theme="light"] .chart-row.surprise-final::before {
  border-right-color: color-mix(in oklab, var(--brand, #5b7cff) 26%, rgba(255,255,255,.88));
  box-shadow:
    -10px 0 16px color-mix(in oklab, var(--brand, #5b7cff) 14%, transparent),
    -18px 0 28px color-mix(in oklab, var(--brand-2, #0fb9b7) 12%, transparent),
    inset 1px 0 0 rgba(255,255,255,.95),
    inset 0 12px 16px rgba(255,255,255,.58),
    inset 0 -10px 16px rgba(176,198,226,.14);
}
html[data-theme="light"] .chart-row.surprise-final .title {
  color: color-mix(in oklab, var(--brand, #3555a8) 72%, #0d1c3f);
  text-shadow:
    0 0 12px color-mix(in oklab, var(--brand, #5b7cff) 16%, transparent),
    0 0 18px color-mix(in oklab, var(--brand-2, #0fb9b7) 10%, transparent);
}
...
@keyframes surpriseBtnShimmer {
  0% { background-position: -200% center; }
  100% { background-position: 200% center; }
}
@keyframes surpriseRevealGlow {
  0% { box-shadow: 0 0 0 0 color-mix(in oklab, var(--brand, #6f92ff) 0%, transparent); transform: scale(1); }
  30% { box-shadow: 0 0 28px 4px color-mix(in oklab, var(--brand, #6f92ff) 28%, transparent), 0 0 56px 8px color-mix(in oklab, var(--brand-2, #00c5b0) 16%, transparent); transform: scale(1.015); }
  60% { box-shadow: 0 0 22px 2px color-mix(in oklab, var(--brand, #6f92ff) 22%, transparent), 0 0 44px 6px color-mix(in oklab, var(--brand-2, #00c5b0) 12%, transparent); transform: scale(1.008); }
  100% { box-shadow: 0 18px 32px rgba(1,4,12,.56), 0 0 0 1px color-mix(in oklab, var(--brand, #6f92ff) 16%, transparent), 0 0 24px color-mix(in oklab, var(--brand-2, #00c5b0) 18%, transparent), inset 0 1px 0 rgba(255,255,255,.04); transform: scale(1); }
}
.surprise-btn-shimmer {
  background-image: linear-gradient(110deg, transparent 25%, rgba(255,255,255,.25) 37%, rgba(255,255,255,.35) 50%, rgba(255,255,255,.25) 63%, transparent 75%) !important;
  background-size: 200% 100% !important;
  animation: surpriseBtnShimmer .8s ease-in-out 1 !important;
}
html[data-theme="light"] .surprise-btn-shimmer {
  background-image: linear-gradient(110deg, transparent 25%, rgba(100,130,255,.15) 37%, rgba(100,130,255,.25) 50%, rgba(100,130,255,.15) 63%, transparent 75%) !important;
}
.chart-row.surprise-reveal {
  animation: surpriseRevealGlow 2.2s ease-out 1;
}
@media (max-width: 760px) {
  .select-container.chart-loading-ring::before {
    inset: -1px;
...
function platformBrandLabel(value) {
  const xiaoyuzhouOptionLabel = buildXiaoyuzhouOptionLabel();
  const xiaoyuzhouLabel = /小宇宙/.test(xiaoyuzhouOptionLabel)
    ? '小宇宙'
    : xiaoyuzhouOptionLabel.replace(/\s+Podcasts$/i, '').trim();
...
function getTopicAccentOverridePalette() {
  try {
    const body = document && document.body;
    if (!body) return null;
    const mode = String(body.getAttribute('data-chart-page-mode') || '').trim().toLowerCase();
    const topicKey = String(body.getAttribute('data-topic-key') || '').trim().toLowerCase();
    if (mode === 'topic' && topicKey === 'news') return ['#ff435f', '#ff5d67', '#ff8a62'];
  } catch {}
  return null;
}
...
function getPlatformAccentColors(platform) {
  const topicOverride = getTopicAccentOverridePalette();
  if (topicOverride) return topicOverride.slice();
  const safePlatform = String(platform || '').trim().toLowerCase();
  const list = PLATFORM_ACCENT_COLORS[safePlatform] || ['#7BD4FF', '#7A8BFF'];
  return Array.isArray(list) && list.length ? list.slice() : ['#7BD4FF', '#7A8BFF'];
}
...
function clearLoadRingVisual(host) {
  ...
  target.style.removeProperty('--brand');
  target.style.removeProperty('--brand-2');
  target.style.removeProperty('--row-accent-bar');
...
function setChartLoadVisual(active, options = {}) {
  ...
  const safePlatform = String(options.platform || (platformSelect ? platformSelect.value : 'apple') || '').trim().toLowerCase() || 'apple';

  const colors = (Array.isArray(options.colors) && options.colors.length)
    ? options.colors
    : getPlatformAccentColors(safePlatform);
  ...
  if (!loadRingState || loadRingState.host !== host) {
    clearLoadRingVisual(host);
    refs = ensureLoadRingOverlay(host);
    applyAccentPaletteToHost(host, colors);
    loadRingState = {
      host,
      refs,
      colors,
      useGradient: safePlatform === 'all',
      platform: safePlatform,
      loading: true,
    ...
    loadRingState.useGradient = safePlatform === 'all';
    loadRingState.platform = safePlatform || loadRingState.platform || 'apple';
...
function getEligiblePlatformsForAll(type = (typeSelect ? typeSelect.value : 'top-podcasts'), country = (countrySelect ? countrySelect.value : 'us')) {
  ...
  return options
    ...
    .filter((value) => value !== 'ximalaya')
    .filter((value) => value !== 'spotify' || SPOTIFY_CONFIRMED_COUNTRIES.has(safeCountry))
...
function buildOrfSoundOptionLabel() {
  return getLocalizedPlatformLabel('platformOrfSound', 'ORF Sound Podcasts', 'ORF Sound', 'ORF Sound Podcasts');
}

function buildRaiPlaySoundOptionLabel() {
  return getLocalizedPlatformLabel('platformRaiPlaySound', 'RaiPlay Sound Podcasts', 'RaiPlay Sound', 'RaiPlay Sound Podcasts');
}

function buildSrfAudioOptionLabel() {
  return getLocalizedPlatformLabel('platformSrfAudio', 'SRF Audio Podcasts', 'SRF Audio', 'SRF Audio Podcasts');
}

function buildDrLydOptionLabel() {
  return getLocalizedPlatformLabel('platformDrLyd', 'DR LYD Podcasts', 'DR LYD', 'DR LYD Podcasts');
}

function buildRtbfAuvioOptionLabel() {
  return getLocalizedPlatformLabel('platformRtbfAuvio', 'RTBF Auvio Podcasts', 'RTBF Auvio', 'RTBF Auvio Podcasts');
}

function buildRneAudioOptionLabel() {
  return getLocalizedPlatformLabel('platformRneAudio', 'RNE Audio Podcasts', 'RNE Audio', 'RNE Audio Podcasts');
}

function buildNpoLuisterOptionLabel() {
  return getLocalizedPlatformLabel('platformNpoLuister', 'NPO Luister Podcasts', 'NPO Luister', 'NPO Luister Podcasts');
}
...
function buildXiaoyuzhouOptionLabel() {
  const pageLang = pageToLanguage(getCurrentFileName()) || 'en';
  if (pageLang === 'zh-hans' || pageLang === 'zh-hant' || pageLang === 'zh-hk') {
    return '小宇宙播客';
  }
  return buildBrandedPodcastsLabel('Xiaoyuzhou', 'Xiaoyuzhou Podcasts');
}
...
function buildSnipdOptionLabel() {
  return getLocalizedPlatformLabel('platformSnipd', 'Snipd Podcasts', 'Snipd', 'Snipd Podcasts');
}

function buildNetflixOptionLabel() {
  return getLocalizedPlatformLabel('platformNetflix', 'Netflix Podcasts', 'Netflix', 'Netflix Podcasts');
}

function buildDisneyplusOptionLabel() {
  return getLocalizedPlatformLabel('platformDisneyplus', 'Disney+ Podcasts', 'Disney+', 'Disney+ Podcasts');
}

function buildSoundOnOptionLabel() {
  return getLocalizedPlatformLabel('platformSoundOn', 'SoundOn Podcasts', 'SoundOn', 'SoundOn Podcasts');
}

function buildKkboxOptionLabel() {
  return getLocalizedPlatformLabel('platformKkbox', 'KKBOX Podcasts', 'KKBOX', 'KKBOX Podcasts');
}

function buildPodplayOptionLabel() {
  return getLocalizedPlatformLabel('platformPodplay', 'Podplay Podcasts', 'Podplay', 'Podplay Podcasts');
}
...
function buildPlatformOptionLabel(value) {
  ...
  if (key === 'snipd') return buildSnipdOptionLabel();
  if (key === 'netflix') return buildNetflixOptionLabel();
  if (key === 'disneyplus') return buildDisneyplusOptionLabel();
  if (key === 'soundon') return buildSoundOnOptionLabel();
  if (key === 'kkbox') return buildKkboxOptionLabel();
  if (key === 'podplay') return buildPodplayOptionLabel();
  if (key === 'xiaoyuzhou') return buildXiaoyuzhouOptionLabel();
  ...
  if (key === 'globalplayer') return getLocalizedPlatformLabel('platformGlobalPlayer', 'Global Player Podcasts', 'Global Player', 'Global Player Podcasts');
...
function ensureXiaoyuzhouPlatformConfig() {
  try {
    if (typeof COUNTRY_PLATFORM_RULES === 'object' && COUNTRY_PLATFORM_RULES) {
      COUNTRY_PLATFORM_RULES.xiaoyuzhou = ['cn'];
    }
    if (Array.isArray(PLATFORM_OPTION_ORDER)) {
      const existingIndex = PLATFORM_OPTION_ORDER.indexOf('xiaoyuzhou');
      if (existingIndex >= 0) PLATFORM_OPTION_ORDER.splice(existingIndex, 1);
      const ximalayaIndex = PLATFORM_OPTION_ORDER.indexOf('ximalaya');
      const insertIndex = ximalayaIndex >= 0 ? ximalayaIndex : PLATFORM_OPTION_ORDER.length - 1;
      PLATFORM_OPTION_ORDER.splice(Math.max(0, insertIndex), 0, 'xiaoyuzhou');
    }
    if (Array.isArray(JP_PLATFORM_OPTION_ORDER)) {
      const index = JP_PLATFORM_OPTION_ORDER.indexOf('xiaoyuzhou');
      if (index >= 0) JP_PLATFORM_OPTION_ORDER.splice(index, 1);
    }
  } catch {}
}

ensureXiaoyuzhouPlatformConfig();
...
function getPlatformOptionOrderForCountry(country) {
  ...
  if (safeCountry === 'jp') {
    ...
    const radikoIndex = order.indexOf('radiko');
    const youtubeIndex = order.indexOf('youtube');
    if (radikoIndex >= 0) {
      const [radiko] = order.splice(radikoIndex, 1);
      const insertIndex = youtubeIndex >= 0 ? Math.min(order.length, youtubeIndex + 1) : 3;
      order.splice(insertIndex, 0, radiko);
    }
    return order;
  }
  if (safeCountry === 'es') {
...
async function fetchSinglePlatformNormalizedData(platform, type, country) {
  ...
  if (safePlatform === 'apple' && isAppleDerivedType(safeType)) {
    return fetchDerivedAppleChartData(safeType, safeCountry, { forceRefresh: false });
  }

  let candidates = getApiCandidates(platform, type, country);
  if (!candidates.length) throw new Error(`No source for ${platform}`);
  const data = await fetchJsonWithFallback(candidates);
  let items = collectNormalizedItemsForData(data, platform, type);
  if (safePlatform === 'bilibili') items = await enrichBilibiliItemsFromDetailCache(items);
  if (!items.length) throw new Error(`Empty data for ${platform}`);
  return { data, items };
}

function isSpotifyFallbackPayloadForRequestedCountry(payload, requestedCountry) {
  if (!payload || typeof payload !== 'object') return false;
  const requested = normalizeCountry(requestedCountry);
  const source = normalizeCountry(payload.sourceCountry || payload.country || requested);
  if (!requested || !source) return false;
  return source !== requested;
}

async function shouldIncludePlatformInAllAggregation(platform, requestedCountry, type, payload = null) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const safeCountry = normalizeCountry(requestedCountry);
  const safeType = normalizeTypeForPlatform(safePlatform, type);
  if (safePlatform !== 'spotify') return true;
  if (!SPOTIFY_CONFIRMED_COUNTRIES.has(safeCountry)) return false;
  if (payload && isSpotifyFallbackPayloadForRequestedCountry(payload, safeCountry)) return false;
  const cacheEntry = await getResponseCacheEntry(responseCacheKeyFor('spotify', safeCountry, safeType));
  const cachedPayload = cacheEntry && Object.prototype.hasOwnProperty.call(cacheEntry, 'payload') ? cacheEntry.payload : null;
  return !isSpotifyFallbackPayloadForRequestedCountry(cachedPayload, safeCountry);
}

function computeAggregateStats(rows) {
  const ranks = (Array.isArray(rows) ? rows : [])
    .map((row) => Number(row && row.rank))
    .filter((value) => Number.isFinite(value) && value > 0);
  const count = ranks.length;
  const total = ranks.reduce((sum, value) => sum + value, 0);
  const averageRank = count ? (total / count) : Number.POSITIVE_INFINITY;
  const bestRank = count ? Math.min(...ranks) : Number.POSITIVE_INFINITY;
  return { count, averageRank, bestRank };
}

function sortDerivedAggregateRows(a, b) {
  if ((b.count || 0) !== (a.count || 0)) return (b.count || 0) - (a.count || 0);
  if ((a.averageRank || Number.POSITIVE_INFINITY) !== (b.averageRank || Number.POSITIVE_INFINITY)) {
    return (a.averageRank || Number.POSITIVE_INFINITY) - (b.averageRank || Number.POSITIVE_INFINITY);
  }
  if ((a.bestRank || Number.POSITIVE_INFINITY) !== (b.bestRank || Number.POSITIVE_INFINITY)) {
    return (a.bestRank || Number.POSITIVE_INFINITY) - (b.bestRank || Number.POSITIVE_INFINITY);
  }
  return String(a.title || '').localeCompare(String(b.title || ''));
}

function buildAppleTopCategoriesPayloadFromSource(sourcePayload, country) {
  const safeCountry = normalizeCountry(country);
  const sourceEntries = sourcePayload && sourcePayload.feed && Array.isArray(sourcePayload.feed.results)
    ? sourcePayload.feed.results
    : [];
  const byCategory = new Map();

  sourceEntries.forEach((entry, index) => {
    const rank = index + 1;
    const categories = Array.isArray(entry && entry.categories) ? entry.categories : [];
    const seen = new Set();
    categories.forEach((category) => {
      ...
      const row = byCategory.get(id) || {
        id,
        term: String(category && category.term || '').trim(),
        label: String(category && category.label || '').trim(),
        ranks: []
      };
      row.ranks.push(rank);
      if (!row.label) row.label = String(category && category.label || category && category.term || '').trim();
      if (!row.term) row.term = String(category && category.term || '').trim();
      byCategory.set(id, row);
    });
  });

  const results = Array.from(byCategory.values())
    .map((row) => {
      const stats = computeAggregateStats(row.ranks.map((rank) => ({ rank })));
      const title = getLocalizedAppleCategoryLabel(row.id, row.label, row.term) || row.label || row.term || row.id;
      return {
        id: row.id,
        name: title,
        artistName: buildAppleDerivedSummaryLabel(stats.count, stats.averageRank),
        artworkUrl100: buildSyntheticCoverDataUri(title, `apple-category-${row.id}`),
        url: buildChartUrlForSelection('apple', buildAppleCategoryTypeValue(row.id, 'podcasts'), safeCountry),
        count: stats.count,
        averageRank: stats.averageRank,
        bestRank: stats.bestRank
      };
    })
    .sort(sortDerivedAggregateRows);

  return {
    feed: {
      updated: extractAppleSourceUpdated(sourcePayload),
      derivedFrom: 'top-podcasts',
      derivedType: APPLE_DERIVED_TOP_CATEGORIES_TYPE,
      sourceFingerprint: buildSnapshotFingerprint(
        sourceEntries.map((entry, index) => ({
          rank: index + 1,
          title: entry && entry.name ? entry.name : '',
          publisher: entry && entry.artistName ? entry.artistName : '',
...
function buildAppleTopHostingPlatformsPayloadFromSource(sourcePayload, sourceItems, country) {
  const safeCountry = normalizeCountry(country);
  const topItems = Array.isArray(sourceItems) ? sourceItems : [];
  const byHost = new Map();

  topItems.forEach((item, index) => {
    const feedUrl = String(item && item.feedUrl || '').trim();
    const detected = detectHostingPlatform(feedUrl);
    if (!detected) return;
    const key = String(detected.key || detected.domain || detected.host || '').trim().toLowerCase();
    if (!key) return;
    const rank = index + 1;
    const row = byHost.get(key) || {
      ...
      title: detected.name,
      url: detected.url,
      seed: detected.domain || detected.host || detected.name,
      ranks: []
    };
    row.ranks.push(rank);
    byHost.set(key, row);
  ...
  const results = Array.from(byHost.values())
    .map((row) => {
      ...
      return {
        ...
        name: row.title,
        artistName: buildAppleDerivedSummaryLabel(stats.count, stats.averageRank),
        artworkUrl100: buildSyntheticCoverDataUri(row.title, `hosting-${row.seed}`),
        url: row.url,
        count: stats.count,
        averageRank: stats.averageRank,
        bestRank: stats.bestRank
  ...
  return {
    feed: {
      updated: extractAppleSourceUpdated(sourcePayload),
      derivedFrom: 'top-podcasts',
      derivedType: APPLE_DERIVED_TOP_HOSTING_PLATFORMS_TYPE,
      sourceFingerprint: buildSnapshotFingerprint(
        topItems.map((entry, index) => ({
          rank: index + 1,
          title: entry && entry.title ? entry.title : '',
          publisher: entry && entry.publisher ? entry.publisher : '',
          url: entry && entry.url ? entry.url : ''
        }))
      ),
      results
...
function doesDerivedApplePayloadMatchSource(payload, sourcePayload, sourceItems) {
  const derivedFeed = payload && payload.feed && typeof payload.feed === 'object' ? payload.feed : {};
  const sourceUpdated = extractAppleSourceUpdated(sourcePayload);
  const derivedUpdated = extractAppleSourceUpdated(payload);
  const sourceFingerprint = buildSnapshotFingerprint(
    (Array.isArray(sourceItems) ? sourceItems : []).map((entry, index) => ({
      rank: Number(entry && entry.rank) || (index + 1),
      title: String(entry && entry.title || entry && entry.name || '').trim(),
      publisher: String(entry && entry.publisher || entry && entry.artistName || '').trim(),
      url: String(entry && entry.url || '').trim()
    }))
  );
  const derivedFingerprint = String(derivedFeed.sourceFingerprint || '').trim();
  if (sourceUpdated && derivedUpdated && sourceUpdated !== derivedUpdated) return false;
  if (sourceFingerprint && derivedFingerprint && sourceFingerprint !== derivedFingerprint) return false;
  return true;
}

async function fetchFreshAppleBaseChartData(country, type = 'top-podcasts') {
  const safeCountry = normalizeCountry(country);
  const safeType = normalizeTypeForPlatform('apple', type);
  const result = await (async () => {
    const candidates = getApiCandidates('apple', safeType, safeCountry);
    if (!candidates.length) throw new Error(`No source for apple ${safeType}`);
    const data = await fetchJsonWithFallback(candidates);
    const items = collectNormalizedItemsForData(data, 'apple', safeType);
    if (!items.length) throw new Error(`Empty data for apple ${safeType}`);
    return { data, items };
  })();
  const cacheKey = responseCacheKeyFor('apple', safeCountry, safeType);
  await setResponseCacheEntry(cacheKey, result.data);
  await saveDailySnapshot('apple', safeCountry, safeType, result.items, {
    fetchedAt: Date.now(),
    payload: result.data
  });
  return result;
...
async function getAppleBaseChartData(country, type = 'top-podcasts', options = {}) {
  const safeCountry = normalizeCountry(country);
  const safeType = normalizeTypeForPlatform('apple', type);
  const forceRefresh = Boolean(options.forceRefresh);
  if (!forceRefresh) {
    const cached = await getCountryPlatformItemsFromLocalCache('apple', safeType, safeCountry, { forceRefresh: false });
    if (cached && Array.isArray(cached.items) && cached.items.length) {
      const cacheKey = responseCacheKeyFor('apple', safeCountry, safeType);
      const cachedEntry = await getResponseCacheEntry(cacheKey);
      const payload = cachedEntry && Object.prototype.hasOwnProperty.call(cachedEntry, 'payload') ? cachedEntry.payload : null;
      if (payload) {
        if (safeType === 'top-podcasts') {
          const first = payload && payload.feed && Array.isArray(payload.feed.results) ? payload.feed.results[0] : null;
          if (first && !Array.isArray(first.categories)) {
            return fetchFreshAppleBaseChartData(safeCountry, safeType);
          }
        }
        return { data: payload, items: cached.items, source: cached.source || 'cache' };
      }
    }
  }
  return fetchFreshAppleBaseChartData(safeCountry, safeType);
...
async function fetchDerivedAppleChartData(type, country, options = {}) {
  const safeType = normalizeTypeForPlatform('apple', type);
  const safeCountry = normalizeCountry(country);
  const forceRefresh = Boolean(options.forceRefresh);
  const base = await getAppleBaseChartData(safeCountry, 'top-podcasts', { forceRefresh });
  const sourceData = base && base.data ? base.data : null;
  const sourceItems = Array.isArray(base && base.items) ? base.items.slice() : [];
  if (!sourceData || !sourceItems.length) throw new Error(`Missing Apple top podcasts source for ${safeType}`);

  const derivedCacheKey = responseCacheKeyFor('apple', safeCountry, safeType);
  const cachedEntry = await getResponseCacheEntry(derivedCacheKey);
  const cachedPayload = cachedEntry && Object.prototype.hasOwnProperty.call(cachedEntry, 'payload') ? cachedEntry.payload : null;
  if (cachedPayload) {
    const cachedItems = collectNormalizedItemsForData(cachedPayload, 'apple', safeType);
    if (cachedItems.length && doesDerivedApplePayloadMatchSource(cachedPayload, sourceData, sourceItems)) {
      return { data: cachedPayload, items: cachedItems, source: 'cache' };
    }
  }

  if (safeType === APPLE_DERIVED_TOP_HOSTING_PLATFORMS_TYPE) {
    await hydrateAppleItemsWithFeedUrls(sourceItems, safeCountry);
  }

  const payload = safeType === APPLE_DERIVED_TOP_CATEGORIES_TYPE
    ? buildAppleTopCategoriesPayloadFromSource(sourceData, safeCountry)
    : buildAppleTopHostingPlatformsPayloadFromSource(sourceData, sourceItems, safeCountry);
  const items = collectNormalizedItemsForData(payload, 'apple', safeType);
  if (!items.length) throw new Error(`Empty derived Apple data for ${safeType}`);

  await setResponseCacheEntry(derivedCacheKey, payload);
  await saveDailySnapshot('apple', safeCountry, safeType, items, {
    fetchedAt: Date.now(),
    payload
  });

  return { data: payload, items, source: 'live' };
}
...
async function fetchCountryPlatformItemsWithCache(platform, type, country, options = {}) {
  ...
  const forceRefresh = Boolean(options.forceRefresh);
  if (String(platform || '').trim().toLowerCase() === 'apple' && isAppleDerivedType(safeType)) {
    return fetchDerivedAppleChartData(safeType, safeCountry, { forceRefresh });
  }
  ...
  await persistPlatformPayloadAliases(platform, safeType, safeCountry, result.data, result.items, {
    fetchedAt: Date.now(),
    payload: result.data
  });
...
async function getCountryPlatformItemsFromLocalCache(platform, type, country, options = {}) {
  ...
  if (cacheFresh) {
    ...
    if (items.length) {
      await persistPlatformPayloadAliases(platform, safeType, safeCountry, cached, items, {
...
function cancelBackgroundPrefetch(options = {}) {
  ...
  if (historyAllOriginsPrefetchTimer) {
    clearTimeout(historyAllOriginsPrefetchTimer);
    historyAllOriginsPrefetchTimer = 0;
  }
  prefetchRunId += 1;
...
async function recordCurrentPrefetchInterest() {
  ...
  if (selectedPlatform === 'xiaoyuzhou') return;
  ...
  if (!isLanguageScopeMode(scope)) {
    ...
    if (selectedPlatform === 'apple' || selectedPlatform === 'spotify' || selectedPlatform === 'youtube') {
...
async function finalizeSuccessfulForegroundRender(items = null) {
  ...
  scheduleHistoryAllOriginsPrefetch();
}
      ...
      (Array.isArray(batch.results) ? batch.results : []).forEach((row) => {
        ...
        try {
          ...
          if (items.length) {
            ...
            persistPlatformPayloadAliases(selectedPlatform, safeType, country, data, items, {
              fetchedAt: Date.now(),
              payload: data
            }).catch(() => { });
...
async function fetchAllPlatformsCompositeData(type, country, options = {}) {
  ...
  for (const platform of platforms) {
    ...
    if (!forceRefresh) {
      ...
      if (snapshot && Array.isArray(snapshot.items) && snapshot.items.length) {
        const includeInAggregation = await shouldIncludePlatformInAllAggregation(platform, safeCountry, safeType);
  ...
  const classifiedTargets = classifyAllPlatformFetchTargets(fetchTargets, safeType, safeCountry);
  const retryTargets = [];
  const batchStages = [classifiedTargets.stageOne, classifiedTargets.stageTwo]
    .map((list) => Array.isArray(list) ? list.filter(Boolean) : [])
    .filter((list) => list.length);

  const recordBatchSuccess = async (platform, data) => {
    const items = collectNormalizedItemsForData(data, platform, safeType);
    if (!items.length) throw new Error(`Empty data for ${platform}`);
    const includeInAggregation = await shouldIncludePlatformInAllAggregation(platform, safeCountry, safeType, data);
    if (includeInAggregation) {
      successes.push({ platform, data, items });
    }
    persistPlatformPayloadAliases(platform, safeType, safeCountry, data, items, {
      fetchedAt: Date.now(),
      payload: data
    }).catch(() => { });
  };

  const queueRetryTarget = (platform) => {
    const safePlatform = String(platform || '').trim().toLowerCase();
    if (!safePlatform || retryTargets.includes(safePlatform)) return;
    retryTargets.push(safePlatform);
  };

  for (let stageIndex = 0; stageIndex < batchStages.length; stageIndex += 1) {
    const batchPlatforms = batchStages[stageIndex];
    const progressStart = 0.18 + (stageIndex / Math.max(1, batchStages.length)) * 0.40;
    const progressEnd = 0.18 + ((stageIndex + 1) / Math.max(1, batchStages.length)) * 0.40;

    onProgress && onProgress({
      stage: 'fetch',
      activePlatform: `batch-${stageIndex + 1}`,
      platforms,
      progress: progressStart
    });

    try {
      const batch = await fetchWorkerPlatformBatchRaw(batchPlatforms, safeType, safeCountry);
      const batchFailures = Array.isArray(batch.failures) ? batch.failures.slice() : [];
      ...
      for (const row of (Array.isArray(batch.results) ? batch.results : [])) {
        ...
        try {
          await recordBatchSuccess(platform, data);
        } catch (_error) {
          if (!batchFailures.includes(platform)) batchFailures.push(platform);
        }
      }

      batchFailures.forEach((platform) => queueRetryTarget(platform));
    } catch (_error) {
      batchPlatforms.forEach((platform) => queueRetryTarget(platform));
    }
  ...
  classifiedTargets.single.forEach((platform) => queueRetryTarget(platform));

  let retryDone = 0;
  const retryTotal = retryTargets.length;
  for (const platform of retryTargets) {
    onProgress && onProgress({
      stage: batchStages.length ? 'retry' : 'fetch',
      activePlatform: platform,
      platforms,
      progress: 0.58 + (retryTotal ? (retryDone / retryTotal) * 0.42 : 0)
    });
    try {
      const result = await fetchSinglePlatformNormalizedData(platform, safeType, safeCountry);
      const includeInAggregation = await shouldIncludePlatformInAllAggregation(platform, safeCountry, safeType, result.data);
      if (includeInAggregation) {
      ...
      onProgress && onProgress({
        ...
        activePlatform: platform,
        platforms,
        progress: 0.58 + (retryTotal ? (retryDone / retryTotal) * 0.42 : 0.42)
      });
    }
  }
...
async function translateTextWithGoogle(text, targetLang) {
  ...
  const translatedText = Array.isArray(data && data[0])
    ? data[0]
...
async function fetchMicrosoftTranslationPreview(text, targetLang, token) {
  const response = await fetch(MICROSOFT_TRANSLATE_API_PREVIEW, {
    method: 'POST',
    headers: {
...
async function fetchMicrosoftTranslationV3(text, targetLang, token) {
  const response = await fetch(`${MICROSOFT_TRANSLATE_API_V3}&to=${encodeURIComponent(targetLang)}`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json; charset=UTF-8'
...
function parseMicrosoftPreviewTranslationResponse(data, originalText) {
  const row = data && Array.isArray(data.value)
    ? data.value[0]
    : (Array.isArray(data) ? data[0] : (data && typeof data === 'object' ? data : null));
  const translation = row && Array.isArray(row.translations)
    ? row.translations[0]
    : (row && Array.isArray(row.targets) ? row.targets[0] : null);
  const translatedText = (
    translation && typeof translation.text === 'string' ? translation.text
      : row && typeof row.text === 'string' ? row.text
        : row && row.translation && typeof row.translation.text === 'string' ? row.translation.text
          : row && row.output && typeof row.output.text === 'string' ? row.output.text
            : originalText
  );
  const detectedLang = String(
    row && row.detectedLanguage && row.detectedLanguage.language
      || row && row.source && row.source.language
      || row && row.language
      || ''
  ).trim().toLowerCase();
  return {
...
function parseMicrosoftV3TranslationResponse(data, originalText) {
  const row = Array.isArray(data) ? data[0] : null;
  const translation = row && Array.isArray(row.translations) ? row.translations[0] : null;
  return {
    translatedText: translation && typeof translation.text === 'string' ? translation.text : originalText,
...
async function translateTextWithMicrosoft(text, targetLang, forceRefresh = false) {
  ...
  const previewResponse = await fetchMicrosoftTranslationPreview(text, targetLang, token);

  if (previewResponse.status === 401 && !forceRefresh) {
    microsoftTranslateAuthState = null;
    return translateTextWithMicrosoft(text, targetLang, true);
  }
  if (previewResponse.ok) {
    const previewData = await previewResponse.json();
    const previewParsed = parseMicrosoftPreviewTranslationResponse(previewData, text);
    const previewText = normalizeTranslatedTextForCompare(previewParsed && previewParsed.translatedText);
    const originalText = normalizeTranslatedTextForCompare(text);
    if (previewText && previewText !== originalText) {
      return previewParsed;
    }
    console.warn(`Microsoft preview translate returned no usable translated text for ${targetLang}. Falling back to v3.`);
  } else {
    console.warn(`Microsoft preview translate failed for ${targetLang}: HTTP ${previewResponse.status}. Falling back to v3.`);
  }

  const v3Response = await fetchMicrosoftTranslationV3(text, targetLang, token);
  if (v3Response.status === 401 && !forceRefresh) {
    microsoftTranslateAuthState = null;
    return translateTextWithMicrosoft(text, targetLang, true);
  }
  if (!v3Response.ok) throw new Error(`Microsoft translate HTTP ${v3Response.status} (preview ${previewResponse.status}, v3 fallback)`);

  const v3Data = await v3Response.json();
  return parseMicrosoftV3TranslationResponse(v3Data, text);
}

function shouldNormalizeTraditionalChineseForTransliteration(detectedLang) {
  const normalized = normalizeBcp47LanguageTag(detectedLang).toLowerCase();
  return normalized === 'zh-hant' || normalized === 'zh-hk' || normalized === 'yue';
}

function getTranslationMarkerSvg() {
  return `
    <svg width="16px" height="16px" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="currentColor" aria-hidden="true" focusable="false">
...
async function normalizeTraditionalChineseForTransliteration(text) {
  const source = String(text || '').trim();
  if (!source) return source;
  try {
    const result = await translateTextWithMicrosoft(source, 'zh-Hans');
    const normalized = normalizeTranslatedTextForCompare(result && result.translatedText);
    return normalized || source;
  } catch (error) {
    console.warn('Traditional Chinese normalization for transliteration failed:', error);
    return source;
  }
}


function shouldAttemptTransliteration(text) {
  const source = String(text || '').trim();
  if (!source) return false;
  if (!/[^\x00-\x7F]/.test(source)) return false;
  return true;
}

function getMicrosoftTransliterationHint(detectedLang) {
  const normalized = normalizeBcp47LanguageTag(detectedLang).toLowerCase();
  if (!normalized) return null;
  return MICROSOFT_TRANSLITERATION_LANGUAGE_HINTS[normalized]
    || MICROSOFT_TRANSLITERATION_LANGUAGE_HINTS[normalized.split('-')[0]]
    || null;
}

function parseMicrosoftPreviewTransliterationResponse(data, originalText) {
  const row = data && Array.isArray(data.value) ? data.value[0] : null;
  return {
    transliteratedText: row && typeof row.text === 'string' ? row.text : originalText,
    script: String(row && row.script || '').trim()
  };
}

async function transliterateTextWithMicrosoft(text, detectedLang = '', forceRefresh = false) {
  const token = await getMicrosoftTranslateToken(forceRefresh);
  const requestUrl = new URL(MICROSOFT_TRANSLITERATE_API_PREVIEW);
  const hint = getMicrosoftTransliterationHint(detectedLang);
  if (hint && hint.language) requestUrl.searchParams.set('language', hint.language);
  if (hint && hint.fromScript) requestUrl.searchParams.set('fromScript', hint.fromScript);
  ...
  if (response.status === 401 && !forceRefresh) {
    microsoftTranslateAuthState = null;
    return transliterateTextWithMicrosoft(text, detectedLang, true);
  }
  if (!response.ok) throw new Error(`Microsoft transliterate HTTP ${response.status}`);

  const data = await response.json();
  return parseMicrosoftPreviewTransliterationResponse(data, text);
}

function parseMicrosoftDetectResponse(data) {
  const row = Array.isArray(data) ? data[0] : null;
  return {
    detectedLang: String(row && row.language || '').trim().toLowerCase(),
    isTransliterationSupported: Boolean(row && row.isTransliterationSupported)
  };
}

async function detectTextLanguageWithMicrosoft(text, forceRefresh = false) {
  const source = String(text || '').trim();
  if (!source) return '';
  ...
  const token = await getMicrosoftTranslateToken(forceRefresh);
  const response = await fetch(MICROSOFT_DETECT_API_V3, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json; charset=UTF-8'
  ...
  if (response.status === 401 && !forceRefresh) {
    microsoftTranslateAuthState = null;
    return detectTextLanguageWithMicrosoft(source, true);
  }
  if (!response.ok) throw new Error(`Microsoft detect HTTP ${response.status}`);

  const data = await response.json();
  const parsed = parseMicrosoftDetectResponse(data);
  if (parsed.detectedLang) await setCachedDetectedLanguage(titleKey, parsed.detectedLang);
  return parsed.detectedLang;
}

async function transliterateText(text) {
  const source = String(text || '').trim();
  if (!shouldAttemptTransliteration(source)) return null;
  const cacheKey = `latn::${source}`;
  if (transliterationCache.has(cacheKey)) return transliterationCache.get(cacheKey);
  const persistentCached = getPersistentTextCacheValue('transliteration', cacheKey);
  if (persistentCached.hit) {
    transliterationCache.set(cacheKey, persistentCached.value);
    return persistentCached.value;
  }

  let finalResult = null;
  try {
    const detectedLang = await detectTextLanguageWithMicrosoft(source).catch((error) => {
      console.warn('Microsoft detect for transliteration failed:', error);
      return '';
    });
    if (detectedLang && shouldSkipTranslate(detectedLang)) {
      transliterationCache.set(cacheKey, null);
      setPersistentTextCacheValue('transliteration', cacheKey, null);
      return null;
    }
    let transliterationSource = source;
    let transliterationLang = detectedLang;
    if (shouldNormalizeTraditionalChineseForTransliteration(detectedLang)) {
      transliterationSource = await normalizeTraditionalChineseForTransliteration(source);
      transliterationLang = 'zh-Hans';
    }
    const result = await transliterateTextWithMicrosoft(transliterationSource, transliterationLang);
    const transliteratedText = normalizeTranslatedTextForCompare(result && result.transliteratedText);
    const originalText = normalizeTranslatedTextForCompare(source);
    if (transliteratedText && transliteratedText !== originalText) {
      finalResult = {
        displayText: transliteratedText,
        script: String(result && result.script || '').trim(),
        detectedLang: String(detectedLang || '').trim().toLowerCase()
      };
    }
  } catch (error) {
  ...
  transliterationCache.set(cacheKey, finalResult);
  setPersistentTextCacheValue('transliteration', cacheKey, finalResult);
  return finalResult;
}
...
async function translateText(text) {
  ...
  const persistentCached = getPersistentTextCacheValue('translation', cacheKey);
  if (persistentCached.hit) {
    translationCache.set(cacheKey, persistentCached.value);
    return persistentCached.value;
  }
  ...
  settled.forEach((entry, index) => {
    ...
    const detectedLang = String(result.detectedLang || '').trim().toLowerCase();
    if (!preferredDetectedLang || provider === 'google') preferredDetectedLang = detectedLang || preferredDetectedLang;
    rawResults.push({
      provider,
      detectedLang,
      translatedText
  ...
  if (shouldSkipTranslate(preferredDetectedLang)) {
    translationCache.set(cacheKey, null);
    setPersistentTextCacheValue('translation', cacheKey, null);
    return null;
  }
  ...
  rawResults.forEach((item) => {
    if (seenTexts.has(item.translatedText)) return;
    seenTexts.add(item.translatedText);
    usableResults.push(item);
  });
  ...
  setPersistentTextCacheValue('translation', cacheKey, finalResult);
  return finalResult;
}


function getPersistentTextCacheConfig(kind) {
  const safeKind = String(kind || '').trim().toLowerCase();
  if (safeKind === 'translation') {
    return {
      prefix: `${TRANSLATION_RESULT_CACHE_PREFIX}${TEXT_RESULT_CACHE_VERSION}::`,
      ttl: TRANSLATION_RESULT_CACHE_TTL_MS,
      maxEntries: TRANSLATION_RESULT_CACHE_MAX_ENTRIES
    };
  }
  return {
    prefix: `${TRANSLITERATION_RESULT_CACHE_PREFIX}${TEXT_RESULT_CACHE_VERSION}::`,
    ttl: TRANSLITERATION_RESULT_CACHE_TTL_MS,
    maxEntries: TRANSLITERATION_RESULT_CACHE_MAX_ENTRIES
  };
}

function getPersistentTextCacheKey(kind, cacheKey) {
  const config = getPersistentTextCacheConfig(kind);
  return `${config.prefix}${cacheKey}`;
}

function prunePersistentTextCache(kind) {
  try {
    const config = getPersistentTextCacheConfig(kind);
    const entries = [];
    ...
    entries.slice(0, entries.length - config.maxEntries).forEach((entry) => {
      try { localStorage.removeItem(entry.key); } catch {}
    });
  } catch {}
}

function getPersistentTextCacheValue(kind, cacheKey) {
  try {
    const config = getPersistentTextCacheConfig(kind);
    const storageKey = getPersistentTextCacheKey(kind, cacheKey);
    const raw = localStorage.getItem(storageKey);
    if (!raw) return { hit: false, value: null };
    const parsed = JSON.parse(raw);
    const updatedAt = Number(parsed && parsed.updatedAt || 0);
    if (!updatedAt || (Date.now() - updatedAt) >= config.ttl) {
...
function setPersistentTextCacheValue(kind, cacheKey, value) {
  try {
    const storageKey = getPersistentTextCacheKey(kind, cacheKey);
    localStorage.setItem(storageKey, JSON.stringify({ updatedAt: Date.now(), value }));
    prunePersistentTextCache(kind);
  } catch {}
}
...
async function processTranslations() {
  ...
  const pending = titles.filter((node) => !node.querySelector('.translated-title') || !node.querySelector('.transliteration-title'));

  for (let i = 0; i < pending.length; i += 5) {
    if (jobId !== translateJobId) return;
    const batch = pending.slice(i, i + 5);
    await Promise.all(batch.map(async (titleNode) => {
      const original = String(titleNode.getAttribute('data-original-title') || titleNode.textContent || '').trim();
      if (!original) return;
      const [transliteration, translation] = await Promise.all([
        titleNode.querySelector('.transliteration-title') ? Promise.resolve(null) : transliterateText(original),
        titleNode.querySelector('.translated-title') ? Promise.resolve(null) : translateText(original)
      ]);

      if (transliteration && transliteration.displayText && !titleNode.querySelector('.transliteration-title')) {
        const inline = document.createElement('span');
        inline.className = 'transliteration-title';
        inline.textContent = `(${transliteration.displayText})`;
        titleNode.appendChild(inline);
      }

      if (translation && translation.displayText && !titleNode.querySelector('.translated-title')) {
        const wrap = document.createElement('span');
        wrap.className = 'translated-title';
        const items = Array.isArray(translation.results) && translation.results.length
          ? translation.results
          : [{ translatedText: String(translation.displayText || '').trim() }];
        const joinedText = items
          .map((item) => {
            const translatedText = String(item && item.translatedText || '').trim();
            return translatedText ? escapeHtml(translatedText) : '';
          })
          .filter(Boolean)
          .join(' <span class="translated-title-sep" aria-hidden="true">·</span> ');
        wrap.innerHTML = joinedText
          ? `<span class="translation-marker" aria-hidden="true">${getTranslationMarkerSvg()}</span><span class="translated-title-text">${joinedText}</span>`
          : '';
        if (wrap.innerHTML) titleNode.appendChild(wrap);
      }
    }));
  }
}
...
function applyUiText() {
  ...
  if (topButton) {
    const label = escapeHtml(t('backToTop'));
    topButton.innerHTML = `<span class="top-button-icon" aria-hidden="true">${getBackToTopSvg()}</span><span class="top-button-label">${label}</span>`;
    topButton.setAttribute('aria-label', t('backToTop'));
    topButton.setAttribute('title', t('backToTop'));
  }
  decorateCommandActionButtons();
...
function getApiCandidates(platform, type, country) {
  ...
  if (
    ...
    || safePlatform === 'netflix'
    || safePlatform === 'disneyplus'
    || safePlatform === 'soundon'
    || safePlatform === 'kkbox'
    || safePlatform === 'podplay'
  ) {
    const params = new URLSearchParams({
      platform: safePlatform,
      type: safeType,
...
async function fetchJsonWithFallback(urls) {
  const requestKey = Array.isArray(urls) ? urls.join('||') : String(urls || '');
  if (requestKey && inflightJsonFetches.has(requestKey)) {
    return inflightJsonFetches.get(requestKey);
  }
  ...
  if (requestKey) inflightJsonFetches.set(requestKey, task);
  try {
    return await task;
  } finally {
    if (requestKey) inflightJsonFetches.delete(requestKey);
  }
}
...
const APPLE_DERIVED_TOP_CATEGORIES_TYPE = 'apple-top-categories';
const APPLE_DERIVED_TOP_HOSTING_PLATFORMS_TYPE = 'apple-top-hosting-platforms';
const APPLE_DERIVED_TYPES = new Set([
  APPLE_DERIVED_TOP_CATEGORIES_TYPE,
  APPLE_DERIVED_TOP_HOSTING_PLATFORMS_TYPE
]);

function isAppleDerivedType(value) {
  return APPLE_DERIVED_TYPES.has(String(value || '').trim().toLowerCase());
}

function extractXmlAttributeValue(attrText, attrName) {
...
function extractAppleCategoriesFromEntry(entryXml) {
  const matches = [...String(entryXml || '').matchAll(/<category\b([^>]*)\/?>/gi)];
  if (!matches.length) return [];
  const out = [];
  const seen = new Set();
  ...
  matches.forEach((match) => {
    const attrs = String(match[1] || '');
    const id = extractXmlAttributeValue(attrs, 'im:id') || extractXmlAttributeValue(attrs, 'id');
    if (!id || seen.has(id)) return;
    const term = extractXmlAttributeValue(attrs, 'term');
    const label = extractXmlAttributeValue(attrs, 'label');
    const scheme = extractXmlAttributeValue(attrs, 'scheme');
    seen.add(id);
    out.push({
      id,
      term,
...
function extractAppleFeedEntry(entryXml) {
  ...
  return {
    ...
    url: String(url || '').trim(),
    categories: extractAppleCategoriesFromEntry(entryXml),
    description: String(
      extractFirstXmlMatch(entryXml, /<summary\b[^>]*>([\s\S]*?)<\/summary>/i)
      || extractFirstXmlMatch(entryXml, /<itunes:summary\b[^>]*>([\s\S]*?)<\/itunes:summary>/i)
      || ''
    ).trim()
  };
}
...
function getAppleCategoryDefinitionById(categoryId) {
  const target = `apple-${String(categoryId || '').trim()}`;
  return getAppleBaseTypeDefinitions().find((def) => String(def && def.value || '').trim().toLowerCase() === target.toLowerCase()) || null;
}

function getLocalizedAppleCategoryLabel(categoryId, fallbackLabel = '', fallbackTerm = '') {
  const def = getAppleCategoryDefinitionById(categoryId);
  if (def) {
    const key = String(def.key || '').trim();
    const localizedByKey = key ? String(t(key) || '').trim() : '';
    const localized = localizedByKey && localizedByKey !== key ? localizedByKey : '';
    const sharedLocalized = !localized ? findLocalizedSharedTypeLabel(def) : '';
    const label = localized || sharedLocalized || String(def.defaultLabel || '').trim();
    if (label) return label;
  }
  return String(fallbackLabel || fallbackTerm || categoryId || '').trim();
}

function buildChartUrlForSelection(platform, type, country) {
  const params = new URLSearchParams();
  params.set('scope', currentScopeValue());
  params.set('platform', String(platform || '').trim().toLowerCase());
  params.set('type', String(type || '').trim().toLowerCase());
  params.set('country', normalizeCountry(country || (countrySelect && countrySelect.value) || 'us'));
...
const SYNTHETIC_COVER_PALETTES = [
  ['#17375E', '#2F79F6', '#7BC9FF'],
  ['#37215B', '#8A57FF', '#DDA5FF'],
  ['#4D1E46', '#D44E9A', '#FFD3E8'],
  ['#4A2A16', '#F08A2B', '#FFD17A'],
  ['#133B35', '#1CB27A', '#92F0C9'],
  ['#2D294B', '#5A73E5', '#B6C7FF'],
  ['#4A2330', '#E55E73', '#FFD0D8'],
  ['#1D3448', '#45A0D8', '#D2F0FF'],
  ['#3D1F56', '#A04CF4', '#F0C2FF'],
  ['#163A4F', '#1AB1C9', '#B9F4FF'],
  ['#493114', '#D78A21', '#FFE39A'],
  ['#2C214A', '#6C5CE7', '#D8D2FF'],
  ['#4B213E', '#D95AA0', '#FFD4EF'],
  ['#173D34', '#27B37E', '#C4FFE6'],
  ['#442634', '#D44D62', '#FFC3CC'],
  ['#24314F', '#4E7CF0', '#C6D6FF'],
  ['#4B2B1D', '#E36D2F', '#FFD0A8'],
  ['#18384A', '#2E9FD8', '#C8EDFF'],
  ['#43204A', '#C04CE0', '#F7C7FF'],
  ['#1F3A2A', '#4DB36A', '#D3FFD8'],
  ['#4A2525', '#DB5B4E', '#FFD0C9'],
  ['#1B3557', '#477FE6', '#D0E2FF'],
  ['#4A1E58', '#9B4DFF', '#E3C8FF']
];

function wrapSyntheticCoverText(text, maxCharsPerLine = 12, maxLines = 3) {
  const source = String(text || '').trim();
  if (!source) return [''];
  ...
  if (lines.length > maxLines) {
    ...
    kept[maxLines - 1] = `${kept[maxLines - 1].slice(0, Math.max(0, maxCharsPerLine - 1))}…`;
    return kept;
  }
  return lines.slice(0, maxLines);
}

function buildSyntheticCoverDataUri(text, seed = '', options = {}) {
  const size = Math.max(180, Number(options.size) || 300);
  const palette = SYNTHETIC_COVER_PALETTES[hashStringToUint(seed || text) % SYNTHETIC_COVER_PALETTES.length];
  const lines = wrapSyntheticCoverText(text, options.maxCharsPerLine || 12, options.maxLines || 3);
  const fontSize = lines.length <= 1 ? 34 : lines.length === 2 ? 28 : 23;
  const startY = lines.length === 1 ? 154 : lines.length === 2 ? 136 : 124;
  const tspans = lines.map((line, index) => (
    `<tspan x="50%" dy="${index === 0 ? 0 : 34}">${escapeHtml(line)}</tspan>`
  )).join('');
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
      ...
      <rect width="${size}" height="${size}" rx="32" fill="url(#g)"/>
      <circle cx="${size - 54}" cy="52" r="34" fill="rgba(255,255,255,0.12)"/>
      <circle cx="48" cy="${size - 46}" r="26" fill="rgba(255,255,255,0.10)"/>
      <text x="50%" y="${startY}" text-anchor="middle" fill="white" font-family="Manrope, system-ui, sans-serif" font-size="${fontSize}" font-weight="800">${tspans}</text>
    </svg>
  `.trim();
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`;
}

function buildAppleDerivedSummaryLabel(count, averageRank) {
  const countNum = Math.max(0, Number(count) || 0);
  if (!countNum) return '';
  return `${countNum} podcasts`;
}
...
function buildAppleDirectSourceUrls(country, type) {
  const safeCountry = normalizeCountry(country);
  const safeType = normalizeTypeForPlatform('apple', type);
  const parsedCategory = parseAppleCategoryTypeValue(safeType);
  ...
  if (parsedCategory && parsedCategory.kind === 'podcasts') {
    const genreId = parsedCategory.id;
    if (genreId) {
      return [`https://itunes.apple.com/${safeCountry}/rss/toppodcasts/genre=${genreId}/limit=200/xml`];
    }
  }
...
async function fetchDirectApplePayloadViaAllOrigins(country, type, signal) {
  ...
  if (isAppleAmpEpisodesType(safeType)) {
    const workerCandidates = getApiCandidates('apple', safeType, country);
    ...
    throw lastError || new Error('Apple worker prefetch failed');
  }
...
function buildBilibiliEmbedUrl(aid, bvid, cid) {
  const aidValue = String(aid || '').trim();
  const bvidValue = String(bvid || '').trim();
  const cidValue = String(cid || '').trim();
  if (!aidValue || !bvidValue || !cidValue) return '';
  return `https://player.bilibili.com/player.html?isOutside=true&aid=${encodeURIComponent(aidValue)}&bvid=${encodeURIComponent(bvidValue)}&cid=${encodeURIComponent(cidValue)}&p=1`;
}

function parseBilibiliEmbedMetaFromUrl(url) {
  const raw = String(url || '').trim();
  if (!raw) return null;
  try {
    const parsed = new URL(raw, window.location.origin);
    const aid = normalizeBilibiliAid(parsed.searchParams.get('aid') || '');
    const bvid = String(parsed.searchParams.get('bvid') || '').trim();
    const cid = String(parsed.searchParams.get('cid') || '').trim();
    if (!aid && !bvid && !cid) return null;
    return { aid, bvid, cid };
  } catch {
    return null;
  }
...
function buildBilibiliPlaybackEmbedUrl(url, options = {}) {
  const raw = String(url || '').trim();
  if (!raw) return '';
  try {
    const parsed = new URL(raw, window.location.origin);
    parsed.searchParams.set('autoplay', options.autoplay === false ? '0' : 'true');
    parsed.searchParams.set('danmaku', '0');
    parsed.searchParams.set('poster', '0');
    parsed.searchParams.set('refer', '1');
    const seekTo = Math.max(0, Number(options.seekTo) || 0);
    if (seekTo > 0) parsed.searchParams.set('t', String(Math.floor(seekTo)));
...
function normalizeBilibiliAid(value) {
  ...
  return /^\d+$/.test(safe) ? safe : '';
}


function normalizeBilibiliDetailRecord(detail) {
  const aid = normalizeBilibiliAid(detail && detail.aid);
  if (!aid) return null;
  const bvid = String(detail && detail.bvid || '').trim();
  const cid = String(detail && detail.cid || '').trim();
  return {
    aid,
    bvid,
    cid,
    title: String(detail && detail.title || '').trim(),
    image: normalizeBilibiliArtworkUrl(detail && detail.image || ''),
    author: String(detail && detail.author || '').trim(),
    embedUrl: String(detail && detail.embedUrl || '').trim() || buildBilibiliEmbedUrl(aid, bvid, cid)
  };
}

function bilibiliDetailMetaKey(aid) {
  return `${IDB_META_KEY_BILIBILI_DETAIL_PREFIX}${normalizeBilibiliAid(aid)}`;
}

async function getCachedBilibiliDetail(aid) {
  const safeAid = normalizeBilibiliAid(aid);
  if (!safeAid) return null;
  try {
    const record = await idbGet(IDB_META_STORE, bilibiliDetailMetaKey(safeAid));
    const updatedAt = Number(record && record.updatedAt || 0);
    if (!record || !record.data || !updatedAt) return null;
    if ((Date.now() - updatedAt) > BILIBILI_DETAIL_CACHE_TTL_MS) return null;
    return normalizeBilibiliDetailRecord(record.data);
  } catch {
    return null;
  }
}

async function setCachedBilibiliDetail(detail) {
  const normalized = normalizeBilibiliDetailRecord(detail);
  if (!normalized) return null;
  try {
    await idbPut(IDB_META_STORE, {
      key: bilibiliDetailMetaKey(normalized.aid),
      updatedAt: Date.now(),
      data: normalized
    });
  } catch {}
...
function applyBilibiliDetailToItem(item, detail) {
  if (!item || !detail) return false;
  const itemAid = normalizeBilibiliAid(item.aid);
  const detailAid = normalizeBilibiliAid(detail.aid);
  if (!itemAid || !detailAid || itemAid !== detailAid) return false;
  let changed = false;
  if (!String(item.bvid || '').trim() && String(detail.bvid || '').trim()) {
    item.bvid = String(detail.bvid || '').trim();
    changed = true;
  }
  if (!String(item.cid || '').trim() && String(detail.cid || '').trim()) {
    item.cid = String(detail.cid || '').trim();
    changed = true;
  }
  if (!String(item.embedUrl || '').trim() && String(detail.embedUrl || '').trim()) {
    item.embedUrl = String(detail.embedUrl || '').trim();
    changed = true;
  }
  if (!String(item.embedPlatform || '').trim() && String(item.embedUrl || '').trim()) {
    item.embedPlatform = 'bilibili';
    changed = true;
  }
  if (!String(item.embedKind || '').trim() && String(item.embedUrl || '').trim()) {
    item.embedKind = 'top-podcasts';
    changed = true;
  }
  if (!String(item.image || '').trim() && String(detail.image || '').trim()) {
    item.image = String(detail.image || '').trim();
    changed = true;
  }
  return changed;
}

async function enrichBilibiliItemsFromDetailCache(items) {
  const list = Array.isArray(items) ? items : [];
  if (!list.length) return list;
  const enriched = list.slice();
  await Promise.all(enriched.map(async (item) => {
    const aid = normalizeBilibiliAid(item && item.aid);
    if (!aid || String(item && item.embedUrl || '').trim()) return;
    const detail = await getCachedBilibiliDetail(aid);
    if (detail) applyBilibiliDetailToItem(item, detail);
  }));
  return enriched;
}

function collectBilibiliKnownDetailRecords(items, map) {
  const target = map instanceof Map ? map : new Map();
  (Array.isArray(items) ? items : []).forEach((item) => {
    const normalized = normalizeBilibiliDetailRecord(item);
    if (!normalized) return;
    if (!target.has(normalized.aid)) target.set(normalized.aid, normalized);
  });
  return target;
}

async function persistBilibiliDetailCacheFromItems(items) {
  const detailMap = collectBilibiliKnownDetailRecords(items, new Map());
  if (!detailMap.size) return;
  await Promise.all(Array.from(detailMap.values()).map((detail) => setCachedBilibiliDetail(detail)));
}
...
function normalizeItems(data, platform, type) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const safeType = safePlatform === 'all'
    ? normalizeTopType(type)
    : normalizeTypeForPlatform(safePlatform, type);
  let arr = [];
  ...
  if (safePlatform === 'youtube') {
    ...
    return arr.map((item, index) => {
      ...
      const podcastTitle = item.showTitle || item.channelTitle || item.author || item.publisher || 'YouTube';
      const publishedText = item.publishedText || item.published || item.date || item.releaseDate || '';
      const durationText = formatLooseMediaDurationLabel(item.durationText || item.duration || item.lengthText || item.runtime || '');
      return {
        rank: Number(item.rank) || (index + 1),
        ...
        podcastTitle: String(podcastTitle || '').trim(),
        publishedText: String(publishedText || '').trim(),
        durationText,
        meta: buildCompactMediaMetaText(publishedText, durationText, '')
      };
    });
  }

  if (safePlatform === 'spotify') {
    arr = Array.isArray(data) ? data : (Array.isArray(data && data.items) ? data.items : []);
    return arr.map((item, index) => {
      if (safeType === 'top-episodes') {
  ...
  if (safePlatform === 'apple') {
    ...
    if (isAppleDerivedType(safeType)) {
      return arr.map((item, index) => ({
        rank: Number(item.rank) || (index + 1),
        title: item.name || item.title || '',
        publisher: item.artistName || item.publisher || '',
        image: String(item.image || item.artworkUrl100 || '').trim(),
        url: item.url || '',
    ...
    return arr.map((item, index) => {
      ...
      const publishedText = String(item.releaseDate || item.releaseDateTime || item.published || '').trim();
      return {
        rank: index + 1,
        title: item.name || '',
        ...
        embedKind: embed.embedKind,
        description: String(item.description || '').trim(),
        podcastTitle: String(item.collectionName || item.artistName || '').trim(),
        publishedText,
        meta: buildCompactMediaMetaText(publishedText, '', ''),
        appleId: String(item.id || item.episodeId || item.contentId || '').trim()
      };
    });
  }
  ...
  if (safePlatform === 'xiaoyuzhou') {
    ...
    return arr.map((item, index) => ({
      rank: Number(item.rank) || (index + 1),
      title: item.name || item.title || '',
      publisher: safeType === 'top-podcasts'
        ? (item.podcastTitle || item.title || item.name || item.publisher || item.author || platformBrandLabel('xiaoyuzhou'))
        : (item.podcastTitle || item.publisher || item.author || platformBrandLabel('xiaoyuzhou')),
      image: String(item.image || '').trim(),
      ...
      description: String(item.description || '').trim(),
      publishedText: String(item.publishedAt || item.pubDate || '').trim(),
      durationText: formatLooseMediaDurationLabel(item.duration || item.durationText || ''),
      podcastTitle: String(
        safeType === 'top-podcasts'
          ? (item.podcastTitle || item.title || item.name || item.publisher || '')
          : (item.podcastTitle || item.publisher || '')
      ).trim(),
  ...
  if (safePlatform === 'bilibili') {
    ...
    return arr.map((item, index) => ({
      ...
      title: item.name || item.title || '',
      publisher: safeType === 'top-podcasts'
        ? (item.author || item.publisher || platformBrandLabel('bilibili'))
        : (item.podcastTitle || item.publisher || item.author || platformBrandLabel('bilibili')),
      podcastTitle: String(
        safeType === 'top-podcasts'
          ? (item.podcastTitle || item.title || item.name || '')
          : (item.podcastTitle || item.publisher || '')
      ).trim(),
      description: String(item.description || '').trim(),
      ...
      publishedText: formatPublishedDisplayDate(item.publishedAt || '') || String(item.publishedText || '').trim(),
      durationText: formatLooseMediaDurationLabel(item.durationText || item.duration || ''),
      durationSeconds: Math.max(0, Number(item.durationSeconds) || parseLooseMediaDurationSeconds(item.durationText || item.duration || '')),
      meta: buildCompactMediaMetaText(formatPublishedDisplayDate(item.publishedAt || '') || String(item.publishedText || '').trim(), formatLooseMediaDurationLabel(item.durationText || item.duration || ''), ''),
      aid: String(item.aid || '').trim(),
      bvid: String(item.bvid || '').trim(),
      cid: String(item.cid || '').trim(),
      embedUrl: String(item.embedUrl || '').trim(),
      embedPlatform: String(item.embedPlatform || '').trim().toLowerCase(),
      embedKind: String(item.embedKind || (safeType === 'top-episodes' ? 'top-episodes' : 'top-podcasts')).trim().toLowerCase()
    }));
  }

  if (safePlatform === 'siriusxm') {
    arr = Array.isArray(data && data.podcasts) ? data.podcasts : [];
    return arr.map((item, index) => ({
      rank: Number(item.rank) || (index + 1),
      title: item.name || item.title || '',
  ...
  if (safePlatform === 'ardsounds' || safePlatform === 'radiofrance' || safePlatform === 'bbcsounds' || safePlatform === 'globalplayer' || safePlatform === 'siriusxm' || safePlatform === 'pandora' || safePlatform === 'podcastapp' || safePlatform === 'rtlplus' || safePlatform === 'tunein' || safePlatform === 'abclisten' || safePlatform === 'nrk' || safePlatform === 'yleareena' || safePlatform === 'sverigesradio' || safePlatform === 'podplay' || safePlatform === 'podimo' || safePlatform === 'podcastaddict' || safePlatform === 'radionet' || safePlatform === 'ivoox' || safePlatform === 'orfsound' || safePlatform === 'raiplaysound' || safePlatform === 'srfaudio' || safePlatform === 'drlyd' || safePlatform === 'rtbfauvio' || safePlatform === 'rneaudio' || safePlatform === 'npoluister' || safePlatform === 'snipd' || safePlatform === 'netflix' || safePlatform === 'disneyplus' || safePlatform === 'soundon' || safePlatform === 'kkbox') {
    arr = Array.isArray(data && data.podcasts) ? data.podcasts : [];
    const authorFallback = safePlatform === 'ardsounds'
      ? 'ARD Sounds'
      : safePlatform === 'radiofrance'
        ? 'Radio France'
        : safePlatform === 'bbcsounds'
...
function buildChartRowsMarkup(items, options = {}) {
  ...
  return items.map((item) => {
    ...
    const itemPlatformForCover = String(item.originPlatform || currentPlatform || item.embedPlatform || '').trim().toLowerCase();
    ...
    const useVideoCover = itemPlatformForCover === 'bilibili'
      || itemPlatformForCover === 'disneyplus'
    ...
    const staticVideoBadge = platformHasDirectVideoBadge(embedPlatformRaw);
    const titleMarkup = `${staticVideoBadge ? buildVideoTitleBadgeMarkup() : ''}${title}`;
    ...
    const canPlayInline = Boolean(embedUrlRaw && embedPlatformRaw && (isEpisodeChartTypeValue(embedKindRaw) || embedKindRaw === 'top-podcasts'));
    ...
    const notePreviewHtml = isNoteRow
      ...
      : '';
    const descriptionAttr = String(item.description || '')
      .replace(/&/g, '&amp;')
      .replace(/"/g, '&quot;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;');
    const itemPodcastTitleAttr = escapeHtml(String(item.podcastTitle || item.publisher || '').trim());
    const itemPublishedAttr = escapeHtml(String(item.publishedText || '').trim());
    const itemDurationAttr = escapeHtml(formatLooseMediaDurationLabel(item.durationText || item.duration || ''));
    const itemDurationSecondsAttr = escapeHtml(String(Math.max(0, Number(item.durationSeconds) || parseLooseMediaDurationSeconds(item.durationText || item.duration || ''))));
    const itemMetaAttr = escapeHtml(buildCompactMediaMetaText(item.publishedText || '', item.durationText || item.duration || '', item.meta || ''));
    const topicMetaBits = [
      { value: String(item.originCountryLabel || '').trim(), className: 'topic-meta-pill-country' },
      { value: String(item.detectedLanguageLabel || '').trim(), className: 'topic-meta-pill-language' },
      { value: String(item.appearanceText || '').trim(), className: 'topic-meta-pill-appearance' }
    ].filter((bit) => bit.value);
    const topicMetaHtml = topicMetaBits.length
      ? `<div class="topic-meta-pills">${topicMetaBits.map((bit) => `<span class="topic-meta-pill ${bit.className}">${escapeHtml(bit.value)}</span>`).join('')}</div>`
      : '';
    const topicFilterMode = String(item.topicFilterMode || '').trim().toLowerCase();
    const topicFilterKeyword = String(item.topicFilterKeyword || '').trim();
    const topicRevealHtml = topicFilterMode === 'blur'
      ? `<button class="topic-row-reveal" type="button" data-topic-reveal="1" aria-label="${escapeHtml(tOr('revealFiltered', 'Show episode'))}">
          <span class="topic-row-reveal-icon" aria-hidden="true">${getEyeClosedRevealSvg()}</span>
        </button>`
      : '';

    return `
      <article class="chart-row${favoritesScope ? ' favorites-row' : ''}${isNoteRow ? ' note-row' : ''}${useVideoCover ? ' has-video-cover' : ''}${topicFilterMode === 'blur' ? ' topic-row-is-blurred' : ''}" data-search="${search}"${topicFilterKeyword ? ` data-topic-filter-keyword="${escapeHtml(topicFilterKeyword)}"` : ''}
        ...
        data-item-podcast-title="${itemPodcastTitleAttr}"
        data-item-published="${itemPublishedAttr}"
        data-item-duration="${itemDurationAttr}"
        data-item-duration-seconds="${itemDurationSecondsAttr}"
        data-item-meta="${itemMetaAttr}"
        ...
        <div class="meta">
          <div class="title-line">
            <h3 class="title title-copy-target" role="button" tabindex="0" data-copy-title="${copyTitle}" data-original-title="${copyTitle}" title="${copyTitle}">${titleMarkup}</h3>
...
function bindAuxiliaryChartContainer(container) {
  ...
  container.addEventListener('click', async (event) => {
    const modeNode = event.target && event.target.closest ? event.target.closest('[data-episode-mode]') : null;
    if (modeNode) {
      event.preventDefault();
      event.stopPropagation();
      const row = modeNode.closest('.chart-row');
      if (row) await switchAppleRowPlayerMode(row, modeNode.getAttribute('data-episode-mode') || 'apple', modeNode);
      return;
    }
    ...
    if (refreshNode) {
      ...
      if (row) await refreshInlineEpisodePlayerRow(row, refreshNode);
      return;
    }
...
function renderItunesSearchResults(items) {
  ...
  if (!window.TOPIC_TRANSLATIONS_SUSPENDED) processTranslations();
}

function removeAds() {
  document.querySelectorAll('.chart-ad, [data-support-shell="list"]').forEach((ad) => ad.remove());
}
  ...
  try {
    requestAnimationFrame(() => {
      try {
        ...
        initializeSupportShells(document);
      }
    });
  } catch (error) {
...
function buildSupportFallbackMarkup(kind = 'inline') {
  const safeKind = String(kind || 'inline').trim().toLowerCase();
  return `
    <div class="support-fallback" data-support-fallback hidden>
      <img class="support-fallback-image" src="ammc.webp" alt="" loading="lazy" decoding="async" data-support-kind="${escapeHtml(safeKind)}">
    </div>
  `;
}

function scheduleSupportFallback(shell) {
  if (!shell || shell.__supportFallbackBound) return;
  shell.__supportFallbackBound = true;
  const live = shell.querySelector('[data-support-live]');
  const fallback = shell.querySelector('[data-support-fallback]');
  const ins = live ? live.querySelector('.adsbygoogle') : null;
  if (!live || !fallback || !ins) return;

  const showFallback = () => {
    fallback.hidden = false;
    shell.setAttribute('data-support-state', 'fallback');
  };
  const hideFallback = () => {
    fallback.hidden = true;
    shell.setAttribute('data-support-state', 'live');
  };

  const hasRenderedPayload = () => {
    if (!ins || !ins.isConnected) return false;
    const insStyle = window.getComputedStyle(ins);
    if (insStyle.display === 'none' || insStyle.visibility === 'hidden') return false;
    const adStatus = String(ins.getAttribute('data-ad-status') || '').trim().toLowerCase();
    if (adStatus === 'filled') return true;
    const iframe = ins.querySelector('iframe');
    if (iframe && iframe.offsetWidth > 0 && iframe.offsetHeight > 0) return true;
    const rect = ins.getBoundingClientRect();
    if (rect.width > 40 && rect.height > 40 && ins.childElementCount > 0) return true;
    return false;
  };

  const shouldShowFallback = () => {
    if (!ins || !ins.isConnected) return true;
    const insStyle = window.getComputedStyle(ins);
    if (insStyle.display === 'none' || insStyle.visibility === 'hidden') return true;
    if (hasRenderedPayload()) return false;
    const rect = ins.getBoundingClientRect();
    return rect.width < 24 || rect.height < 24 || ins.childElementCount === 0;
  };
  ...
  const tick = () => {
    ...
    window.setTimeout(tick, attempts < 3 ? 250 : 500);
  };

  const observer = new MutationObserver(() => {
    if (hasRenderedPayload()) hideFallback();
  });
  observer.observe(ins, { childList: true, subtree: true, attributes: true, attributeFilter: ['style', 'data-ad-status', 'hidden'] });
  shell.__supportFallbackObserver = observer;
  tick();
}

function initializeSupportShells(root = document) {
  if (!root || !root.querySelectorAll) return;
  root.querySelectorAll('[data-support-shell]').forEach((shell) => {
    scheduleSupportFallback(shell);
  });
}

function buildPlayerAdMarkup() {
  return `
    <div class="support-shell" data-support-shell="inline" data-support-state="live">
      <div class="support-live" data-support-live>
      ...
      ${buildSupportFallbackMarkup('inline')}
    </div>
  `;
}
...
function buildPlaylistAdMarkup() {
  return `
    <div class="support-shell" data-support-shell="playlist" data-support-state="live">
      <div class="support-live" data-support-live>
      ...
      ${buildSupportFallbackMarkup('playlist')}
    </div>
  `;
}
...
function insertAds() {
  ...
  for (const rowIndex of insertionIndexes) {
    ...
    adWrap.className = 'support-shell chart-ad';
    adWrap.setAttribute('data-support-shell', 'list');
    adWrap.setAttribute('data-support-state', 'live');

    const liveWrap = document.createElement('div');
    liveWrap.className = 'support-live';
    liveWrap.setAttribute('data-support-live', '');
    ...
    liveWrap.appendChild(ins);
    adWrap.appendChild(liveWrap);
    adWrap.insertAdjacentHTML('beforeend', buildSupportFallbackMarkup('list'));
    rows[rowIndex].after(adWrap);
    initializeAdsbygooglePush();
    initializeSupportShells(adWrap);
  }
}
...
async function randomPick() {
  ...
  const clearMarks = () => {
    document.querySelectorAll('.chart-row.spotlight, .chart-row.surprise-hover, .chart-row.surprise-flicker, .chart-row.surprise-final, .chart-row.surprise-reveal')
      .forEach((row) => row.classList.remove('spotlight', 'surprise-hover', 'surprise-flicker', 'surprise-final', 'surprise-reveal'));
  };

  try {
    clearMarks();

    // Phase 1: Button shimmer (brief suspense)
    if (randomBtn) {
      randomBtn.classList.add('surprise-btn-shimmer');
    }
    await sleep(800);
    if (runId !== surpriseRunId) return;
    if (randomBtn) {
      randomBtn.classList.remove('surprise-btn-shimmer');
    }

    // Phase 2: Pick winner and smooth scroll
    const chosen = rows[Math.floor(Math.random() * rows.length)];
    chosen.scrollIntoView({ behavior: 'smooth', block: 'center' });

    // Phase 3: Wait for scroll to settle, then cinematic reveal
    await sleep(500);
    if (runId !== surpriseRunId) return;
    chosen.classList.add('spotlight', 'surprise-final', 'surprise-reveal');

    setTimeout(() => {
      if (runId !== surpriseRunId) return;
      chosen.classList.remove('surprise-final', 'surprise-reveal');
    }, 2500);
  } finally {
...
function isAppleAmpEpisodesType(type) {
  const raw = String(type || '').trim().toLowerCase();
  return raw === 'top-episodes' || /^apple-\d{4}-episodes$/.test(raw) || /^genre-\d{4}-episodes$/.test(raw);
}
...
function getPayloadAliasCountries(platform, requestedCountry, payload) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const countries = [];
  const pushCountry = (value) => {
    const safe = normalizeCountry(value);
    if (!safe || countries.includes(safe)) return;
    countries.push(safe);
  };

  pushCountry(requestedCountry);

  if (safePlatform === 'spotify' && payload && typeof payload === 'object') {
    pushCountry(payload.requestedCountry);
    pushCountry(payload.sourceCountry);
    pushCountry(payload.country);
  }

  return countries;
}

async function persistPlatformPayloadAliases(platform, type, requestedCountry, payload, items = null, options = {}) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const safeType = safePlatform === 'all'
    ? normalizeTopType(type)
    : normalizeTypeForPlatform(safePlatform, type);
  const countries = getPayloadAliasCountries(safePlatform, requestedCountry, payload);
  if (!countries.length || !payload) return;

  const normalizedItems = Array.isArray(items) ? items : collectNormalizedItemsForData(payload, safePlatform, safeType);
  const fetchedAt = Number(options.fetchedAt) || Date.now();
  const savePayload = Object.prototype.hasOwnProperty.call(options || {}, 'payload') ? options.payload : payload;

  if (safePlatform === 'bilibili' && normalizedItems.length) {
    await persistBilibiliDetailCacheFromItems(normalizedItems);
  }

  await Promise.all(countries.map(async (country) => {
    await setResponseCacheEntry(responseCacheKeyFor(safePlatform, country, safeType), payload);
    if (normalizedItems.length) {
      await saveDailySnapshot(safePlatform, country, safeType, normalizedItems, {
        fetchedAt,
        payload: savePayload
      });
    }
...
const XIAOYUZHOU_BUNDLE_TYPES = [
  'top-episodes',
  'top-podcasts',
  'xiaoyuzhou-skyrocket-episodes',
  'xiaoyuzhou-new-star-episodes'
];

function responseCacheKeyForXiaoyuzhouBundle(country) {
  return `podcast_data_xiaoyuzhou_${normalizeCountry(country)}_bundle`;
}

function getXiaoyuzhouPayloadFromBundle(bundle, type) {
  const safeType = normalizeTypeForPlatform('xiaoyuzhou', type);
  const results = bundle && typeof bundle === 'object' && bundle.results && typeof bundle.results === 'object'
    ? bundle.results
    : null;
  if (!results) return null;
  const payload = results[safeType];
  return payload && typeof payload === 'object' ? payload : null;
}

async function primeXiaoyuzhouBundleCaches(bundle, country) {
  const safeCountry = normalizeCountry(country);
  if (!bundle || typeof bundle !== 'object') return;
  await setResponseCacheEntry(responseCacheKeyForXiaoyuzhouBundle(safeCountry), bundle);
  for (const type of XIAOYUZHOU_BUNDLE_TYPES) {
    const payload = getXiaoyuzhouPayloadFromBundle(bundle, type);
    if (!payload) continue;
    try {
      const items = collectNormalizedItemsForData(payload, 'xiaoyuzhou', type);
      await persistPlatformPayloadAliases('xiaoyuzhou', type, safeCountry, payload, items, { fetchedAt: Date.now(), payload });
    } catch (_error) { }
  }
}

async function fetchAndPrimeXiaoyuzhouBundle(country, options = {}) {
  const safeCountry = normalizeCountry(country);
  const params = new URLSearchParams({
    platform: 'xiaoyuzhou',
    type: 'bundle',
  ...
  const bundle = await fetchJsonWithFallback([`${WORKER_API_BASE}/?${params.toString()}`]);
  await primeXiaoyuzhouBundleCaches(bundle, safeCountry);
  return bundle;
}

function appendForceRefreshParams(urls) {
  const list = Array.isArray(urls) ? urls.filter(Boolean) : [];
  if (!list.length) return list;
  const stamp = String(Date.now());
  return list.map((url) => `${url}${url.includes('?') ? '&' : '?'}refresh=1&_ts=${stamp}`);
}
...
function getCacheTtlMs() {
  return isEpisodeChartTypeValue(typeSelect && typeSelect.value) ? EPISODE_CACHE_TTL_MS : CACHE_TTL_MS;
}

function getCacheTtlMsForType(type) {
  return isEpisodeChartTypeValue(type) ? EPISODE_CACHE_TTL_MS : CACHE_TTL_MS;
}
...
async function fetchPodcastsData(options = {}) {
  ...
  if (cached) {
    try {
      ...
      if (period === 'daily' && !selectedHistoryDateKey) {
        ...
        if (hasFresh && cachedItems.length) {
          await persistPlatformPayloadAliases(
            platformSelect.value,
            typeSelect.value,
            getEffectiveCountryKeyForScope(),
            cached,
            cachedItems,
            { fetchedAt: cachedTime || Date.now(), payload: cached }
          );
        }
        const snapshots = await getSnapshotsForCurrentSelection();
        await refreshHistoryDateOptions(snapshots);
  ...
  try {
    ...
    if (languageScopeActive) {
      ...
      let candidates = null;
      if (selectedPlatform === 'apple' && isAppleDerivedType(typeSelect.value)) {
        const derived = await fetchDerivedAppleChartData(typeSelect.value, countrySelect.value, { forceRefresh });
        data = derived.data;
      } else if (selectedPlatform === 'xiaoyuzhou') {
        const bundle = await fetchAndPrimeXiaoyuzhouBundle(countrySelect.value, {
          forceRefresh,
          type: typeSelect.value,
          renderedCache
        });
        data = getXiaoyuzhouPayloadFromBundle(bundle, typeSelect.value);
        if (!data) {
          throw new Error(`Missing Xiaoyuzhou bundle payload for ${String(typeSelect.value || '').trim()}`);
        }
      } else if (selectedPlatform === 'ximalaya') {
      ...
      if (!data) {
        if (forceRefresh && (selectedPlatform === 'bilibili')) {
          candidates = appendForceRefreshParams(candidates);
        }
        candidates = await appendWorkerPrefetchToCandidates(
          candidates,
          platformSelect.value,
          typeSelect.value,
          countrySelect.value,
        ...
        if (isWorkerPrefetchEnvelope(data)) {
          void persistInlinePrefetchedPayloads(data.prefetched);
          data = data.data;
        }
      }
    }
    ...
    if (String(platformSelect.value || '').trim().toLowerCase() === 'bilibili') {
      normalizedItems = await enrichBilibiliItemsFromDetailCache(normalizedItems);
    }
    ...
    await persistPlatformPayloadAliases(
      ...
      data,
      normalizedItems,
      { fetchedAt: Date.now(), payload: data }
    );
    await refreshHistoryDateOptions();
...
platformSelect.addEventListener('change', () => {
  updateHeroTypeSubtitle();
  globalRssPlayerController.ensureMounted().syncAccent();
...
chartContainer.addEventListener('click', async (event) => {
  const modeNode = event.target && event.target.closest
    ? event.target.closest('[data-episode-mode]')
    : null;
  if (modeNode) {
    event.preventDefault();
    event.stopPropagation();
...
const _plmColorCache = new Map();

function extractDominantColors(imgSrc, count = 4) {
  return new Promise((resolve) => {
    if (_plmColorCache.has(imgSrc)) { resolve(_plmColorCache.get(imgSrc)); return; }
    const fallback = [[80,100,200],[40,180,160],[160,80,220],[200,120,60]];
    if (!imgSrc) { resolve(fallback); return; }
    ...
    img.onload = () => {
      try {
        ...
        for (let i = 0; i < data.length; i += 8) {
          ...
          if (brightness > 15 && brightness < 240) pixels.push([r, g, b]);
        }
        if (!pixels.length) { resolve(fallback); return; }

        const distSq = (a, b) => (a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2 + (a[2] - b[2]) ** 2;

        let seed = pixels[0], maxSat = -1;
        for (let i = 0; i < pixels.length; i += 8) {
          const c = pixels[i];
          const sat = Math.max(c[0], c[1], c[2]) - Math.min(c[0], c[1], c[2]);
          if (sat > maxSat) { maxSat = sat; seed = c; }
        }

        const selected = [seed];
        const samplePool = [];
        const sampleSize = Math.min(pixels.length, 600);
        for (let i = 0; i < sampleSize; i++) {
          samplePool.push(pixels[Math.floor(Math.random() * pixels.length)]);
        }

        for (let k = 1; k < count; k++) {
          let bestDist = -1, best = samplePool[0];
          for (const c of samplePool) {
            let minD = Infinity;
            for (const s of selected) { const d = distSq(c, s); if (d < minD) minD = d; }
            if (minD > bestDist) { bestDist = minD; best = c; }
          }
          selected.push(best);
        }
        while (selected.length < count) selected.push(selected[0]);

        _plmColorCache.set(imgSrc, selected);
        resolve(selected);
...
let _plmAmbientReq = null;
let _plmAmbientBlobs = [];

function _lerpPlm(start, end, amt) {
  return start + (end - start) * amt;
}

function _initAmbientBlobs() {
  // Use orbital Lissajous curve parameters instead of linear velocities
  _plmAmbientBlobs = [
    { cx: 32, cy: 32, aX: 38, aY: 34, spX: 0.00012, spY: 0.00009, px: 0, py: 1, r: 48, c: [0,0,0], t: [0,0,0] },
    { cx: 32, cy: 32, aX: 42, aY: 38, spX: -0.00008, spY: 0.00015, px: 2, py: 3, r: 52, c: [0,0,0], t: [0,0,0] },
    { cx: 32, cy: 32, aX: 35, aY: 45, spX: 0.00014, spY: -0.00011, px: 4, py: 2, r: 45, c: [0,0,0], t: [0,0,0] },
    { cx: 32, cy: 32, aX: 45, aY: 40, spX: -0.00011, spY: -0.00014, px: 1, py: 5, r: 55, c: [0,0,0], t: [0,0,0] }
  ];
}

function _startAmbientAnimation(canvas) {
  if (!canvas || _plmAmbientReq) return;
  const ctx = canvas.getContext('2d', { alpha: false });
  canvas.width = 64;
  canvas.height = 64;
  if (!_plmAmbientBlobs.length) _initAmbientBlobs();

  let lastTime = performance.now();

  function loop(time) {
    _plmAmbientReq = requestAnimationFrame(loop);
    ...
    for (let i = 0; i < 4; i++) {
      const b = _plmAmbientBlobs[i];

      // Organic swirling movement based on Lissajous curves (mimics noise)
      const x = b.cx + Math.sin(time * b.spX + b.px) * b.aX;
      const y = b.cy + Math.cos(time * b.spY + b.py) * b.aY;
      
      b.c[0] = _lerpPlm(b.c[0], b.t[0], 0.05 * dt);
      b.c[1] = _lerpPlm(b.c[1], b.t[1], 0.05 * dt);
      b.c[2] = _lerpPlm(b.c[2], b.t[2], 0.05 * dt);

      const grad = ctx.createRadialGradient(x, y, 0, x, y, b.r);
      const r = Math.round(b.c[0]);
      const g = Math.round(b.c[1]);
      const bl = Math.round(b.c[2]);
...
function _stopAmbientAnimation() {
  if (_plmAmbientReq) {
    cancelAnimationFrame(_plmAmbientReq);
    _plmAmbientReq = null;
  }
}

function updatePlaylistDynamicBg(overlay, coverSrc) {
  if (!overlay) return;
  const canvas = overlay.querySelector('.plm-ambient-canvas');
  if (canvas && !_plmAmbientReq) _startAmbientAnimation(canvas);

  extractDominantColors(coverSrc).then((colors) => {
    if (!overlay.isConnected) return;
    if (!_plmAmbientBlobs.length) _initAmbientBlobs();
    for (let i = 0; i < 4; i++) {
      const c = colors[i] || colors[0];
      _plmAmbientBlobs[i].t = c;
    }
  }).catch(() => {});
}
...
const playlistController = {
  ...
  _buildOverlay() {
    ...
    el.innerHTML = `
      <div class="plm-dynamic-bg">
        <canvas class="plm-ambient-canvas"></canvas>
      </div>
  ...
  _collectItems() {
    ...
    rows.forEach((row) => {
      ...
      items.push({
        ...
        isVideo: platformHasDirectVideoBadge(embedPlatform) ? true : null,
        _videoDetected: platformHasDirectVideoBadge(embedPlatform)
      });
      realItemCount++;
  ...
  async _detectVideoAsync(item) {
    ...
    try {
      if (item.embedPlatform === 'spotify') {
        const fn = isEpisodeChartTypeValue(item.embedKind)
      ...
      } else if (item.embedPlatform === 'apple' && isEpisodeChartTypeValue(item.embedKind)) {
  ...
  _buildIframeForItem(item) {
    ...
    const playerMode = this._getItemPlayerMode(item);
    ...
    if (p === 'apple') {
      const cls = isEpisodeChartTypeValue(item.embedKind) ? 'plm-dock-apple-episode' : 'plm-dock-apple-show';
  ...
  _buildCardHtml(item, direction) {
    ...
    const playerMode = this._getItemPlayerMode(item);
    const showingRssPlayer = playerMode === 'rss';
    ...
    let extraCardClass = '';
    if (showingRssPlayer) {
      extraCardClass = 'plm-card-rss-show';
      if (item.embedPlatform === 'apple' && !isEpisodeChartTypeValue(item.embedKind)) {
        extraCardClass += ' plm-card-apple-show';
      }
    } else if (!isVideo && item.embedPlatform === 'apple' && !isEpisodeChartTypeValue(item.embedKind)) {
      extraCardClass = 'plm-card-apple-show';
    } else if (isVideo && item.embedPlatform === 'apple') {
      extraCardClass = 'plm-card-apple-video';
    }
    const platformCardClass = item.embedPlatform ? `plm-card-platform-${String(item.embedPlatform).trim().toLowerCase().replace(/[^\w-]+/g, '-')}` : '';
    const kindCardClass = item.embedKind ? `plm-card-kind-${String(item.embedKind).trim().toLowerCase().replace(/[^\w-]+/g, '-')}` : '';
    const titleMarkup = `${(!showingRssPlayer && (isVideo || platformHasDirectVideoBadge(item.embedPlatform))) ? buildVideoTitleBadgeMarkup() : ''}${this._escapeHtml(item.title)}`;

    return `
      <div class="plm-card ${dirClass} ${extraCardClass} ${platformCardClass} ${kindCardClass}" data-plm-index="${this.currentIndex}" style="--brand: ${brandColor1}; --brand-2: ${brandColor2};">
        ${mediaSection}
        <div class="plm-info">
          ...
          <div class="plm-info-text">
            <h2 class="plm-title">${titleMarkup}</h2>
        ...
        <div class="plm-actions">
          ...
          ${this._canToggleItemToRss(item) ? `
          <button class="plm-action-btn plm-mode-toggle-btn" type="button" data-plm-mode-toggle="${this._escapeAttr(showingRssPlayer ? String(item.embedPlatform || '').trim().toLowerCase() : 'rss')}" aria-label="${this._escapeAttr(showingRssPlayer ? (getMyPodcastsImportModeLabel(item.embedPlatform) || item.embedPlatform) : 'RSS')}" title="${this._escapeAttr(showingRssPlayer ? (getMyPodcastsImportModeLabel(item.embedPlatform) || item.embedPlatform) : 'RSS')}">
            ${showingRssPlayer
              ? (String(item.embedPlatform || '').trim().toLowerCase() === 'apple' ? getAppleModeSvg() : getMyPodcastsImportModeIconSvg(item.embedPlatform))
              : getRssFeedSvg()}
          </button>` : ''}
          ${!isVideo && (item.embedPlatform === 'spotify' || item.embedPlatform === 'apple' || item.embedPlatform === 'youtube' || showingRssPlayer || item.embedPlatform === 'rss' || item.embedPlatform === 'castbox' || item.embedPlatform === 'pocketcasts') ? `
  ...
  _getItemPlayerMode(item) {
    const platform = String(item && item.embedPlatform || '').trim().toLowerCase();
    if (platform === 'rss' || platform === 'castbox' || platform === 'pocketcasts') return 'rss';
    if (platform === 'apple' || platform === 'spotify') {
      const explicitMode = String(item && item.__plmPlayerMode || '').trim().toLowerCase();
      if (explicitMode === 'rss') return 'rss';
      return getDefaultRssPlayerMode(platform);
    }
    return platform;
  },

  _canToggleItemToRss(item) {
    if (!item || item.isAd) return false;
    const platform = String(item.embedPlatform || '').trim().toLowerCase();
    return platform === 'apple' || platform === 'spotify';
  },

  _getItemRssSourceUrl(item) {
    if (!item) return '';
    const platform = String(item.embedPlatform || '').trim().toLowerCase();
    if (platform === 'rss' || platform === 'castbox' || platform === 'pocketcasts') {
      return String(item.embedUrl || '').trim();
    }
    return normalizeFavoriteFeedUrl(item.resolvedFeedUrl || (item.favAttrs && item.favAttrs['data-favorite-feed']) || '');
  },

  async _switchCurrentItemPlayerMode(item, requestedMode, trigger = null) {
    if (!item || item.isAd) return false;
    const platform = String(item.embedPlatform || '').trim().toLowerCase();
    if (!(platform === 'apple' || platform === 'spotify')) return false;
    const nextMode = String(requestedMode || '').trim().toLowerCase() === 'rss' ? 'rss' : platform;
    const currentMode = this._getItemPlayerMode(item);
    ...
    try {
      if (nextMode === 'rss') {
        if (platform === 'spotify' && typeof globalThis !== 'undefined') {
          globalThis.__spotifyRssSwitchSuppressToastUntil = Date.now() + 3000;
        }
        const resolved = platform === 'apple'
          ? await resolveAppleFeedUrlForPlaylistItem(item)
          : await resolveSpotifyFeedUrlForPlaylistItem(item);
        if (!resolved.feedUrl) {
          if (platform === 'spotify' && typeof globalThis !== 'undefined') {
            globalThis.__spotifyRssSwitchSuppressToastUntil = 0;
          }
          showToast(t('loadFailedRefresh'));
          return false;
        }
        item.resolvedFeedUrl = resolved.feedUrl;
        item.__plmPlayerMode = 'rss';
        setDefaultRssPlayerModePreference('rss');
        if (platform === 'spotify' && typeof globalThis !== 'undefined') {
          globalThis.__spotifyRssSwitchSuppressToastUntil = 0;
        }
      } else {
        item.__plmPlayerMode = platform;
        setDefaultRssPlayerModePreference(platform);
      }
      if (this.isOpen && this.items[this.currentIndex] === item) {
        this._renderCard(this.currentIndex);
      }
      return true;
    } finally {
  ...
  _renderCard(index, direction) {
    ...
    // Update dynamic background with cover colors
    if (!item.isAd && item.coverSrc) {
      updatePlaylistDynamicBg(this.overlay, item.coverSrc);
    }
    ...
    const p = item.embedPlatform;
    const playerMode = this._getItemPlayerMode(item);
    if (((p === 'castbox' || p === 'pocketcasts' || p === 'rss') || playerMode === 'rss') && typeof renderCastboxRssPlayerInSlot === 'function') {
      const rssSlot = this.feedEl.querySelector('.plm-rss-slot');
      if (rssSlot) {
        const rssUrl = this._getItemRssSourceUrl(item) || item.embedUrl;
        renderCastboxRssPlayerInSlot(rssSlot, p, rssUrl, item.title, true, { includeAd: false }).catch(() => {});
      }
    }
  ...
  _bindCardActions() {
    ...
    const modeToggleBtn = card.querySelector('.plm-mode-toggle-btn');
    if (modeToggleBtn) {
      modeToggleBtn.addEventListener('click', async () => {
        const item = this.items[this.currentIndex];
        if (!item || item.isAd) return;
        const targetMode = String(modeToggleBtn.getAttribute('data-plm-mode-toggle') || '').trim().toLowerCase();
        await this._switchCurrentItemPlayerMode(item, targetMode, modeToggleBtn);
      });
    }
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', () => {
        ...
        const platform = String(item.embedPlatform || '').trim().toLowerCase();
        const playerMode = this._getItemPlayerMode(item);
        if (playerMode === 'rss' || platform === 'rss' || platform === 'castbox' || platform === 'pocketcasts') {
          const rssSlot = card.querySelector('.plm-rss-slot');
          if (!rssSlot) return;
          refreshBtn.disabled = true;
          refreshBtn.classList.add('is-loading');
          const rssUrl = this._getItemRssSourceUrl(item) || item.embedUrl;
          renderCastboxRssPlayerInSlot(rssSlot, platform, rssUrl, item.title, true, { includeAd: false })
            .catch(() => {})
            .finally(() => {
...
function ensurePlaylistModeButton() {
  ...
  btn.type = 'button';
  decorateCommandButton(btn, 'playlist', tOr('playlistMode', 'Playlist Mode'));
...
function ensureAudioModeButton() {
  const actions = document.querySelector('.command-actions');
  if (!actions || actions.querySelector('#audio-mode-btn')) return;
  const btn = document.createElement('button');
  btn.className = 'plain-btn';
  btn.id = 'audio-mode-btn';
  btn.type = 'button';
  decorateCommandButton(btn, 'audio-mode', tOr('audioMode', 'Audio Mode'));
  const refreshNode = actions.querySelector('#refresh-btn');
  const playlistNode = actions.querySelector('#playlist-mode-btn');
  if (playlistNode && playlistNode.parentNode === actions && playlistNode.nextSibling) {
    actions.insertBefore(btn, playlistNode.nextSibling);
  } else if (refreshNode && refreshNode.parentNode === actions) {
    actions.insertBefore(btn, refreshNode);
  } else {
    actions.appendChild(btn);
  }
  btn.addEventListener('click', () => {
    void enterAudioModeFromCurrentChart(btn);
  });
}
...
window.addEventListener('resize', () => {
  ...
  refreshVisibleEpisodeDescriptions();
});
...
function bootPage() {
  ...
  try {
    applyThemeMode(currentThemeMode, { persist: false, refreshAppleEmbeds: false });
  ...
  try { globalRssPlayerController.ensureMounted(); } catch (error) { console.warn('Global RSS player init failed:', error); }
...
window.addEventListener('load', () => {
  if (document.body && document.body.dataset && document.body.dataset.chartPageMode === 'topic') return;
  bootPage();
}, { once: true });
document.addEventListener('DOMContentLoaded', () => {
  if (document.body && document.body.dataset && document.body.dataset.chartPageMode === 'topic') return;
...
if (!(document.body && document.body.dataset && document.body.dataset.chartPageMode === 'topic')) {
  setTimeout(() => {
    if (!pageBooted) bootPage();
  }, 1200);
}
...
function getItunesBackfillLookupTitle(item) {
  if (!item || typeof item !== 'object') return '';
  ...
  const originType = String(item.originType || item.embedKind || '').trim().toLowerCase();
  if (isEpisodeChartTypeValue(originType)) return '';
...
function isEligibleForItunesAppleBackfill(item) {
  if (!item || typeof item !== 'object') return false;
  const title = getItunesBackfillLookupTitle(item);
...
function buildAppleEmbedUrlFromPodcastPage(url, countryCode) {
  ...
  if (!localized) return '';
  return appendAppleEmbedThemeParam(localized.replace('://podcasts.apple.com/', '://embed.podcasts.apple.com/'));
}
...
function applyItunesBackfillMatchToItem(item, match) {
  ...
  if (!String(item.description || '').trim() && String(match.description || '').trim()) {
    item.description = String(match.description || '').trim();
  }
...
function fetchItunesLookupPayloadByIds(ids, countryCode, timeoutMs = ITUNES_SEARCH_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const callbackName = '__podcastChartsItunesLookupCb_' + Date.now() + '_' + Math.floor(Math.random() * 100000);
    const script = document.createElement('script');
    let settled = false;
    let timer = 0;
    ...
    timer = window.setTimeout(() => {
      cleanup();
      reject(new Error('iTunes lookup timeout'));
    }, Math.max(2500, Number(timeoutMs) || ITUNES_SEARCH_TIMEOUT_MS));

    const country = normalizeCountry(countryCode || 'us');
    const url =
      'https://itunes.apple.com/lookup?id=' +
      encodeURIComponent(safeIds.join(',')) +
      '&media=podcast&entity=podcast&country=' +
      encodeURIComponent(country) +
      '&callback=' +
      encodeURIComponent(callbackName);
    ...
    script.onerror = () => {
      cleanup();
      reject(new Error('iTunes lookup network error'));
    };
    document.head.appendChild(script);
  });
}
...
function pickItunesPodcastDescription(entry) {
  if (!entry || typeof entry !== 'object') return '';
  return String(
    entry.description
...
function normalizeItunesCatalogSearchEntry(entry, countryCode, themePlatform) {
  ...
  return {
    ...
    description: pickItunesPodcastDescription(entry),
    embedUrl,
    embedPlatform: 'apple',
    embedKind: 'top-podcasts',
    sourceType: 'chart',
  ...
  for (let attempt = 0; attempt <= ITUNES_SEARCH_MAX_RETRIES; attempt += 1) {
    try {
      ...
      const match = {
        ...
        description: pickItunesPodcastDescription(found),
...
async function hydrateAppleItemsWithFeedUrls(items, countryCode) {
  const list = Array.isArray(items) ? items : [];
  if (!list.length) return list;

  const unresolved = [];
  await Promise.all(list.map(async (item) => {
    if (!item || String(item.feedUrl || '').trim()) return;
    const cached = await getCachedItunesBackfillMatch(item.title, countryCode);
    if (cached) {
      item.feedUrl = String(cached.feedUrl || '').trim();
      if (!String(item.url || '').trim() && String(cached.url || '').trim()) item.url = String(cached.url || '').trim();
      return;
    }
  ...
  unresolved.forEach((item) => {
    const podcastId = extractApplePodcastIdFromUrl(item && item.url);
    if (podcastId) unresolvedById.set(podcastId, item);
  });

  const unresolvedIds = Array.from(unresolvedById.keys());
  for (let i = 0; i < unresolvedIds.length; i += 25) {
    const batchIds = unresolvedIds.slice(i, i + 25);
    let payload = null;
    try {
      payload = await fetchItunesLookupPayloadByIds(batchIds, countryCode, ITUNES_SEARCH_TIMEOUT_MS);
    } catch (error) {
      console.warn('iTunes batch lookup failed:', batchIds, error);
      continue;
    }
    const results = Array.isArray(payload && payload.results) ? payload.results : [];
    results.forEach((entry) => {
      const trackId = String(entry && (entry.trackId || entry.collectionId) || '').trim();
      if (!trackId || !unresolvedById.has(trackId)) return;
      const item = unresolvedById.get(trackId);
      const pageUrl = localizeApplePodcastUrl(entry.collectionViewUrl || entry.trackViewUrl || item.url || '', countryCode);
      const embedUrl = buildAppleEmbedUrlFromPodcastPage(pageUrl, countryCode);
      const match = {
        url: pageUrl,
        embedUrl,
        embedPlatform: 'apple',
        ...
        artworkUrl100: String(entry.artworkUrl100 || entry.artworkUrl600 || '').trim(),
        trackId
      };
      item.feedUrl = match.feedUrl;
      if (pageUrl) item.url = pageUrl;
      if (!String(item.description || '').trim() && String(match.description || '').trim()) {
        item.description = String(match.description || '').trim();
      }
      const sourceTitleKey = buildItunesBackfillCacheKey(item.title, countryCode);
      if (sourceTitleKey) {
        const cacheValue = { fetchedAt: Date.now(), value: match };
        itunesBackfillCache.set(sourceTitleKey, cacheValue);
        void persistItunesBackfillCache(sourceTitleKey, match, Date.now());
      }
      const returnedTitleKey = buildItunesBackfillCacheKey(entry.trackName || entry.collectionName || '', countryCode);
      if (returnedTitleKey && returnedTitleKey !== sourceTitleKey) {
        const cacheValue = { fetchedAt: Date.now(), value: match };
        itunesBackfillCache.set(returnedTitleKey, cacheValue);
        void persistItunesBackfillCache(returnedTitleKey, match, Date.now());
      }
    });
  }

  const stillMissing = list.filter((item) => !String(item && item.feedUrl || '').trim());
  await mapWithConcurrency(stillMissing, 2, async (item) => {
    const match = await searchApplePodcastByExactTitleOnItunes(item.title, countryCode);
    if (!match) return;
    item.feedUrl = String(match.feedUrl || '').trim();
    if (!String(item.url || '').trim() && String(match.url || '').trim()) item.url = String(match.url || '').trim();
    if (!String(item.description || '').trim() && String(match.description || '').trim()) item.description = String(match.description || '').trim();
  });

  return list;
}

function normalizeHostingPlatformRuleUrl(raw) {
...
function parseHostingPlatformRulesRaw(rawText) {
  return String(rawText || '')
    ...
    .map((line) => {
      ...
      const name = parts[1];
      const url = normalizeHostingPlatformRuleUrl(parts.slice(2).join('|'));
      if (!host || !name || !url) return null;
      return { hosts: [host], name, url };
    })
    .filter(Boolean);
}

function mergeHostingPlatformRules(...groups) {
  const out = [];
  const seenHosts = new Set();
  groups.forEach((group) => {
    (Array.isArray(group) ? group : []).forEach((rule) => {
      if (!rule || !Array.isArray(rule.hosts) || !rule.hosts.length) return;
      ...
      out.push({
        ...
        url: normalizeHostingPlatformRuleUrl(rule.url)
      });
    });
  });
  return out;
...
const HOSTING_PLATFORM_RULES_BASE = [
  { hosts: ['omnycontent.com', 'omny.fm', 'api.omny.fm'], name: 'Omny Studio', url: 'https://www.omnycontent.com/' },
  { hosts: ['megaphone.fm', 'traffic.megaphone.fm'], name: 'Megaphone', url: 'https://megaphone.fm/' },
  { hosts: ['acast.com', 'feeds.acast.com', 'sphinx.acast.com', 'ateam.acast.cloud'], name: 'Acast', url: 'https://acast.com/' },
  { hosts: ['libsyn.com', 'traffic.libsyn.com'], name: 'Libsyn', url: 'https://libsyn.com/' },
  { hosts: ['simplecast.com', 'cdn.simplecast.com'], name: 'Simplecast', url: 'https://simplecast.com/' },
  { hosts: ['art19.com', 'rss.art19.com'], name: 'ART19', url: 'https://art19.com/' },
  { hosts: ['buzzsprout.com', 'feeds.buzzsprout.com'], name: 'Buzzsprout', url: 'https://www.buzzsprout.com/' },
  { hosts: ['captivate.fm', 'feeds.captivate.fm'], name: 'Captivate', url: 'https://www.captivate.fm/' },
  { hosts: ['transistor.fm', 'feeds.transistor.fm'], name: 'Transistor', url: 'https://transistor.fm/' },
  { hosts: ['podbean.com', 'mcdn.podbean.com'], name: 'Podbean', url: 'https://www.podbean.com/' },
  { hosts: ['rss.com', 'media.rss.com'], name: 'RSS.com', url: 'https://rss.com/' },
  { hosts: ['spreaker.com'], name: 'Spreaker', url: 'https://www.spreaker.com/' },
  { hosts: ['redcircle.com', 'feeds.redcircle.com'], name: 'RedCircle', url: 'https://redcircle.com/' },
  { hosts: ['audioboom.com'], name: 'Audioboom', url: 'https://audioboom.com/' },
  { hosts: ['anchor.fm'], name: 'Spotify for Creators', url: 'https://creators.spotify.com/' },
  { hosts: ['blubrry.com'], name: 'Blubrry', url: 'https://blubrry.com/' },
  { hosts: ['castos.com', 'feeds.castos.com'], name: 'Castos', url: 'https://castos.com/' },
  { hosts: ['pinecast.com'], name: 'Pinecast', url: 'https://pinecast.com/' },
  { hosts: ['substack.com'], name: 'Substack', url: 'https://substack.com/' },
  { hosts: ['sounder.fm'], name: 'Sounder', url: 'https://sounder.fm/' },
  { hosts: ['prx.org', 'publicradio.org'], name: 'PRX', url: 'https://prx.org/' }
];

const HOSTING_PLATFORM_RULES_RAW = `
100komma7.lu | Radio 100,7 | 100komma7.lu
101.ru | 101.ru | 101.ru
102fm.co.il | Radio Tel Aviv | 102fm.co.il
3cat.cat | 3Cat | 3cat.cat
3cr.org.au | 3CR Community Radio | 3cr.org.au
9natree.com | 9Natree | 9natree.com
aalto.fi | Aalto University | aalto.fi
abc.net.au | ABC | abc.net.au
accessmedia.nz | Access Media NZ | accessmedia.nz
accessmore.com | AccessMore | accessmore.com
acikradyo.com.tr | Açık Radyo | acikradyo.com.tr
adnosiny.by | Adnosiny | adnosiny.by
afr.net | American Family Radio | afr.net
afripods.com | Afripods | afripods.africa
aipolyglot.jp | AI Polyglot | aipolyglot.jp
alarabiya.net | Al Arabiya | alarabiya.net
alitu.com | Alitu | alitu.com
allplane.tv | Allplane | allplane.tv
amazonaws.com | Amazon S3 | aws.amazon.com
amperwave.net | AmperWave | amperwave.com
ancientfaith.com | Ancient Faith Ministries | ancientfaith.com
apostrophepodcasts.ca | Apostrophe Podcast Network | apostrophepodcasts.ca
apparatus.si | Apparatus | apparatus.si
apple.com | Apple Podcasts | podcasts.apple.com
apprendrelanglaisrapidement.fr | Apprendre l'anglais | apprendrelanglaisrapidement.fr
arabicpod101.com | ArabicPod101 | arabicpod101.com
archive.org | Internet Archive | archive.org
arteradio.com | ARTE Radio | arteradio.com
articlesofinterest.club | Articles of Interest | articlesofinterest.co
arzamas.academy | Arzamas | arzamas.academy
async.com | Async | async.com
audio.com | Audio.com | audio.com
audiocon.de | audiocom multimedia | audiocom.de
audiodesires.com | Audio Desires | audiodesires.com
audiomack.com | Audiomack | audiomack.com
audiomeans.fr | Audiomeans | audiomeans.fr
audion.fm | Audion | audion.fm
audiorella.com | Audiorella | audiorella.com
ausha.co | Ausha | ausha.co
avivanuestroscorazones.com | Aviva Nuestros Corazones | avivanuestroscorazones.com
azatutyun.am | Radio Azatutyun | azatutyun.am
azerion.com | Azerion | azerion.com
babybus.com | BabyBus | babybus.com
backbonebroadcast.com | Backbone | backbonebroadcast.com
banjohangout.org | Banjo Hangout | banjohangout.org
barakservicos.com | Barak Serviços | barakservicos.com
barrons.com | Barron's | barrons.com
barstoolsports.com | Barstool Sports | barstoolsports.com
bbci.co.uk | BBC Sounds | bbc.co.uk/sounds
beehiiv.com | beehiiv | beehiiv.com
bennyhinn.org | Benny Hinn Ministries | bennyhinn.org
beyondwords.io | BeyondWords | beyondwords.io
bibletalk.tv | BibleTalk.tv | bibletalk.tv
blubrry.net | Blubrry | blubrry.com
bnipodcast.com | The Official BNI Podcast | bnipodcast.com
bomban.is | Bomban Podcast | bomban.is
br.de | BR | br.de
bremenzwei.de | Bremen Zwei | bremenzwei.de
byui.edu | BYU-Idaho | byui.edu
capital.bg | Capital | capital.bg
cardionerds.com | CardioNerds | cardionerds.com
castbox.fm | Castbox | castbox.fm
cbc.ca | CBC | cbc.ca
cbn.com | CBN | cbn.com
ccphilly.org | Calvary Chapel Philadelphia | ccphilly.org
cdnstream1.com | CDNStream | cdnstream.com
cgtn.com | CGTN | cgtn.com
chcemviac.com | Chcemviac | chcemviac.com
choq.ca | CHOQ | choq.ca
christourlife.ca | Christ Our Life | christourlife.ca
citr.ca | CiTR 101.9 FM | citr.ca
cloudfront.net | Amazon CloudFront | aws.amazon.com
cloudplay.ro | CloudPlay | cloudplay.ro
cloudworks.ro | Soundis | soundis.gr
coffeeklatchcrew.com | Coffee Klatch Crew | coffeeklatchcrew.com
cohostpodcasting.com | CoHost | cohostpodcasting.com
cope.es | COPE | cope.es
cornerstonechapel.net | Cornerstone Chapel | cornerstonechapel.net
csbible.com | Christian Standard Bible | csbible.com
cuonda.com | Cuonda | cuonda.com
danlirencomedy.com | Danliren Comedy | danlirencomedy.com
dasding.de | DASDING | dasding.de
delfi.ee | Delfi | delfi.ee
democracynow.org | Democracy Now! | democracynow.org
desiringgod.org | Desiring God | desiringgod.org
deutschlandfunk.de | Deutschlandfunk | deutschlandfunk.de
deutschlandfunkkultur.de | Deutschlandfunk Kultur | deutschlandfunkkultur.de
deutschlandfunknova.de | Deutschlandfunk Nova | deutschlandfunknova.de
dfja.eu | DFJA | dfja.eu
digitaloceanspaces.com | DigitalOcean Spaces | digitalocean.com
djpod.com | DJPod | djpod.com
dotcast.org | DotCast | dotcast.org
dou.ua | DOU | dou.ua
downloadsvr.com | Bill Winston Ministries | billwinston.org
dr.dk | DR | dr.dk
dropboxusercontent.com | Dropbox | dropbox.com
dw.com | DW | dw.com
easynewsitalian.com | Easy News Italian | easynewsitalian.com
eccdubai.org | ECC Dubai | eccdubai.org
edgesuite.net | Akamai | akamai.com
eff.org | Electronic Frontier Foundation | eff.org
elevationchurch.org | Elevation Church | elevationchurch.org
elsitiocristiano.com | OnePlace | oneplace.com
embassychurch.net | Embassy Church | embassychurch.net
emcitv.com | EMCI TV | emcitv.com
encontacto.org | In Touch Ministries | encontacto.org
engineeringmanagementinstitute.org | Engineering Management Institute | engineeringmanagementinstitute.org
erkamradyo.com | Erkam Radyo | erkamradyo.com
err.ee | ERR | err.ee
europafm.com | Europa FM | europafm.com
europafm.ro | Europa FM | europafm.ro
ewtn.com | EWTN | ewtn.com
faceoff.lv | FaceOff | faceoff.lv
faithfm.com.au | Faith FM | faithfm.com.au
fame.so | Fame | fame.so
fangraphs.com | FanGraphs | fangraphs.com
fastcast.ai | FastCast | fastcast.ai
feedbueno.es | FeedBueno | feedbueno.es
feedburner.com | FeedBurner | feedburner.google.com
feedpress.me | FeedPress | feedpress.com
filalag.is | Fílalag | filalag.is
fireside.fm | Fireside | fireside.fm
flightcast.com | FlightCast | flightcast.com
flightradar24.com | Flightradar24 | flightradar24.com
folcayeduase.org | Fountain of Life Cathedral | folcayeduase.org
fortisinstitute.org | Fortis Institute | fortisinstitute.org
fotbolti.net | Fótbolti.net | fotbolti.net
fountain.fm | Fountain | fountain.fm
fourble.co.uk | Fourble | fourble.co.uk
france24.com | France 24 | france24.com
fritz.de | Radio Fritz | fritz.de
fusebox.fm | Fusebox | fusebox.fm
futurock.fm | Futurock | futurock.fm
genthosmedia.com | Genthos Media | genthosmedia.com
georgetown.edu | Georgetown University | georgetown.edu
gilbut.co.kr | Gilbut Publishing | gilbut.co.kr
github.io | GitHub Pages | pages.github.com
githubusercontent.com | GitHub | github.com
gitlab.io | GitLab Pages | gitlab.com
giuseppeflandoli.com | Giuseppe Flandoli | giuseppeflandoli.com
global.com | Global | global.com
globo.com | Globo | globo.com
googleapis.com | Google Cloud Storage | cloud.google.com
govoritmoskva.ru | Govorit Moskva | govoritmoskva.ru
gracia.org | Gracia a Vosotros | gracia.org
graffitiradio.fr | Graffiti Radio | graffitiradio.fr
greatgame.blog | The Great Game | greatgame.blog
gty.org | Grace to You | gty.org
guerrillaradio.ro | Radio Guerrilla | guerrillaradio.ro
haaretz.co.il | Haaretz | haaretz.co.il
hakawati.fm | Hakawati | hakawati.fm
harvardbusiness.org | Harvard Business Impact | harvardbusiness.org
heimildin.is | Heimildin | heimildin.is
helloaudio.fm | Hello Audio | helloaudio.fm
henlagt.no | Henlagt TV | henlagt.no
hoerspielundfeature.de | Hörspiel und Feature | hoerspielundfeature.de
hopetv.org | Hope Channel | hopetv.org
hozana.si | HOZANA.si | hozana.si
hrt.hr | HRT | hrt.hr
hubhopper.com | Hubhopper | hubhopper.com
iamlmp.com | LMP | iamlmp.com
ifrs.org | IFRS Foundation | ifrs.org
ihumand.com | iHuman | ihuman.com
ilo.org | International Labour Organization | ilo.org
ilsole24ore.com | Il Sole 24 Ore | ilsole24ore.com
imbc.com | MBC | imbc.com
blog.in.th | Blognone | blog.in.th
infostart.hu | Infostart | infostart.hu
infowarproductions.com | InfoWar Productions | infowarproductions.com
insight.org | Insight for Living | insight.org
intouch.org | In Touch Ministries | intouch.org
iono.fm | iono.fm | iono.fm
islamway.net | IslamWay | islamway.net
italianpod101.com | Innovative Language | italianpod101.com
jamit.app | Jamit | jamit.app
japanesepod101.com | Innovative Language | japanesepod101.com
joincci.org | Celebration Church International | joincci.org
jtbc.co.kr | JTBC | jtbc.co.kr
julephosting.de | Julep | julep.de
justpodmedia.com | JustPod | justpod.com
kajabi.com | Kajabi | kajabi.com
kakadu.de | Deutschlandfunk Kultur | kakadu.de
kbs.co.kr | KBS | kbs.co.kr
khz.se | KHZ | khz.se
kidscorner.net | Kids Corner | kidscorner.net
kissfm.ua | Kiss FM Ukraine | kissfm.ua
klubradio.hu | Klubrádió | klubradio.hu
konnektor.org | Konnektor | konnektor.org
koreanclass101.com | Innovative Language | koreanclass101.com
kpcg.fm | KPCG | kpcg.fm
kpfa.org | KPFA | kpfa.org
kpfk.org | KPFK | kpfk.org
kroraina.com.cn | Kroraina | kroraina.com.cn
languagepodcasts.com | LanguagePodcasts | languagepodcasts.com
larepublica.pe | La República | larepublica.pe
lastdaysnews.com | Last Days News | lastdaysnews.com
latrobe.edu.au | La Trobe University | latrobe.edu.au
lepodcast.fr | podCloud | podcloud.fr
letscast.fm | LetsCast.fm | letscast.fm
libertaddigital.com | Libertad Digital | libertaddigital.com
lightcast.com | Lightcast | lightcast.com
lightsource.com | LightSource | lightsource.com
ligonier.org | Ligonier Ministries | ligonier.org
littlestoriestinypeople.com | Little Stories for Tiny People | littlestoriestinypeople.com
lizhi.fm | Lizhi | lizhi.fm
long.vn | Radio Long | long.vn
loudandfound.com | Loud & Found | loudandfound.com
loyalbooks.com | Loyal Books | loyalbooks.com
lse.ac.uk | LSE | lse.ac.uk
lsm.lv | Latvijas Radio | latvijasradio.lsm.lv
lu-church.de | LU Church | lu-church.de
lunfafm.com | Lunfa FM | lunfafm.com
machaneh.org | The Machaneh | machaneh.org
mahasi.org.mm | Mahasi | mahasi.org.mm
maranata.se | Maranata | maranata.se
masjidds.org | Masjid DarusSalam | masjidds.org
mave.digital | mave | mave.digital
mc-doualiya.com | Monte Carlo Doualiya | mc-doualiya.com
mdr.de | MDR | mdr.de
mdstrm.com | Mediastream | mediastream.co
medi1tv.ma | Medi 1 TV | medi1tv.com
meduza.io | Meduza | meduza.io
meldingcloud.com | AEZFM | aezfm.com
millasreggeli.hu | Millásreggeli | millasreggeli.hu
mountaingoatsoftware.com | Mountain Goat Software | mountaingoatsoftware.com
mtggoldfish.com | MTGGoldfish | mtggoldfish.com
mujrozhlas.cz | Český rozhlas | rozhlas.cz
mundomais.com.br | MundoMais | mundomais.com.br
music-zone.es | Music Zone | music-zone.es
nbcnews.com | NBC News | nbcnews.com
ndr.de | NDR | ndr.de
nexx.cloud | nexxPLAY | nexx.tv
nhk.or.jp | NHK | nhk.or.jp
nimmalo.com | Nimmalo | nimmalo.com
noagendaassets.com | No Agenda | noagendashow.net
notisistema.net | Notisistema | notisistema.net
npo.nl | NPO | npo.nl/luister
npr.org | NPR | npr.org
nrjaudio.fm | NRJ Audio | nrjaudio.fm
nrk.no | NRK | nrk.no
nsca.com | NSCA | nsca.com
ntr.nl | NTR | ntr.nl
nucleuscdn.com | Nucleus | nucleus.church
nv.ua | New Voice | nv.ua
nyimbompya.com | Nyimbo Mpya | nyimbompya.com
obedira.com.py | Radio Obedira | obedira.com.py
ognjisce.si | Radio Ognjišče | ognjisce.si
ondacero.es | Onda Cero | ondacero.es
oneplace.com | OnePlace | oneplace.com
open.ac.uk | Open University | open.ac.uk
openlanguage.com | OpenLanguage | openlanguage.com
oper.ru | Tynu40k Goblina | oper.ru
orf.at | ORF | orf.at
ox.ac.uk | University of Oxford | ox.ac.uk
oyez.org | Oyez | oyez.org
panyaprateep.org | Ajahn Jayasaro | panyaprateep.org
pastor-chuck.com | The Word For Today | pastorchuck.org
patreon.com | Patreon | patreon.com
pavillon-arsenal.com | Pavillon de l'Arsenal | pavillon-arsenal.com
pawsandtales.org | Paws & Tales | pawsandtales.org
pbs.org | PBS | pbs.org
pdrl.fm | Podreal | pdrl.fm
pepsradio.be | Pep's Radio | pepsradio.be
pippa.io | Acast | acast.com
pjisrael.org | Sifriyat Pijama | pjisrael.org
planningcenteronline.com | Planning Center | planningcenter.com
pleier.ee | Pleier | pleier.ee
plumvillage.org | Plum Village | plumvillage.org
pod.co | Pod.co | pod.co
pod.space | Podspace | podspace.com
podcaster.de | podcaster.de | podcaster.de
podcastfeed.nl | PodcastFeed | podcastfeed.nl
podcastics.com | Podcastics | podcastics.com
podcastle.ai | Async | async.com
podcastmirror.com | Podcast Mirror | podcastmirror.com
podcasts.com | Podcasts.com | podcasts.com
podeo.co | Podeo | podeo.co
podfm.ru | PodFM | podfm.ru
podhome.fm | Podhome | podhome.fm
podigee.io | Podigee | podigee.com
podomatic.com | Podomatic | podomatic.com
podplaystudio.com | Podplay Studio | studio.podplay.com
podpoint.com | Pod Point | podpoint.com
podsast.ru | ТЕХНОПОДСАСТ | podsast.ru
podscribe.ai | Podscribe | podscribe.com
podserve.fm | PodServe.fm | podserve.fm
podster.fm | Podster | podster.fm
podtoo.com | PodToo | podtoo.com
podtree.com | Podtree | podtree.com
polityka.pl | Polityka | polityka.pl
polskieradio.pl | Polskie Radio | polskieradio.pl
postimees.ee | Postimees | postimees.ee
posttv.com | Washington Post | washingtonpost.com
pray.com | Pray.com | pray.com
primaverasound.com | Radio Primavera Sound | primaverasound.com
prisasd.com | PRISA Radio | prisa.com
prologiq.co.za | DJ PH | iamph.prologiq.co.za
promodj.com | PromoDJ | promodj.com
protocol-radio.com | Protocol Radio | protocol-radio.com
prototype202.com | Prototype202 | prototype202.com
publicfeeds.net | Public Feeds | publicfeeds.net
pulsradio.com | Puls'Radio | pulsradio.com
punkto.info | punkto.info | punkto.info
qingting.fm | Qingting FM | qingting.fm
qq.com | QQ Music | y.qq.com
r2.dev | Cloudflare R2 | cloudflare.com/developer-platform/r2
rac1.cat | RAC1 | rac1.cat
radio.si | Radio Si | radio.si
radio1.ch | Radio 1 | radio1.ch
radio886.at | Radio 88.6 | radio886.at
radiocampusparis.org | Radio Campus Paris | radiocampusparis.org
radioeins.de | radioeins | radioeins.de
radiofrance-podcast.net | Radio France | radiofrance.fr
radiokp.ru | Komsomolskaya Pravda | radiokp.ru
radiomaria.es | Radio Maria | radiomaria.es
radiomelodie.com | Radio Mélodie | radiomelodie.com
radiomisfits.com | Radio Misfits | radiomisfits.com
radionacional.com.ar | Radio Nacional | radionacional.com.ar
radionikkei.jp | Radio NIKKEI | radionikkei.jp
radionovaera.pt | Radio Nova Era | radionovaera.pt
radioomega.fr | Radio Omega | radioomega.fr
radiorecord.ru | Radio Record | radiorecord.ru
radiosputnik.ru | Radio Sputnik | radiosputnik.ru
radiotavisupleba.ge | რადიო თავისუფლება | radiotavisupleba.ge
radiozet.pl | Radio ZET | radiozet.pl
rarejob.com | RareJob | rarejob.com
rbb-online.de | rbb | rbb-online.de
rbnenergy.com | RBN Energy | rbnenergy.com
redbarn.ru | Red Barn | redbarn.ru
redbasset.tech | Red Basset | redbasset.tech
reflex.cz | Reflex | reflex.cz
retrorocketnetwork.pl | Retro Rocket | retrorocket.pl
revealradio.org | Reveal | revealnews.org
reveillenoscoeurs.com | Réveille Nos Cœurs | reveillenoscoeurs.com
reviveourhearts.com | Revive Our Hearts | reviveourhearts.com
revma.com | Revma | revma.com
rfi.fr | RFI | rfi.fr
rhema.org | Rhema Ministries | rhema.org
ricamconsultoria.com.br | RICAM Consultoria | ricamconsultoria.com.br
riverside.fm | Riverside | riverside.fm
rmf.pl | RMF FM | rmf.pl
rnz.co.nz | RNZ | rnz.co.nz
robotstart.jp | Robot Start | robotstart.jp
rockfm.fm | Rock FM | rockfm.fm
rockfm.ro | Rock FM | rockfm.ro
romanianpod101.com | RomanianPod101 | romanianpod101.com
rthk.hk | RTHK | rthk.hk
rtl.it | RTL 102.5 | rtl.it
rtl.lu | RTL Luxembourg | rtl.lu
rtv.rs | Radio Television of Vojvodina | rtv.rs
rtve.es | RTVE | rtve.es
rtvslo.si | RTV Slovenija | rtvslo.si
rumcast.com | Rumcast | rumcast.com
run.app | Google Cloud Run | cloud.google.com
rus.fm | Rus.fm | rus.fm
russianpod101.com | RussianPod101 | russianpod101.com
ruv.is | RÚV | ruv.is
s3mts.ru | mave | mave.digital
saooti.com | Saooti | saooti.com
sardius.media | Sardius Media | sardius.media
sbs.co.kr | SBS | sbs.co.kr
sbs.com.au | SBS | sbs.com.au
sceneonradio.org | Scene on Radio | sceneonradio.org
schibsted.media | Schibsted | schibsted.com
seesaa.net | Seesaa | seesaa.net
sermonaudio.com | SermonAudio | sermonaudio.com
servers8.com | Triple8 | triple8.net
sesame.org | Sesame Workshop | sesameworkshop.org
seznam.cz | Seznam Podcasty | podcasty.seznam.cz
sherwood.online | Sherwood | sherwood.online
sigli.com | Sigli | sigli.com
simpletruthsforlife.org | Simple Truths for Life | simpletruthsforlife.org
sisradio.be | SIS Radio | sisradio.be
siyertv.com | Siyer TV | siyertv.com
skai.gr | SKAI | skai.gr
skivamusic.com | SK Infinity World Media | skivamusic.com
slate.com | Slate | slate.com
slobodnaevropa.mk | Радио Слободна Европа | slobodnaevropa.mk
slobodnaevropa.org | Radio Slobodna Evropa | slobodnaevropa.org
smartpod.no | SmartPod | smartpod.no
sochcast.com | Sochcast | sochcast.com
sofapps.it | Sofapps | sofapps.it
sonicbowl.cloud | SonicBowl | sonicbowl.cloud
soundcloud.com | SoundCloud | soundcloud.com
soundon.fm | SoundOn | soundon.fm
sportschau.de | Sportschau | sportschau.de
springcast.app | Springcast | springcast.io
squarespace.com | Squarespace | squarespace.com
srf.ch | SRF | srf.ch
ssenhosting.com | Podbbang | podbbang.com
stationista.com | Stationista | stationista.com
storycast.ru | StoryCast | storycast.ru
storyfm.cn | Story FM | storyfm.cn
storynory.com | Storynory | storynory.com
streamguys1.com | StreamGuys | streamguys.com
studeo.fm | Studeo | studeo.fm
stvr.sk | STVR | stvr.sk
subsplash.com | Subsplash | subsplash.com
svmaudio.com | SVM Audio | svmaudio.com
svoboda.org | Radio Svoboda | svoboda.org
svobodnaevropa.bg | Svobodna Evropa | svobodnaevropa.bg
swr.de | SWR | swr.de
swr3.de | SWR3 | swr3.de
t13.cl | T13 | t13.cl
tagesschau.de | Tagesschau | tagesschau.de
talkshoe.com | TalkShoe | talkshoe.com
teknoseyir.com | TeknoSeyir | teknoseyir.com
thaipbs.or.th | Thai PBS | thaipbs.or.th
theblackpodcastapp.com | The Black Podcast App | theblackpodcastapp.com
theguardian.com | The Guardian | theguardian.com
theheartradio.org | The Heart | theheartradio.org
themasters.ai | Masters of Automation | themasters.ai
themoth.org | The Moth | themoth.org
thespecialsaucepodcast.com | Special Sauce | thespecialsaucepodcast.com
thinkerview.com | ThinkerView | thinkerview.com
thisamericanlife.org | This American Life | thisamericanlife.org
thisisdistorted.com | This Is Distorted | thisisdistorted.com
thmanyah.com | Thmanyah | thmanyah.com
tilos.hu | Tilos Rádió | tilos.hu
tnhaudio.org | Thich Nhat Hanh Audio | tnhaudio.org
tokfm.pl | TOK FM | tokfm.pl
topalbaniaradio.com | Top Albania Radio | topalbaniaradio.com
topchretien.com | TopChrétien | topchretien.com
torahclass.com | Torah Class | torahclass.com
tumedadtunnid.ee | Tumedad Tunnid | tumedadtunnid.ee
tuoitre.vn | Tuoi Tre | tuoitre.vn
turkishclass101.com | TurkishClass101 | turkishclass101.com
tvradio.show | Televiziya po radioto | tvradio.show
twit.tv | TWiT | twit.tv
twr.org | Thru the Bible | ttb.bible
tygodnikpowszechny.pl | Tygodnik Powszechny | tygodnikpowszechny.pl
typlog.io | Typlog | typlog.com
tziun3.co.il | Tziun 3 | tziun3.co.il
uber.space | Uberspace | uberspace.de
ubook.com | Ubook | ubook.com
ufc.tv | UFC | ufc.com
umputun.com | Feed Master | umputun.com
news.un.org | United Nations News | news.un.org
uni-muenchen.de | LMU Munich | uni-muenchen.de
uni-regensburg.de | University of Regensburg | uni-regensburg.de
universalstudioshomeentertainment.com | Universal Studios | universalpictures.com
ur.se | UR | ur.se
valgusetee.ee | Valguse Tee | valgusetee.ee
vistopia.com.cn | Vistopia | vistopia.com.cn
vk.com | VK | vk.com
vnexpress.net | VnExpress | vnexpress.net
voaafrique.com | VOA Afrique | voaafrique.com
voanews.com | Voice of America | voanews.com
vpr.net | Vermont Public | vermontpublic.org
vrt.be | VRT | vrt.be
warroom.org | War Room | warroom.org
wavpub.com | WavPub | wavpub.com
wbur.org | WBUR | wbur.org
wdr.de | WDR | wdr.de
wearebeachhouse.com | Beach House | wearebeachhouse.com
weeralwo2.be | Weeral WO2 | weeralwo2.be
wgbh.org | GBH | wgbh.org
windows.net | Microsoft Azure | azure.microsoft.com
wistia.com | Wistia | wistia.com
witcastthailand.com | WiTcast | witcastthailand.com
womeninoceanscience.com | Women in Ocean Science | womeninoceanscience.com
wordpress.com | WordPress | wordpress.com
workers.dev | Cloudflare Workers | workers.cloudflare.com
wsj.com | Wall Street Journal | wsj.com
ximalaya.com | Ximalaya | ximalaya.com
xsfm.co.kr | XSFM | xsfm.co.kr
xyzfm.space | XiaoYuzhou | xiaoyuzhoufm.com
xn--helait-5ya.ee | Helaait | helaait.ee
zavtracast.ru | Zavtracast | zavtracast.ru
zdf.de | ZDF | zdf.de
zencast.fm | ZenCast | zencast.fm
zencastr.com | Zencastr | zencastr.com
`;

const HOSTING_PLATFORM_RULES = mergeHostingPlatformRules(
  HOSTING_PLATFORM_RULES_BASE,
  parseHostingPlatformRulesRaw(HOSTING_PLATFORM_RULES_RAW)
);

function getApproxRegistrableDomain(hostname) {
  ...
  const knownMulti = new Set(['co.uk', 'com.au', 'co.jp', 'com.br', 'com.mx', 'com.tr', 'com.cn', 'co.nz', 'com.tw', 'com.hk']);
  if (knownMulti.has(suffix2) && parts.length >= 3) return parts.slice(-3).join('.');
  if (knownMulti.has(parts.slice(-3).join('.')) && parts.length >= 4) return parts.slice(-4).join('.');
  return suffix2;
}

function titleizeDomainLabel(domain) {
  const base = String(domain || '').trim().replace(/\.[a-z0-9.-]+$/i, '');
  if (!base) return '';
  return base
    .split(/[-_]+/)
    .filter(Boolean)
...
function detectHostingPlatform(feedUrl) {
  ...
  const matched = HOSTING_PLATFORM_RULES.find((rule) => rule.hosts.some((candidate) => host === candidate || host.endsWith(`.${candidate}`)));
  if (matched) {
    return {
      key: matched.name.toLowerCase(),
      name: matched.name,
      url: matched.url,
      host,
      domain: getApproxRegistrableDomain(host)
    };
  }
  const domain = getApproxRegistrableDomain(host);
  return {
    key: domain,
    name: titleizeDomainLabel(domain) || host,
...
function updateRowFavoriteEmbedDataset(row, item) {
  ...
  if (String(item.url || '').trim()) {
    favBtn.setAttribute('data-favorite-url', String(item.url || '').trim());
  }
  if (String(item.feedUrl || '').trim()) {
...
function ensureRowPlayButtonPatched(row, item) {
  ...
  const openLink = row.querySelector('.open-link');
  if (openLink && String(item.url || '').trim()) {
    openLink.setAttribute('href', String(item.url || '').trim());
  }
...
function ensureRowDescriptionPatched(row, item) {
  if (!row || !item) return false;
  const description = String(item.description || '').trim();
  if (!description) return false;

  let changed = false;
  if (!String(row.getAttribute('data-description') || '').trim()) {
    row.setAttribute('data-description', description);
    changed = true;
  }

  const slot = row.querySelector('.episode-player-wrap');
  if (slot && !slot.hasAttribute('hidden') && !slot.querySelector('.episode-player-description')) {
    const descriptionMarkup = buildEpisodePlayerDescriptionMarkup(description);
    if (descriptionMarkup) {
      const adNode = slot.querySelector('.episode-player-ad');
      if (adNode) adNode.insertAdjacentHTML('beforebegin', descriptionMarkup);
      else slot.insertAdjacentHTML('beforeend', descriptionMarkup);
...
function patchVisibleRowsForItunesBackfillItem(item) {
  ...
  let patched = 0;
  rows.forEach((row) => {
    const playChanged = ensureRowPlayButtonPatched(row, item);
    const descriptionChanged = ensureRowDescriptionPatched(row, item);
    if (playChanged || descriptionChanged) patched += 1;
  });
  return patched;
}
...
async function backfillMissingApplePlaybackFromItunes(items, runMeta = null) {
  ...
  for (let start = 0; start < pending.length; start += ITUNES_SEARCH_BATCH_SIZE) {
    ...
    const batch = pending.slice(start, start + ITUNES_SEARCH_BATCH_SIZE).filter((item) => {
      return item && !hasPlayableAppleBackfill(item) && getItunesBackfillLookupTitle(item);
    });
