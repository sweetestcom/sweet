...
async function maybeTranslateEpisodePlayerDescription(wrap, sourceText) {
  ...
  if (wrap.dataset.translationState === 'loading' || wrap.dataset.translationState === 'done') return;
...
function refreshEpisodePlayerDescriptionWrap(wrap) {
  ...
  const plainText = String(preview.textContent || expanded.textContent || '').trim();
...
async function transliterateTextWithMicrosoft(text, forceRefresh = false) {
  const token = await getMicrosoftTranslateToken(forceRefresh);
  const response = await fetch(MICROSOFT_TRANSLITERATE_API_PREVIEW, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json; charset=UTF-8'
  ...
  if (response.status === 401 && !forceRefresh) {
    microsoftTranslateAuthState = null;
    return transliterateTextWithMicrosoft(text, true);
  }
...
async function transliterateText(text) {
  ...
  try {
    ...
    const result = await transliterateTextWithMicrosoft(source);
  ...
  for (let i = 0; i < pending.length; i += 5) {
    ...
    await Promise.all(batch.map(async (titleNode) => {
      ...
      if (translation && translation.displayText && !titleNode.querySelector('.translated-title')) {
        ...
        span.textContent = translation.displayText;
