...
function getMyPodcastsImportModeLabel(mode = myPodcastsImportMode) {
  const safeMode = normalizeMyPodcastsImportMode(mode);
  if (safeMode === 'youtube') return 'YouTube';
  if (safeMode === 'apple') return 'Apple';
  if (safeMode === 'spotify') return 'Spotify';
  if (safeMode === 'opml') return 'OPML';
  return 'RSS';
}

function getMyPodcastsImportActionLabel(mode = myPodcastsImportMode) {
  const safeMode = normalizeMyPodcastsImportMode(mode);
  if (safeMode === 'youtube') return 'Add YouTube';
  if (safeMode === 'apple') return 'Add Apple Podcasts';
  if (safeMode === 'spotify') return 'Add Spotify';
  if (safeMode === 'opml') return 'Import OPML';
  return 'Add RSS';
}
...
function buildMyPodcastsImportModeButton(mode, active = false) {
  const safeMode = normalizeMyPodcastsImportMode(mode);
  return `<button class="my-podcasts-mode${active ? ' is-active' : ''}" type="button" data-my-podcasts-import-mode="${safeMode}" aria-pressed="${active ? 'true' : 'false'}"><span class="my-podcasts-mode-icon" aria-hidden="true">${getMyPodcastsImportModeIconSvg(safeMode)}</span><span class="my-podcasts-mode-label">${escapeHtml(getMyPodcastsImportModeLabel(safeMode))}</span></button>`;
}

function getMyPodcastsImportPlaceholder(mode = myPodcastsImportMode) {
  const safeMode = normalizeMyPodcastsImportMode(mode);
  if (safeMode === 'youtube') return 'YouTube URL';
  if (safeMode === 'apple') return 'Apple Podcasts URL';
  if (safeMode === 'spotify') return 'Spotify URL';
  return 'RSS URL';
}
...
function parseYouTubeImportCandidate(rawUrl) {
  ...
  return {
    ...
    channelId,
    channelLabel,
    fallbackTitle: channelLabel || 'YouTube Channel',
    fallbackPublisher: channelLabel || 'YouTube'
  };
}
...
async function resolveYouTubeImportMeta(candidate) {
  ...
  const runtimeMeta = (!oembedMeta || !oembedMeta.title || !oembedMeta.publisher || !oembedMeta.image)
    ...
    : null;
  const publisher = String((oembedMeta && oembedMeta.publisher) || (runtimeMeta && runtimeMeta.publisher) || candidate.fallbackPublisher || 'YouTube').trim();
  let title = String((oembedMeta && oembedMeta.title) || (runtimeMeta && runtimeMeta.title) || candidate.fallbackTitle || publisher || 'YouTube').trim();
  ...
  if (candidate && candidate.kind === 'channel') {
    title = publisher || title || 'YouTube Channel';
  }
...
async function addFavoriteFromYouTubeUrl(rawUrl, options = {}) {
  ...
  if (!candidate) throw new Error('Invalid YouTube URL');
  if (candidate.kind === 'unsupported') {
    throw new Error('Unsupported YouTube channel URL. Use a single video, a playlist URL, or a /channel/UC... URL.');
  }
  ...
  if (!record.title || !record.embedUrl) throw new Error('YouTube import metadata missing');
...
async function addFavoriteFromPlatformUrl(platform, rawUrl, options = {}) {
  ...
  if (!candidate) throw new Error(`Invalid ${platform === 'apple' ? 'Apple Podcasts' : 'Spotify'} URL`);
  ...
  if (!record.title || !record.embedUrl) {
    throw new Error(`${brandLabel} import metadata missing`);
  }
...
async function handleMyPodcastsAddYouTube() {
  ...
  try {
    ...
    setMyPodcastsProgress('!');
    showToast(String(error && error.message || '').trim() || t('loadFailedRefresh'));
  } finally {
    setMyPodcastsBusyState(false);
    setTimeout(() => setMyPodcastsProgress(''), 1600);
  }
    ...
    node.innerHTML = `
      <div class="my-podcasts-library-head">
        <div class="my-podcasts-toolbar">
          ...
          <div class="my-podcasts-import-modes" id="my-podcasts-import-modes" role="group" aria-label="Import type">
        ...
        <div class="my-podcasts-import-row">
          <div class="my-podcasts-input-shell">
            <input id="my-rss-input" class="my-podcasts-input" type="url" inputmode="url" placeholder="RSS URL" autocomplete="off" spellcheck="false">
            <button id="my-opml-import" class="my-podcasts-opml-pick" type="button" hidden>Select OPML file</button>
          </div>
          <button id="my-rss-add" class="my-podcasts-action-btn" type="button" aria-label="Add RSS" title="Add RSS">${getMyPodcastsImportActionSvg('rss')}</button>
...
async function translateText(text) {
  ...
  const preferMicrosoft = shouldPreferMicrosoftTranslation(text);
  const googleTarget = getTranslateTarget('google');
  const microsoftTarget = getTranslateTarget('microsoft');
  const services = preferMicrosoft
    ? [
      () => translateTextWithMicrosoft(text, microsoftTarget),
      () => translateTextWithGoogle(text, googleTarget)
    ]
    : [
      () => translateTextWithGoogle(text, googleTarget),
      () => translateTextWithMicrosoft(text, microsoftTarget)
  ...
  for (let i = 0; i < services.length; i += 1) {
    try {
      ...
      console.warn(`Translation ${preferMicrosoft ? ['microsoft', 'google'][i] : ['google', 'microsoft'][i]} failed:`, error);
    }
  }
...
function getMyPodcastsTabLabel(tab) {
  ...
  if (safe === 'episodes') {
    const localized = String(t('topEpisodes') || '').trim();
    return /^top episodes$/i.test(localized) ? 'Episodes' : (localized || 'Episodes');
  }
  if (safe === 'notes') return isEnglishUiPage() ? 'Notes' : noteText('note');
