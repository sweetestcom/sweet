...
let japanBackfillCache = { updatedAt: 0, map: null };
...
function getTypeDefinitionsForPlatform(platform) {
  ...
  if (p === 'castbox' || p === 'pocketcasts' || p === 'audible' || p === 'iheartradio' || p === 'amazonmusic' || p === 'radiko') {
...
function buildFavoritePreferredKeyFromRecord(record) {
  ...
  // Unify cross-platform favorite identity by podcast title first.
...
function expandFavoriteCandidateKeys(record) {
  ...
  // Legacy key compatibility for old cached data.
  const explicit = String(row.favoriteKey || '').trim();
...
function triggerFavoriteAnimation(btn) {
  ...
  setTimeout(() => btn.classList.remove('is-animating'), 420);
}
...
async function maybeNotifyFavoriteRankChanges(items) {
  ...
  const maxNotify = Math.min(3, changes.length);
  for (let i = 0; i < maxNotify; i += 1) {
    const change = changes[i];
    const oldText = Number.isFinite(change.oldRank) ? `#${change.oldRank}` : '#-';
    const newText = Number.isFinite(change.newRank) ? `#${change.newRank}` : '#-';
    const title = String(t('rankUpdateTitle') || 'Rank update').trim() || 'Rank update';
    const body = `${change.title}
${oldText} → ${newText}`;
    await showBrowserRankNotification(title, body, `fav-rank-${contextKey}-${change.key}`);
  }
}
...
async function toggleFavoriteFromButton(button) {
  ...
  if (existingKey && favoritePodcastMap.has(existingKey)) {
    ...
    saveFavoriteStorageMap();
    showToast(getFavoriteRemovedMessage());
  } else {
    ...
    showToast(getFavoriteAddedMessage());
    await ensureNotificationPermissionFromGesture();
  }

  if (isFavoritesScopeMode()) {
    renderNormalizedItems(getFavoriteItemsForRender(), { rankTrendMap: null, historySeriesMap: null });
  }
...
function buildScopeSelectOptionLabel(value) {
  ...
  if (v === MY_PODCASTS_SCOPE_VALUE) {
    return buildMyPodcastsScopeLabel();
  }
  if (v.startsWith('lang-')) { const cfg = LANGUAGE_SCOPE_CONFIGS[v]; if (cfg && cfg.label) return cfg.label; return buildLanguageScopeLabel(v.slice(5)); }
  const explicit = String(t('languageChart') || '').trim();
  if (explicit) return explicit;
  const label = String(t('periodLabel') || '').trim();
  return label || UI_TEXT_DEFAULT.languageChart || '';
...
function supportsWorkerCountryBatch(platform, type) {
  ...
  if (p === 'youtube') return false; // YouTube is IP-localized and not country-param based.
  if (p === 'iheartradio' || p === 'iheart') return false; // Country-fixed source.
...
async function saveDailySnapshot(platform, country, type, items, fetchedAt = Date.now()) {
  ...
  let snapshotDate = getStorageDateKey(fetchedAt);
...
function aggregateSnapshotsToItems(snapshots) {
  ...
  rows.forEach((snapshot) => {
    ...
    // Repair historical double-counting caused by old local-date bucketing around midnight.
...
function shouldApplyJapanBackfill(platform, country, type) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  const safeCountry = normalizeCountry(country);
  const safeType = String(type || '').trim().toLowerCase();
  return safeCountry === 'jp'
    && safeType === 'top-podcasts'
    && (safePlatform === 'amazonmusic' || safePlatform === 'radiko');
}

async function buildJapanBackfillMapFromSnapshots(type = 'top-podcasts') {
  const safeType = normalizeTopType(type);
  if (safeType !== 'top-podcasts') return new Map();

  const now = Date.now();
  if (japanBackfillCache.map instanceof Map && (now - Number(japanBackfillCache.updatedAt || 0)) < JAPAN_BACKFILL_CACHE_TTL_MS) {
    return japanBackfillCache.map;
  }

  const byTitle = new Map();

  for (const platform of JAPAN_BACKFILL_PLATFORMS) {
    const comboKey = comboKeyFor(platform, 'jp', safeType);
    let rows = [];
    try {
      ...
      rows = snapshotRowsCache.get(comboKey) || [];
      if (!Array.isArray(rows)) rows = [];
      console.warn('Japan backfill snapshot read failed:', platform, error);
    }
    ...
    for (const row of sortedRows) {
      ...
      for (const item of list) {
        ...
        const nextEmbedPlatform = String(item && item.embedPlatform ? item.embedPlatform : platform).trim().toLowerCase();
...
async function enrichJapaneseItemsFromLocalCache(items, platform, country, type) {
  const list = Array.isArray(items) ? items : [];
  if (!list.length) return list;
  if (!shouldApplyJapanBackfill(platform, country, type)) return list;

  const backfillMap = await buildJapanBackfillMapFromSnapshots(type);
...
function buildHistorySeriesMapFromSnapshots(snapshots, currentItems = []) {
  const rows = Array.isArray(snapshots) ? snapshots : [];
...
function buildEpisodePlayerIframeMarkup(embedUrl, platform, title, kind, options = {}) {
  ...
  if (p === 'youtube') {
    return `<iframe class="episode-player-frame youtube ${safeKind === 'top-podcasts' ? 'is-show' : 'is-episode'}" src="${safeUrl}" title="${safeTitle}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen loading="lazy"></iframe>`;
  }
  return '';
}
...
async function resolveSpotifyEpisodeEmbedMeta(url) {
  ...
  const cacheKey = episodeId.toLowerCase();
  const now = Date.now();
  ...
  const requestUrl = `https://open.spotify.com/oembed?url=${encodeURIComponent(openUrl)}`;
...
async function resolveSpotifyShowEmbedMeta(url) {
  ...
  const cacheKey = showId.toLowerCase();
  const now = Date.now();
...
async function resolveAppleEpisodeEmbedMeta(url) {
  ...
  const cacheKey = `${ids.showId}:${ids.episodeId}`;
  const now = Date.now();
  ...
  const task = (async () => {
    ...
    if (!target || typeof target !== 'object') {
      const data = { showId: ids.showId, episodeId: ids.episodeId, isVideo: false, kind: '' };
...
async function renderCastboxRssPlayerInSlot(slot, embedPlatform, rssUrl, fallbackTitle, forceRefresh = true) {
  ...
  slot.innerHTML = `<div class="episode-player-shell platform-${platform}"><div class="castbox-rss-loading">${escapeHtml(t('loading'))}…</div></div>`;
  ...
  try {
    ...
    slot.innerHTML = `<div class="episode-player-shell platform-${platform}">${buildCastboxRssPlayerMarkup(feed, safeFallbackTitle, safeRssUrl)}</div>`;
    ...
    slot.innerHTML = `<div class="episode-player-shell platform-${platform}"><div class="castbox-rss-error-wrap"><div class="castbox-rss-error">${escapeHtml(t('loadFailedRefresh'))}</div><button class="castbox-rss-refresh" type="button" data-castbox-retry>${escapeHtml(t('refresh'))}</button></div></div>`;
...
async function openEpisodePlayerRow(row, btn) {
  ...
  const embedKind = String(btn.getAttribute('data-embed-kind') || (typeSelect ? typeSelect.value : '') || '').trim().toLowerCase();
  ...
  slot.innerHTML = `<div class="episode-player-shell platform-${embedPlatform}">${iframeHtml}</div>`;
  slot.removeAttribute('hidden');
  if (embedPlatform === 'apple' && embedKind === 'top-episodes') {
...
function buildHistoryPanelMarkup(series, platform, titleText = '') {
  ...
  // Keep global 1-100 context when needed, but zoom in a bit for small fluctuations so the line is readable.
  let scaleMin = 1;
  ...
  const pointLabels = points.map((pt, index) => {
    ...
    // Keep labels away from point markers at all times.
    if (preferAbove) {
...
function setMyPodcastsBusyState(busy) {
  myPodcastsBusy = !!busy;
  if (myPodcastsRssInputEl) myPodcastsRssInputEl.disabled = myPodcastsBusy;
  if (myPodcastsAddBtnEl) myPodcastsAddBtnEl.disabled = myPodcastsBusy;
...
function parseOpmlFeedCandidates(opmlText) {
  ...
  if (!xml || xml.querySelector('parsererror')) throw new Error('Invalid OPML XML');
  ...
  await mapWithConcurrency(queue, 4, async (candidate) => {
    try {
      ...
      if (out && out.result && out.result.ok) {
        ...
        // Persist each successful import immediately.
...
function ensureMyPodcastsControls() {
  ...
  if (!node) {
    ...
    node.innerHTML = `
      <div class="my-podcasts-input-row">
        <input id="my-rss-input" class="my-podcasts-input" type="url" inputmode="url" placeholder="https://.../feed.xml" autocomplete="off" spellcheck="false">
        <button id="my-rss-add" class="my-podcasts-btn" type="button">+ RSS</button>
        <button id="my-opml-import" class="my-podcasts-btn" type="button">OPML</button>
        <button id="my-opml-retry" class="my-podcasts-btn" type="button" hidden>${t('retry')}</button>
    ...
    // Keep this tools panel as an independent section right below the search bar area.
  ...
  if (myPodcastsAddBtnEl && !myPodcastsAddBtnEl.dataset.boundClick) {
    myPodcastsAddBtnEl.addEventListener('click', () => {
      handleMyPodcastsAddRss().catch((error) => {
        console.warn('Add RSS handler failed:', error);
      });
    });
  ...
  if (myPodcastsRssInputEl && !myPodcastsRssInputEl.dataset.boundEnter) {
    myPodcastsRssInputEl.addEventListener('keydown', (event) => {
      ...
      event.preventDefault();
      handleMyPodcastsAddRss().catch((error) => {
        console.warn('Add RSS enter handler failed:', error);
      });
    });
...
function ensureRuntimeEffectsStyles() {
  ...
  style.textContent = `
.select-container.chart-loading-ring { position: relative; border-radius: 22px; }
...
.chart-row.surprise-hover { transform: translateY(-3px) scale(1.008); box-shadow: 0 10px 24px rgba(0,0,0,.28); }
.chart-row.surprise-flicker {
  animation: surprisePulse 240ms ease;
  box-shadow: 0 0 0 1px rgba(255,255,255,.18), 0 0 0 3px rgba(118,255,215,.18), 0 0 18px rgba(118,255,215,.28);
}
.chart-row.surprise-final {
  box-shadow: 0 0 0 1px rgba(255,255,255,.26), 0 0 0 4px rgba(118,255,215,.28), 0 0 22px rgba(118,255,215,.36), inset 0 0 0 1px rgba(255,255,255,.06);
  animation: surpriseFinalPulse .7s ease;
}
.chart-row.surprise-final .title {
  background: linear-gradient(90deg, rgba(255,255,255,.96), rgba(126,255,214,.98));
...
@media (max-width: 760px) {
  ...
  .chart-row.surprise-final {
    box-shadow: 0 0 0 1px rgba(255,255,255,.18), 0 0 0 3px rgba(118,255,215,.18), 0 0 16px rgba(118,255,215,.28);
  }
}
...
function applyRowAccentTheme() {
  ...
  const first = String((palette && palette[0]) || '#6f92ff');
  const second = String((palette && palette[1]) || first);
  ...
  const steps = palette.map((color, index) => {
...
function layoutLoadRingOverlay(host, refs) {
  ...
  const length = typeof refs.progress.getTotalLength === 'function' ? refs.progress.getTotalLength() : (2 * (rw + rh));
...
function buildCastboxOptionLabel() {
  return buildBrandedPodcastsLabel('Castbox', 'Castbox Podcasts');
}

function buildPocketcastsOptionLabel() {
  return buildBrandedPodcastsLabel('Pocket Casts', 'Pocket Casts Podcasts');
}

function buildAudibleOptionLabel() {
  return buildBrandedPodcastsLabel('Audible', 'Audible Podcasts');
}
...
function buildAmazonmusicOptionLabel() {
  return buildBrandedPodcastsLabel('Amazon Music', 'Amazon Music Podcasts');
}
...
async function getCountryPlatformItemsFromLocalCache(platform, type, country, options = {}) {
  ...
  const safeCountry = String(country || 'us').trim().toLowerCase() || 'us';
  const forceRefresh = Boolean(options.forceRefresh);
  const todayKey = getStorageDateKey();
  ...
  const cacheFresh = Boolean(cached && cachedTime && (Date.now() - cachedTime < getCacheTtlMsForType(safeType)));
...
async function hasFreshResponseCacheForCombo(platform, country, type) {
  ...
  return (Date.now() - updatedAt) < getCacheTtlMsForType(normalizeTypeForPlatform(platform, type));
}
...
async function finalizeSuccessfulForegroundRender() {
  try {
      ...
      (Array.isArray(batch.results) ? batch.results : []).forEach((row) => {
        ...
        try {
          ...
          if (items.length) {
            ...
            saveDailySnapshot(selectedPlatform, country, safeType, items).catch(() => {});
...
async function getSnapshotRecordForComboDate(platform, country, type, dateKey = getStorageDateKey()) {
  const comboKey = comboKeyFor(platform, country, type);
  const id = `${comboKey}::${String(dateKey || '').trim()}`;
  if (!id.endsWith('::')) {
    try {
  ...
  try {
    ...
    if (Array.isArray(rows)) snapshotRowsCache.set(comboKey, rows);
    return (Array.isArray(rows) ? rows : []).find((row) => String((row && row.snapshotDate) || '') === String(dateKey || '')) || null;
  } catch (error) {
    console.warn('Snapshot combo read failed:', error);
    const cachedRows = snapshotRowsCache.get(comboKey) || [];
    return cachedRows.find((row) => String((row && row.snapshotDate) || '') === String(dateKey || '')) || null;
  }
}
...
async function fetchAllPlatformsCompositeData(type, country, options = {}) {
  ...
  const storageDateKey = getStorageDateKey();
  const fetchTargets = [];
  ...
  for (const platform of platforms) {
    ...
    if (!forceRefresh) {
      const snapshot = await getSnapshotRecordForComboDate(platform, safeCountry, safeType, storageDateKey);
      if (snapshot && Array.isArray(snapshot.items) && snapshot.items.length) {
...
function getTranslateTarget() {
  ...
  return map[pageLang] || document.documentElement.lang || pageLang || 'en';
}

function shouldSkipTranslate(detectedLang) {
  if (!detectedLang) return false;
  const target = getTranslateTarget().toLowerCase();
  const detected = detectedLang.toLowerCase();
  if (target === detected) return true;
  if (target.startsWith('es') && detected.startsWith('es')) return true;
  if (target.startsWith('pt') && detected.startsWith('pt')) return true;
  if (target.startsWith('zh') && detected.startsWith('zh')) return true;
  return false;
}
...
function inferPodcastsWordFromTopLabel(label, pageLang = '') {
  ...
  // Generic fallback: derive the noun from localized top label to avoid mixed-language UI words.
...
async function translateText(text) {
  const targetLang = getTranslateTarget();
  ...
  try {
    ...
    const result = {
      translatedText: data[0] && data[0][0] ? data[0][0][0] : text,
      detectedLang: data[2]
    };
...
async function detectTextLanguage(text, preferredPrefix = '') {
  ...
  // For Chinese scope, trust script-based Han detection first to avoid JA false positives.
  if (targetPrefix === 'zh') {
...
async function processTranslations() {
  ...
  for (let i = 0; i < pending.length; i += 5) {
    ...
    await Promise.all(batch.map(async (titleNode) => {
      ...
      if (!res || shouldSkipTranslate(res.detectedLang) || res.translatedText === original) return;
      if (titleNode.querySelector('.translated-title')) return;
      const span = document.createElement('span');
      span.className = 'translated-title';
      span.textContent = res.translatedText;
...
async function localizeUiBundle() {
  const pageLang = pageToLanguage(getCurrentFileName());
  Object.assign(uiText, UI_TEXT_DEFAULT, UI_TEXT_OVERRIDES[pageLang] || {});
...
function buildWallMarkup(covers, className, startOffset = 0, maxCount) {
  ...
  const limit = Math.max(0, Math.min(Number.isFinite(maxCount) ? maxCount : covers.length, covers.length));
  if (!limit) return '';

  let html = '';
  for (let i = 0; i < limit; i += 1) {
    const image = escapeHtml(covers[(startOffset + i) % covers.length]);
    html += `<div class="wall-tile ${className}" style="--i:${i}"><img src="${image}" alt="" loading="lazy" decoding="async" /></div>`;
  }
  return html;
}
...
function renderHeroWall(items) {
  ...
  const visibleCount = Math.min(metrics.count, covers.length);
...
function getApiCandidates(platform, type, country) {
  ...
  if (safePlatform === 'apple' || safePlatform === 'spotify' || safePlatform === 'castbox' || safePlatform === 'pocketcasts' || safePlatform === 'podbbang' || safePlatform === 'audioclip') {
    const params = new URLSearchParams({
      platform: safePlatform,
      type: safeType,
      country: safeCountry
...
async function fetchDirectApplePayloadViaAllOrigins(country, type, signal) {
  ...
  for (const sourceUrl of urls) {
    try {
      if (normalizeTypeForPlatform('apple', type) === 'top-episodes') {
  ...
  if (platform === 'apple') {
    ...
    return arr.map((item, index) => {
      ...
      return {
        rank: index + 1,
        title: item.name || '',
        publisher: item.artistName || item.publisher || 'Apple Podcasts',
...
function renderItems(items) {
  ...
  chartContainer.innerHTML = items.map((item) => {
    ...
    const image = escapeHtml(item.image || 'logo.webp');
    const rawUrl = String(item.url || '').trim();
    const hasOpenUrl = /^https?:\/\//i.test(rawUrl);
    const url = hasOpenUrl ? escapeHtml(rawUrl) : '';
    const search = escapeHtml(`${item.title} ${item.publisher}`.toLowerCase());
    ...
    const favoriteRecord = normalizeFavoriteRecord({
      ...
      embedUrl: item.embedUrl,
      embedPlatform: item.embedPlatform,
      embedKind: item.embedKind || (typeSelect ? typeSelect.value : 'top-podcasts'),
      sourceType: item.sourceType || (favoritesScope ? 'rss' : 'chart'),
      originPlatform: item.originPlatform || (favoritesScope ? '' : String((platformSelect && platformSelect.value) || '').trim().toLowerCase()),
      originCountry: item.originCountry || (favoritesScope ? '' : String((countrySelect && countrySelect.value) || '').trim().toLowerCase()),
      originType: item.originType || (favoritesScope ? '' : String((typeSelect && typeSelect.value) || '').trim().toLowerCase())
    });
    ...
    const favoriteActive = favoritesScope ? true : Boolean(existingFavoriteKey);
    ...
    const historySeries = favoritesScope ? null : getHistorySeriesForItem(item);
    ...
    return `
      <article class="chart-row${favoritesScope ? ' favorites-row' : ''}" data-search="${search}">
        ${rankPart}
        <img class="cover" src="${image}" alt="${title}" loading="lazy" />
        <div class="meta">
  ...
  chartContainer.querySelectorAll('.favorite-link').forEach((btn) => {
...
function visibleRows() {
  return Array.from(document.querySelectorAll('.chart-row:not(.chart-ad):not(.hidden)'));
}
...
function insertAds() {
  ...
  for (let i = 19; i < rows.length; i += 20) {
    ...
    ins.style.width = '100%';
    ins.style.minHeight = window.innerWidth <= 760 ? '92px' : '104px';
...
function responseCacheKeyFor(platform, country, type) {
  return `podcast_data_${platform}_${country}_${type}`;
}
...
async function renderAggregateFromHistory() {
  ...
  items = await enrichJapaneseItemsFromLocalCache(items, platformSelect.value, countrySelect.value, typeSelect.value);
...
async function fetchPodcastsData(options = {}) {
  ...
  if (favoritesScopeActive) {
    ...
    const favoriteItems = getFavoriteItemsForRender();
    renderNormalizedItems(favoriteItems, { rankTrendMap: null, historySeriesMap: null });
    setStatus(favoriteItems.length ? 'cache' : 'noData');
    return;
  }
  ...
  const cached = cachedEntry && Object.prototype.hasOwnProperty.call(cachedEntry, 'payload') ? cachedEntry.payload : null;
  const cachedTime = Number((cachedEntry && cachedEntry.updatedAt) || 0);
  const hasFresh = cached && cachedTime && (Date.now() - cachedTime < getCacheTtlMs());
  ...
  if (cached) {
    try {
      if (period === 'daily' && !selectedHistoryDateKey) {
        ...
        cachedItems = await enrichJapaneseItemsFromLocalCache(cachedItems, platformSelect.value, countrySelect.value, typeSelect.value);
        const snapshots = await getSnapshotsForCurrentSelection();
        await refreshHistoryDateOptions(snapshots);
  ...
  if (period === 'daily' && !selectedHistoryDateKey && hasFresh && !forceRefresh && renderedCache) {
  ...
  try {
    ...
    if (languageScopeActive) {
      ...
      data = await fetchAllPlatformsCompositeData(typeSelect.value, countrySelect.value, {
        onProgress: (progressState) => {
          if (requestId !== activeRequestId) return;
          // Keep all-platform animation visually consistent with single-platform mode:
          // one full lap, complete flash, then breathe until data is ready.
          // We still receive progress here for fetch orchestration, but we do not
          // drive the ring length with it because cache/planning stages cause
          // jumpy partial fills that break the intended "start from one point" effect.
          if (progressState && progressState.stage === 'retry') {
            setChartLoadVisual(true, { platform: 'all', colors: LANGUAGE_SCOPE_RING_GRADIENT });
          }
        }
      ...
      const candidates = getApiCandidates(platformSelect.value, typeSelect.value, countrySelect.value);
      if (!candidates.length) {
        setStatus('noSource');
        setLoading(false);
        setSkeleton(false);
    ...
    normalizedItems = await enrichJapaneseItemsFromLocalCache(normalizedItems, platformSelect.value, countrySelect.value, typeSelect.value);
...
function initLanguageSwitcher() {
  ...
  if (languageSelectEl.querySelector(`option[value="${currentLang}"]`)) {
...
function bootPage() {
  ...
  try {
    ...
    if (themeColorEl) themeColorEl.setAttribute('content', '#080d1a');
  } catch (error) {
