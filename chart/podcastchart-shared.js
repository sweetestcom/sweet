...
function platformHasDirectVideoBadge(platform) {
  const safePlatform = String(platform || '').trim().toLowerCase();
  return safePlatform === 'youtube' || safePlatform === 'bilibili';
}
...
const RSS_PLAYBACK_RATE_OPTIONS = Array.from({ length: 15 }, (_, index) => 0.5 + (index * 0.25));
...
function buildCastboxSpeedMenuMarkup(activeRate = 1) {
  const currentRate = Number(activeRate) || 1;
  const options = RSS_PLAYBACK_RATE_OPTIONS.map((rate) => {
    const label = formatRssPlaybackRateLabel(rate);
    const active = Math.abs(rate - currentRate) < 0.001;
    return `<button class="castbox-rss-speed-option${active ? ' is-active' : ''}" type="button" data-castbox-rate="${label}" data-castbox-rate-value="${rate}" aria-pressed="${active ? 'true' : 'false'}">${label}</button>`;
  }).join('');
  return `
    <details class="castbox-rss-speed-menu">
      <summary class="castbox-rss-speed" data-castbox-speed-trigger aria-label="Playback speed" title="Playback speed">${escapeHtml(formatRssPlaybackRateLabel(currentRate))}</summary>
      <div class="castbox-rss-speed-pop">${options}</div>
    </details>
  `;
}
...
function bindCastboxRssPlayerInteractions(slot, options = {}) {
  ...
  const speedMenu = root.querySelector('.castbox-rss-speed-menu');
  ...
  function syncSpeedButtonState() {
    ...
    root.querySelectorAll('[data-castbox-rate-value]').forEach((btn) => {
      const buttonRate = Number(btn.getAttribute('data-castbox-rate-value') || 1);
      const active = Math.abs(buttonRate - rate) < 0.001;
  ...
  if (speedMenu) {
    speedMenu.addEventListener('click', (event) => {
      const option = event.target && event.target.closest ? event.target.closest('[data-castbox-rate-value]') : null;
      if (!option) return;
      event.preventDefault();
      const nextRate = Number(option.getAttribute('data-castbox-rate-value') || 1);
