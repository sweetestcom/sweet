...
function shouldApplyDataBackfill(platform, type) {
  ...
  if (safePlatform !== 'rtlplus' && safeType !== 'top-podcasts' && !isEpisodeChartTypeValue(safeType)) {
    return false;
  }
...
function getPlatformAccentPair(platform) {
  return PLATFORM_ACCENT_COLORS[String(platform || '').trim().toLowerCase()] || PLATFORM_ACCENT_COLORS.apple || ['#6F8BFF', '#00C5B0'];
}
...
function normalizeItems(data, platform, type) {
  ...
  if (safePlatform === 'xiaoyuzhou') {
    ...
    return arr.map((item, index) => ({
      rank: Number(item.rank) || (index + 1),
      title: item.name || item.title || '',
      publisher: item.podcastTitle || item.publisher || item.author || platformBrandLabel('xiaoyuzhou'),
      image: String(item.image || '').trim(),
      ...
      podcastTitle: String(item.podcastTitle || item.publisher || '').trim(),
