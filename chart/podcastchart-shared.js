...
function findLocalizedSharedTypeLabel(def) {
  ...
  const pools = [
    Array.isArray(APPLE_TYPE_DEFINITIONS) ? APPLE_TYPE_DEFINITIONS : [],
...
function getAppleMainPrefetchTypes() {
  return (Array.isArray(APPLE_TYPE_DEFINITIONS) ? APPLE_TYPE_DEFINITIONS : [])
    .filter((def) => isAppleTypeValue(def && def.value) && !isAppleDerivedType(def && def.value) && !Number(def && def.indent))
  ...
  const results = Array.from(byCategory.values())
    .map((row) => {
      ...
      return {
        ...
        url: buildChartUrlForSelection('apple', `apple-${row.id}`, safeCountry),
        count: stats.count,
...
function getAppleCategoryDefinitionById(categoryId) {
  ...
  return (Array.isArray(APPLE_TYPE_DEFINITIONS) ? APPLE_TYPE_DEFINITIONS : []).find((def) => String(def && def.value || '').trim().toLowerCase() === target.toLowerCase()) || null;
}
