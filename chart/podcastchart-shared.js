...
function buildNoteContext(values = {}) {
  ...
  return { id, title, publisher, link, platform, kind };
}
...
function buildNotePanelMarkup(context, record = null) {
  ...
  const content = escapeHtml(String(record && record.content || ''));
...
const playlistController = {
  ...
  _buildOverlay() {
    ...
    el.innerHTML = `
      ...
      <button class="plm-nav plm-nav-prev" type="button" aria-label="${this._escapeAttr(tOr('previous', 'Previous'))}">▲</button>
      <button class="plm-nav plm-nav-next" type="button" aria-label="${this._escapeAttr(tOr('next', 'Next'))}">▼</button>
...
const playlistNoteController = {
  ...
  _buildMarkup(context, record = null) {
    ...
    const content = escapeHtml(String(record && record.content || ''));
    const publisher = escapeHtml(String(context.publisher || '').trim());
    const link = String(context.link || '').trim();
    return `
