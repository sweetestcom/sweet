...
function getEligiblePlatformsForAll(type = (typeSelect ? typeSelect.value : 'top-podcasts'), country = (countrySelect ? countrySelect.value : 'us')) {
  ...
  return options
    ...
    .filter((value) => value !== 'ardsounds' && value !== 'radiofrance' && value !== 'podimo')
  ...
  chartContainer.innerHTML = items.map((item) => {
    ...
    return `
      <article class="chart-row${favoritesScope ? ' favorites-row' : ''}" data-search="${search}">
        ${rankPart}
        <img class="cover" src="${image}" alt="${title}" loading="lazy" data-image-candidates="${imageCandidates}" data-image-fallback="logo.webp" />
        <div class="meta">
