...
const PLATFORM_ACCENT_COLORS = {
  ...
  netflix: ['#E50914', '#B20710', '#221F1F'],
  disneyplus: ['#0F53FF', '#1D7BFF', '#081C66'],
...
const NETFLIX_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: 'topPodcasts', defaultLabel: 'Top Podcasts' },
  { value: 'netflix-pop-culture', key: 'typeSocietyCulture', defaultLabel: 'Pop Culture' },
  { value: 'netflix-knowledge', key: 'typeEducation', defaultLabel: 'Knowledge' },
...
function setTitleVideoBadgeState(titleEl, shouldShow) {
  ...
  if (shouldShow) {
    if (!badge) {
      titleEl.insertAdjacentHTML('beforeend', buildVideoTitleBadgeMarkup());
    }
    return;
  }
...
function normalizeItems(data, platform, type) {
  ...
  if (platform === 'ardsounds' || platform === 'radiofrance' || platform === 'bbcsounds' || platform === 'siriusxm' || platform === 'pandora' || platform === 'podcastapp' || platform === 'rtlplus' || platform === 'tunein' || platform === 'abclisten' || platform === 'nrk' || platform === 'yleareena' || platform === 'sverigesradio' || platform === 'podplay' || platform === 'podimo' || platform === 'podcastaddict' || platform === 'radionet' || platform === 'ivoox' || platform === 'orfsound' || platform === 'raiplaysound' || platform === 'srfaudio' || platform === 'drlyd' || platform === 'rtbfauvio' || platform === 'rneaudio' || platform === 'npoluister' || platform === 'snipd' || platform === 'netflix' || platform === 'disneyplus' || platform === 'soundon' || platform === 'kkbox') {
    arr = Array.isArray(data && data.podcasts) ? data.podcasts : [];
...
function buildChartRowsMarkup(items, options = {}) {
  ...
  return items.map((item) => {
    ...
    const titleMarkup = `${title}${staticVideoBadge ? buildVideoTitleBadgeMarkup() : ''}`;
...
const playlistController = {
  ...
  _buildCardHtml(item, direction) {
    ...
    const titleMarkup = `${this._escapeHtml(item.title)}${(isVideo || platformHasDirectVideoBadge(item.embedPlatform)) ? buildVideoTitleBadgeMarkup() : ''}`;
