...
function getActiveEnglishTextMode() {
  ...
  try {
    ...
    return mode === ENGLISH_TEXT_MODE_GENZ ? ENGLISH_TEXT_MODE_GENZ : '';
  } catch {
    return '';
  }
}
...
function initLanguageSwitcher() {
  ...
  if (currentLang === 'en' && currentEnglishMode === ENGLISH_TEXT_MODE_GENZ && languageSelectEl.querySelector(`option[value="en-${ENGLISH_TEXT_MODE_GENZ}"]`)) {
    languageSelectEl.value = `en-${ENGLISH_TEXT_MODE_GENZ}`;
  ...
  const buildEnglishModeUrl = (mode = '') => {
    const params = new URLSearchParams(window.location.search);
    if (mode === ENGLISH_TEXT_MODE_GENZ) params.set(ENGLISH_TEXT_MODE_QUERY_KEY, ENGLISH_TEXT_MODE_GENZ);
    else params.delete(ENGLISH_TEXT_MODE_QUERY_KEY);
