...
function getEligiblePlatformsForAll(type = (typeSelect ? typeSelect.value : 'top-podcasts'), country = (countrySelect ? countrySelect.value : 'us')) {
  ...
  return options
    ...
    .filter((value) => value !== 'ximalaya' && value !== 'bilibili' && value !== 'ardsounds' && value !== 'radiofrance' && value !== 'podimo' && value !== 'podcastaddict')
...
function buildPodcastAddictOptionLabel() {
  return buildBrandedPodcastsLabel('Podcast Addict', 'Podcast Addict Podcasts');
}
...
function normalizeItems(data, platform, type) {
  ...
  if (platform === 'ardsounds' || platform === 'radiofrance' || platform === 'podimo' || platform === 'podcastaddict' || platform === 'radionet' || platform === 'ivoox') {
    arr = Array.isArray(data && data.podcasts) ? data.podcasts : [];
...
function buildPlayerAdMarkup() {
  return `
    <div class="episode-player-ad" aria-label="Sponsored">
...
function insertAds() {
  ...
  for (let i = 7; i < rows.length; i += 20) {
    ...
    ins.style.width = '100%';
    ins.style.minHeight = window.innerWidth <= 760 ? '138px' : '156px';
    ins.dataset.adClient = 'ca-pub-9206135835124064';
    ins.dataset.adSlot = '7495680049';
