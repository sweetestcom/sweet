...
function normalizeGlobalRssQueueItem(raw = {}) {
  ...
  const guid = String(raw.guid || '').trim() || src || videoId || playlistId;
  const title = String(raw.title || '').trim();
  if ((!src && !isYouTube) || !title || (isYouTube && !videoId && !playlistId)) return null;
  ...
  return {
    id: String(raw.id || `${isYouTube ? 'youtube' : (feedUrl || 'feed')}::${guid}`).trim(),
    src,
    guid,
    title,
    meta,
    ...
    feedUrl,
    platform: isYouTube ? 'youtube' : 'audio',
    videoId,
    playlistId,
...
const globalRssPlayerController = {
  ...
  emit() {
    ...
    document.dispatchEvent(new CustomEvent(GLOBAL_RSS_PLAYER_EVENT, {
      detail: {
        ...
        muted: !this.isYouTubeItem() && !!(this.audio && this.audio.muted)
      }
    }));
  },
  updateUi() {
    ...
    if (volumeWrap) {
      ...
      const muted = !this.isYouTubeItem() && (this.audio?.muted || safeVolume <= 0.001);
  ...
  persist(force = false) {
    ...
    const item = this.getCurrentItem();
    if (item && item.feedUrl && item.guid && !this.isYouTubeItem(item)) {
  ...
  findItemIndex(item) {
    ...
    return this.queue.findIndex((entry) => entry && (
      ...
      || (!this.isYouTubeItem(entry) && entry.src === normalized.src && entry.feedUrl === normalized.feedUrl)
...
function bindInlinePlayerModeControls(row) {
  ...
  document.addEventListener(GLOBAL_RSS_PLAYER_EVENT, () => {
    try { syncInlineYoutubeQueueButton(row); } catch {}
  });
}
