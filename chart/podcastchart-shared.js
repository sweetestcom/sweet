...
function buildCastboxSpeedControlMarkup(activeRate = 1) {
  const currentRate = Math.max(0.5, Math.min(4, Number(activeRate) || 1));
  return `
...
const globalRssPlayerController = {
  ...
  getMediaVolume() {
    ...
    return Number(this.audio?.volume) || 1;
  },
  ...
  bind() {
    ...
    if (speedBtn && speedSlider) {
      ...
      speedSlider.addEventListener('input', () => {
        const nextRate = Math.max(0.5, Math.min(4, Number(speedSlider.value) || 1));
  ...
  updateUi() {
    ...
    if (volumeWrap) {
      const safeVolume = Math.max(0, Math.min(1, this.getMediaVolume()));
    ...
    if (volumeSlider) volumeSlider.value = String(Math.round(this.getMediaVolume() * 100));
    if (speedBtn) speedBtn.textContent = formatRssPlaybackRateLabel(this.getMediaPlaybackRate());
    if (speedSlider) speedSlider.value = String(Math.max(0.5, Math.min(4, this.getMediaPlaybackRate())));
...
function bindCastboxRssPlayerInteractions(slot, options = {}) {
  ...
  function syncVolumeUi() {
    const safeVolume = Math.max(0, Math.min(1, Number(audio.volume) || 0));
  ...
  if (volumeSlider) {
    volumeSlider.addEventListener('input', () => {
      ...
      if (nextVolume > 0.001) root.__castboxLastVolume = nextVolume;
      syncVolumeUi();
    });
  }
