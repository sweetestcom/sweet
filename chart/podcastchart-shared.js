...
function getAppleFeedResolutionMetaForRow(row, button = null) {
  ...
  const showId = extractApplePodcastIdFromUrl(pageUrl) || String(episodeIds && episodeIds.showId || '').trim();
  return {
...
async function switchAppleRowPlayerMode(row, mode, trigger = null) {
  ...
  try {
    if (requestedMode === 'rss') {
      ...
      applyPlayerAccentVars(slot, 'apple');
      const modeHost = slot.querySelector('[data-episode-mode-host]');
      if (!modeHost) return false;
      await renderCastboxRssPlayerInSlot(modeHost, 'apple', resolved.feedUrl, titleText, true, { includeAd: false });
