...
function buildEpisodePlayerDescriptionMarkup(rawDescription) {
  ...
  return `
    <div class="episode-player-description">
      <div class="episode-player-description-preview">${prepared.html || escapeHtml(prepared.text)}</div>
      ...
      <div class="episode-player-description-expanded" hidden>${prepared.html || escapeHtml(prepared.text)}</div>
    </div>
  `;
}
...
function syncEpisodePlayerDescription(row) {
  ...
  if (!slot) return;
  slot.querySelectorAll('.episode-player-description-toggle').forEach((btn) => {
    const wrap = btn.closest('.episode-player-description');
    const preview = wrap ? wrap.querySelector('.episode-player-description-preview') : null;
    const expanded = wrap ? wrap.querySelector('.episode-player-description-expanded') : null;
    if (!wrap || !preview || !expanded) return;
...
async function translateTextWithMicrosoft(text, targetLang, forceRefresh = false) {
  ...
  if (previewResponse.ok) {
    ...
    return parseMicrosoftPreviewTranslationResponse(previewData, text);
  }
