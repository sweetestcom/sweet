...
function shouldApplyDataBackfill(platform, type) {
  ...
  if (safeType !== 'top-podcasts' && safeType !== 'top-episodes') {
    return false;
  }

  const backfillPlatforms = [
    'ximalaya', 'amazonmusic', 'audible', 'iheartradio', 'siriusxm',
    'tunein', 'podbbang', 'audioclip', 'ardsounds', 'radiofrance',
    'bbcsounds', 'deezer', 'abclisten', 'nrk', 'yleareena',
    'sverigesradio', 'radionet', 'podcastaddict', 'radiko',
    'ivoox', 'podimo'
  ];
...
async function initSponsorBlockForYouTubePlayer(row) {
  if (!row) return;
  const iframe = row.querySelector('iframe.episode-player-frame.youtube');
  if (!iframe || !iframe.id) return;

  const iframeSrc = iframe.getAttribute('src') || '';
  const videoId = extractVideoIdFromEmbedUrl(iframeSrc);
  if (!videoId) return;
  ...
  const shell = iframe.closest('.episode-player-shell');

  // Add badge showing SponsorBlock is active
  if (shell && segments.length > 0) {
    const existingBadge = shell.querySelector('.sponsorblock-badge');
    if (!existingBadge) {
      const badge = document.createElement('div');
      badge.className = 'sponsorblock-badge';
      badge.innerHTML = '<span class="sponsorblock-badge-icon"></span> SponsorBlock · ' + segments.length + ' segment' + (segments.length === 1 ? '' : 's');
    ...
    const ytPlayer = new window.YT.Player(iframe.id, {
      events: {
        onReady: () => {
          ...
          activeSponsorBlockMonitor = new SponsorBlockMonitor(ytPlayer, segments, shell);
        }
      }
    });
  } catch (err) {
...
function renderItems(items) {
  ...
  chartContainer.innerHTML = items.map((item) => {
    ...
    return `
      <article class="chart-row${favoritesScope ? ' favorites-row' : ''}${useVideoCover ? ' has-video-cover' : ''}" data-search="${search}">
        ${rankPart}
        <img class="cover${useVideoCover ? ' cover-video' : ''}" src="${image}" alt="${title}" loading="lazy"${imageReferrerPolicy} data-image-candidates="${imageCandidates}" data-image-fallback="podcast-placeholder.webp" />
        <div class="meta">
  ...
  chartContainer.querySelectorAll('.chart-row').forEach((row, i) => {
...
const playlistController = {
  ...
  _buildOverlay() {
    ...
    el.innerHTML = `
      <div class="plm-topbar">
        <button class="plm-close" type="button" aria-label="Close">✕</button>
        <span class="plm-counter"></span>
      </div>
      <div class="plm-feed"></div>
      <button class="plm-nav plm-nav-prev" type="button" aria-label="Previous">▲</button>
      <button class="plm-nav plm-nav-next" type="button" aria-label="Next">▼</button>
    `;
  ...
  _buildIframeForItem(item) {
    ...
    if (p === 'youtube') {
      const ytSrc = url + (url.includes('?') ? '&' : '?') + 'enablejsapi=1&origin=' + encodeURIComponent(window.location.origin);
      return `<iframe class="plm-dock-youtube" src="${ytSrc}" title="${safeTitle}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen loading="lazy"></iframe>`;
    }
    if (p === 'bilibili') {
  ...
  _buildCardHtml(item, direction) {
      ...
      return `
        <div class="plm-card plm-ad-card ${dirClass}" data-plm-index="${this.currentIndex}">
          <div class="plm-ad-container">
            ${typeof buildPlayerAdMarkup === 'function' ? buildPlayerAdMarkup() : ''}
          </div>
        </div>
      `;
    }
    ...
    if (isVideo) {
      ...
      mediaSection = `
        <div class="plm-audio-cover">
          ...
          <img class="plm-audio-cover-img" src="${coverSrc}" alt="" loading="lazy" onerror="this.style.display='none'">
        </div>
    ...
    return `
      <div class="plm-card ${dirClass}" data-plm-index="${this.currentIndex}" style="--brand: ${brandColor1}; --brand-2: ${brandColor2};">
        ${mediaSection}
        <div class="plm-info">
  ...
  _checkIfFavorited(item) {
    if (!item.favKey || typeof getFavoriteEntries !== 'function') return false;
    try {
      const entries = getFavoriteEntries();
      return entries.some((e) => String(e.key || '').trim() === String(item.favKey || '').trim());
    } catch { return false; }
  },
  ...
  _renderCard(index, direction) {
    ...
    if (item.isAd && typeof initializeAdsbygooglePush === 'function') {
      const adEl = this.feedEl.querySelector('.episode-player-ad .adsbygoogle');
    ...
    // Init SponsorBlock for YouTube video
    if (item.isVideo && item.embedPlatform === 'youtube' && this.activeCardIframeId) {
  ...
  async _initSBForCurrentCard(item) {
    ...
    const videoId = extractVideoIdFromEmbedUrl(item.embedUrl);
    if (!videoId) return;
    ...
    try {
      ...
      if (!window.YT || !window.YT.Player || !segments.length) return;

      const iframe = document.getElementById(this.activeCardIframeId);
      if (!iframe) return;

      const shell = iframe.closest('.plm-video-area') || iframe.closest('.plm-card');
      const ytPlayer = new window.YT.Player(this.activeCardIframeId, {
        events: {
          onReady: () => {
            this._destroySBMonitor();
            this.activeCardSBMonitor = new SponsorBlockMonitor(ytPlayer, segments, shell);
          }
        }
      });
    } catch (err) {
...
function ensurePlaylistModeButton() {
  ...
  btn.type = 'button';
  btn.innerHTML = `<span class="plm-icon">${PLM_ICON_SVG}</span> Playlist`;
  btn.setAttribute('title', 'Playlist Mode');
...
function fetchItunesSearchPayload(title, countryCode, timeoutMs = ITUNES_SEARCH_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const country = normalizeCountry(countryCode || 'us');
    const url =
      'https://itunes.apple.com/search?media=podcast&entity=podcast&limit=5&country=' +
