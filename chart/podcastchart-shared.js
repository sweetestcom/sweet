...
function getTransportPrevSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="currentColor" d="M6.2 4.35c.45 0 .8.35.8.8v9.7a.8.8 0 1 1-1.6 0v-9.7c0-.45.35-.8.8-.8m1.95 5.65 6.27 4.22a.82.82 0 0 0 1.28-.69V5.47a.82.82 0 0 0-1.28-.69L8.15 9c-.23.15-.37.4-.37.63 0 .24.14.48.37.37Z"/></svg>';
}

function getTransportNextSvg() {
  return '<svg viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path fill="currentColor" d="M13.8 4.35c-.45 0-.8.35-.8.8v9.7a.8.8 0 1 0 1.6 0v-9.7c0-.45-.35-.8-.8-.8M11.85 10 5.58 5.78a.82.82 0 0 0-1.28.69v8.06c0 .65.73 1.04 1.28.69L11.85 11c.23-.15.37-.4.37-.63 0-.24-.14-.48-.37-.37Z"/></svg>';
}
...
function normalizeGlobalRssQueueItem(raw = {}) {
  ...
  const guid = String(raw.guid || '').trim() || src;
  const title = String(raw.title || '').trim();
  if (!src || !title) return null;
  const accentPlatform = String(raw.accentPlatform || raw.embedPlatform || raw.originPlatform || 'rss').trim().toLowerCase() || 'rss';
  return {
    id: String(raw.id || `${feedUrl || 'feed'}::${guid}`).trim(),
    src,
    guid,
    title,
    ...
    feedUrl,
    podcastTitle: String(raw.podcastTitle || raw.feedTitle || '').trim(),
    accentPlatform,
...
const globalRssPlayerController = {
  ...
  bind() {
    ...
    if (playBtn) {
      playBtn.addEventListener('click', () => {
        if (this.audio.paused || this.audio.ended) this.playCurrent();
    ...
    if (progress) {
      ...
      progress.addEventListener('change', () => {
        const duration = Number(this.audio.duration) || 0;
        if (duration > 0) this.audio.currentTime = Math.max(0, Math.min(duration, duration * ((Number(progress.value) || 0) / 1000)));
        this.persist(true);
        this.updateUi();
      });
    }
    if (volumeBtn && volumeSlider) {
      ...
      volumeSlider.addEventListener('input', () => {
        ...
        this.audio.muted = nextVolume <= 0.001;
        this.persist(false);
        this.emit();
        this.updateUi();
      });
  ...
  emit() {
    ...
    document.dispatchEvent(new CustomEvent(GLOBAL_RSS_PLAYER_EVENT, {
      detail: {
        ...
        duration: Number(this.audio?.duration) || 0,
  ...
  updateUi() {
    ...
    if (playBtn) playBtn.classList.toggle('is-playing', !!(item && this.audio && !this.audio.paused && !this.audio.ended));
    if (prevBtn) prevBtn.disabled = !(this.currentIndex > 0);
    if (nextBtn) nextBtn.disabled = !(this.currentIndex >= 0 && this.currentIndex < this.queue.length - 1);
    if (currentEl) currentEl.textContent = formatCastboxAudioTime(this.audio?.currentTime || 0);
    if (totalEl) totalEl.textContent = formatCastboxAudioTime(this.audio?.duration || 0);
    if (progress) {
      const duration = Number(this.audio?.duration) || 0;
    ...
    if (volumeWrap) {
      const safeVolume = Math.max(0, Math.min(1, Number(this.audio?.volume) || 0));
      volumeWrap.dataset.volumeState = this.audio?.muted || safeVolume <= 0.001 ? 'muted' : (safeVolume < 0.34 ? 'low' : (safeVolume < 0.67 ? 'medium' : 'high'));
    }
    if (volumeSlider) volumeSlider.value = String(Math.round((Number(this.audio?.volume) || 0) * 100));
    if (speedBtn) speedBtn.textContent = formatRssPlaybackRateLabel(Number(this.audio?.playbackRate) || 1);
    if (speedSlider) speedSlider.value = String(Math.max(0.5, Math.min(4, Number(this.audio?.playbackRate) || 1)));
  ...
  persist(force = false) {
    ...
    const item = this.getCurrentItem();
    if (item && item.feedUrl && item.guid) {
  ...
  findItemIndex(item) {
    ...
    return this.queue.findIndex((entry) => entry && (entry.id === normalized.id || (entry.src === normalized.src && entry.feedUrl === normalized.feedUrl)));
  },
  ...
  removeAt(index) {
    ...
    if (!this.queue.length) {
      ...
      this.playIndex(this.currentIndex, { autoplay: !this.audio.paused });
      return;
...
function buildApplePlayerModeAreaMarkup(mode = 'apple', canUseRss = true, descriptionMarkup = '', includeAd = true) {
  return `
    ...
    <div class="episode-player-mode-controls-wrap">
      ${buildAppleRssModeControlsMarkup(mode, canUseRss)}
    </div>
    ${descriptionMarkup}
