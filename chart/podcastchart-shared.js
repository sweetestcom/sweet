...
const WORKER_API_BASE = '/api/podcast';
...
function isEpisodeChartTypeValue(type) {
  const raw = String(type || '').trim().toLowerCase();
  return raw === 'top-episodes' || /^apple-\d{4}-episodes$/.test(raw);
}
...
async function selectWorkerPrefetchTasks(platform, country, type) {
  ...
  if (!currentTask || !isWorkerAttachEligibleTask(currentTask.platform, currentTask.type)) return [];
...
async function persistInlinePrefetchedPayloads(entries) {
  ...
  for (const entry of list) {
    ...
    try {
      await setResponseCacheEntry(responseCacheKeyFor(platform, country, type), data);
      const items = collectNormalizedItemsForData(data, platform, type);
      if (items.length) {
        await saveDailySnapshot(platform, country, type, items, {
          fetchedAt: Date.now(),
          payload: data
        });
      }
...
async function buildDataBackfillMapFromSnapshots(country, type = 'top-podcasts') {
  ...
  for (const sourcePlatform of backfillSources) {
    ...
    } else if (!rowsHaveDescriptions(rows)) {
      try {
...
async function fetchAllPlatformsCompositeData(type, country, options = {}) {
  ...
  const recordBatchSuccess = (platform, data) => {
    ...
    setResponseCacheEntry(responseCacheKeyFor(platform, safeCountry, safeType), data).catch(() => { });
    saveDailySnapshot(platform, safeCountry, safeType, items, {
      fetchedAt: Date.now(),
      payload: data
    }).catch(() => { });
  };
