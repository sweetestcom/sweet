...
const RADIOFRANCE_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: '', defaultLabel: 'Les émissions les plus écoutées' },
  { value: 'radiofrance-enfants', key: '', defaultLabel: 'Podcasts pour enfants' },
  { value: 'radiofrance-true-crime', key: '', defaultLabel: 'Crimes et histoires vraies' },
  { value: 'radiofrance-fictions', key: '', defaultLabel: 'Fictions audio' },
  { value: 'radiofrance-histoire', key: '', defaultLabel: 'Nos podcasts histoire' },
  { value: 'radiofrance-sciences', key: '', defaultLabel: 'Nos podcasts sciences' },
  { value: 'radiofrance-monde', key: '', defaultLabel: 'Le monde en podcasts' },
  { value: 'radiofrance-musique', key: '', defaultLabel: 'La musique en podcasts' },
  { value: 'radiofrance-arts-divertissement', key: '', defaultLabel: 'Les arts et divertissement en podcasts' },
  { value: 'radiofrance-humour', key: '', defaultLabel: "Le coin de l'humour" },
  { value: 'radiofrance-vie-quotidienne', key: '', defaultLabel: 'La vie quotidienne en podcasts' }
];
...
const XIMALAYA_TYPE_DEFINITIONS = [
  { value: 'ximalaya-story-podcasts', key: '', defaultLabel: '故事播客' },
  { value: 'ximalaya-opinion-podcasts', key: '', defaultLabel: '观点播客' }
];
const BILIBILI_TYPE_DEFINITIONS = [
  { value: 'top-podcasts', key: '', defaultLabel: '热门播客' }
];
...
function getTypeDefinitionsForPlatform(platform) {
  ...
  if (p === 'castbox' || p === 'pocketcasts' || p === 'audible' || p === 'iheartradio' || p === 'amazonmusic' || p === 'radiko' || p === 'radionet') {
...
function platformBrandLabel(value) {
  ...
  const ximalayaLabel = lang === 'zh-hans' ? '喜马拉雅' : ((lang === 'zh-hant' || lang === 'zh-hk') ? '喜馬拉雅' : 'Ximalaya');
  const bilibiliLabel = lang === 'zh-hans' ? '哔哩哔哩' : ((lang === 'zh-hant' || lang === 'zh-hk') ? '嗶哩嗶哩' : 'BiliBili');
  const map = {
    apple: 'Apple',
    spotify: 'Spotify',
    youtube: 'YouTube',
...
function buildDeezerOptionLabel() {
  return buildBrandedPodcastsLabel('Deezer', 'Deezer Podcasts');
}

function buildXimalayaOptionLabel() {
  const lang = pageToLanguage(getCurrentFileName()) || 'en';
  if (lang === 'zh-hans') return '喜马拉雅';
  if (lang === 'zh-hant' || lang === 'zh-hk') return '喜馬拉雅';
  return 'Ximalaya';
}

function buildBilibiliOptionLabel() {
  const lang = pageToLanguage(getCurrentFileName()) || 'en';
  if (lang === 'zh-hans') return '哔哩哔哩';
  if (lang === 'zh-hant' || lang === 'zh-hk') return '嗶哩嗶哩';
  return 'BiliBili';
}

function buildPodimoOptionLabel() {
  return buildBrandedPodcastsLabel('Podimo', 'Podimo Podcasts');
}

function buildRadionetOptionLabel() {
  return buildBrandedPodcastsLabel('radio', 'radio Podcasts');
}

function buildIvooxOptionLabel() {
  return buildBrandedPodcastsLabel('iVoox', 'iVoox Podcasts');
}
...
function normalizeItems(data, platform, type) {
  ...
  if (platform === 'apple') {
    ...
    return arr.map((item, index) => {
      ...
      return {
        rank: index + 1,
        title: item.name || '',
        publisher: item.artistName || item.publisher || (itemKind === 'podcast-channels' ? 'Apple Podcast Channels' : 'Apple Podcasts'),
  ...
  if (platform === 'ardsounds' || platform === 'radiofrance' || platform === 'podimo' || platform === 'radionet' || platform === 'ivoox') {
    arr = Array.isArray(data && data.podcasts) ? data.podcasts : [];
...
function renderItems(items) {
  ...
  chartContainer.innerHTML = items.map((item) => {
    const currentPlatform = String((platformSelect && platformSelect.value) || '').trim().toLowerCase();
    const currentType = String((typeSelect && typeSelect.value) || '').trim().toLowerCase();
    const useVideoCover = !favoritesScope && (
      (currentPlatform === 'youtube' && currentType === 'top-episodes')
    ...
    const imageCandidatesRaw = Array.isArray(item.imageCandidates)
      ...
      : [];
    const imageSrc = item.image || imageCandidatesRaw[0] || 'logo.webp';
    const image = escapeHtml(imageSrc);
    const imageCandidates = escapeHtml(imageCandidatesRaw.join('\n'));
    const imageReferrerPolicy = needsNoReferrerImage(imageSrc, item.embedPlatform || item.originPlatform || platformSelect?.value)
    ...
    return `
      <article class="chart-row${favoritesScope ? ' favorites-row' : ''}${useVideoCover ? ' has-video-cover' : ''}" data-search="${search}">
        ${rankPart}
        <img class="cover${useVideoCover ? ' cover-video' : ''}" src="${image}" alt="${title}" loading="lazy"${imageReferrerPolicy} data-image-candidates="${imageCandidates}" data-image-fallback="logo.webp" />
        <div class="meta">
  ...
  chartContainer.querySelectorAll('img.cover').forEach((img) => {
    const fallback = String(img.getAttribute('data-image-fallback') || 'logo.webp').trim() || 'logo.webp';
