...
function buildYouTubePlayerHostMarkup(id, videoId, title, extraClass = '') {
  ...
  return `<div id="${safeId}" class="episode-player-frame youtube youtube-player-host ${safeClass}" data-youtube-video-id="${safeVideoId}" data-youtube-title="${safeTitle}"></div>`;
}
...
function buildEpisodePlayerIframeMarkup(embedUrl, platform, title, kind, options = {}) {
  ...
  if (p === 'youtube') {
    ...
    if (videoId) {
      return buildYouTubePlayerHostMarkup(ytId, videoId, safeTitle, safeKind === 'top-podcasts' ? 'is-show' : 'is-episode');
    }
...
function collectSponsorBlockVideoIdsFromItems(items, limit = 6) {
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    const videoId = extractVideoIdFromEmbedUrl(String(item && item.embedUrl || '').trim());
...
function collectSponsorBlockVideoIdsFromRows(container, limit = 6) {
  ...
  rows.forEach((btn) => {
    ...
    const videoId = extractVideoIdFromEmbedUrl(String(btn.getAttribute('data-embed-url') || '').trim());
...
function ensureSponsorBlockBadge(shell, segments) {
  if (!shell || !shell.parentNode || !Array.isArray(segments) || !segments.length) return;
  const adjacent = shell.nextElementSibling;
  if (adjacent && adjacent.classList && adjacent.classList.contains('sponsorblock-badge')) return;
...
async function initSponsorBlockForYouTubePlayer(row) {
  ...
  const segmentsPromise = fetchSponsorBlockSegments(videoId);
  ...
  try {
    const ytPlayer = new window.YT.Player(host.id, {
      videoId,
      playerVars: {
        playsinline: 1,
        rel: 0,
      ...
      events: {
        onReady: async () => {
          const segments = await segmentsPromise.catch(() => []);
          if (!row.classList.contains('player-open')) return;
          ensureSponsorBlockBadge(shell, segments);
...
const playlistController = {
  ...
  _buildIframeForItem(item) {
    ...
    if (item.isVideo && p === 'youtube') {
      ...
      if (videoId) {
        return buildYouTubePlayerHostMarkup(ytId, videoId, safeTitle, 'plm-dock-youtube');
      }
  ...
  async _initSBForCurrentCard(item) {
    ...
    try {
      ...
      const shell = host.closest('.plm-video-area') || host.closest('.plm-card');
  ...
  _primeSponsorBlockAroundIndex(index) {
    ...
    for (let i = index; i < this.items.length && ids.length < 6; i += 1) {
      ...
      const videoId = extractVideoIdFromEmbedUrl(String(item.embedUrl || '').trim());
      if (!videoId || ids.includes(videoId)) continue;
      ids.push(videoId);
    }
