...
function formatRssPlaybackRateLabel(rate) {
  ...
  return Number.isInteger(safeRate) ? `${safeRate}x` : `${safeRate.toFixed(2).replace(/0+$/, '').replace(/\.$/, '')}x`;
}
...
function buildCastboxSpeedControlMarkup(activeRate = 1) {
  ...
  return `
    <div class="castbox-rss-speed-control" data-castbox-speed-control data-open="false">
      ...
      <input class="castbox-rss-speed-slider" data-castbox-speed-slider type="range" min="0.5" max="4" step="0.1" value="${currentRate}" aria-label="Playback speed">
    </div>
  `;
}
...
function bindCastboxRssPlayerInteractions(slot, options = {}) {
  ...
  if (speedSlider) {
    speedSlider.addEventListener('input', () => {
      ...
      audio.playbackRate = Number(nextRate.toFixed(1));
      syncSpeedButtonState();
    });
  }
