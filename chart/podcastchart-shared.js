...
const GLOBAL_RSS_PLAYER_QUEUE_LIMIT = 160;
...
async function switchAppleRowPlayerMode(row, mode, trigger = null) {
  ...
  try {
    ...
    return await openEpisodePlayerRow(row, playBtn);
  } finally {
...
function buildChartRowsMarkup(items, options = {}) {
  ...
  return items.map((item) => {
    ...
    const topicRevealHtml = topicFilterMode === 'blur'
      ? `<button class="topic-row-reveal" type="button" data-topic-reveal="1" aria-label="${escapeHtml(tOr('open', 'Open'))}">
          <span class="topic-row-reveal-label">${escapeHtml(tOr('revealFiltered', 'Show episode'))}</span>
          ${topicFilterKeyword ? `<strong class="topic-row-reveal-keyword">${escapeHtml(topicFilterKeyword)}</strong>` : ''}
        </button>`
      : '';
...
async function fetchAndPrimeXiaoyuzhouBundle(country, options = {}) {
  const safeCountry = normalizeCountry(country);
  const forceRefresh = Boolean(options.forceRefresh);
  const bundleKey = responseCacheKeyForXiaoyuzhouBundle(safeCountry);
  if (!forceRefresh) {
    const cachedEntry = await getResponseCacheEntry(bundleKey);
    const cached = cachedEntry && Object.prototype.hasOwnProperty.call(cachedEntry, 'payload') ? cachedEntry.payload : null;
    const cachedTime = Number((cachedEntry && cachedEntry.updatedAt) || 0);
    if (cached && isResponseCacheFreshForPlatform('xiaoyuzhou', 'top-episodes', cached, cachedTime)) {
      return cached;
    }
  }
  const params = new URLSearchParams({
...
const playlistController = {
  ...
  _getItemPlayerMode(item) {
    ...
    if ((platform === 'apple' || platform === 'spotify') && String(item && item.__plmPlayerMode || '').trim().toLowerCase() === 'rss') return 'rss';
    return platform;
  },
